...
\begin{equation*}
	...
	\quad
	\uv{b}\cdot\vb{\epsilon} = \begin{pmatrix}0&i&0\\-i&0&0\\0&0&0\end{pmatrix}.
\end{equation*}
这与前面的结果完全一致.
...
\begin{example}
	...
	\begin{equation*}
		...
		&\qquad= \pm\sum_\alpha\frac{\omega_{p\alpha}^2\mathrm{e}^{-\kappa_\alpha}}{\omega\kappa_\alpha}\sum_{n=-\infty}^{\infty}n\frac{I_n(\kappa_\alpha)-I_n^\prime(\kappa_\alpha)}{\omega-n\Omega_\alpha}.
	\end{equation*}
\end{example}
...
\begin{example}
	...
	\begin{equation*}
		...
		&v_{\mathrm{the}} = \sqrt{\frac{m_i T_e}{m_e T_i}}v_{\mathrm{thi}} = \sqrt{\frac{2k_B T_e}{m_e}},\quad \lambda_{\mathrm{De}} = \sqrt{\frac{ZT_i}{T_e}}\lambda_{\mathrm{Di}} = \sqrt{\frac{\varepsilon_0 k_B T_e}{n_e e^2}}.
	\end{equation*}
	只考虑纵波模式的色散方程
	\begin{equation*}
	...
	\begin{equation*}
		1 - \frac{\omega_{p\mathrm{e}}^2}{\omega^2}\bigg(1+\frac{3k^2v_{\mathrm{the}}^2}{2\omega^2}\bigg) - i\frac{2\sqrt{\pi}\omega_{p\mathrm{e}}^2}{k^3v_{\mathrm{the}}^3}\mathrm{e}^{-\frac{\omega^2}{k^2v_{\mathrm{the}}^2}} = 0.
	\end{equation*}
	解得
	\begin{equation*}
		\frac{\omega}{\omega_{p\mathrm{e}}}\approx\bigg(1+\frac{3k^2v_{\mathrm{the}}^2}{2\omega_{p\mathrm{e}}^2}\bigg) - i\sqrt{\pi}\bigg(\frac{\omega_{p\mathrm{e}}}{k v_{\mathrm{the}}}\bigg)^3\mathrm{e}^{-\frac{\omega_{p\mathrm{e}}^2}{k^2v_{\mathrm{the}}^2}}.
	\end{equation*}
	由于 \(\omega_{p\mathrm{e}}/k v_{\mathrm{the}}\approx\zeta_e\gg1\)，可见阻尼的确是小量.

	对于低频电磁波，考虑到电子热运动速率较快，取 \(|\zeta_i|\gg1, |\zeta_e|\ll1\)，近似得
	\begin{equation*}
		1 - \frac{\omega_{p\mathrm{i}}^2}{\omega^2} + \frac{1}{k^2\lambda_{\mathrm{De}}^2} + i\frac{2\sqrt{\pi}\omega_{p\mathrm{i}}^2}{k^3v_{\mathrm{thi}}^3}\bigg(\mathrm{e}^{-\frac{\omega^2}{k^2v_{\mathrm{thi}}^2}}+\frac{\omega_{p\mathrm{e}}^2v_{\mathrm{thi}}^3}{\omega_{p\mathrm{i}}^2v_{\mathrm{the}}^3}\mathrm{e}^{-\frac{\omega^2}{k^2v_{\mathrm{the}}^2}}\bigg) = 0.
	\end{equation*}
	解得
	\begin{equation*}
		\omega = \frac{k c_s}{\sqrt{1+k^2\lambda_{\mathrm{De}}^2}}\Bigg(1 - \frac{i\sqrt{\pi}}{2}\bigg(\frac{c_s}{v_{\mathrm{thi}}\sqrt{1+k^2\lambda_{\mathrm{De}}^2}}\bigg)^3\bigg(\mathrm{e}^{-\frac{c_s^2}{v_{\mathrm{thi}}^2\left(1+k^2\lambda_{\mathrm{De}}^2\right)}} + \frac{1}{Z}\sqrt{\frac{m_e T_i^3}{m_i T_e^3}}\mathrm{e}^{-\frac{c_s^2}{v_{\mathrm{the}}^2\left(1+k^2\lambda_{\mathrm{De}}^2\right)}}\bigg)\Bigg).
	\end{equation*}
