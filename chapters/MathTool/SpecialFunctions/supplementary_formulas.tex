...
与平面周期运动有关的积分恒等式
\begin{equation*}
	&\int_{0}^{2\pi} e^{i m (\xi - \alpha \cos\xi)} \sin\xi \, \d \xi = -\frac{2\pi}{\alpha} \, (-i)^{m} J_{m}(m \alpha), \quad
	\int_{0}^{2\pi} e^{i m (\xi - \alpha \cos\xi)} \cos\xi \, \d \xi = 2\pi \, (-i)^{m-1} J_{m}'(m \alpha). \\
	&\frac{1}{4\pi} \int_{0}^{2\pi} \frac{1 - \sin^{2}\varphi \cos^{2}\theta + \beta^{2} \cos^{2}\theta - 2 \beta \cos\varphi \cos\theta}{(1 - \beta \cos\theta \cos\varphi)^{5}} \, \d \varphi= \frac{4(1 + \sin^{2}\theta) - \beta^{2} \cos^{4}\theta (1 + 3\beta^{2})}{16 (1 - \beta^{2} \cos^{2}\theta)^{7/2}}\\
