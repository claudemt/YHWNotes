...
另由斯托克斯关系式得：
\begin{equation*}
	r_L' = -r_R \dfrac{\cos\theta_r}{\cos\theta_r'} t_R, \quad t_L' = \dfrac{1 - r_R^2}{t_R},
\end{equation*}
\begin{equation*}
...
特别地，若 \(i = 0\)，则：
\begin{equation*}
	\theta_N' &= \theta_N = 0, \quad \theta_r' = -\theta_r = -\xi + \arctan\left( \dfrac{n_o}{n_e} \tan\xi \right),\\
	r_R &= r_L = -r_R' = -r_L' = \dfrac{n_o n_e - n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi}}{n_o n_e + n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi}},\\
	t_R &= t_L = \dfrac{2 n (n_e^2 \cos^2\xi + n_o^2 \sin^2\xi)}{\left( n_e \cos^2\xi + n_o \sin^2\xi \right) \left( n_o n_e + n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi} \right)},\\
	t_R' &= t_L' = \dfrac{2 n_o n_e (n_e \cos^2\xi + n_o \sin^2\xi)}{\sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi} \left( n_o n_e + n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi} \right)},\\
	...
	T_R &= T_L = \dfrac{4 n_o n_e n (n_e^2 \cos^2\xi + n_o^2 \sin^2\xi)}{\left( n_e \cos^2\xi + n_o \sin^2\xi \right) \left( n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi} + n_o n_e \right)^2},\\
	T_R' &= T_L' = \dfrac{4 n_o n_e n (n_e \cos^2\xi + n_o \sin^2\xi)}{\left( n \sqrt{n_e^2 \cos^2\xi + n_o^2 \sin^2\xi} + n_o n_e \right)^2}.
\end{equation*}
