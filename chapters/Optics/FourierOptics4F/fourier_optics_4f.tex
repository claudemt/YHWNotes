...
\begin{figure}[htbp]
	...
	\label{4f系统示意图}
\end{figure}
4\(f\)系统光路如图\ref{4f系统示意图}.记物面振幅分布 \( U_o(x, y) \) ，对球面波因子进行傍轴近似、忽略轴向转播因子，得频谱面振幅分布：
...
则频谱面的振幅为
\begin{equation*}
	\tilde{U}_f(\vb{\kappa}) = \frac{1}{\lambda f} \int U_o(\vb{r}) * (W(\vb{r}) \amalg(\vb{r})) e^{-i \vb{\kappa} \cdot \vb{r}} d\vb{r} = \tilde{U}_f^{\text{unit}}(\vb{\kappa}) \sum_{h=-\infty}^{\infty} \sum_{l=-\infty}^{\infty} \frac{(2\pi)^2}{A_c} \tilde{W}(\vb{\kappa} - \vb{G}_{hl}).
\end{equation*}
...
\begin{equation*}
	...
	\tilde{U}_f(\vb{\kappa}) &= \tilde{U}_f^{\text{unit}}(\vb{\kappa}) \frac{L_1 L_2}{A_c} \sum_{h=-\infty}^{\infty} \sum_{l=-\infty}^{\infty} (2\pi)^2 \sinc\left( \frac{(\vb{\kappa} - \vb{G}_{hl}) \cdot \uv{u}_1}{2} L_1 \right) \sinc\left( \frac{(\vb{\kappa} - \vb{G}_{hl}) \cdot \uv{u}_2}{2} L_2 \right).
\end{equation*}
...
\begin{example}
	...
	\end{equation*}
	\begin{equation*}
		\tilde{U}_f(\vb{\kappa}) = \tilde{U}_f^{\text{unit}}(\vb{\kappa}) \frac{(2\pi L)^2}{d^2} \sum_{h=-\infty}^{\infty} \sum_{l=-\infty}^{\infty} \sinc\left( \frac{\kappa_x L_1}{2} - \pi h \frac{L_1}{d} \right) \sinc\left( \frac{\kappa_y L_2}{2} - \pi l \frac{L_2}{d} \right)
	\end{equation*}
	极大斑点位于
	\begin{equation*}
		\kappa_x=\frac{2h}{d},\quad \kappa_y=\frac{2l}{d}
	\end{equation*}
\end{example}
...
\begin{example}
	...
	\end{equation*}
	\begin{equation*}
		\tilde{U}_f(\vb{\kappa}) = \tilde{U}_f^{\text{unit}}(\vb{\kappa}) \frac{2 (2\pi L)^2}{\sqrt{3} d^2} \sum_{h=-\infty}^{\infty} \sum_{l=-\infty}^{\infty} \sinc\left( \frac{\kappa_x L_1}{2} - \pi h \frac{L_1}{d} \right) \sinc\left( \frac{\kappa_y L_2}{2} - \frac{\pi}{\sqrt{3}} (-h + 2l) \frac{L_2}{d} \right)
	\end{equation*}
	极大斑点位于
	\begin{equation*}
		\kappa_x=\frac{2h}{d},\quad \kappa_y=\frac{2(-l+2h)}{\sqrt{3}d}
	\end{equation*}
\end{example}

\subsubsection{频谱模拟}
最后，用mathematica的离散傅里叶变换功能模拟频谱面（如图\ref{物面及频谱面模拟图}）.
