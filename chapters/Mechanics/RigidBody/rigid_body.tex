...
\begin{equation*}
	\vb{I} = \int \rho(\vb{r}) (r^2 \vb{e} - \vb{r} \vb{r}) \, \d V.
\end{equation*}
其中\(\vb{r}\)是体积元\(\d V\)相对于质心的位矢.

对于相对于质心的位矢为\(\vb{a}\)的点\(P\)，定义刚体相对\(P\)点的惯量张量为：
\begin{equation*}
	\vb{I}_P = \int \rho(\vb{r}) \left((r + a)^2 \vb{e} - (\vb{r} + \vb{a})(\vb{r} + \vb{a})\right) \, \d V = \vb{I} + M (a^2 \vb{e} - \vb{a} \vb{a}),
\end{equation*}
其中\(M = \int \rho(\vb{r}) \, \d V\)是刚体的总质量.

当刚体以质心速度\(\vb{v}_C\)平动，以相对质心的角速度\(\vb{\omega}\)转动时，总动能为
\begin{equation*}
	T = \frac{1}{2} \int \rho(\vb{r}) |\vb{v}_C + \vb{\omega} \times \vb{r}|^2 \, \d V = \frac{1}{2} M v_c^2 + \frac{1}{2} \vb{I} : \vb{\omega} \vb{\omega}.
\end{equation*}

相对质心的角动量为
\begin{equation*}
	\vb{L}_c = \int \rho(\vb{r}) \vb{r} \times (\vb{\omega} \times \vb{r}) \, \d V = \vb{I} \cdot \vb{\omega}.
\end{equation*}

定义刚体相对穿过质心且单位方向为\(\uv{n}\)的转轴的转动惯量为：
\begin{equation*}
	I(\uv{n}) = \int \rho(\vb{r}) |\uv{n} \times \vb{r}|^2 \, \d V = \vb{I} : \uv{n} \uv{n}.
\end{equation*}

对于相对于质心的位矢为\(\vb{a}\)的点\(P\)，定义刚体相对穿过\(P\)且单位方向为\(\uv{n}\)的转轴的转动惯量为：
\begin{equation*}
	I_P(\uv{n}) = \int \rho(\vb{r}) |\uv{n} \times (\vb{r} + \vb{a})|^2 \, \d V = \vb{I}_P : \uv{n} \uv{n} = I(\uv{n}) + M |\vb{a} \times \uv{n}|^2.
\end{equation*}
...
则刚体相对\(O\)的动能为：
\begin{align*}
	T & = \frac{1}{2} \int \rho(\vb{r}) |\vb{\omega} \times \vb{r}|^2 \, \d V = \frac{1}{2} \vb{I} : \vb{\omega} \vb{\omega} = \frac{1}{2} I_1 \omega_1^2 + \frac{1}{2} I_2 \omega_2^2 + \frac{1}{2} I_3 \omega_3^2\notag           \\
...
\begin{equation*}
	\vb{F} = M \frac{\d \vb{v}_c}{\d t}, \quad \vb{\tau} = \frac{\d \vb{L}_c}{\d t} = \left(\frac{\d \vb{L}_c}{\d t}\right)_c + \vb{\omega}\times\vb{L}_c,
\end{equation*}
...
\begin{equation*}
	...
	\omega_3^2 &= \frac{1}{I_3(I_1-I_3)}\left[(L^2 - 2EI_1) - I_2(I_2-I_1)\omega_2^2\right].\\
	&\left(\frac{\d \omega_2}{\d t}\right)^2 = \frac{1}{I_2^2 I_1 I_3}\left[(2EI_3 - L^2) - I_2(I_3-I_2)\omega_2^2\right]\left[(L^2 - 2EI_1) - I_2(I_2-I_1)\omega_2^2\right].
\end{equation*}
...
\begin{equation*}
	...
	s = \omega_2\sqrt{\frac{I_2(I_2-I_1)}{2EI_1-L^2}}
\end{equation*}
演化方程简化为
\begin{equation*}
	\left(\frac{\d s}{\d \tau}\right)^2 = (1-s^2)(1-k^2s^2)
\end{equation*}
...
\begin{align*}
	...
	\dot{\varphi} & = \frac{(2EI_3-L^2)+(L^2-2EI_1)\sn^2(\tau,k)}{I_1(2EI_3-L^2)+I_3(L^2-2EI_1)\sn^2(\tau,k)}.
\end{align*}

运动周期（同样也是\(\vb{\omega}\)的演化周期）为
\begin{equation*}
	T = 4\sqrt{\frac{I_1I_2I_3}{(I_2-I_1)(2EI_3-L^2)}}\int_0^1\frac{\d x}{\sqrt{(1-x^2)(1-k^2x^2)}}= 4\sqrt{\frac{I_1I_2I_3}{(I_2-I_1)(2EI_3-L^2)}}F(k)
\end{equation*}
...
运动周期为
\begin{equation*}
	T = 4 \sqrt{\frac{I_1 I_2 I_3}{(I_3 - I_2)(L^2 - 2EI_1)}} \int_0^1 \frac{\d x}{\sqrt{(1-x^2)(1-k^2 x^2)}}= 4 \sqrt{\frac{I_1 I_2 I_3}{(I_3 - I_2)(L^2 - 2EI_1)}} F(k)
\end{equation*}
...
当\(t \to \infty\)时，角速度将仅保留沿2轴方向的分量\(\omega_0\).由于系统的动力学行为具有时间反演对称性，因此若初始时刻角速度仅有2轴分量\(\omega_0\)，则系统达到稳态时
\begin{equation*}
...
这表明角速度会从转动惯量居中的轴（2轴）转移到转动惯量最大和最小的两个轴（1轴和3轴）.
...
\begin{notes}
	...
	\begin{equation*}
		\sn(\tau,k)&=\frac{1}{k}\sn(k\tau,\frac{1}{k}),\quad \cn(\tau,k)=\dn(k\tau,\frac{1}{k}),\quad \dn(\tau,k)=\dn(k\tau,\frac{1}{k}).\\
...
\begin{equation*}
	T= \frac{1}{2}\, M v_c^2 + \frac{1}{2}\, \vb{I} : \vb{\omega} \vb{\omega} = \frac{1}{2}\, M | \vb{\omega} \times \vb{a} |^2 + \frac{1}{2}\, \vb{I} : \vb{\omega} \vb{\omega} = \frac{1}{2}\, \bm{\vb{I}_P} : \vb{\omega} \vb{\omega}.
\end{equation*}
...
\begin{equation*}
	\mathcal{L} = T - V + \int \vb{A} \cdot \vb{v} \, \d q = T - V + \int \left( \frac{1}{2} \vb{B} \times \vb{r} \right) \cdot \vb{v} \, \d q = T - V + \vb{\Omega} \cdot \vb{L}.
\end{equation*}
其中
\begin{equation*}
...
\begin{example}
	...
	则
	\begin{equation*}
		a = \frac{6}{5} R, \quad M = \frac{8 \pi}{3} \rho R^3, \quad I_1 = \frac{192}{35} \pi \rho R^5, \quad I_3 = \frac{64}{35} \pi \rho R^3,\quad
		\alpha = \frac{7}{12}, \quad \gamma = \frac{1}{3}.
	\end{equation*}

	给定数值 \(\beta = 7.684,\, g/R\,\Omega^2=1.2,\, \theta_0=1.47\,\mathrm{rad}\)，可计算得
	\begin{align*}
		\frac{\dot{\psi}_0}{\Omega} & = 1.926, \quad \frac{\omega_p}{\Omega} = 8.640.
	\end{align*}
\end{example}
...
\begin{example}
	...
	\begin{equation*}
		...
		\omega_p^2=\sqrt{\dot{\phi}_0^2-\frac{2Mga}{I_1}\cos\theta_0+\left(\frac{Mga}{I_1\dot{\phi}_0}\right)^2}.
	\end{equation*}
\end{example}
...
\begin{notes}
	...
	记质心相对参考点位矢为 \(\vb{r}_c\,\)，刚体上某一点相对质心的位矢为 \(\vb{r}'\)，刚体的质量密度为 \(\rho(\vb{r}')\)，相对质心的惯量张量为 \(\vb{I}= \int \rho(\vb{r}') \left( r'^2 \vb{e} - \vb{r}' \vb{r}' \right) \d \vb{r}'\)，
	总质量为 \(M = \int \rho(\vb{r}') \d \vb{r}'\).电荷密度为 \(\rho_e(\vb{r}')\)，总电荷量为 \(Q = \int \rho_e(\vb{r}') \d \vb{r}'\).假设其荷质比为常数 \(Q/M\)，即 \(\rho_e(\vb{r}) = Q \,\rho(\vb{r})/M\).
	...
	其中用到\(\d \vb{r}'/\d t = \vb{\omega} \times \vb{r}'\).

	进而，刚体所受电磁合外力为
	\begin{equation*}
		\vb{F}_{\text{em}} = \int \rho_e(\vb{r}') \vb{v}(\vb{r}') \times \vb{B} \d \vb{r}' = Q \vb{v}_c \times \vb{B},
	\end{equation*}

	电磁合外力矩为
	\begin{align*}
		\vb{\tau}_{\text{em}} & = \int \rho_e(\vb{r}') (\vb{r}_c + \vb{r}') \times (\vb{v}(\vb{r}') \times \vb{B}) \d \vb{r}' = Q \vb{r}_c \times (\vb{v}_c \times \vb{B}) + \int 2 \rho(\vb{r}') \vb{r}' \times [(\vb{\omega} \times \vb{r}') \times \vb{\Omega}]                                           \\
		                      & = Q \vb{r}_c \times (\vb{v}_c \times \vb{B}) + \int \rho(\vb{r}') \left[ \frac{\d}{\d t} \left( \vb{r}' (\vb{r}' \cdot \vb{\Omega}) - r'^2 \vb{\Omega} \right) + \vb{\Omega} \times \left( \vb{r}' (\vb{r}' \cdot \vb{\omega}) - r'^2 \vb{\omega} \right) \right] \d \vb{r}' \\
		                      & = Q \vb{r}_c \times (\vb{v}_c \times \vb{B}) - \frac{\d}{\d t} (\vb{I} \cdot \vb{\Omega}) - \vb{\Omega} \times (\vb{I} \cdot \vb{\omega})
	\end{align*}
