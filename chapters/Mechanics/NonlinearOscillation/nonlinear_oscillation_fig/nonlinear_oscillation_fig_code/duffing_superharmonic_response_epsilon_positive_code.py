...
def make_plot(eps,xlim,ylim,filename):
    ...
    with figure_style():
        fig,ax=plt.subplots(figsize=(12.8,8.0)); handles=[]
        ...
        ax.set(xlabel=r"$s$",ylabel=r"$a_s$",title=rf"Responce curve: $\epsilon={eps:g}$, $\zeta={zeta:g}$")
        ax.set_xlim(*xlim); ax.set_ylim(*ylim); polish_axes(ax); ax.legend(handles=handles,loc="upper left")
