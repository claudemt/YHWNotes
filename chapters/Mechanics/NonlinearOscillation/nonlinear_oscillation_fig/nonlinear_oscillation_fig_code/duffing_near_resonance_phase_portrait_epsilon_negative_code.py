...
def main():
    eps,zeta=-0.2,0.25; Bs=[1.8,2.4,3.0,3.6]
    s=np.linspace(0,1.3,850); A=np.linspace(0,3,750); S,AA=np.meshgrid(s,A)
    with figure_style():
        fig,ax=plt.subplots(figsize=(13.0,8.0))
        ax.contourf(S,AA,f_region(S,AA,eps,zeta),levels=[-1e9,0],colors=[LIGHT_BLUE])
        handles=[]
        for i,b in enumerate(Bs):
            ax.contour(S,AA,B_value(S,AA,eps,zeta,True)-b,levels=[0],colors=[COLORS[i]],linewidths=2.8)
        ...
        ax.set(xlabel=r"$s$",ylabel=r"$a_s$",title=r"$B(s,a_s)$ for $\epsilon=-0.2$, $\zeta=0.25$")
        ax.set_xlim(0,1.3); ax.set_ylim(0,3); polish_axes(ax); ax.legend(handles=handles,loc="center left",bbox_to_anchor=(1.02,.5))
        save_figure(fig,"duffing_near_resonance_phase_portrait_epsilon_negative.png")
