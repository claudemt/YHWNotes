...
进而哈密顿量修正为
\begin{equation*}
	\mathcal{H} = \mathcal{H}_{0} + \mathcal{H}_{1}, \quad \mathcal{H}_{0} = -\frac{\hbar^{2}}{2m} \sum_{i} \nabla_{i}^{2}, \quad \mathcal{H}_{1} = \frac{4\pi a \hbar^{2}}{m} \sum_{i<j} \delta(\vb{r}_{i} - \vb{r}_{j}) \left( \frac{\partial}{\partial r_{ij}} r_{ij} \right) = \frac{2\pi a \hbar^{2}}{m} \sum_{i,j} \delta(\vb{r}_{i} - \vb{r}_{j}) \left( \frac{\partial}{\partial r_{ij}} r_{ij} \right).
\end{equation*}
其中\(i, j\)为粒子编号， \(r_{ij} = |\vb{r}_{i} - \vb{r}_{j}|\), \(\nabla_{i}^{2}\)以 \(\vb{r}_{i}\) 为微分坐标.
...
\begin{equation*}
	...
	&= -8 \left( \frac{2\pi a \hbar^{2}}{m V} \right)^{2} \sum_{\vb{p}_{\alpha},\vb{p}_{\beta},\vb{p}_{\gamma},\vb{p}_{\lambda}} \frac{\langle n_{\gamma} \rangle \langle n_{\lambda} \rangle \langle n_{\alpha} \rangle}{\varepsilon_{\alpha} + \varepsilon_{\beta} - \varepsilon_{\gamma} - \varepsilon_{\lambda}} \delta_{\vb{p}_{\alpha} + \vb{p}_{\beta}, \vb{p}_{\gamma} + \vb{p}_{\lambda}}.
\end{equation*}
代入
\begin{equation*}
...
\begin{equation*}
	...
	&F(z) \big|_{\ln z \gg 1} = \frac{16}{105} \frac{11 - 2\ln 2}{\pi^{3/2}} (\ln z)^{7/2} \left( 1 - \frac{35}{8} \frac{2\ln 2 - 1}{11 - 2\ln 2} \frac{\pi^{2}}{(\ln z)^{2}} \right) + \mathcal{O}\left( (\ln z)^{5/4} \right).
\end{equation*}
...
\subsubsection{占据数平均值的计算}

在求解硬球势对巨配分函数的修正时，经常要求解占据数的各种幂次的平均值，在此一并求解.
...
记总粒子数、粒子数偏差为
\begin{equation*}
	N = g \sum_{\alpha} \langle n_{\alpha} \rangle, \quad \delta N = \sum_{\alpha} \left( n_{\alpha} - \langle n_{\alpha} \rangle \right).
\end{equation*}

求和连续化为积分得
\begin{equation*}
	&N = g \sum_{\alpha} \frac{1}{z^{-1} e^{\beta \varepsilon_{\alpha}} \mp 1} = g \int_{0}^{\infty} \frac{s B V \varepsilon^{s-1}}{z^{-1} e^{\beta \varepsilon} \mp 1} \, \d \varepsilon = B V \Gamma(s+1) (kT)^{s} \Phi_{s}(z).\\
	&\langle (\delta N)^{2} \rangle = B V \Gamma(s+1) (kT)^{s} \Phi_{s-1}(z).\quad
	\langle (\delta N)^{3} \rangle = B V \Gamma(s+1) (kT)^{s} \Phi_{s-2}(z).\\
	&\langle (\delta N)^{4} \rangle - 3 \langle (\delta N)^{2} \rangle^{2} = B V \Gamma(s+1) (kT)^{s} \Phi_{s-3}(z).
\end{equation*}

根据统计数学的定义，不难给出
\begin{equation*}
	\left\langle 2 \sum_{\alpha \neq \beta} n_{\alpha} n_{\beta} + \sum_{\alpha} n_{\alpha} (n_{\alpha} - 1) \right\rangle
	&= \langle N \rangle^{2} - \langle N \rangle + \left\langle (\delta N)^{2} \right\rangle. \\
	\mathrm{Var}\!\left( 2 \sum_{\alpha \neq \beta} n_{\alpha} n_{\beta} + \sum_{\alpha} n_{\alpha} (n_{\alpha} - 1) \right)
	&= 4 (\langle N \rangle^2 - \langle N \rangle) \left\langle (\delta N)^{2} \right\rangle
	+ \left\langle (\delta N)^{4} \right\rangle+(4 \langle N \rangle - 2) \left\langle (\delta N)^{3} \right\rangle
	\\&\qquad- \left\langle (\delta N)^{2} \right\rangle \left( \left\langle (\delta N)^{2} \right\rangle - 1 \right).
\end{equation*}
