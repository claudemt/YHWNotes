...
def main():
    ...
    with figure_style():
        ...
        l1=ax.legend(handles=[Line2D([0],[0],color="black",ls="-",label="Bethe"),Line2D([0],[0],color="black",ls="--",label="B--W")],loc="upper right"); ax.add_artist(l1)
        add_legend(ax, outside=False, loc="lower left"); ax.set(xlabel=r"$kT/J$",ylabel=r"$m$",xlim=(0,5.5),ylim=(0,1.03)); polish_axes(ax)
