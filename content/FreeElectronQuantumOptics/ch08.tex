...
\begin{equation*}
 ...
 v_0=\frac{c^2p_0}{E_0},
\end{equation*}
纵向二阶色散质量固定写为 $m_\parallel=\gamma_0^3m_{\mathrm e}$。对选中的交换角频率 $\omega$，理想能量梯用 $|\ell\rangle$ 表示，$\ell\in\mathbb Z$ 是电子相对于中心通道交换的净量子数；真实通道能量始终写 $\mathcal E_\ell=E(p_\ell)$，只有在均匀梯近似成立后才写 $E_\ell^{(0)}=E_0+\ell\hbar\omega$。经典场作用在整个路径上积累的无量纲耦合记为 $\beta$。它将由沿轨迹的场积分确定；$|\beta|$ 控制边带分布的宽度，$\arg\beta$ 控制各边带振幅的相位。光场量子化后的耦合与算符将在量子交换一节引入。
...
\end{equation*}
参数化外场，其中 $\phi$ 与 $\mathbf A$ 分别是标势和矢势；电子电荷为 $q_{\mathrm e}=-e$。最小耦合 Dirac 哈密顿量由\eqnrefs{eq:dirac-hamiltonian-em} 定义。给定这一哈密顿量，在共同稠密定义域与通常保证非自治演化存在的连续性条件下，定义唯一幺正传播子 $\hat U_A(t_f,t_i)$，满足
\begin{equation}
\boxed{
...
为了区分自由传播与外场作用，写
\begin{equation*}
\hat H_0=c\boldsymbol\alpha\cdot\hat{\mathbf p}+\beta_Dm_{\mathrm e} c^2,
\qquad
\hat V_A(t)=q_{\mathrm e}\phi(\hat{\mathbf r},t)-q_{\mathrm e} c\boldsymbol\alpha\cdot\mathbf A(\hat{\mathbf r},t),
\end{equation*}
...
=
\ee^{+\ii q_{\mathrm e}\chi(\mathbf r,t)/\hbar}
\Psi(\mathbf r,t).
\]
对变换后的态求时间导数：
\begin{align*}
...
\[
E(\mathbf p)=\sqrt{m_{\mathrm e}^2c^4+c^2\mathbf p^2}
\]
...
其中自由正能旋量满足
\[
\left(c\boldsymbol\alpha\cdot\mathbf p_0+\beta_Dm_{\mathrm e} c^2\right)u_0=E_0u_0,
\qquad u_0^\dagger u_0=1.
\]
对载波求导时
\begin{align*}
\ii\hbar\partial_t(f_{\mathrm e}\ee^{\ii(\mathbf p_0\cdot\mathbf r-E_0t)/\hbar})
&=\left(E_0f_{\mathrm e}+\ii\hbar\partial_tf_{\mathrm e}\right)
\ee^{\ii(\mathbf p_0\cdot\mathbf r-E_0t)/\hbar},\\
\hat{\mathbf p}(f_{\mathrm e}\ee^{\ii(\mathbf p_0\cdot\mathbf r-E_0t)/\hbar})
&=\left(\mathbf p_0f_{\mathrm e}-\ii\hbar\nabla f_{\mathrm e}\right)
\ee^{\ii(\mathbf p_0\cdot\mathbf r-E_0t)/\hbar}.
\end{align*}
代入
\[
\ii\hbar\partial_t\Psi=
\left[c\boldsymbol\alpha\cdot(\hat{\mathbf p}-q_{\mathrm e}\mathbf A)
...
+q_{\mathrm e}(\phi-c\boldsymbol\alpha\cdot\mathbf A)u_0f_{\mathrm e}
+\mathcal R_D.
\]
...
=q_{\mathrm e}(\phi-\mathbf v_0\cdot\mathbf A)f_{\mathrm e}
...
=q_{\mathrm e}[\phi(\mathbf r_e,t)-\mathbf v_0\cdot\mathbf A(\mathbf r_e,t)]f_{\mathrm e}.}
...
\begin{equation*}
\frac{\dd}{\dd t}f_{\mathrm e}[\mathbf r_e(t),t]
...
=-q_{\mathrm e} v_0A_z(z,t)f_{\mathrm e}+O(\epsilon_{\rm eik}).}
...
$c_\ell^{(n)}(t_f)\to\mathcal A_{\ell|n}(g_{\mathrm Q})$，\eqnrefs{eq:qpinem_finite_recoil_number_forward_dp46} 随即退化为\secref{subsec:single-mode-number-instrument-dp19}使用的条件响应核。
