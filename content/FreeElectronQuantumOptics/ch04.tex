...
\input{sections/02math-photon-polarization-si}
\input{sections/02-lsz-dirac-photon}
\input{sections/04qed-covariant-gauge-derivations}
...
\begin{figure}[htbp]
 \centering
 \input{figures/tikz/qed-tree-atlas-dp7}
...
\begin{figure}[htbp]
 \centering
 \input{figures/tikz/qed-loop-atlas-dp7}
...
\input{sections/02si-reduced-rules}
\input{sections/02a3-diagram-combinatorics}
\input{sections/02a-spin-polarization-algebra}
\input{sections/02a-helicity-projectors-dp9}

\input{sections/04qed-feynman-foundations-dp7}
\input{sections/04qed-feynman-rules-derivations}
...
\input{sections/02b-yukawa-scattering}
\input{sections/02b2-yukawa-decays}

Yukawa 散射与 QED 角分布采用同一套国际单位制运动学约定；耦合常数可以无量纲化，但四动量、质量壳、相空间和截面仍显式保留 $\hbar$ 与 $c$。

\input{sections/04qed-yukawa-extra-detail-dp21}
...
\end{equation}
所以 Coulomb 势由完整单光子交换在 $|q^0|/|\mathbf q|\ll1$ 与 $|\mathbf p|/(m_{\mathrm e} c)\ll1$ 的静态非相对论极限中匹配得到；其 $1/r$ 形式继承自质量为零的光子传播子。

\begin{figure}[htbp]
 \centering
 \input{figures/tikz/qed-exchange}
...
\input{sections/02c-qed-fermion-scattering}
\input{sections/02c2-qed-observables}
\input{sections/04qed-charged-scattering-derivations}
...
\input{sections/02d-photon-scattering-loops}
\input{sections/04qed-compton-ward-worked-dp64}
\input{sections/02d2-photon-cross-sections}
\input{sections/02d3-loop-calculation}
\input{sections/04qed-spectral-cuts-lsz-dp9}
\input{sections/04qed-ward-takahashi}
\input{sections/04qed-one-loop-ward-renormalization}
\input{sections/02d4-vertex-schwinger-si}
\input{sections/02e-landau-advanced-qed}
\input{sections/04qed-soft-inclusive-dp10}
\input{sections/04qed-ward-takahashi-derivations-dp43}
\input{sections/04qed-one-loop-ward-renormalization-derivations-dp43}
\input{sections/04qed-soft-inclusive-derivations-dp43}
\input{sections/04qed-photon-loop-derivations}
...
\input{sections/04-macroscopic-qed-reservoir}
...
\[
 \hat H_{jA}
 =-q_{\mathrm e} c\int\dd^3 r:\!\hat{\bar\psi}(\mathbf r)
...
\input{sections/04qed-macroscopic-green-derivations}
