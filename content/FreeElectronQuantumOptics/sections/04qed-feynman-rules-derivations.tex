...
\begin{equation*}
 ...
 =\sigma_h\left(
 \frac{|\mathbf p|}{\me c},
 \frac{E_{\mathbf p}}{\me c^2}\widehat{\mathbf p}
 \right).}
 \end{equation*}
逐项检查：
\begin{align*}
 p\!\cdot s_h
 &=\frac{E_{\mathbf p}}{c}\frac{\sigma_h|\mathbf p|}{\me c}
 -\mathbf p\cdot
 \frac{\sigma_hE_{\mathbf p}}{\me c^2}\widehat{\mathbf p}
 =0,\\
 s_h^2
 &=\frac{|\mathbf p|^2}{\me^2c^2}
 -\frac{E_{\mathbf p}^2}{\me^2c^4}
 =-1,
\end{align*}
其中最后一步用了 $E_{\mathbf p}^2=\me^2c^4+c^2|\mathbf p|^2$。所以它确实是 \eqnrefs{eq:polarized-spinor-projector-r9} 所要求的单位类空自旋四矢量。
...
$\sum_hu_h\bar u_h=\slashed p+\me c$。因此未偏振自旋求和就是两个螺旋度投影算符的完备和。
...
\begin{equation*}
 ...
 \right\rangle .
 \end{equation*}
把\eqnrefs{eq:qed-gell-mann-low-dp7} 的分子展开到 $\qe$ 一阶，连通部分为
\begin{align*}
 G_{3,c}^{\mu,(1)}(x,z;y)
 ={}&-\frac{\ii \qe}{\hbar}
 \int\dd^4 w\,
...
\begin{equation*}
 -\frac{\ii \qe}{\hbar}\gamma^\nu,
 \end{equation*}
而把统一的 SI 场归一化常数提出后，约化规则变成前面定义的
\begin{equation*}
 {-\ii g_e\gamma^\nu,\qquad
 g_e=\frac{\qe}{\sqrt{\varepsilon_0\hbar c}}.}
 \end{equation*}
