...
\node[draw,rounded corners,right=of act,align=center] (vev) {choose Higgs vacuum\\diagonalize mass matrices};
...
\draw[->] (act)--node[line label,midway,above,font=\scriptsize]{EWSB + basis change}(vev);
\draw[->] (vev)--(phys);
\draw[->] (vev)--(gf);
\draw[->] (gf)--(unphys);
\draw[->] (phys)--(rules);
