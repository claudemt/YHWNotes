...
\input{sections/04c-sm-gauge-transformation-detail-dp11}
...
\begin{figure}[htbp]
\centering
\begin{tikzpicture}[>=Latex,line width=0.8pt,line label/.style={fill=white,fill opacity=.96,text opacity=1,inner sep=1.2pt}]
\node[draw,rounded corners,align=center,font=\small,text width=3.3cm,minimum height=1.3cm] (rep) at (-3.2, 2.1) {chiral representations\\$Q_L,L_L,u_R,d_R,e_R$};
\node[draw,rounded corners,align=center,font=\small,text width=3.3cm,minimum height=1.3cm] (mass) at ( 3.2, 2.1) {bare Dirac mass\\not a gauge singlet};
\node[draw,rounded corners,align=center,font=\small,text width=3.3cm,minimum height=1.3cm] (h)    at ( 3.2,-2.1) {add $\Phi:(\mathbf1,\mathbf2)_{1/2}$};
\node[draw,rounded corners,align=center,font=\small,text width=3.3cm,minimum height=1.3cm] (y)    at (-3.2,-2.1) {gauge-invariant\\Yukawa operators};
\draw[->] (rep)--node[line label,midway,above=5pt,font=\scriptsize]{compare $L/R$ reps}(mass);
\draw[->] (mass)--node[line label,midway,right=5pt,font=\scriptsize]{missing index}(h);
\draw[->] (h)--node[line label,midway,below=5pt,font=\scriptsize]{contract reps}(y);
\draw[->] (rep)--node[line label,midway,left=5pt,font=\scriptsize]{same reps}(y);
\end{tikzpicture}
...
\input{sections/04c-sm-operator-classification-detail-dp11}
