...
\draw[->,line width=1.0pt] (-4.5,1.45)--(3.8,1.45) node[process line label,midway,above,font=\small] {$C_+$: forward branch};
\draw[->,line width=1.0pt] (3.8,-0.05)--(-4.5,-0.05) node[process line label,midway,below,font=\small] {$C_-$: backward branch};
...
\centering
\begin{tikzpicture}[x=1cm,y=1cm,>=Stealth]
\node[draw,rounded corners,text width=2.8cm,minimum height=1.25cm,align=center] (J) at (-5.3,1.25)
{transition current\\$\mathbf j(\mathbf r,\omega)$};
\node[draw,rounded corners,text width=2.8cm,minimum height=1.25cm,align=center] (M) at (-5.3,-1.25)
{material + geometry\\Maxwell operator};
\node[draw,rounded corners,text width=3.0cm,minimum height=1.25cm,align=center] (G) at (-1.2,-1.25)
{retarded resolvent\\$\mathbf G^R(\mathbf r,\mathbf r';\omega)$};
\node[draw,rounded corners,text width=2.7cm,minimum height=1.15cm,align=center] (E) at (2.4,1.25)
{coherent field\\$\widetilde{\mathbf E}=\ii\mu_0\omega\,\mathbf G^R\!\ast\mathbf j$};
\node[draw,rounded corners,text width=2.7cm,minimum height=1.15cm,align=center] (R) at (2.4,-1.25)
{environment spectrum\\$\operatorname{Im}\mathbf G^R$};
\node[draw,rounded corners,text width=3.1cm,minimum height=1.45cm,align=center] (O) at (6.1,0)
{observables\\EELS / emission / LDOS\\Lamb shift / mode coupling};
\draw[->] (M.east)--(G.west) node[process line label,midway,below=3pt,font=\small] {invert};
\draw[->] (G.north east)--(E.south west);
\draw[->] (J.east)--(E.west) node[process line label,midway,above=3pt,font=\small] {source};
\draw[->] (G.east)--(R.west);
\draw[->] (E.east)--(O.north west);
\draw[->] (R.east)--(O.south west);
\end{tikzpicture}
...
\draw[dashed,line width=0.9pt] (4.2,-0.8)--(4.2,2.0);
\node[fvertex] at (l2) {}; \node[fvertex] at (r2) {};
\node[font=\small] at (4.2,-0.35) {$\sum_n \int \!\d\Phi_n\,\mathcal M_{in}\mathcal M_{in}^*$};
\end{tikzpicture}
