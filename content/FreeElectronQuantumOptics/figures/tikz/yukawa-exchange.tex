\begin{tikzpicture}[x=1cm,y=1cm,>=Latex,baseline,line label/.style={fill=white,fill opacity=.96,text opacity=1,inner sep=1.2pt}]
  ...
  \draw[thick,->] (L1) -- (V1) node[line label,midway,above left] {$p_1$};
  \draw[thick,->] (V1) -- (R1) node[line label,midway,above right] {$p_1'$};
  \draw[thick,->] (L2) -- (V2) node[line label,midway,below left] {$p_2$};
  \draw[thick,->] (V2) -- (R2) node[line label,midway,below right] {$p_2'$};
  \draw[thick,dashed] (V1) -- (V2) node[line label,midway,above right] {$q$};
