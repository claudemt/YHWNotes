...
\input{sections/02qft-oneparticle-causality}
...
\input{sections/02qft-oneparticle-causality-detail-dp18}
...
\begin{figure}[htbp]
\centering
\input{figures/tikz/feynman-poles}
...
\input{sections/03qft-harmonic-kernel-dp64}
...
\input{sections/03qft-gaussian-to-diagrams-explicit}
...
\input{sections/03qft-gaussian-to-diagrams-detail-dp18}
...
\input{sections/02a0-exact-scattering}
...
\input{sections/03qft-fermionic-wick-dp8}
...
\begin{figure}[htbp]
 ...
 \begin{tikzpicture}[line width=.8pt,line label/.style={fill=white,fill opacity=.96,text opacity=1,inner sep=1.15pt}]
  ...
  \fill (v) circle (2pt) node[above right]{$z$};
 \end{tikzpicture}
...
\input{sections/03qft-lsz-worked-example-dp64}
\input{sections/03qft-wick-lsz-detail-dp18}
...
\input{sections/02a-scattering-kinematics}
\input{sections/03-scattering-kinematics-derivations}

\subsection{交叉与色散}
同一个截肢核在不同外态指派下产生交叉过程；解析性又把不同能区的实部、虚部和阈值连接起来。因而交叉不是“换符号口诀”，色散关系也不是独立技巧，而是同一复解析振幅的两种读取方式。
\input{sections/03qft-crossing-dispersion-dp9}

\subsection{标量散射实验室}
最简单的标量相互作用已经足以同时产生图、振幅、衰变率、截面与静态势，并且不引入自旋和规范自由度。它因此隔离出散射论本身的运动学、相空间与解析结构；自旋和规范代数只在更复杂的费米子与光子模型中加入。
\input{sections/01si-scalar-reduced-rules}
\input{sections/02a1-scalar-toy-full}
...
\input{sections/01-exact-correlators-spectral}
...
\input{sections/03qft-unitarity-cuts-dp8}
\input{sections/03-unitarity-cuts-derivations}
...
\input{sections/03qft-renormalization-rg-dp10}
\input{sections/03qft-renorm-allorders-ope-dp11}
\input{sections/03qft-heat-kernel-dp11}

\input{sections/03qft-renorm-allorders-detail-dp18}
\input{sections/03qft-heat-kernel-detail-dp18}
\input{sections/03qft-renormalization-detail-dp21}
...
\input{sections/03qft-gauge-brst-dp11}
...
\input{sections/03qft-ssb-higgs-general-dp11}
