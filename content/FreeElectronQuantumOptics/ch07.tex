...
\input{sections/electron-channel-coherence-worked}
...
\input{sections/03-qnm-residue-normalization}
\input{sections/03-continuum-branchcut-normalization}
...
\end{equation}
其中 $E(\mathbf p)=\sqrt{\me^2c^4+c^2\mathbf p^2}$ 是完整正能色散，$\omega_\lambda>0$ 是模式角频率。这个模型的普适范围是“谐振环境、线性正常坐标耦合、正能单电子扇区”；若环境本身非谐、耦合显著非线性，或负能/多电子过程不可忽略，就应回到更一般的块方程或场论描述，而不是重新定义一个有效 $\mathcal G$ 来吸收缺失的自由度。
...
$E(\mathbf p)=\sqrt{\me^2c^4+c^2\mathbf p^2}$；低反冲同步关系必须在此之后作为受控展开取得。
...
\]
若电子主要沿 $z$ 传播，定义入射 Lorentz 因子 $\gamma_{{\rm L},i}\equiv E_i/(\me c^2)$，并记 $\mathbf K\equiv\boldsymbol\kappa+\mathbf G$，则
\[
\mathbf K^{\mathsf T}H_E\mathbf K
=\frac{K_\perp^2}{m_\perp}+\frac{K_z^2}{m_\parallel},
\qquad
m_\perp=\gamma_{{\rm L},i}\me,
\quad
m_\parallel=\gamma_{{\rm L},i}^3\me.
\]
...
\input{sections/electron-finite-window-worked}
...
\begin{equation*}
\hat h_D
=c\boldsymbol\alpha\cdot[-\ii\hbar\boldsymbol\nabla-\qe\mathbf A(\mathbf r)]
+\beta\me c^2+\qe\phi(\mathbf r)
\end{equation*}
中有两个定态
\begin{equation*}
...
\begin{equation*}
\rho_{fi}(\mathbf r,t)
=\qe\Psi_f^\dagger\Psi_i,
\qquad
\mathbf j_{fi}(\mathbf r,t)
=\qe c\Psi_f^\dagger\boldsymbol\alpha\Psi_i.
\end{equation*}
由两组 Dirac 方程
...
\begin{equation*}
\epsilon_p\equiv\frac{\Delta p}{\max(p_0,\me c)},
\qquad
...
=\qe\mathbf v_0\,|\varphi(\mathbf r-\mathbf r_e(t))|^2
+O(\epsilon_p,\epsilon_{\rm disp}),
\qquad
\mathbf J_{\rm wp}\Longrightarrow\qe\mathbf v_0\,\delta^{(3)}[\mathbf r-\mathbf r_e(t)]
\quad(\epsilon_x\to0).}
...
\begin{equation*}
\mathbf J_e(\mathbf r,t)=\qe\mathbf v(t)\delta^{(3)}[\mathbf r-\mathbf r_e(t)].
\end{equation*}
若再取匀速 $\mathbf v(t)=\mathbf v_0$，空间积分可以完全执行，得到 EELS/辐射问题反复使用的轨迹投影式
\begin{equation}
\boxed{
\frac{\dd P}{\dd\omega}
=\frac{\mu_0\qe^2}{\pi\hbar}
\int\!\dd t\,\dd t'\,
\ee^{\ii\omega(t-t')}
\mathbf v_0\cdot
...
\input{sections/03-green-loss-cumulants-dp84}
...
Dirac 电荷和空间电流分别为
\[
\rho=\qe\Psi^\dagger\Psi,
\qquad
\mathbf J=\qe c\,\Psi^\dagger\boldsymbol\alpha\Psi.
\]
正能自由自旋子满足速度恒等式
\[
...
\[
\rho(\mathbf r,t)
=\qe|\varphi(\mathbf r-\mathbf r_e(t))|^2
+O(\epsilon_p,\epsilon_{\rm disp}),
\]
\[
\mathbf J(\mathbf r,t)
...
\end{derivation}
\input{sections/03-green-loss-cumulants-derivation-dp88}
...
则点电荷源
\begin{equation*}
\rho_{\rm ch}(\mathbf r,t)=\qe\delta^{(3)}(\mathbf r-\mathbf vt),
\qquad
...
\begin{equation*}
\boxed{
\rho_{\rm ch}(\boldsymbol\kappa,\omega)=2\pi\qe\delta(\omega-\boldsymbol\kappa\cdot\mathbf v),
\qquad
\mathbf J_{\rm ch}(\boldsymbol\kappa,\omega)=2\pi\qe\mathbf v\delta(\omega-\boldsymbol\kappa\cdot\mathbf v).}
\end{equation*}
...
\delta h_\lambda
=\qe\phi_\lambda
-\qe c\,\boldsymbol\alpha\cdot\mathbf A_\lambda.
\end{equation*}
...
单模频率隔离之外，模式的空间 Fourier 谱还必须选出一个主导纵向动量交换 $\hbar \kappa_z$；若多个 $\kappa_z$ 分量具有可比权重，就得到带有多种跳跃程的动量图，而不是一维相邻格点链。若材料吸收使模式严重展宽、多个共振在 $1/T$ 内重叠，或连续谱本身不可忽略，则不应定义单个 $\hat a$ 和单个 $\gq$，而应继续使用多模振幅层级或 Green 张量表示。
...
\end{align*}
满足 $|I_2|\le T/\Sigma+2/\Sigma^2$，乘上两个顶点后得到相应的 $|\gq|^2T/\Sigma$ 虚过程尺度。单模谱投影的传播子界与有限时间泄漏由同一个 Duhamel 恒等式控制，并与 RWA 的时间平均条件保持独立。
\end{derivation}
\subsection{反冲格振幅链}
...
并保留每个通道的真实相对论自由能量
\begin{equation*}
\boxed{\mathcal E_\ell=\sqrt{\me^2c^4+c^2p_\ell^2}.}
\end{equation*}
相邻格点的真实能量间隔为
\[
...
\begin{equation}
\boxed{
\gq=-\ii\int_{t_i}^{t_f}\dd t\,\mathcal G_0(t)\ee^{\ii\Delta_0t},}
...
则
\begin{equation*}
 \beta=\alpha_0\gq+\delta\beta,
 \qquad
 |\delta\beta|\le\epsilon_\alpha|\alpha_0|A_w.
\end{equation*}
若相位匹配因子 $C_w\equiv|\int w\,\dd t|/A_w$ 不接近零，则相对误差还满足 $|\delta\beta|/|\alpha_0\gq|\le\epsilon_\alpha/C_w$。因此 $\beta=\alpha_0\gq$ 是包络缓变且不存在强相消时的受控首项，而不是一般恒等式。
