...
\[
U_p\equiv\frac{\qe^2E_0^2}{4\me\omega^2},
\]
并得到有效驻波势
\begin{equation}
\boxed{
...
\[
\omega_{\rm KD}\equiv\frac{\hbar\kappa_{\rm L}^2}{2\me},
\]
...
\boxed{
\ii\hbar\dot c_n
=\frac{(p_x+n\hbar\kappa_{\rm KD})^2}{2\me}c_n
...
\[
E_{\rm KD}\equiv\frac{\hbar^2\kappa_{\rm L}^2}{2\me}.
\]
...
\[
\hat H=\frac{\hat{\mathbf p}^{\,2}}{2\me}
-\frac{\qe}{2\me}\bigl(\hat{\mathbf p}\!\cdot\!\mathbf A+\mathbf A\!\cdot\!\hat{\mathbf p}\bigr)
+\frac{\qe^2A^2}{2\me}.
\]
...
于是
\[
\frac{\qe^2}{2\me}\langle A_y^2\rangle_t
=\frac{\qe^2A_0^2}{4\me}\cos^2(\kappa_{\rm L}x)
=\frac{\qe^2E_0^2}{4\me\omega^2}\cos^2(\kappa_{\rm L}x),
\]
...
\[
\Delta E_n
=\frac{(n\hbar\kappa_{\rm KD})^2}{2\me}
...
=\left[-\frac{\hbar^2}{2\me}\partial_x^2+
...
=\sqrt{\me^2c^4+c^2(p_0+\ell\hbar \kappa_z)^2}.
\end{equation*}
...
=\frac{\hbar \kappa_z^2}{2\gamma_0^3\me}.}
...
E(p_0+\delta p)
=E_0+v_0\delta p
+\frac{(\delta p)^2}{2\gamma_0^3\me}
-\frac{v_0(\delta p)^3}{2\gamma_0^4\me^2c^2}
+O((\delta p)^4).
...
\end{equation}
其中 $g_c$ 是频率量纲的腔模耦合率，而\eqnrefs{eq:qpinem_S} 中的 $\gq$ 是有限作用时间积分得到的无量纲单程参数。按上述两种 Hamiltonian 的相位约定，若同一投影交换矩阵元在长度为 $T$ 的窗口内严格常量且处于共振，则 $g_c=\ii\upsilon$，从而 $\gq=-\ii g_cT$，特别有 $|\gq|=|g_c|T$；若耦合包络或失谐随时间变化，则必须保留 $\gq=\int\dd t\,\upsilon(t)$，不能用 $g_cT$ 代替。
...
\qquad E'(p_0)=v_0.
\end{equation*}
利用 $E_0=\gamma_0\me c^2$，
\begin{align*}
E''(p)
&=\frac{c^2}{E}-\frac{p^2c^4}{E^3}
=\frac{\me^2c^6}{E^3},\\
E''(p_0)&=\frac{1}{\gamma_0^3\me},
\end{align*}
而
\begin{align*}
E'''(p)
