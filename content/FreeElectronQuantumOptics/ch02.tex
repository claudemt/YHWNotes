...
\end{equation}
这里 $s=0,\frac12,1,\ldots$ 是相对论意义下的粒子自旋。电子对应 $s=1/2$。因此寻找电子的局域波动方程，可以表述为：寻找一个局域场表示，使其一粒子激发落在 $P^2=\me^2c^2$ 且 $W^2=-3\me^2c^2\hbar^2/4$ 的 Poincar\'e 不可约表示中。
...
\begin{equation*}
 \sigma^\mu=(\id_2,\boldsymbol\sigma_{\rm P}),
 \qquad
 \bar\sigma^\mu=(\id_2,-\boldsymbol\sigma_{\rm P}),
\end{equation*}
它们满足
\begin{equation}
 \sigma^\mu\bar\sigma^\nu+\sigma^\nu\bar\sigma^\mu
...
\begin{equation*}
 p^2=\mu_L\mu_R.
\end{equation*}
对质量为 $\me$ 的粒子，质量壳是 $p^2=\me^2c^2$。要求自由二次作用量厄米给出 $\mu_R=\mu_L^*$；再用左右手场之间的相对常数相位重定义，可把共同质量参数取成正实数 $\me c$。于是得到
\begin{equation}
 \boxed{
 (p_\mu\sigma^\mu)\psi_R=\me c\,\psi_L,
 \qquad
 (p_\mu\bar\sigma^\mu)\psi_L=\me c\,\psi_R.}
...
\begin{equation}
 p^0=\me c\cosh\zeta,
 \qquad
 \mathbf p=\me c\,\hat{\mathbf n}\sinh\zeta,
 \label{eq:rapidity-momentum-r7}
\end{equation}
\eqnrefs{eq:weyl-boost-r7} 可作为这两条方程的显式一致性检验，而不是一般动力学的来源。$\me\neq0$ 时左右手 Weyl 表示由质量项耦合；$\me=0$ 时两式解耦。把两式合并得到四分量动量空间方程
\begin{equation}
 \begin{pmatrix}
 -\me c\,\id_2 & p_\mu\sigma^\mu\\
 p_\mu\bar\sigma^\mu & -\me c\,\id_2
 \end{pmatrix}
...
\begin{equation}
 \boxed{
 \left(\ii\hbar\gamma^\mu\partial_\mu-\me c\right)\psi(x)=0.}
...
特别地，
\begin{equation*}
 \sld\equiv\gamma^\mu\partial_\mu,
 \qquad
 \slashed A\equiv\gamma^\mu A_\mu,
 \qquad
 \slD\equiv\gamma^\mu D_\mu,
 \qquad
 \slp\equiv\gamma^\mu p_\mu.
\end{equation*}
斜杠把 Lorentz 指标收缩与旋量矩阵结构写在同一个符号中，例如自由 Dirac 算符就是 $\ii\hbar\sld-\me c$。
...
\begin{equation*}
\psi'(x)=\exp\!\left(\frac{i \qe}{\hbar}\chi(x)\right)\psi(x),
\qquad \qe=-e,
\end{equation*}
其中 $\chi$ 是任意实函数。要求导数项与 $\psi$ 具有相同的局域相位变换律，就必须定义
\begin{equation}
\boxed{D_\mu\equiv\partial_\mu+\frac{i \qe}{\hbar}A_\mu,}
...
\end{equation}
于是 $D'_\mu\psi'=\ee^{\ii\qe\chi/\hbar}D_\mu\psi$。这就是“协变”二字的具体含义：$D_\mu\psi$ 与 $\psi$ 属于同一个局域表示，因此可以把自由 Dirac 项中的 $\partial_\mu$ 替换为 $D_\mu$ 而不破坏局域 $U(1)$ 对称性。
...
\qquad
U_\chi(x)=\ee^{\ii\qe\chi(x)/\hbar}.
\]
若局域联络仍取一阶形式
\[
D'_\mu=\partial_\mu+\frac{\ii\qe}{\hbar}A'_\mu,
\]
则作用在任意 $\psi$ 上给
\begin{align*}
D'_\mu(U_\chi\psi)
={}&U_\chi\partial_\mu\psi
+\frac{\ii\qe}{\hbar}U_\chi(\partial_\mu\chi)\psi
+\frac{\ii\qe}{\hbar}A'_\mu U_\chi\psi,\\
U_\chi D_\mu\psi
={}&U_\chi\partial_\mu\psi
+\frac{\ii\qe}{\hbar}U_\chi A_\mu\psi.
\end{align*}
...
局域 $U(1)$ 已经确定带电 Dirac 场必须通过 $D_\mu=\partial_\mu+\ii\qe A_\mu/\hbar$ 与四势耦合。由同一作用量变分，可以同时得到物质方程、规范场方程和守恒流；再给波动算符指定因果边界条件，就得到 Green 张量与 QED 传播子共同使用的逆算符结构。
...
\begin{equation}
 ...
 =\frac1c\int\dd^4 x\,
 \left[
 \bar\psi\left(\ii\hbar c\gamma^\mu D_\mu-\me c^2\right)\psi
 -\frac{1}{4\mu_0}F_{\mu\nu}F^{\mu\nu}
 \right].}
...
\begin{equation}
 \boxed{
 \Lag_{\rm int}
 =-\qe c\,\bar\psi\gamma^\mu\psi A_\mu.}
...
\begin{equation}
 \boxed{
 \left(\ii\hbar c\slD-\me c^2\right)\psi=0,}
 \label{eq:dirac-em}
\end{equation}
以及
\begin{equation}
...
\begin{equation}
 ...
 +\beta_D \me c^2+\qe\phi,
 }
 \label{eq:dirac-hamiltonian-em}
\end{equation}
其中
\begin{equation}
 \hat{\vb{\Pi}}\equiv\hat{\mathbf p}-\qe\mathbf A
...
\begin{equation}
 \boxed{
 \left[
 \hbar^2D_\mu D^\mu+\me^2c^2
 +\frac{\qe\hbar}{2}\sigma^{\mu\nu}F_{\mu\nu}
 \right]\psi=0.
 }
 \label{eq:dirac-squared}
\end{equation}
前三个对象的来源彼此独立而在这一式中汇合：$D_\mu D^\mu$ 是规范协变的二阶平移，$\me^2c^2$ 来自相对论质量壳，$\sigma^{\mu\nu}F_{\mu\nu}$ 则把旋量 Lorentz 生成元与电磁曲率收缩。其低能展开将产生 Pauli 磁矩、Darwin 项和自旋--轨道耦合。
...
\begin{equation}
 \hat H_D=\beta_D\me c^2+\hat{\mathcal V}+\hat{\mathcal O},
 \qquad
 \hat{\mathcal V}\equiv \qe\phi,
 \qquad
...
\begin{equation*}
 \Delta_-
 \equiv2\me c^2-\delta_\star-V_\star.
\end{equation*}
...
\end{equation}
在弱外场且典型动量为 $p_\star$ 时，$\|\hat{\mathcal O}\|\sim cp_\star$、$\Delta_-\simeq2\me c^2$，所以 $\epsilon_{\rm NR}\sim p_\star/(2\me c)$。该比值把正能投影与真实的正负能谱隙联系起来，从而给出消去负能子空间的定量条件。
...
Foldy--Wouthuysen 变换用幺正变换逐阶消去关于 $\beta_D$ 的奇算符。对静态外场展开到 $1/(\me c^2)$ 的相对论低能修正阶，分块对角的偶算符结构为
\begin{equation}
 \boxed{
 H_{\rm FW}
 =\beta_D\me c^2+\hat{\mathcal V}
 +\frac{\beta_D\hat{\mathcal O}^2}{2\me c^2}
 -\frac{\beta_D\hat{\mathcal O}^4}{8\me^3c^6}
 -\frac{1}{8\me^2c^4}[\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]]
 +\cdots.}
...
\begin{equation}
 ...
 \hat{\mathcal O}^2
 =c^2\left(
 \boldsymbol\Pi^2-\qe\hbar\boldsymbol\Sigma\cdot\mathbf B
 \right).}
 \label{eq:fw-O2-r16}
\end{equation}
第一项给出动能，第二项已经包含自旋--磁场耦合。对静态标势 $\hat{\mathcal V}=\qe\phi$，双重对易子进一步给出
\begin{equation}
 \boxed{
 \begin{aligned}
 [\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]]
 ={}&\qe\hbar^2c^2\,\boldsymbol\nabla\cdot\mathbf E\\
 &+\qe\hbar c^2\,
...
\begin{equation}
 ...
 \hat H_{+}
 ={}&\me c^2+\qe\phi
 +\frac{\hat{\boldsymbol\Pi}^2}{2\me}
 -\frac{\qe\hbar}{2\me}\boldsymbol\sigma_{\rm P}\cdot\mathbf B
 -\frac{\left(\hat{\boldsymbol\Pi}^2-\qe\hbar\boldsymbol\sigma_{\rm P}\cdot\mathbf B\right)^2}{8\me^3c^2}\\
 &-\frac{\qe\hbar^2}{8\me^2c^2}\boldsymbol\nabla\cdot\mathbf E
 -\frac{\qe\hbar}{8\me^2c^2}
 ...
 \label{eq:fw-pauli}
\end{equation}
依次可辨认出静质量、标势、非相对论动能、磁矩、相对论动能与弱场联合修正、Darwin 项和自旋--轨道项。这里四次奇算符项保留为完整算符平方；由于 $\hat{\boldsymbol\Pi}^2$ 与位置依赖的 $\mathbf B$ 一般不对易，不能在没有额外弱场计数时把它无条件替换成 $\hat{\boldsymbol\Pi}^4$。若进一步满足 $|\qe|\hbar B_\star\ll p_\star^2$，才可在这一项内舍去磁场交叉项并恢复常见的 $-\hat{\boldsymbol\Pi}^4/(8\me^3c^2)$ 形式。所有这些项仍来自同一个 Dirac Hamiltonian 的分块对角化。
为把上述低能展开写成定量适用条件，令 $p_\star$ 是电子典型动力学动量，$\phi_\star,E_\star,B_\star$ 是外场在作用区的典型幅值，$L_\star$ 与 $\omega_\star$ 分别是其最短空间变化尺度和最高相关角频率，并定义 $\lambda_C\equiv\hbar/(\me c)$。一组足够的无量纲控制量是
\begin{equation}
 \max\!\left\{
 \frac{p_\star}{\me c},
 \frac{|\qe|\,|\phi_\star|}{\me c^2},
 \frac{\hbar\omega_\star}{\me c^2},
 \frac{\lambda_C}{L_\star},
 \frac{|\qe|\hbar B_\star}{\me^2c^2},
 \frac{|\qe|E_\star\lambda_C}{\me c^2}
 \right\}\ll1.
...
由 $\hat\Pi_i=\hat p_i-\qe A_i(\mathbf r,t)$ 与 $[\hat p_i,f(\mathbf r,t)]=-\ii\hbar\partial_i f$，逐项展开
\begin{align*}
 [\hat\Pi_i,\hat\Pi_j]
 &=[\hat p_i-\qe A_i,\hat p_j-\qe A_j]\\
 &=[\hat p_i,\hat p_j]
 -\qe[\hat p_i,A_j]
 -\qe[A_i,\hat p_j]
 +\qe^2[A_i,A_j].
...
\begin{align*}
 [\hat\Pi_i,\hat\Pi_j]
 &=-\qe(-\ii\hbar\partial_iA_j)
 -\qe(+\ii\hbar\partial_jA_i)\\
 &=\ii \qe\hbar(\partial_iA_j-\partial_jA_i).
\end{align*}
...
\begin{equation*}
 (\ii\hbar\slD-\me c)\psi=0.
\end{equation*}
左乘共轭的一阶因子 $\ii\hbar\slD+\me c$：
\begin{align*}
 0
 &=(\ii\hbar\slD+\me c)(\ii\hbar\slD-\me c)\psi\\
...
\begin{align*}
 ...
 (D_\mu D_\nu-D_\nu D_\mu)\\
 &=\frac12\sigma^{\mu\nu}[D_\mu,D_\nu]\\
 &=\frac{\ii \qe}{2\hbar}\sigma^{\mu\nu}F_{\mu\nu}.
\end{align*}
于是
\begin{align*}
 \gamma^\mu\gamma^\nu D_\mu D_\nu
 &=D_\mu D^\mu
 +\frac{\qe}{2\hbar}\sigma^{\mu\nu}F_{\mu\nu}.
\end{align*}
代回二阶方程并整体乘以 $-1$：
\begin{align*}
 0
...
\begin{equation*}
 A_1\equiv\ii S_1=\frac{\beta_D\hat{\mathcal O}}{2\me c^2},
 \qquad U_1=\ee^{A_1}.
\end{equation*}
...
对静质量与奇算符的嵌套对易子逐阶计算，
\begin{align*}
 [A_1,\beta_D\me c^2]&=-\hat{\mathcal O},\\
 [A_1,\hat{\mathcal O}]&=\frac{\beta_D\hat{\mathcal O}^2}{\me c^2},\\
 [A_1,[A_1,\beta_D\me c^2]]&=-\frac{\beta_D\hat{\mathcal O}^2}{\me c^2},\\
 [A_1,[A_1,\hat{\mathcal O}]]&=-\frac{\hat{\mathcal O}^3}{\me^2c^4}.
\end{align*}
...
\begin{equation*}
 ...
 =-\frac{1}{4\me^2c^4}[\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]],
\end{equation*}
乘上 BCH 的 $1/2!$ 后得到
\begin{equation*}
 -\frac{1}{8\me^2c^4}[\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]].
...
\begin{equation*}
 \hat{\mathcal O}_1
 =\frac{\beta_D[\hat{\mathcal O},\hat{\mathcal V}]}{2\me c^2}
 -\frac{\hat{\mathcal O}^3}{3\me^2c^4}+\cdots .
\end{equation*}
再取 $A_2=\beta_D\hat{\mathcal O}_1/(2\me c^2)$，其与静质量的对易子正好给 $-\hat{\mathcal O}_1$，因此这一奇块被下一步变换消去。按 $\|\hat{\mathcal O}\|/(\me c^2)=O(\epsilon)$、$\|\hat{\mathcal V}\|/(\me c^2)=O(\epsilon^2)$ 计数，第二步对偶部分的首个新修正高于当前保留阶数。于是得到正文\eqnrefs{eq:fw-even-operator-form-r16}。
\end{derivation}
...
\begin{equation*}
 \frac{\ii}{2}\epsilon_{ijk}\Sigma_k[\Pi_i,\Pi_j]
 =-\qe\hbar\boldsymbol\Sigma\cdot\mathbf B,
\end{equation*}
...
正文\eqnrefs{eq:dirac-even-odd-decomposition-r16} 中 $\hat{\mathcal V}=\qe\phi$、$\hat{\mathcal O}=c\boldsymbol\alpha\cdot\boldsymbol\Pi$。对静态标势，第一层对易子只依赖 $[\Pi_i,\phi]$；再与 $\hat{\mathcal O}$ 对易一次后，Pauli 代数的对称部分产生 $\nabla\cdot\mathbf E$，反对称部分产生自旋与 $\mathbf E\times\boldsymbol\Pi$ 的耦合。

因为 $\mathbf E=-\boldsymbol\nabla\phi$，
\begin{align*}
 [\hat{\mathcal O},\hat{\mathcal V}]
 &=\qe c\alpha_i[\Pi_i,\phi],\\
 &=\ii \qe\hbar c\,\boldsymbol\alpha\cdot\mathbf E.
\end{align*}
于是
\begin{align*}
 [\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]]
 &=\ii \qe\hbar c^2[\alpha_i\Pi_i,\alpha_jE_j],\\
 &=\ii \qe\hbar c^2
 \left
 \{[\Pi_i,E_i]
...
\begin{equation*}
 [\hat{\mathcal O},[\hat{\mathcal O},\hat{\mathcal V}]]
 =\qe\hbar^2c^2\boldsymbol\nabla\cdot\mathbf E
 +\qe\hbar c^2\boldsymbol\Sigma\cdot
...
=c^2\Bigl(
\hat{\boldsymbol\Pi}^2
-\qe\hbar\boldsymbol\sigma_{\rm P}\cdot\mathbf B
\Bigr)P_+.
\]
...
={}&\Biggl[
\me c^2+\qe\phi
+\frac{\hat{\boldsymbol\Pi}^2-\qe\hbar\boldsymbol\sigma_{\rm P}\cdot\mathbf B}{2\me}\\
&-\frac{\left(\hat{\boldsymbol\Pi}^2-\qe\hbar\boldsymbol\sigma_{\rm P}\cdot\mathbf B\right)^2}{8\me^3c^2}
-\frac{\qe\hbar^2}{8\me^2c^2}\boldsymbol\nabla\cdot\mathbf E\\
&-\frac{\qe\hbar}{8\me^2c^2}
\boldsymbol\sigma_{\rm P}\cdot
...
\epsilon_{B/p}
\equiv
\frac{|\qe|\hbar B_\star}{p_\star^2}\ll1;
\]
...
\begin{equation}
 ...
 p^2=\me^2c^2,
 \qquad
 E_{\mathbf p}=\sqrt{\me^2c^4+c^2\mathbf p^2}.}
...
\begin{equation}
 ...
 \begin{pmatrix}
 \xi_s^{(+)}\\[2mm]
 \dfrac{\boldsymbol\sigma_{\rm P}\cdot\mathbf p}{E/c+\me c}\,\xi_s^{(+)}
 \end{pmatrix},
 \label{eq:explicit-u}
\end{equation}
...
\begin{equation}
 ...
 \sqrt{\frac{E}{c}+\me c}
 \begin{pmatrix}
 \dfrac{\boldsymbol\sigma_{\rm P}\cdot\mathbf p}{E/c+\me c}\,\xi_s^{(-)}\\[2mm]
 \xi_s^{(-)}
 \end{pmatrix},
 \label{eq:explicit-v}
\end{equation}
并采用
$\bar u_su_{s'}=2\me c\,\delta_{ss'}$、
$\bar v_sv_{s'}=-2\me c\,\delta_{ss'}$。
比显式分量更常用的是在任意质量壳四动量上直接构造能量投影算符。由 $\slashed p^{\,2}=\me^2c^2$ 定义
\begin{equation*}
\Lambda_+(p)\equiv\frac{\slashed p+\me c}{2\me c},
\qquad
\Lambda_-(p)\equiv\frac{\me c-\slashed p}{2\me c}.
\end{equation*}
它们满足 $\Lambda_\pm^2=\Lambda_\pm$、$\Lambda_+\Lambda_-=0$ 且迹均为 $2$；$\Lambda_+$ 的像空间正是 $(\slashed p-\me c)u=0$ 的二维正能解空间，$\Lambda_-$ 的像空间正是 $(\slashed p+\me c)v=0$ 的二维负频解空间。结合上面的 Dirac 归一化，得到协变完备关系
\begin{align}
 \boxed{\sum_su_s(p)\bar u_s(p)=\slp+\me c,}
 &\qquad \label{eq:u-complete}\\
 \boxed{\sum_sv_s(p)\bar v_s(p)=\slp-\me c.}
...
记
\begin{equation*}
 A\equiv\sqrt{\frac{E}{c}+\me c},
 \qquad
 \kappa\equiv\frac{p}{E/c+\me c},
 \qquad
 E=\sqrt{\me^2c^4+c^2p^2}.
\end{equation*}
...
\end{equation}
这里小分量与大分量之比恰为 $\kappa$。低速时 $p\ll\me c$，有 $\kappa\simeq p/(2\me c)\ll1$；超相对论时 $E\gg\me c^2$，则 $\kappa\to1$。因此所谓 Dirac 旋量的“大、小分量”不是绝对分类，而由动量尺度控制。

以 $u_\uparrow$ 为例验证归一化。Dirac 表示中 $\gamma^0=\operatorname{diag}(1,1,-1,-1)$，所以
\begin{align*}
 \bar u_\uparrow u_\uparrow
 &=A^2(1-\kappa^2)\\
 &=\left(\frac Ec+\me c\right)
 \left[1-\frac{p^2}{(E/c+\me c)^2}\right]\\
 &=\frac{(E/c+\me c)^2-p^2}{E/c+\me c}\\
 &=2\me c,
\end{align*}
最后一步使用 $E^2/c^2-p^2=\me^2c^2$。类似地
\begin{align*}
...
\end{equation}
且结果与选取 $\uparrow$ 或 $\downarrow$ 无关。这个算例把三个彼此相关的抽象公式连在一起：旋量归一化 $\bar uu=2\me c$、概率/粒子数密度 $u^\dagger u=2E/c$，以及协变四流与四动量同向。散射迹中的 $\bar u\gamma^\mu u$ 使用已经建立的一般协变恒等式；这里的显式分量计算提供对旋量归一化、号规和四流方向的独立核验。
\end{exbox}
...
\begin{equation}
 \boxed{
 m_\perp\equiv\gamma_0\me,
 \qquad
 m_\parallel\equiv\gamma_0^3\me,}
 \qquad
...
\end{equation}
$m_\perp,m_\parallel$ 定义为色散在横向与纵向方向的曲率质量；静质量仍记为 $\me$。其中纵向关系也可写成
\begin{equation}
...
\begin{figure}[htbp]
 ...
 \caption{动能 $K_0=200\,\mathrm{keV}$ 时的精确相对论色散与在 $p_0$ 处的一、二阶展开。横轴为 $\delta p/p_0$，纵轴为 $(E-E_0)/(\me c^2)$；二阶曲线偏离精确曲线的位置直接显示高阶色散开始重要的区域。}
...
\end{equation}
这里 $K^\mu$ 固定描述正频模式本身，$\sigma$ 单独记录能量--动量流向，因此吸收与发射共享同一组 $\omega>0$、$\mathbf k$ 模式参数。初末电子都在同一质量壳 $P_i^2=P_f^2=\me^2c^2$，因此
\begin{equation}
...
\begin{align*}
 p^2&=\me^2c^2,\\
 \frac{E_{\mathbf p}^2}{c^2}-\mathbf p^2&=\me^2c^2,\\
 E_{\mathbf p}&=\sqrt{\me^2c^4+c^2\mathbf p^2},
\end{align*}
其中最后选择正能分支。对正文\eqnrefs{eq:v-dirac} 左乘 $\slp-\me c$ 得到同一二阶条件，因此电子与正电子外线共享同一正能质量壳参数化。
\end{derivation}
...
\[
\chi_s
=\frac{c\boldsymbol\sigma_{\rm P}\cdot\mathbf p}{E+\me c^2}\phi_s
=\frac{\boldsymbol\sigma_{\rm P}\cdot\mathbf p}{E/c+\me c}\phi_s.
\]
取
$\phi_s=\sqrt{E/c+\me c}\,\xi_s^{(+)}$，便得到正文\eqnrefs{eq:explicit-u}。用质量壳 $E^2=\me^2c^4+c^2\mathbf p^2$ 检查归一化：
\begin{align*}
\bar u_su_{s'}
&=\left(\frac Ec+\me c\right)
\left[
1-\frac{\mathbf p^2}{(E/c+\me c)^2}
\right]\delta_{ss'}\\
&=2\me c\,\delta_{ss'}.
\end{align*}
...
\[
\phi'_s
=\frac{\boldsymbol\sigma_{\rm P}\cdot\mathbf p}{E/c+\me c}\chi'_s.
\]
取
$\chi'_s=\sqrt{E/c+\me c}\,\xi_s^{(-)}$，即得到正文\eqnrefs{eq:explicit-v}。同样利用质量壳，
\begin{align*}
\bar v_sv_{s'}
&=\left(\frac Ec+\me c\right)
\left[
\frac{\mathbf p^2}{(E/c+\me c)^2}-1
\right]\delta_{ss'}\\
&=-2\me c\,\delta_{ss'}.
\end{align*}
\end{derivation}
...
质量壳给 $\slashed p^{\,2}=\me^2c^2$，因此
\begin{align*}
\Lambda_+^2
&=\frac{(\slashed p+\me c)^2}{4\me^2c^2}
=\frac{\slashed p+\me c}{2\me c}=\Lambda_+,\\
\Lambda_-^2
&=\frac{(\me c-\slashed p)^2}{4\me^2c^2}
=\Lambda_-.
\end{align*}
并且
\begin{equation*}
(\slashed p-\me c)\Lambda_+=0,
\qquad
(\slashed p+\me c)\Lambda_-=0.
\end{equation*}
由于 $\operatorname{tr}\slashed p=0$，两投影的迹均为 $2$，所以其像空间分别与正能、负频 Dirac 解空间完全相同。由 $\bar u_su_{s'}=2\me c\,\delta_{ss'}$，正能解空间上的恒等投影为
\begin{equation*}
\Lambda_+=\frac1{2\me c}\sum_su_s\bar u_s,
\end{equation*}
故得到\eqnrefs{eq:u-complete}。负频基满足 $\bar v_sv_{s'}=-2\me c\,\delta_{ss'}$，于是
\begin{equation*}
\Lambda_-=-\frac1{2\me c}\sum_sv_s\bar v_s,
\end{equation*}
...
\]
先从精确色散 $E(p)=\sqrt{\me^2c^4+p^2c^2}$ 计算所需导数：
\begin{align*}
E'(p)&=\frac{pc^2}{E(p)},\\
E''(p)&=\frac{\me^2c^6}{E(p)^3},\\
E^{(3)}(p)
&=-\frac{3\me^2c^8p}{E(p)^5}.
\end{align*}
在 $p=p_0$ 处用 $E_0=\gamma_0\me c^2$ 与 $p_0c/E_0=\beta_0$，两项之比为
\begin{align*}
\epsilon_{\rm curv}
