...
\begin{equation*}
\rho(\mathbf r,t)=\qe\delta^{(3)}[\mathbf r-\mathbf r_e(t)],
\qquad
\mathbf J(\mathbf r,t)=\qe\mathbf v(t)\delta^{(3)}[\mathbf r-\mathbf r_e(t)]
\end{equation*}
...
=
\frac{\qe^2}{16\pi^2\varepsilon_0c}
\frac{
\left|
...
\boxed{
P_{\rm rad}
=\frac{\qe^2\gamma_{\rm L}^6}{6\pi\varepsilon_0c^3}
...
=\frac{\qe^2}{4\pi^3\varepsilon_0c}
...
理想导体正入射时，真空侧边界条件可由真实电荷与镜像电荷的两个相干端点实现。令真实电荷以 $+\beta_{\rm rel}\mathbf e_z$ 入射，镜像电荷 $-\qe$ 以 $-\beta_{\rm rel}\mathbf e_z$ 镜像运动，并定义
\begin{equation*}
...
\begin{align*}
\mathcal A_{\rm TR}
&=-\qe\frac{\beta_{\rm rel}\mathbf N}{1-\beta_{\rm rel}\cos\theta}
-\qe\frac{\beta_{\rm rel}\mathbf N}{1+\beta_{\rm rel}\cos\theta}\\
&=-\frac{2\qe\beta_{\rm rel}\mathbf N}{1-\beta_{\rm rel}^2\cos^2\theta}.
\end{align*}
...
P_{\rm rad}
\longrightarrow
\frac{\qe^2a^2}{6\pi\varepsilon_0c^3}.
\]
\end{derivation}
...
\begin{equation*}
\omega_p^2
\equiv\frac{n_0e^2}{\varepsilon_0\me},
\qquad
[\omega_p]={\rm s^{-1}}.
\end{equation*}
...
\]
自由电子电流为 $J_L=\qe n_0u_L$，而极化电流满足 $J_L=-\ii\omega P_L$，所以
\begin{align*}
P_L
&=\frac{\ii}{\omega}\qe n_0u_L\\
&=-\varepsilon_0
\frac{\omega_p^2}
{\omega(\omega+\ii\Gamma_{\rm mat})-v_H^2\kappa^2}E_L.
\end{align*}
...
\qquad
E_f^2=\me^2c^4+c^2\!\left(p_{f,z}^2+p_{f,\perp}^2\right).}
...
\end{equation}
这里 $E_{\omega,z}$ 是同步纵向复电场幅度，单位 ${\rm V\,m^{-1}}$。积分中的 $\qe E_{\omega,z}\dd z$ 是焦耳，除以单光子能量 $\hbar\omega$ 后 $\beta$ 无量纲。指数因子只保留与电子飞行相位 $\omega z/v_0$ 同步的纵向 Fourier 分量，因此 $\beta(\mathbf R)$ 同时携带局域场幅度和相位信息。
