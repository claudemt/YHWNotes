...
\section{光学介质膜}
如\cref{fig:optical_film}，在空气（a端）和基片（g端）之间贴有一张膜，膜由\(N\)层介质堆叠而成.空气的介电常数为\(\varepsilon_a\)，磁导率为\(\mu_a\)，记其特征导纳\(\zeta_a = \sqrt{\varepsilon_a/\mu_a}\).基片的介电常数为\(\varepsilon_g\)，磁导率为\(\mu_g\)，记\(\zeta_g = \sqrt{\varepsilon_g/\mu_g}\).膜的介电常数为\(\varepsilon_m\)，磁导率为\(\mu_m\)，厚度为\(h_m\)，记\(\zeta_m = \sqrt{\varepsilon_m/\mu_m}\).空气、基片、膜在膜平面方向无限延展，可视为一维问题.
...
对于a端和g端分析得：
\begin{align*}
	E_s^a = E_s^i + E_s^r,\quad H_\tau^t = \zeta_g \cos\theta_g E_s^t,\quad H_\tau^a = \zeta_a \cos\theta_a (E_s^i - E_s^r).
\end{align*}
\begin{align*}
	H_s^a = \zeta_a (E_p^i + E_p^r),\quad E_\tau^t = \zeta_g^{-1} \cos\theta_g H_s^t = E_p^t \cos\theta_g,\quad E_\tau^a = \cos\theta_a (E_p^i - E_p^r).
\end{align*}
...
\begin{align*}
	|r|^2+\dfrac{\zeta_g|t|^2\cos\theta_g}{\zeta_a \cos\theta_a}=1.
\end{align*}
...
具体考虑如\cref{fig:elastic_film}的介质膜.在入射端\(a\)和出射端\(g\)之间有\(N\)层无穷大平板介质膜，第\(m\)层介质的Lame系数为\(\lambda_m, \mu_m\)，厚度为\(h_m\)，质量密度为 \(\rho_s \eta_m\)（\(\rho_s\)是用于无量纲化的标准质量密度），\(a\)和\(g\)介质的Lame系数为\(\lambda_a, \mu_a\)和\(\lambda_g, \mu_g\)，质量密度为 \(\rho_s \eta_a, \rho_s \eta_g\)，各介质的 \(k, c_S, c_P\) 参量定义同上，角频率为 \(\omega\)的弹性波从\(a\)端入射.
...
\begin{align*}
	S_{zP} = \dfrac{1}{2} \rho c_P \omega^2 k_P^2 |\phi|^2 \cos\theta_P = \dfrac{1}{2} \rho_s \eta \omega^4 \dfrac{|\phi|^2}{c_P} \cos\theta_P, \quad S_{zS} = \dfrac{1}{2} \rho_s \eta \omega^4 \dfrac{|\vb{\psi}|^2}{c_S} \cos\theta_S.
\end{align*}
...
\begin{align*}
	S_{zS} = \dfrac{1}{2} \rho c_S \omega^2 k_S^2 |\psi|^2 \cos\theta_S = \dfrac{1}{2} \zeta \omega^2 k_S^2 |\psi|^2 \cos\theta_S.
\end{align*}
...
\begin{exbox}
	...
	\begin{align*}
		\begin{pmatrix}
			1  & \dfrac{\eta_g \zeta_{Pa}}{\eta_a \zeta_{Pg}} & \tan\theta_{Pa} \\[8pt]
...
\begin{figure}[H]
	...
	\begin{subfigure}[b]{0.48\textwidth}
		\includegraphics[width=\textwidth]{optical_dielectric_film.png}
		\caption{光学介质膜}
		\label{fig:thin_film_optical}
		\label{fig:optical_film}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.48\textwidth}
		\includegraphics[width=\textwidth]{elastic_dielectric_film.png}
		\caption{弹性介质膜}
		\label{fig:thin_film_elastic}
		\label{fig:elastic_film}
	\end{subfigure}
	\caption{介质膜示意图}
	\label{fig:thin_film}
\end{figure}