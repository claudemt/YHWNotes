...
% Jacobi elliptic-function operators (\sn, \cn, \dn) used only in this chapter.
% Use \newcommand+\operatorname (not \DeclareMathOperator) because this is in the body.
...
如\cref{fig:euler_angles}，定义欧拉角\(\theta\)，\(\varphi\)，\(\psi\)，使得\(x_1\)-\(x_2\)-\(x_3\)坐标系可以由\(x\)-\(y\)-\(z\)坐标系通过以下操作得到：
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.3\textwidth]{euler_angles_schematic_diagram.jpg}
	\caption{欧拉角示意图}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.4\textwidth]{fixed_point_rigid_body_rotation_example_schematic_diagram.png}
	\caption{刚体定点转动例题示意图}
	\label{fig:rigid_body_fixed_rotation_example}
\end{figure}
