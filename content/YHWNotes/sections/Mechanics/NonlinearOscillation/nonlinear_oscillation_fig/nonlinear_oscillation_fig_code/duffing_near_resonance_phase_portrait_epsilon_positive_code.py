...
def main():
    ...
    with figure_style():
        ...
        ax.set(xlabel=r"$s$",ylabel=r"$a_s$",title=r"$B(s,a_s)$ for $\epsilon=0.14$, $\zeta=0.12$")
        ax.set_xlim(0,2); ax.set_ylim(0,2.75); polish_axes(ax); add_legend(ax, handles=handles)
        save_figure(fig,"duffing_near_resonance_phase_portrait_epsilon_positive.png")
