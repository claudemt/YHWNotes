...
\begin{align*}
	\int_{V_{\varepsilon}} r^{l} \left( \frac{1}{r^{2}} \frac{\dd}{\dd r} \left( r^{2} \frac{\dd}{\dd r} \right) - \frac{l(l+1)}{r^{2}} \right) n_{l}(kr) \, \dd \vb{r} = \int_{V_{\varepsilon}} \left( r^{l} \nabla^{2} n_{l}(kr) - n_{l}(kr) \nabla^{2} r^{l} \right) \dd \vb{r} = \frac{4\pi}{k^{l+1}} (2l+1)!!.
\end{align*}
给出
\begin{align*}
...
\begin{align*}
	\mathcal{P}_{l}(\uv{r}) = \sum_{m=-l}^{l} Y_{lm}(\uv{r}) \int \psi(\vb{r}') Y_{lm}^{*}(\uv{r}') \, \dd \Omega'.
\end{align*}
...
\begin{align*}
	\varepsilon_{\alpha} \to \varepsilon, \quad \sum_{\alpha} \to \frac{1}{h^{d}} \int \prod_{i=1}^{d} \dd q_{i} \, \dd p_{i}.
\end{align*}

\section{一般态密度与热力学闭式公式}

对于自由粒子系统，以上对位置空间的积分得到体积\(V\)，对动量空间的积分可以转换至能量空间，如上积分总可以写作
\begin{align*}
	\frac{1}{h^{d}} \int \prod_{i=1}^{d} \dd q_{i} \, \dd p_{i} = \int \frac{\dd \Sigma(\varepsilon)}{\dd \varepsilon} \, \dd \varepsilon.
\end{align*}

不妨假设 \(\Sigma(\varepsilon) = B V \varepsilon^{s}, s \ge 1\)，\(B\) 是与体积无关的常数，进而
\begin{align*}
	\sum_{\alpha} \to B V \int \dd \varepsilon^{s} = B V \int_{0}^{\infty} s \varepsilon^{s-1} \, \dd \varepsilon.
\end{align*}
...
\begin{align*}
	&\mathcal{N} = \sum_{\alpha} \frac{1}{z^{-1} e^{\beta \varepsilon_{\alpha}} \mp 1} = \int_{0}^{\infty} \frac{s B_0 V \varepsilon^{s-1}}{z^{-1} e^{\beta \varepsilon} \mp 1} \, \dd \varepsilon = B_0 V \Gamma(s+1) (kT)^{s} \Phi_{s}(z).\\
...
进一步换元，并将求和化为积分
\begin{align*}
	\vb{p}_{\alpha,\beta} = \frac{\vb{\kappa}}{2} \pm \vb{q}, \quad \vb{p}_{\gamma,\lambda} = \frac{\vb{\kappa}}{2} \pm \vb{p}, \quad \sum_{\alpha} \to \frac{V}{h^{3}} \int \dd \vb{p}_{\alpha}, \quad \dd \vb{p}_{\alpha} \dd \vb{p}_{\beta} \dd \vb{p}_{\gamma} = \dd \vb{\kappa} \, \dd \vb{p} \, \dd \vb{q}.
\end{align*}
代入
\begin{align*}
...
并用到公式
\begin{align*}
	\int_{0}^{\infty} \int_{0}^{\infty} \frac{\sinh(u p q)}{q^{2} - p^{2}} p q e^{-v q^{2} - w p^{2}} \, \dd p \, \dd q = \frac{\pi}{4} \frac{u (w - v)}{((w+v)^{2} - u^{2}) \sqrt{4wv - u^{2}}}.
\end{align*}
