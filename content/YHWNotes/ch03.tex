...
\begin{align*}
	...
	\dfrac{\dd}{\dd t} = \sum_{n=0}^m \epsilon^n \dfrac{\partial}{\partial T_n} = \sum_{n=0}^m \epsilon^n D_n, \quad \dfrac{\dd^2}{\dd t^2} = \left( \sum_{n=0}^m \epsilon^n D_n \right)^2.
\end{align*}
...
\end{align*}
\begin{align*}
	\dfrac{\dd}{\dd t} = D_0 + \epsilon D_1 + \epsilon^2 D_2, \quad \dfrac{\dd^2}{\dd t^2} = D_0^2 + 2 \epsilon D_0 D_1 + \epsilon^2 (D_1^2 + 2 D_0 D_2).
\end{align*}
...
\begin{align*}
	D_0^2 x_1 + \omega_0^2 x_1 = 2 \omega_0 \left( D_1 a + \dfrac{\omega_0 \zeta a}{\epsilon} \right) \sin\psi + 2 \omega_0 a (D_1 \theta) \cos\psi + f(a \cos\psi, -\omega_0 a \sin\psi).
\end{align*}
...
\end{align*}
\begin{align*}
	c_n = \dfrac{1}{(1 + \delta_{n0})\pi} \int_0^{2\pi} f(a \cos\psi', -\omega_0 a \sin\psi') \cos(n\psi') \,\dd \psi', \quad d_n = \dfrac{1}{\pi} \int_0^{2\pi} f(a \cos\psi', -\omega_0 a \sin\psi') \sin(n\psi') \,\dd \psi'.
\end{align*}

为避免久期项 （即关于  \(x_1\) 的驱动力中会导致共振发散的  \(\cos\psi\) 和\(\sin\psi\) 项），要求
\begin{align*}
	2 \omega_0 \left( D_1 a + \dfrac{\omega_0 \zeta a}{\epsilon} \right) + d_1 = 0,\quad
	2 \omega_0 a (D_1 \theta) + c_1 = 0.
\end{align*}
其中
\begin{align*}
	\Phi(a) = \dfrac{1}{2 \pi \omega_0^2} \int_0^{2\pi} f(a \cos\psi', -\omega_0 a \sin\psi') \sin\psi' \,\dd \psi',
\end{align*}
\begin{align*}
	\Psi(a) = \dfrac{1}{2 \pi \omega_0^2} \int_0^{2\pi} f(a \cos\psi', -\omega_0 a \sin\psi') \cos\psi' \,\dd \psi'.
\end{align*}
...
\begin{align*}
	D_0^2 x_1 + \omega^2 x_1 = 2 \omega \left( D_1 a + \dfrac{\omega_0 \zeta a}{\epsilon} \right) \sin\psi + 2 \omega a (D_1 \theta) \cos\psi + f(a \cos\psi, -\omega a \sin\psi) + \omega_0^2 B \cos(\psi - \theta) + \omega_0^2 \sigma a \cos\psi.
\end{align*}

同上，将\(f\)展开为傅里叶级数，为避免久期项得
\begin{align*}
	2 \omega \left( D_1 a + \dfrac{\omega_0 \zeta a}{\epsilon} \right) + \omega_0^2 B \sin\theta + 2 \omega_0^2 \Phi(s, a) = 0,
\end{align*}
\begin{align*}
...
其中
\begin{align*}
	\Phi(s, a) = \dfrac{1}{2 \pi \omega_0^2} \int_0^{2\pi} f(a \cos\psi', -\omega a \sin\psi') \sin\psi' \,\dd \psi',
\end{align*}
\begin{align*}
	\Psi(s, a) = \dfrac{1}{2 \pi \omega_0^2} \int_0^{2\pi} f(a \cos\psi', -\omega a \sin\psi') \cos\psi' \,\dd \psi'.
\end{align*}
...
\begin{align*}
	B \sin\theta_{\mathrm{s}} &= -\dfrac{2 \zeta s a_{\mathrm{s}}}{\epsilon} - 2 \Phi(s, a_{\mathrm{s}})\\
	B \cos\theta_{\mathrm{s}} &= \dfrac{a_{\mathrm{s}}}{\epsilon} (1 - s^2) - 2 \Psi(s, a_{\mathrm{s}}).
\end{align*}
进而
\begin{align*}
	\left( \Phi_s + \dfrac{\zeta s a_{\mathrm{s}}}{\epsilon} \right)^2 + \left( \Psi_s + \dfrac{a_{\mathrm{s}} (s^2 - 1)}{2 \epsilon} \right)^2 = \dfrac{B^2}{4},\quad
...
\begin{exbox}
	...
	\begin{figure}[H]
		...
		\begin{subfigure}[b]{0.49\textwidth}
			\includegraphics[width=\textwidth]{duffing_near_resonance_phase_portrait_epsilon_positive.png}
			\caption{\(\epsilon>0\)}
			\label{fig:duffing_phase_eps_pos}
		\end{subfigure}
		\hfill
		\begin{subfigure}[b]{0.49\textwidth}
			\includegraphics[width=\textwidth]{duffing_near_resonance_phase_portrait_epsilon_negative.png}
			\caption{\(\epsilon<0\)}
			\label{fig:duffing_phase_eps_neg}
		\end{subfigure}
		\caption{达芬系统固有频率附近驱动响应相图}
		\label{fig:duffing_phase_near_natural}
	\end{figure}

	相图如\cref{fig:duffing_phase_near_natural}，不稳定区域用阴影标出，阴影的边界（\(\partial s/\partial a_{\mathrm{s}} = 0\)）为
	\begin{align*}
...
\begin{align*}
	\dot{a} = \omega_0 \left( -\zeta a + \dfrac{\epsilon}{8} A^3 \sin\gamma \right),\quad
	\dot{\gamma} = \omega_0 \epsilon \left( -\sigma + \dfrac{3}{8} (a^2 + 2 A^2) + \dfrac{1}{8} \dfrac{A^3}{a} \cos\gamma \right).
\end{align*}

稳态下 \(\dot{a} = 0\)，\(\dot{\gamma} = 0\)，对应
\begin{align*}
	A^3 \sin\gamma_{\mathrm{s}} = \dfrac{8 \zeta a_{\mathrm{s}}}{\epsilon},\quad
	A^3 \cos\gamma_{\mathrm{s}} = \left( 8\sigma - 3(a_{\mathrm{s}}^2 + 2A^2) \right) a_{\mathrm{s}}.
\end{align*}
也即
\begin{align*}
	\left( \dfrac{8\zeta a_{\mathrm{s}}}{\epsilon} \right)^2 + \left( 8\sigma - 3(a_{\mathrm{s}}^2 + 2A^2) \right)^2 a_{\mathrm{s}}^2 = A^6, \quad \gamma_{\mathrm{s}} = \arctan\left( \dfrac{8 \zeta}{\epsilon\left[8\sigma - 3(a_{\mathrm{s}}^2 + 2A^2)\right]} \right).
\end{align*}
记 \(s =\omega/\omega_0\)，代入 \(\sigma = (3s - 1)/\epsilon,\,A = F (1 - s^2)^{-1}\)，则上式等价为
\begin{align*}
		\dfrac{\zeta^2}{\epsilon^2} + \left( \dfrac{3s - 1}{\epsilon} - \dfrac{3}{8} a_{\mathrm{s}}^2 - \dfrac{3}{4} \dfrac{F^2}{(1 - s^2)^2} \right)^2 = \dfrac{F^6}{64 a_{\mathrm{s}}^2(1 - s^2)^6}.
\end{align*}
...
\begin{figure}[H]
	\centering
	\begin{subfigure}[b]{0.49\textwidth}
		\includegraphics[width=\textwidth]{duffing_superharmonic_response_epsilon_positive.png}
		\caption{改变驱动力}
		\label{fig:duffing_superharmonic_force}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.49\textwidth}
		\includegraphics[width=\textwidth]{duffing_superharmonic_response_epsilon_negative.png}
		\caption{改变阻尼}
		\label{fig:duffing_superharmonic_damping}
	\end{subfigure}

	\caption{达芬系统超谐波共振幅频响应图}
	\label{fig:duffing_superharmonic_response}
\end{figure}
...
\begin{align*}
	\dot{a} = \omega_0 \left( -\zeta a + \dfrac{3\epsilon a^2 A}{8} \sin\gamma \right),\quad \dot{\gamma} = \omega_0 \epsilon \left( \dfrac{9}{8} (a^2 + 2A^2 + aA \cos\gamma) - \sigma \right).
\end{align*}
稳态下 \(\dot{a} = 0\)，\(\dot{\gamma} = 0\)，对应
\begin{align*}
	A a_{\mathrm{s}} \sin\gamma_{\mathrm{s}} = \dfrac{8\zeta}{3\epsilon},\quad A a_{\mathrm{s}} \cos\gamma_{\mathrm{s}} = \dfrac{8\sigma}{9} - a_{\mathrm{s}}^2 - 2A^2.
\end{align*}
也即
\begin{align*}
	\gamma_{\mathrm{s}} = \arctan\left( \dfrac{24\zeta}{\epsilon\left[8\sigma - 9(a_{\mathrm{s}}^2 + 2A^2)\right]} \right), \quad a_{\mathrm{s}}^2 = p \pm \sqrt{p^2 - q}.
\end{align*}
其中
\begin{align*}
...
\begin{align*}
	...
	\dfrac{63}{32} F^4 - (1 - s^2)^2 \dfrac{s - 3}{\epsilon} F^2 + \dfrac{8 (1 - s^2)^4 \zeta^2}{\epsilon^2} \leq 0.
\end{align*}
...
\begin{figure}[H]
	\centering
	\includegraphics[width=0.4\textwidth]{duffing_system_subharmonic_resonance_response_plot.png}
	\caption{达芬系统亚谐波共振响应图}
	\label{fig:duffing_subharmonic_response}
\end{figure}
