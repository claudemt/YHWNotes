...
Mathieu方程形如
\begin{align*}
	\frac{\dd^2 y}{\dd z^2} + (\lambda - 2q \cos(2z)) y = 0.
\end{align*}
展开为Floquent解
\begin{align*}
...
易见，有正交关系
\begin{align*}
	\int_{0}^{2\pi} \mathrm{ce}_m(z, q) \mathrm{ce}_n(z, q) \,\dd z = \int_{0}^{2\pi} \mathrm{se}_m(z, q) \mathrm{se}_n(z, q) \,\dd z = 0, \quad m \neq n.
\end{align*}
\begin{align*}
	\int_{0}^{2\pi} \mathrm{ce}_m(z, q) \mathrm{se}_n(z, q) \,\dd z = 0.
\end{align*}

另外，规定各系数的归一化关系为
\begin{align*}
	\frac{1}{\pi} \int_{0}^{2\pi} \mathrm{ce}_m^2(z, q) \,\dd z = \frac{1}{\pi} \int_{0}^{2\pi} \mathrm{se}_m^2(z, q) \,\dd z = 1.
\end{align*}
也即（适用于任何上标）
\begin{align*}
...
\begin{figure}[htbp]
	...
	\begin{subfigure}[b]{0.49\textwidth}
		\includegraphics[width=\textwidth]{eigenvalue_curve_shaded_region_stable.png}
		\caption{本征值曲线（阴影区稳定）}
		\label{fig:mathieu_eigenvalue_curve}
	\end{subfigure}
	\hfill
	\begin{subfigure}[b]{0.49\textwidth}
		\includegraphics[width=\textwidth]{mathieu_function.png}
		\caption{Mathieu函数}
		\label{fig:mathieu_functions}
	\end{subfigure}
	\caption{Mathieu方程解的示意图}
	\label{fig:mathieu_equation_solutions}
\end{figure}
