...
对 \(d\) 个方向进行对称化及周期化操作（即如\cref{fig:cubic_resistor_network_symmetry}），由于原问题电流无法从边界流出，故操作后边界条件仍然满足，原区域的解就是原问题的解。
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.8\textwidth]{cubic_resistor_network_symmetrization_schematic_diagram.png}
	\caption{立方电阻网对称化示意图}
	\label{fig:cubic_resistor_network_symmetry}
\end{figure}
...
\begin{align*}
	&U[\xi_1, \ldots, \xi_d] = \dfrac{1}{(2\pi)^d} \prod_q \int_{-\pi}^{\pi} A(k_1, \ldots, k_d) \exp\left( i \sum_q k_q \xi_q \right) \prod_q \dd k_q.\\
...
\section{椭球的静电性质}
\section{导体椭球的电容}
记椭球的三个轴的半长为  \(a, b, c (a\geq b \geq c)\)，选取椭球坐标\(\xi,\,\eta,\,\zeta\)，相关数学公式见\cref{sec:ellipsoidal_coordinates}.令椭球带电量为\(Q\)，考虑到椭球表面 (对应\(\xi = 0\)) 是等势面，猜测空间电势分布只与\(\xi\)有关，即：
\begin{align*}
	\Phi(\xi,\eta,\zeta)=\Phi(\xi),
	\qquad
	\nabla^2\Phi(\xi)
	=\frac{4\sqrt{\varphi(\xi)}}{(\xi-\eta)(\xi-\zeta)}
	\frac{\dd}{\dd \xi}\Bigl(\sqrt{\varphi(\xi)}\,
	\frac{\dd \Phi}{\dd \xi}\Bigr)
	=0,
	\quad
	\Phi(\xi\to\infty)=0,
\end{align*}
...
\begin{align*}
	\Phi(\xi)
	&=A \int_{\xi}^{\infty}\frac{\dd \Theta}{\sqrt{\varphi(\Theta)}},\quad
	\vb{E}
	=-\nabla\Phi
	=-\frac{\dd \Phi/\dd \xi}{h_{\xi}}\,
	\uv{e}_{\xi}
...
\begin{align*}
	\vb{D} = \int_S \left(3\,\vb{r}\,\vb{r} - r^2\, \vb{I}\,\right) \sigma \, \dd S
	= \frac{Q}{3} \left[
...
\begin{align*}
	...
	\begin{cases}
		A_1,                                                                                                  & -c^2 \leq \xi < 0, \\
		1 - A_2 \displaystyle\int_{\xi}^{\infty} \frac{1}{(\theta + a^2) \sqrt{\varphi(\theta)}}\, \dd \theta, & \xi > 0.
	\end{cases}
\end{align*}
...
\begin{align*}
	\vb{p}_{\text{ellipsoid}} = \int_S \vb{r}\,\sigma \, \dd S = \frac{4\pi}{3} a b c\,\varepsilon_0 \left(
...
\section{导体平面上的圆形孔}
\label{sec:circular_hole_on_plane}
如\cref{fig:circular_hole_on_plane}，在无限大的接地理想导体平面上有一半径为 \(a\) 的圆形孔，以圆心为原点、垂直于导体平面方向为 \(z\) 轴，建立坐标系.
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.4\textwidth]{circular_hole_on_conducting_plane_schematic_diagram.png}
	\caption{导体平面上的圆孔示意图}
	\label{fig:circular_hole_on_plane}
\end{figure}
...
\begin{align*}
	\Phi^{(1)}(\rho, z) = \int_0^\infty A(k) e^{-k|z|} J_0(k\rho) \,\dd k.
\end{align*}
则边界条件给出
\begin{align*}
	\int_0^\infty k A(k) J_0(k\rho) \,\dd k = \frac{E_0}{2}, \quad \rho < a. \qquad
	\int_0^\infty A(k) J_0(k\rho) \,\dd k = 0, \quad \rho \ge a.
\end{align*}
解得
\begin{align*}
	&A(k) = \frac{E_0}{\pi} a^2 j_1(ka).\\
	&\Phi^{(1)}(\rho, z)
	= \frac{E_0}{\pi} a^2 \int_0^\infty e^{-k|z|} J_0(k\rho) j_1(ka) \,\dd k
...
\begin{align*}
	\Phi_M^{(1)}(\rho, \phi, z) = \int_0^\infty A(k) e^{-k|z|} J_1(k\rho) \sin\phi \,\dd k.
\end{align*}
则边界条件给出
\begin{align*}
	\int_0^\infty A(k) J_1(k\rho) \,\dd k = \frac{H_0}{2} \rho, \quad \rho < a. \qquad
	\int_0^\infty k A(k) J_1(k\rho) \,\dd k = 0, \quad \rho \ge a.
\end{align*}
解得
\begin{align*}
	A(k) & = \frac{2H_0}{\pi} a^2 j_1(ka).                                                                                                                                                         \\
	\Phi_M^{(1)}(\rho, \phi, z)
	     & = \frac{2H_0}{\pi} a^2 \sin\phi \int_0^\infty e^{-k|z|} J_1(k\rho) j_1(ka) \,\dd k                                                                                                       \\
...
\section{旋转对称荷流分布的电磁场}
在球坐标系下，若 \(\vb{J}(\vb{r}) = J(r,\theta) \uv{\phi} = J(r,\theta) (-\sin\phi \uv{x} + \cos\phi \uv{y})\)，则 \(A_z = 0\)，
\begin{align*}
	\tilde{A}(\vb{r}) = A_y + iA_x
	= \frac{\mu_0}{4\pi} \int \frac{\vb{J}(\vb{r}') \cdot (\uv{y}' + i\uv{x}')}{|\vb{r} - \vb{r}'|} \,\dd \vb{r}'
	= \frac{\mu_0}{4\pi} \int \frac{J(r',\theta') e^{-i\phi}}{|\vb{r} - \vb{r}'|} \,\dd \vb{r}'.
\end{align*}
运用数学公式
\begin{align*}
...
\begin{align*}
	\tilde{A}(\vb{r})
	               & = \frac{\mu_0}{2} e^{-i\phi} \sum_{l=1}^\infty P_l^1(\cos\theta) \frac{1}{l(l+1)} \iint \frac{r_<^l}{r_>^{l+1}} J(r',\theta') P_l^1(\cos\theta') (r')^2 \sin\theta' \,\dd \theta' \dd r', \\[4pt]
	\vb{A}(\vb{r})
	               & = A_\phi \uv{\phi} = (\cos\phi \operatorname{Re}\tilde{A} - \sin\phi \operatorname{Im}\tilde{A}) \uv{\phi}
	= \frac{\mu_0}{2}\uv{\phi} \sum_{l=1}^\infty P_l^1(\cos\theta) \frac{1}{l(l+1)} \iint \frac{r_<^l}{r_>^{l+1}} J(r',\theta') P_l^1(\cos\theta') (r')^2 \sin\theta' \,\dd \theta' \dd r' .                   \\
	\vb{B}(\vb{r}) & = \nabla \times \vb{A}
	= \frac{1}{r\sin\theta} \frac{\partial (\sin\theta A_\phi)}{\partial \theta} \uv{r} - \frac{1}{r} \frac{\partial (r A_\phi)}{\partial r} \uv{\theta}
	\\&= -\frac{\mu_0}{2r} \sum_{l=1}^\infty \left( \uv{r} P_l(\cos\theta) + \uv{\theta} \frac{P_l^1(\cos\theta)}{l(l+1)} \frac{\partial}{\partial r} r \right) \iint \frac{r_<^l}{r_>^{l+1}} J(r',\theta') P_l^1(\cos\theta') (r')^2 \sin\theta' \,\dd \theta' \dd r'.
\end{align*}

同理，在球坐标系下，若 \(\rho(\vb{r}) = \rho(r,\theta)\)，则
\begin{align*}
	\Phi(\vb{r})
	 & = \frac{1}{4\pi\varepsilon_0} \int \frac{\rho(\vb{r}')}{|\vb{r} - \vb{r}'|} \,\dd \vb{r}' = \frac{1}{2\varepsilon_0} \sum_{l=0}^\infty P_l(\cos\theta) \iint \frac{r_<^l}{r_>^{l+1}} \rho(r',\theta') P_l(\cos\theta') (r')^2 \sin\theta' \,\dd \theta' \dd r',                   \\[4pt]
	\vb{E}(\vb{r})
	 & = -\nabla\Phi = -\frac{1}{2\varepsilon_0} \sum_{l=0}^\infty \left( \frac{\uv{\theta}}{r} P_l^1(\cos\theta) + \uv{r} P_l(\cos\theta) \frac{\partial}{\partial r} \right) \iint \frac{r_<^l}{r_>^{l+1}} \rho(r',\theta') P_l(\cos\theta') (r')^2 \sin\theta' \,\dd \theta' \dd r'.
\end{align*}
...
\begin{align*}
	g(\vb{r}, t) &= \int_0^\infty \sum_{l=0}^\infty \sum_{m=-l}^l e^{-k^2 t/(\mu\sigma)} a_{lm}(k) j_l(kr) Y_{lm}(\theta,\phi) \,\dd k,\\
	a_{lm}(k) &= \frac{2k^2}{\pi} \int_0^{2\pi} \int_0^\pi \int_0^\infty g(\vb{r}, t=0) j_l(kr) Y_{lm}^*(\theta,\phi) r^2 \sin\theta \,\dd r \,\dd \theta \,\dd \phi.
\end{align*}

特别地，若 \(g(\vb{r}, t) = a(r, t=0) \,b(\theta,\phi)\)，则
\begin{align*}
	g(\vb{r}, t) = a(r,t)\, b(\theta,\phi),\quad
	a(r,t) = \sum_{l=0}^\infty \int_0^\infty \int_0^\infty \frac{2k^2}{\pi} r'^2 j_l(kr') a(r',t=0) e^{-k^2 t/(\mu\sigma)} j_l(kr) \,\dd r' \,\dd k.
\end{align*}
...
\begin{align*}
	...
	f_\phi(r,t) = \int_0^\infty \int_0^\infty \frac{2k^2}{\pi} r'^2 j_1(kr') f_\phi(r',t=0) e^{-k^2 t/(\mu\sigma)} j_1(kr) \,\dd r' \,\dd k.
\end{align*}
...
\begin{note}
	...
	\begin{align*}
		...
		= \frac{\Theta(t)}{(2\pi)^3} \int e^{-k^2 t/(\mu\sigma)} e^{i\vb{k}\cdot(\vb{r} - \vb{r}')} \,\dd \vb{k}
		= \Theta(t) \left(\frac{\mu\sigma}{4\pi t}\right)^{3/2} e^{-\mu\sigma |\vb{r} - \vb{r}'|^2/(4t)}.
	\end{align*}
	进而，扩散方程的解为
	\begin{align*}
		\vb{A}(\vb{r},t) = \int G(\vb{r} - \vb{r}',t) \vb{A}(\vb{r}',t=0) \,\dd \vb{r}'
		= \int \left(\frac{\mu\sigma}{4\pi t}\right)^{3/2} e^{-\mu\sigma |\vb{r} - \vb{r}'|^2/(4t)} \vb{A}(\vb{r}',t=0) \,\dd \vb{r}'.
	\end{align*}
\end{note}
...
\begin{exbox}
	电导率为 \(\sigma\)、磁导率为 \(\mu\) 的良导体液中，半径为 \(a\) 的球面上固定有导线，在球面内产生 \(B_0 \uv{z}\) 的匀强磁场，在球面外产生等效磁偶极矩为 \(2\pi a^3 B_0 \uv{z}/\mu\) 的偶极矩磁场.在 \(t=0\) 时刻突然切断导线的电流，使空间中无电流分布，此后磁场将扩散.
	...
	用分离变量法解得
	\begin{align*}
		A_\phi(r,t) & = \int_0^\infty \int_0^\infty \frac{2k^2}{\pi} r'^2 j_1(kr') \frac{B_0 a^2}{2} \frac{r'_<}{(r'_>)^2} e^{-k^2 t/(\mu\sigma)} j_1(kr) \,\dd r' \,\dd k                                             \\
		            & = \frac{3B_0 a^2}{\pi} \int_0^\infty j_1(kr) j_1(ka) e^{-k^2 t/(\mu\sigma)} \,\dd k                                                                                                             \\
	...
	在 \(r \ll a\) 处，最低阶为 \(n=0\) 项。球心磁场的时间演化如\cref{fig:sphere_magnetic_diffusion}所示。
\begin{center}
\includegraphics[width=0.68\textwidth]{magnetic_field_diffusion_from_sphere_center_schematic_diagram.png}
\captionof{figure}{球域内部磁场扩散到球心附近的时间演化。}
\label{fig:sphere_magnetic_diffusion}
\end{center}
	\begin{align*}
	...
	进而
	\begin{align*}
		J_\phi(r,t) & = \int_0^\infty \int_0^\infty \frac{2k^2}{\pi} r'^2 j_1(kr') \frac{3B_0}{2\mu} \delta(r'-a) e^{-k^2 t/(\mu\sigma)} j_1(kr) \,\dd r' \,\dd k                                                     \\
		            & = \frac{3B_0 a^2}{\pi\mu} \int_0^\infty k^2 j_1(ka) j_1(kr) e^{-k^2 t/(\mu\sigma)} \,\dd k                                                                                                     \\
	...
	进而，空间中电磁场的总能量为：
	\begin{align*}
		W_m(t) & = \frac12 \int \vb{J}(\vb{r},t) \cdot \vb{A}(\vb{r},t) \,\dd \vb{r}
		= \frac{4\pi}{3} \int_0^\infty J_\phi(r,t) A_\phi(r,t) r^2 \,\dd r                                                                                                            \\
...
\begin{exbox}
	...
	\begin{align*}
		...
		\vb{H}(\vb{r},t) = \mathcal{L}\{\tilde{H}(z,p)\}(t) \uv{x} = \uv{x} \int_0^\infty h(k) \cos(kz) e^{-k^2 t/(\mu\sigma)} \,\dd k.
	\end{align*}
	代入初始条件得
	\begin{align*}
		h(k) = \frac{1}{\pi} \int_{-\infty}^\infty H(z,t=0) \cos(kz) \,\dd z = \frac{2H_0}{\pi k} \sin(ka).
	\end{align*}
	进而（演化图像如\cref{fig:one_d_magnetic_diffusion}）
	\begin{align*}
		\vb{H}(\vb{r},t) & = \uv{x} \int_0^\infty h(k) \cos(kz) e^{-k^2 t/(\mu\sigma)} \,\dd k
	...
	\begin{figure}[H]
		...
		\begin{minipage}[b]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{magnetic_field_diffusion_from_sphere_center_schematic_diagram.png}
			\caption{球心磁场扩散示意图}
			\label{fig:sphere_center_magnetic_diffusion}
		\end{minipage}
		\hfill
		\begin{minipage}[b]{0.49\textwidth}
			\centering
			\includegraphics[width=\textwidth]{one_dimensional_magnetic_field_diffusion_schematic_diagram.png}
			\caption{一维磁场扩散示意图}
			\label{fig:one_d_magnetic_diffusion}
		\end{minipage}
	\end{figure}
\end{exbox}
