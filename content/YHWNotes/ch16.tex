...
\begin{equation}
	...
	\end{pmatrix}.
\end{equation}
令右上角为零、左上角为一，就得到\cref{eq:matrix-principal-plane}.
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.72\textwidth]{principal_planes_nodes_and_foci.png}
	\caption{主面、节点和焦点的有向距离约定}
	\label{fig:principal_plane_nodes_foci}
\end{figure}
...
\begin{figure}[htbp]
	\centering
	\includegraphics[width=0.78\textwidth]{gaussian_beam_through_lens_group.png}
	\caption{Gaussian beam 通过理想光具组}
	\label{fig:gaussian_beam_lens_system}
\end{figure}
...
\cref{eq:q-abcd} 使用局部波数 \(k=nk_0\) 定义 \(q\). 因而平面折射界面的矩阵
\[
...
\end{equation}
于是\cref{eq:cavity-stability} 退化为熟知的稳定性条件
\begin{equation}
	0<g_1g_2<1.
\end{equation}
...
对 Gaussian beam，\cref{eq:q-abcd} 给出
\begin{equation}
	q_{\mathrm{out}}
