...
洛伦兹代数 & $\mathfrak{so}(3,1)$ & 生成元 $J_i$（转动）、$K_i$（推动）；复化分解为 $A_i,B_i$。 \\
...
\chapref{chap:lorentz-fields} 起的相对论章节采用时间分量取正号的约定：闵可夫斯基度规 $\eta_{\mu\nu}=\diag(+1,-1,-1,-1)$。为突出群表示而不反复携带单位，这两章暂取 $c=\hbar=1$，恢复 SI 单位时四动量为 $p^\mu=(E/c,\vb p)$。此约定下 $[K_i,K_j]=-\ii\epsilon_{ijk}J_k$（洛伦兹推动的交换子带负号），与 \chapref{chap:rotation-group} 的 $SU(2)$ 约定 $[J_i,J_j]=\ii\epsilon_{ijk}J_k$ 一致。Lie 代数章节（\chapref{chap:lie-groups-algebras}）的 Killing 形式对紧群为负定，根与权的内积由 $-B$ 诱导为正定。
