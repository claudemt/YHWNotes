...
\chapref{chap:rotation-group}讨论的 $SO(3)$ 只改变空间方向，时间仍是独立参数。狭义相对论把时间与空间合成四维时空，允许不同惯性系之间的 推进；相应的连续时空对称性由正常正时洛伦兹群 $SO^+(3,1)$ 描述，再加入平移便得到 Poincar\'e 群。前者决定场的 Lorentz 类型，后者的不可约幺正表示决定单粒子的质量与自旋。这两类表示承担不同任务，不能混为一谈。
...
\qquad v=\tanh\zeta .
\]
转角是周期变量，而 $\zeta\in\mathbb R$ 无界；因此 $SO^+(3,1)$ 非紧。非紧性有一个直接后果：除平凡表示外，它没有非平凡有限维幺正表示。原因是有限维表示中 推进 生成元 $K_i$ 必须取反厄米矩阵（与 $J_i$ 满足 $[K_i,K_j]=-\ii\epsilon_{ijk}J_k$），无法与全部生成元同时厄米化。场仍可处在有限维但非幺正的 Lorentz 表示中；量子态所需的幺正性则由无限维的 Poincar\'e 表示实现。
...
\]
$J_i$ 生成空间转动，$K_i$ 生成 推进。取生成元为厄米算符的约定时，洛伦兹代数为
\begin{equation}
...
\end{equation}
最后一个负号把双曲 推进 与欧氏转动区分开来；两个不共线 推进 的交换子是一个转动生成元。
...
宇称保持转动而反转 推进，时间反演在量子理论中是反幺正算符，作用为
\begin{align*}
