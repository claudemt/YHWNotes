...
四矢量写作 $x^\mu=(t,\mathbf x)$，闵可夫斯基内积为
\[
...
\qquad v=\tanh\zeta .
\]
转角是周期变量，而 $\zeta\in\mathbb R$ 无界；因此 $SO^+(3,1)$ 非紧。非紧性有一个直接后果：除平凡表示外，它没有非平凡有限维幺正表示。场仍可处在有限维但非幺正的 Lorentz 表示中；量子态所需的幺正性则由无限维的 Poincar\'e 表示实现。
...
=\exp\!\left[-\ii\boldsymbol\theta\cdot\rho(\mathbf J)
-\ii\boldsymbol\zeta\cdot\rho(\mathbf K)\right].
...
\end{equation}
所以 $\slashed p=\gamma^\mu p_\mu$ 按相似变换协变，Dirac 方程 $(\slashed p-m)\Psi=0$ 与 Lorentz 变换相容。

手征算符 $\gamma^5=\ii\gamma^0\gamma^1\gamma^2\gamma^3$ 满足 $(\gamma^5)^2=I$，投影 $P_{L,R}=(I\mp\gamma^5)/2$ 把 Dirac 旋量分成左右 Weyl 分量。复 Clifford 代数决定矩阵结构；是否能施加 Majorana 实条件还取决于实形式和时空签名，不能仅由复维数判断。
...
\end{align}
第一 Casimir 是 $P^2=P_\mu P^\mu$。第二个由 Pauli--Lubanski 四矢量
\begin{equation}
W^\mu\equiv\frac12\epsilon^{\mu\nu\rho\sigma}P_\nu M_{\rho\sigma}
\label{eq:pauli-lubanski}
\end{equation}
构成。它满足 $W\cdot P=0$，而 $W^2$ 与所有 Poincar\'e 生成元对易。对质量 $m>0$ 的不可约表示，可取静止动量 $k=(m,\mathbf0)$；此时 $W^0=0$、$\mathbf W=m\mathbf J$，所以
\begin{equation}
W^2=-m^2j(j+1).
\label{eq:W2-massive}
\end{equation}
因此 $P^2=m^2$ 与 $W^2$ 分别读出质量和自旋。
...
对正能有质量轨道 $p^2=m^2$、$p^0>0$，可取 $k=(m,\mathbf0)$，小群为 $SO(3)$；若同时允许半整数自旋，应使用其覆盖群 $SU(2)$。表示空间可写成
\begin{equation}
\mathcal H_{m,j}=L^2(\mathcal O_m,\dd\mu_m)\otimes\mathbb C^{2j+1},
\qquad
\dd\mu_m(p)=\frac{\dd^3p}{2E_{\mathbf p}},
...
\begin{equation}
\mathsf T\,\Psi(t,\mathbf x)\,\mathsf T^{-1}
=\eta_T\,\ii\gamma^1\gamma^3\Psi(-t,\mathbf x),
...
Lorentz 群上的 Haar 测度可由 $SL(2,\mathbb C)$ 覆盖群给出；在 Cartan 分解 $g=k_1a(\zeta)k_2$ 中，它具有
\begin{equation}
\dd\mu_G(g)=C\,\sinh^2\!\zeta\,\dd\zeta\,\dd\mu_{SU(2)}(k_1)\,\dd\mu_{SU(2)}(k_2),
\label{eq:haar-lorentz}
\end{equation}
其中 $C$ 取决于归一化。群自身的主系列表示可在边界函数上写成
\begin{equation}
[\pi_{n,\rho}(g)f](z)
=|cz+d|^{-2(1+\ii\rho)}
...
\end{equation}
这描述群上函数的调和分析，不等同于按 $(m,j)$ 或 $(0,h)$ 分类的 Poincar\'e 单粒子态；二者分别回答“怎样展开群上函数”和“粒子态怎样实现时空对称性”。
