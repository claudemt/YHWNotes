...
\subsection{Cartan 子代数与根空间}

前一节已经从连续群得到 Lie 代数和伴随表示。现在取有限维复半单 Lie 代数 $\mathfrak g$。伴随表示 $\operatorname{ad}_X(Y)=[X,Y]$ 把括号变成线性算符，而 Killing 形式
\begin{equation}
...
\begin{definitionplain}[Cartan 子代数]
...
\begin{proposition}[$\mathfrak{sl}(3,\mathbb C)$ 的 Killing 型在 Cartan 子代数上非退化]
取 $\mathfrak h$ 的基 $H_1,H_2$，使其满足 $\alpha_i(H_j)=\delta_{ij}$。$\mathfrak{sl}(3,\mathbb C)$ 的六个根为
$\pm\alpha_1,\pm\alpha_2,\pm(\alpha_1+\alpha_2)$。Killing 型限制到 $\mathfrak h$ 后非退化。
\end{proposition}

\begin{proof}
对 $H\in\mathfrak h$，$\operatorname{ad}_H$ 在根空间 $\mathfrak g_\alpha$ 上作用为标量 $\alpha(H)$，而在 $\mathfrak h$ 上作用为零，故
\[
...
\begin{pmatrix}h_1'\\h_2'\end{pmatrix}.
\]
矩阵行列式为 $12$，所以限制型非退化。这里计算的是复代数
$\mathfrak{sl}(3,\mathbb C)$ 上选定实根坐标后的形式。若回到紧实形式
$\mathfrak{su}(3)$，生成元取反厄米矩阵，Killing 型为负定；用来定义正定根内积的是 $-B$。这一区分避免把复化代数与紧实形式的符号混在一起。
\end{proof}

\subsection{正根、单根与最高权}
...
\begin{definitionplain}[单根、基本权与 Dynkin 标号]
...
最高权为什么必须满足整数条件，可以沿任意一条根方向退回熟悉的
$\mathfrak{sl}(2)$。为每个根 $\alpha$ 选取生成元
\[
E_\alpha,\qquad E_{-\alpha},\qquad
H_\alpha=[E_\alpha,E_{-\alpha}],
\]
并归一化到
\[
[H_\alpha,E_{\pm\alpha}]=\pm2E_{\pm\alpha}.
\]
这三个元素组成一个 $\mathfrak{sl}(2)$ 子代数。于是高秩代数中的每一条根方向都带着一条普通角动量升降链。

\begin{theorem}[根串与整性条件]
设 $\mu$ 是有限维表示的一个权，$\alpha$ 是根。沿 $\alpha$ 方向出现的权恰构成有限根串
\[
\mu-p\alpha,\ldots,\mu,\ldots,\mu+q\alpha,
\qquad p,q\in\mathbb Z_{\ge0},
\]
并满足
\begin{equation}
p-q=\frac{2(\mu,\alpha)}{(\alpha,\alpha)}.
\label{eq:root-string-integrality}
\end{equation}
因此右端必为整数。特别地，有限维不可约表示的最高权必须是支配整权。
\end{theorem}

\begin{proof}
把表示限制到根 $\alpha$ 生成的 $\mathfrak{sl}(2)$ 子代数。算符
$H_\alpha$ 在权态 $v_\mu$ 上的本征值为
\[
m_\alpha=\frac{2(\mu,\alpha)}{(\alpha,\alpha)}.
\]
$E_\alpha$ 和 $E_{-\alpha}$ 分别把这个本征值改变 $2$ 与 $-2$。
有限维 $\mathfrak{sl}(2)$ 表示分解成有限个最高权模；在包含
$v_\mu$ 的那条链中，若向下还能走 $p$ 步、向上还能走 $q$ 步，
链两端的 $H_\alpha$ 本征值互为相反数。因此
\[
m_\alpha-2p=-(m_\alpha+2q),
\]
整理得 $m_\alpha=p-q$，即\eqnrefs{eq:root-string-integrality}。
若 $\lambda$ 是最高权，对每个简单根都不能继续升高，所以 $q=0$，
$2(\lambda,\alpha_i)/(\alpha_i,\alpha_i)=p_i\in\mathbb Z_{\ge0}$。
按基本权定义，$\lambda=\sum_i p_i\omega_i$。
\end{proof}
...
\begin{exampleplain}[Freudenthal 递推在 $SU(3)$ 八重态 $(1,1)$ 上走一步]
