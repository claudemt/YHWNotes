...
$\cyclic{r}$ 正规。反射共轭旋转给出其逆：$srs^{-1}=r^2\in\cyclic{r}$，$sr^2s^{-1}=r\in\cyclic{r}$。几何上，"先反射、转 $120^\circ$、再反射回去"等于反向转 $120^\circ$，仍是旋转。旋转共轭旋转当然仍是旋转。因此 $\cyclic{r}$ 在所有共轭下保持自身，$g\cyclic{r}g^{-1}=\cyclic{r}$。

$\cyclic{s}$ 不正规。旋转共轭反射给出另一个反射：$rsr^{-1}=sr\neq s$，而 $sr\notin\cyclic{s}=\{e,s\}$。几何上，旋转把过顶点 $1$ 的反射轴转到过顶点 $2$ 的反射轴，因此共轭把 $\cyclic{s}$ 里的反射 $s$ 送到属于另一个子群 $\cyclic{sr}$ 的反射 $sr$。于是 $r\cyclic{s}r^{-1}=\cyclic{sr}\neq\cyclic{s}$。
...
由第一同构定理 $H/\Ker\varphi\cong\Img\varphi$，即 $H/(H\cap N)\cong HN/N$。
\hfill$\square$
\end{theorem}
...
第一同构定理于是给出 $(G/N)/\Ker\varphi\cong\Img\varphi$，即 $(G/N)/(K/N)\cong G/K$。
\hfill$\square$
\end{theorem}
...
\begin{proof}
先自同构的形式。设 $\sigma\in\Aut(C_n)$。因 $a$ 生成 $C_n$，$\sigma$ 由 $\sigma(a)$ 完全确定。设 $\sigma(a)=a^k$（$0\le k<n$），则 $\sigma(a^m)=a^{km}$。
...
接着群结构。$\sigma_k\circ\sigma_l(a)=\sigma_k(a^l)=(a^k)^l=a^{kl}=\sigma_{kl}(a)$，故 $\sigma_k\circ\sigma_l=\sigma_{kl}$。映射 $k\mapsto\sigma_k$ 是 $(\ZZ/n\ZZ)^\times$ 到 $\Aut(C_n)$ 的同构。
\end{proof}
...
\begin{proof}
(1) 内自同构是自同构的特例，故 $\varphi(H)=H$ 对所有 $\varphi\in\Aut(G)$ 成立蕴含 $c_g(H)=H$ 对所有 $g\in G$ 成立，即 $gHg^{-1}=H$，故 $H\normal G$。

(2) 对任意 $\varphi\in\Aut(G)$，$\varphi|_K\in\Aut(K)$（因 $K\operatorname{char}G$ 保证 $\varphi(K)=K$），故 $\varphi(H)=\varphi|_K(H)=H$（因 $H\operatorname{char}K$）。

(3) 对任意 $g\in G$，$c_g\in\Inn(G)\subseteq\Aut(G)$，$c_g(K)=K$（因 $K\normal G$），故 $c_g|_K\in\Aut(K)$，$c_g(H)=c_g|_K(H)=H$（因 $H\operatorname{char}K$）。

(4) 对任意 $\varphi\in\Aut(G)$ 和 $z\in Z(G)$：$\varphi(z)x\varphi(z)^{-1}=\varphi(z\varphi^{-1}(x)z^{-1})=\varphi(\varphi^{-1}(x))=x$，故 $\varphi(z)\in Z(G)$，即 $\varphi(Z(G))\subseteq Z(G)$；同理论 $\varphi^{-1}$ 得反向包含。导群 $G'=[G,G]$ 由换位子生成，$\varphi([x,y])=[\varphi(x),\varphi(y)]$，故 $\varphi(G')=G'$。
\end{proof}
...
核。$\Phi(g)=\id(H)$ 当且仅当 $ghg^{-1}=h$ 对所有 $h\in H$，即 $g\in C_G(H)$。故 $\Ker\Phi=C_G(H)$。
...
先构造二元组集合。对每个 $x\in X$，设 $r_x$ 为使 $x$ 不动的群元个数，即 $r_x=\lvert\Stab_G(x)\rvert$。考虑二元组集合
\[
...
接着按 $x$ 计数。固定 $x\in X$，使 $x$ 不动的 $g$ 恰有 $r_x=\lvert\Stab_G(x)\rvert$ 个（$x$ 的稳定子大小）。对 $x$ 求和：
\[
...
再按轨道分组。对同一轨道中的元素，稳定子大小相同：若 $y=h\cdot x$，则 $\Stab(y)=h\Stab(x)h^{-1}$（共轭子群，同阶）。对轨道 $\Orb(x)$ 中每个元素，由轨道--稳定子定理
\[
...
这个作用的不动点只有 $P$ 自身。事实上，若 $Q$ 被 $P$ 固定，则 $P\le N_G(Q)$。此时 $P,Q$ 都是 $N_G(Q)$ 的 Sylow $p$-子群，而 $Q\normal N_G(Q)$；由已证的共轭性，只能有 $P=Q$。
...
$\cyclic{r}=\{e,r,r^2\}$ 是唯一的 $3$ 阶子群并且正规。这与\cref{ex:d3-normal-subgroup-geometry} 的几何结论一致。

对 $p=2$，约束为 $n_2\equiv1\pmod2$ 且 $n_2\mid3$。群中确有
$\cyclic{s},\cyclic{sr},\cyclic{sr^2}$ 三个二阶子群，故 $n_2=3$；它们彼此共轭而都不正规。
\end{example}
...
(iii) $p$-群可解. 设 $|G|=p^a$。由类方程
\[
|G|=|Z(G)|+\sum_{i}[G:C_G(g_i)],
\]
其中 $g_i$ 跑遍非中心共轭类代表。每个 $[G:C_G(g_i)]>1$ 是 $p$ 的幂，故 $p\mid|Z(G)|$，即中心非平凡。

取 $Z(G)$ 中 $p$ 阶子群 $C_p\le Z(G)$（由 Cauchy 定理，因 $p\mid|Z(G)|$）。因 $C_p\le Z(G)$，$C_p\normal G$。商群 $G/C_p$ 的阶为 $p^{a-1}$，仍是 $p$-群。由归纳假设 $G/C_p$ 可解，$C_p$ 亦可解，再由 (ii) 的扩张性质知 $G$ 可解。

归纳基础：$|G|=p^0=1$ 时 $G=\{e\}$ 可解（空链）。归纳步骤：$|G|=p^a$ 时，上述论证把问题降到 $p^{a-1}$ 阶，由归纳假设完成。

(iv) $S_n$ 不可解（$n\ge5$）. $A_n$（$n\ge5$）是单群且非 Abel（\cref{ex:a5-simplicity} 对 $n=5$ 的证明可推广到一般 $n\ge5$）。若 $S_n$ 可解，则由 (i) 的子群遗传性，$A_n\le S_n$ 也应可解。但 $A_n$ 的合成列只能是
\[
A_n\rhd\{e\},
\]
...
\]
$N$ 是保持三角形手性的三个真转动，$H$ 由一条反射生成。由\cref{ex:d3-normal-subgroup-geometry}，$N\normal D_3$；又有 $N\cap H=\{e\}$ 以及 $|N||H|=|D_3|$，故 $NH=D_3$。
