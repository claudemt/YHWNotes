...
保持晶格的转动在晶格基底下由 $M\in\GL(3,\ZZ)$ 表示，所以 $\tr M\in\ZZ$。它与正交基底下的 $R$ 相似，而三维转动的本征值为 $1,\ee^{\ii\theta},\ee^{-\ii\theta}$，故
\[
...
\[
F(\vb G)=\int\rho(\vb r)\ee^{-i\vb G\cdot\vb r}\,d^3r,
\]
...
先结构因子的对称约束。若 $\{R|\vb\tau\}\in\mathcal G$ 把原子 $\vb r_j$ 映到同类型原子 $\vb r_{j'}$，则 $f_j=f_{j'}$ 且
\[
...
随后$R^{\mathsf T}\vb G=\vb G$ 的情形。若 $\vb G$ 在 $R$ 下不变（即 $R$ 属于 $\vb G$ 的小群），则
\[
