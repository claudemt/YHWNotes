...
\]
称为与 $V_\alpha$ 对应的\textbf{同类分量}（等型 分量）。
...
\begin{example}[投影算符在 对称性适配线性组合 构造中的应用]
...
\]
归一化后给出 $A_1$ 对称的 对称性适配线性组合 $\psi_{A_1}=\frac{1}{\sqrt3}(\phi_1+\phi_2+\phi_3)$。
...
\]
作用在 $\phi_1$ 上给出 $E$ 表示的一个基矢。这就是\chapref{chap:group-theory-quantum} 中 对称性适配线性组合 构造的群论基础——投影算符公式把"从对称性提取适配基"变成了一个机械计算。
\end{example}
...
\]
三个 Kronecker δ 分别表达三层正交：不同不可约表示正交；同一个不可约表示中不同行指标正交；不同列指标也正交。
...
\item $\nu(\chi)=-1$ 当且仅当 $\chi$ 取实值但表示不可写成实矩阵（ 四元数 型）；
...
\]
的表示论翻译：群代数同构于若干个全矩阵代数的直和。这一结构在后续构造投影算符和 对称性适配线性组合 时直接使用。
