...
群的乘法、陪集和商群揭示了群本身的结构，但物理量通常是向量：晶格位移、原子轨道的系数、波函数和态矢量都可以相加，也可以乘一个复数。要让抽象群元参与实际计算，就要把每个 $g\in G$ 对应到向量空间上的可逆线性变换 $\rho(g)$，并使群乘法变成算符复合，
\[
\rho(gh)=\rho(g)\rho(h).
\]
这个对应就是群表示。
...
若表示空间能分成若干彼此不混合的不变子空间，表示矩阵便能同时化成分块形式。不能继续分裂的块称为不可约表示。能级简并、选择定则和能带标记之所以能用少数标签概括，正是因为复杂的表示可以分解成这些最小块。
...
\begin{definitionplain}[子表示与不变子空间]
...
\begin{exampleplain}[$S_3$ 的二维标准表示]
...
\begin{definitionplain}[交织算子与表示等价]
...
\begin{definitionplain}[可约表示与不可约表示]
