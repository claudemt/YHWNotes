...
设 $A=\groupalg{G}$ 为有限群的复群代数，$p\in A$ 为非零幂等元 $p^2=p$。左理想 $Ap$ 是不可约左 $A$-模，当且仅当
\begin{equation}
pAp=\CC p,
...
\tau a_T=a_T.
\]
因为它们在 $S$ 同列，所以 $\tau\in C_S$，且 $\sgnperm(\tau)=-1$。列反对称子的定义给出
\[
b_S\tau=-b_S.
\]
于是
...
\end{align}
线性延拓后，对任意 $x\in\groupalg{S_n}$ 都有
\[
e_Sxe_T=0.
\]
这条式子只需要在字典序较大形状 $\lambda$ 到较小形状 $\mu$ 的一个方向成立；它已经足以排除两个 Specht 模之间存在同构，因为若存在同构，其逆映射会给出该方向上的非零模同态。

\section{\texorpdfstring{为什么 $e_Txe_T$ 只能与 $e_T$ 成比例}{为什么 eT x eT 只能与 eT 成比例}}

\begin{lemma}[Young 一维性引理]
固定 Young 盘 $T$。若 $x\in\groupalg{S_n}$ 满足
\begin{equation}
rxc=\sgnperm(c)x,
\qquad
...
\]
故分解 $rc$ 唯一。设单位元系数 $x_e=\theta$。由 $rx=x$ 可知左乘行置换不改变系数；由 $xc=\sgnperm(c)x$ 可知右乘列置换只给出相应符号，因此
\[
x_{rc}=\theta\,\sgnperm(c).
\]
于是
\[
x
=\theta\sum_{r\in R_T}\sum_{c\in C_T}\sgnperm(c)rc
=\theta a_Tb_T
=\theta e_T.
\]
\end{lemma}

现在令 $x=e_Tye_T$。由于 $r a_T=a_T$ 以及 $b_Tc=\sgnperm(c)b_T$，它自动满足 \eqnrefs{eq:young-bi-equivariance}，于是
\begin{equation}
e_Tye_T=\lambda_T(y)e_T,
\qquad \forall y\in\groupalg{S_n}.
\label{eq:ete-scalar}
\end{equation}
特别取 $y=e$，得到
\[
...
设 $T$ 已经把每一行按从小到大排列，但还不是标准盘。取最靠左的一处列逆序，设它发生在相邻两行 $i,i+1$ 的第 $j$ 列。令 $A$ 为第 $i$ 行从第 $j$ 格起到行末的数字集合，$B$ 为第 $i+1$ 行从行首到第 $j$ 格的数字集合。记 $\mathcal D(A,B)$ 为把 $A\cup B$ 重新分配回这两段位置、同时保持 $A$ 内部与 $B$ 内部原有相对次序的 shuffle 置换集合，并令恒等置换属于 $\mathcal D(A,B)$。每个 shuffle 的系数固定取其置换奇偶性 $\sgnperm(\sigma)$。则
\begin{equation}
\sum_{\sigma\in\mathcal D(A,B)}\sgnperm(\sigma)\,\varepsilon_{\sigma T}=0.
...
\[
\boxed{f^\lambda=\frac{n!}{\displaystyle\prod_{u\in\lambda}h(u)}}.
\]
...
\[
P_-=\frac1{n!}\sum_{\pi\in S_n}\sgnperm(\pi)U(\pi).
\]
