...
\begin{exercise}
从 Cartan 第一、第二结构方程出发，不使用分量公式，完整推导两条 Bianchi 恒等式\eqnrefs{eq:math-dg-bianchi-first}与\eqnrefs{eq:math-dg-bianchi-second}。
\end{exercise}
...
\qquad f(r)>0,
\]
取正交余标架 \(\theta^1=\dd r\)、\(\theta^2=f(r)\dd\phi\)。先由无挠条件求 Levi--Civita 联络一形式，再由第二结构方程证明 Gaussian curvature 为
\[
K=-\frac{f''(r)}{f(r)}.
\]
...
\begin{exercise}
设 \(X\) 为 Killing 场。利用\eqnrefs{eq:math-dg-synthesis-hodge-commute}与 \([\Lie_X,\dd]=0\) 证明 \([\Lie_X,\Delta]=0\)。说明为什么这个结论意味着每个 Hodge--Laplacian 特征空间在 Killing 流下保持。
\end{exercise}
...
\]
不用直接引用正文结论，证明 \(\delta\gamma=0\)，从而说明 \([\alpha]=[\alpha_{\mathcal H}]\)。再说明为什么 harmonic 分量 \(\alpha_{\mathcal H}\) 唯一而 \(\beta\) 不必唯一。
\end{exercise}
...
\begin{exercise}
设 \(g(t)\) 为度量族，记 \(\dot g=\partial_tg\)。从 \(g^{ik}g_{kj}=\delta^i{}_j\) 与 Jacobi 行列式恒等式分别推导\eqnrefs{eq:math-dg-inverse-metric-variation}和\eqnrefs{eq:math-dg-volume-variation}。然后令 \(\dot g=-2\Ric\)，推出\eqnrefs{eq:math-dg-ricci-flow-volume}。
\end{exercise}
...
\begin{exercise}
对单位球面取外法向，按照本书约定 \(\overline\nabla_Xn=-AX\) 求 shape operator、两个主曲率、Gaussian curvature 与有向平均曲率 \(\mathsf H\)。反转法向后重新计算，并说明哪些量改变符号、哪些量保持不变。
\end{exercise}
