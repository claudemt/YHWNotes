...
物理建模：膜与肥皂膜。极小曲面方程 $\mathsf H=0$ 是肥皂膜的平衡条件，对应面积泛函的临界点。平行曲面公式给出膜在法向小扰动下面积的一阶响应：$\mathsf H=0$ 时一阶项消失，膜处于平衡；二阶项 $\frac12\int_M(m^2\mathsf H^2-|A|^2)\operatorname{vol}_g=-\frac12\int_M|A|^2\operatorname{vol}_g$ 在非全测地时为负，反映膜在大多数情形下不稳定，需要边界约束维持。这与 Plateau 问题的解存在性密切相关。
\end{derivation}
...
-\left(|A|^2+\overline{\operatorname{Ric}}(n,n)\right).
