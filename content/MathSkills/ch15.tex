...
退化自检：一维法丛（如 $\mathbb R^3$ 中曲面）中 $[A_\xi,A_\eta]=0$，Ricci 方程 的法向部分自动满足；$\mathbb R^4$ 中 Clifford 环面 的两个法向 $A$ 分别为 $\pm J$，其法曲率 $[A_{n_1},A_{n_2}]=0$ 是 Lawson 不等式 的关键。
\end{derivation}
...
高阶项：继续展开到二阶，$\det(I-sA)=1-s\sigma_1+\frac{s^2}{2}(\sigma_1^2-\sigma_2)+O(s^3)$（$\sigma_k=\sum_{i_1<\cdots<i_k}\kappa_{i_1}\cdots\kappa_{i_k}$ 为主曲率的第 $k$ 个初等对称多项式），二阶项 $\frac12(\sigma_1^2-\sigma_2)\operatorname{vol}_g=\frac12(m^2\mathsf H^2-|A|^2)\operatorname{vol}_g$ 给出二阶变分，是研究 极小 子流形稳定性的关键；$m^2\mathsf H^2=|A|^2$ 时二阶项消失，对应全测地子流形。管状邻域体积 $\operatorname{Vol}(T_r(M))=\omega_{n-m}\int_M\int_0^r\prod_{i=1}^m(1-s\kappa_i)\,\dd s\,\operatorname{vol}_g$ 展开后只有偶数阶 $\sigma_{2k}$ 出现（法向反射对称），即 Weyl 管公式；这些积分是 Lipschitz--Killing 曲率，与热核渐近展开系数一致。例：球面 $S^m(r)\subset\RR^{m+1}$ 主曲率全为 $1/r$，平行曲面面积元 $(1+s/r)^m\operatorname{vol}_g$ 的一阶项 $m\mathsf Hs$ 与公式一致；圆柱 $S^1(r)\times\RR\subset\RR^3$ 主曲率为 $(1/r,0)$，$\mathsf H=1/(2r)$，一阶项 $s/r=2\mathsf Hs$，验证余维一情形。膜与肥皂膜：极小曲面方程 $\mathsf H=0$ 是肥皂膜的平衡条件，一阶项消失即平衡；二阶项 $-\frac12\int_M|A|^2\operatorname{vol}_g$ 在非全测地为负，反映膜多数情形不稳定，需要边界约束维持，与 Plateau 问题的解存在性密切相关。
...
典型例：平面 $B_{ij}=0$、$\mathbf H=0$；球面 $\mathbf H=(1/R)n\neq0$；悬链面 $\mathbf H=0$，即两圆环间肥皂膜的平衡形状。
\end{derivation}
...
\begin{derivation}[Simons 恒等式 的结构来源]
...
应用：Bernstein 定理。对 极小 图 $u:\Omega\subset\RR^n\to\RR$，Simons 恒等式 结合 Bochner 技巧给出 $\frac12\Delta_{\mathrm{LB}}|A|^2\ge(\frac{2}{n}-1)|A|^4$；$n\le7$ 时系数为负，配合 $|A|^2$ 的有界性给出 $|A|^2\equiv0$，即整 极小 图 必为超平面——$n=8$ 时 Bombieri--De Giorgi--Giusti 给出反例，临界维数正是系数变号处。Chern--Simons 形式 $\operatorname{CS}(A)=\operatorname{tr}(A\wedge\dd A+\frac23A\wedge A\wedge A)$ 的变分给出 $F\wedge F$，与 $|A|^4$ 项有类似的'曲率平方'结构，是 Chern--Simons 拓扑场论与极小曲面理论在变分结构上的深层联系。例：$S^3\subset\RR^4$ 中 Clifford 极小环面 $|A|^2=2$ 处处为常数，Simons 恒等式 给出 $\Delta_{\mathrm{LB}}|A|^2=0$，对应 $|\nabla A|^2=|A|^4=4$，验证 $A$ 不能平行。弦论中开弦的世界面是 Dirichlet 边界条件下的极小曲面，Simons 恒等式 给出世界面方程在量子涨落下稳定性的判据：$|A|^2$ 的椭圆不等式控制弦世界面在背景时空中的涨落尺度，与 $\alpha'$ 修正的几何展开直接相关。
