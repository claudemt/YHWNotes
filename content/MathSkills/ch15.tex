...
\begin{equation}\label{eq:math-dg-gauss-formula-general}
\boxed{
\overline\nabla_XY
=\nabla_XY+B(X,Y),}
\end{equation}
...
\begin{equation}\label{eq:math-dg-weingarten-formula-general}
\boxed{
\overline\nabla_X\xi
=-A_\xi X+\nabla_X^\perp\xi.}
\end{equation}
...
\]
平均曲率向量定义为
\begin{equation}\label{eq:math-dg-mean-curvature-vector}
\mathbf H
...
\]
的符号约定。对第二基本形式定义协变导数
\begin{equation}\label{eq:math-dg-second-form-covariant-derivative}
(\nabla_X^\perp B)(Y,Z)
:=\nabla_X^\perp(B(Y,Z))
-B(\nabla_XY,Z)-B(Y,\nabla_XZ).
\end{equation}
...
\textbf{实例。}$\mathbb R^3$ 中的球面 $\mathbb S^2(R)$：法丛一维，$A_\xi=(1/R)\,\mathrm{id}$，$[A_\xi,A_\eta]=0$（一维法丛无交换子），Ricci 方程退化为 $\overline R$ 的切向分量。对 $\mathbb R^3$ 中环面 $T^2$：$A=0$（平面是极小且全测地），所有兼容方程平凡。对 $\mathbb R^4$ 中的 Clifford 环面 $T^2\subset\mathbb S^3\subset\mathbb R^4$：$A_{n_1}$ 和 $A_{n_2}$ 在切空间上分别为 $\pm J$（$J$ 为复结构），$[A_{n_1},A_{n_2}]=0$——法曲率消失，这是 Lawson 不等式的关键。
\end{derivation}
...
\]
环境第一结构方程的法向分量给出
\begin{equation}\label{eq:math-dg-surface-cartans-lemma-input}
\omega^3{}_1\wedge\theta^1
...
\]
代回上式得到
\begin{equation}\label{eq:math-dg-second-form-symmetry}
h_{12}=h_{21},
\end{equation}
这正是一般结论“\(B\) 对称”在二维余维一移动标架中的分量形式。第二基本形式写成
\begin{equation}\label{eq:math-dg-second-fundamental-form}
II=h_{ij}\theta^i\theta^j.
\end{equation}
...
\begin{equation}\label{eq:math-dg-gauss-weingarten}
\boxed{
\begin{aligned}
\overline\nabla_XY&=\nabla_XY+II(X,Y)n,\\
\overline\nabla_Xn&=-AX.
...
\]
即
\begin{equation}\label{eq:math-dg-gauss-equation}
\boxed{K=\det A.}
\end{equation}
...
\]
则移动标架的两个独立方程可写为
\begin{equation}\label{eq:math-dg-codazzi-form-one}
\dd\omega^3{}_1+\omega^3{}_2\wedge\omega^2{}_1=0,
\end{equation}
\begin{equation}\label{eq:math-dg-codazzi-form-two}
\dd\omega^3{}_2+\omega^3{}_1\wedge\omega^1{}_2=0,
\end{equation}
并等价于分量式
\begin{equation}\label{eq:math-dg-codazzi-component}
\boxed{h_{ij;k}=h_{ik;j}.}
\end{equation}
余维一只有一个 shape operator，所以 Ricci 方程中的交换子自动为零；normal bundle 也是一维，Ricci 方程不再给出额外独立条件。



接下来考察Gauss map、平行超曲面与主曲率的几何意义。
...
单位法向本身定义 Gauss map
\begin{equation}
\mathcal N:M\longrightarrow S^m,
\qquad
\mathcal N(p)=n(p).
\label{eq:math-dg-gauss-map}
\end{equation}
...
\begin{equation}
(\mathcal N^*g_{S^m})(X,Y)
=g(AX,AY).
\label{eq:math-dg-third-fundamental-form}
\end{equation}
右边常称为第三基本形式。另一方面，Gauss map 的有向 Jacobian 为
\begin{equation}
\det(\dd\mathcal N)
=(-1)^m\det A.
\label{eq:math-dg-gauss-map-jacobian}
\end{equation}
...
\dd F_s(X)
=(I-sA)X.
\label{eq:math-dg-parallel-hypersurface-differential}
\end{equation}
...
\label{eq:math-dg-parallel-shape-operator}
\end{equation}
同时面积元按
\begin{equation}
\vol_{g_s}
=|\det(I-sA)|\,\vol_g
=\prod_{i=1}^m|1-s\kappa_i|\,\vol_g
...
\begin{equation*}
\vol_{g_s}=(1-sm\mathsf H+O(s^2))\,\vol_g.
\end{equation*}
对 $s$ 求导并在 $s=0$ 取值，
\begin{align*}
\left.\frac{\dd}{\dd s}\vol_{g_s}\right|_{s=0}
&=\left.\frac{\dd}{\dd s}\bigl[(1-sm\mathsf H+O(s^2))\,\vol_g\bigr]\right|_{s=0}\\
&=\bigl[-m\mathsf H+O(s)\bigr]\Big|_{s=0}\vol_g\\
&=-m\mathsf H\,\vol_g.
\end{align*}
...
\begin{equation*}
\frac12\left.\frac{\dd^2}{\dd s^2}\vol_{g_s}\right|_{s=0}
=\frac12(\sigma_1^2-\sigma_2)\vol_g
=\frac{m^2\mathsf H^2-|A|^2}{2}\vol_g
\end{equation*}
给出二阶变分，是研究 minimal 子流形稳定性的关键。当 $m^2\mathsf H^2=|A|^2$ 时二阶项消失，对应全测地子流形。

\emph{管状邻域与 Weyl 公式。}对 $\RR^n$ 中紧子流形 $M^m$，管状邻域体积为
\begin{equation*}
\operatorname{Vol}(T_r(M))
=\omega_{n-m}\int_M\int_0^r\prod_{i=1}^m(1-s\kappa_i)\,\dd s\,\vol_g,
\end{equation*}
其中 $\omega_{n-m}$ 是单位 $(n-m)$-球体积。展开后被积函数是 $\sigma_k$ 的组合，对偶数余维给出 Weyl 管公式
\begin{equation*}
\operatorname{Vol}(T_r(M))
=\sum_{k=0}^{\lfloor(n-m)/2\rfloor}\frac{\omega_{n-m-2k}r^{n-m-2k}}{(n-m-2k)\cdots(n-m)}\int_M\sigma_{2k}\,\vol_g.
\end{equation*}
只有偶数阶 $\sigma_{2k}$ 出现，反映管体积对法向反射对称。这些积分是 Lipschitz--Killing 曲率，与热核渐近展开系数一致。

\emph{举例：球面与圆柱。}对 $S^m(r)\subset\RR^{m+1}$，主曲率全为 $\kappa_i=1/r$，平均曲率 $\mathsf H=1/r$。平行曲面 $S^m(r+s)$ 面积元为 $(1+s/r)^m\vol_g$，一阶项 $m/r\cdot s=m\mathsf H s$ 与公式一致。对圆柱 $S^1(r)\times\RR\subset\RR^3$，主曲率为 $(1/r,0)$，$\mathsf H=1/(2r)$，平行曲面面积元为 $(1+s/r)\vol_g$，一阶项 $s/r=2\mathsf H s$，验证余维一情形。

\emph{物理建模：膜与肥皂膜。}极小曲面方程 $\mathsf H=0$ 是肥皂膜的平衡条件，对应面积泛函的临界点。平行曲面公式给出膜在法向小扰动下面积的一阶响应：$\mathsf H=0$ 时一阶项消失，膜处于平衡；二阶项 $\frac12\int_M(m^2\mathsf H^2-|A|^2)\vol_g=-\frac12\int_M|A|^2\vol_g$ 在非全测地时为负，反映膜在大多数情形下不稳定，需要边界约束维持。这与 Plateau 问题的解存在性密切相关。
\end{derivation}
...
=-m\int_M
\overline g(\mathbf H,V^\perp)\,\vol_g
}.
...
\textbf{意义。}一阶变分公式 $\delta\operatorname{Area}=-\int_M m\,\overline g(\mathbf H,V^\perp)\dd\vol_g$ 说明面积泛函的 Euler--Lagrange 方程是平均曲率 $\mathbf H=0$——这就是极小子流形方程。切向变分只产生散度项（边界项），不改变面积；法向变分通过平均曲率与面积变化耦合。物理上，肥皂膜在表面张力下平衡时正是极小曲面：$\mathbf H=0$ 是 Young--Laplace 压力平衡在零压差时的特例。

\textbf{实例。} (1) $\mathbb R^3$ 中的平面：第二基本形式 $B_{ij}=0$，故 $\mathbf H=0$，面积在任意紧支撑变分下稳定。(2) $\mathbb R^3$ 中的球面 $\mathbb S^2(R)$：$B_{ij}=(1/R)g_{ij}$，$\mathbf H=(1/R)n\neq0$，球面不是极小曲面——缩小半径可减少面积。(3) 悬链面 $z=\cosh^{-1}(r)$：可验证 $\mathbf H=0$，这是 $\mathbb R^3$ 中唯一的非平凡旋转极小曲面，对应于两圆环间肥皂膜的平衡形状。
\end{derivation}
...
\left(
|\nabla\phi|^2
-\bigl(|A|^2+\overline{\Ric}(n,n)\bigr)\phi^2
\right)\vol_g
}.
...
\begin{equation}
\delta^2\operatorname{Vol}[\phi]
=\int_M\phi\,L_J\phi\,\vol_g,
\qquad
L_J:=-\Delta_{\mathrm{LB}}
-\left(|A|^2+\overline\Ric(n,n)\right).
\label{eq:math-dg-minimal-jacobi-operator}
\end{equation}
...
\emph{Bernstein 定理的 Simons 证明。}对 minimal graph $u:\Omega\subset\RR^n\to\RR$，Simons identity 结合 Bochner 技巧给出
\begin{equation*}
...
\emph{Chern--Simons 形式的关联。}对三维流形上的 $SU(2)$ 联络，Chern--Simons 形式
\begin{equation*}
...
\emph{举例：Clifford 极小环面。}在 $S^3\subset\RR^4$ 中，Clifford 极小环面
\begin{equation*}
...
\emph{物理建模：弦论中的极小曲面。}弦论中开弦的世界面是 Dirichlet 边界条件下的极小曲面，闭弦的世界面是体积泛函的临界点。Simons identity 给出世界面方程在量子涨落下稳定性的判据：$|A|^2$ 满足的椭圆不等式控制弦世界面在背景时空中的涨落尺度，与弦论中 $\alpha'$ 修正的几何展开直接相关。
\end{derivation}
...
\mathcal A[u]
=\int_\Omega\sqrt{1+|\nabla u|^2}\,\dd^m x.
\label{eq:math-dg-minimal-graph-area}
\end{equation}
...
\label{eq:math-dg-minimal-surface-equation}
\end{equation}
在小斜率 $|\nabla u|\ll1$ 下展开
\[
\frac{1}{\sqrt{1+|\nabla u|^2}}
...
=-m^2\int_{M_t}|\mathbf H|^2\,\vol_{g_t}\le0.
\]
