...
用缩放同伦 $F_t(x)=tx$ 将 $t=1$ 的形式 $\alpha$ 连续拉回到 $t=0$（原点），沿途的"变化率"由 Cartan 公式控制，积分后得到 同伦 恒等式。
...
闭 implies 恰当。若 $k\ge1$ 且 $\dd\alpha=0$，常值映射 $F_0$ 把 $k$-形式拉回为零（$k\ge1$ 时原点处切空间退化），故 $F_0^*\alpha=0$。代入 同伦 恒等式：
\begin{equation*}
 \alpha=\dd(K\alpha),
\end{equation*}
即 $\alpha$ 恰当，势为 $K\alpha$。

这说明局部上"闭 implies 恰当"来自空间可以沿径向连续缩到一点；全局 上同调 正是这种收缩在整体上失败留下的信息。
\end{derivation}
...
\begin{derivation}[正则 水平集 的切空间为何是微分的核]
分两步证明双向包含。先证 $T_pF^{-1}(y)\subseteq\ker F_{*p}$。任取切向量 $v\in T_pF^{-1}(y)$，由切空间定义存在光滑曲线 $\gamma:(-\delta,\delta)\to F^{-1}(y)$ 满足 $\gamma(0)=p$、$\dot\gamma(0)=v$。因 $\gamma$ 像落在 水平集 中，
\[
F(\gamma(t))\equiv y
\]
...
再证 $\ker F_{*p}\subseteq T_pF^{-1}(y)$。因 $y$ 是 正则 值，$F_{*p}$ 满射，由 淹没 正规 形式 可取 $p$ 附近局部坐标使
\[
...
举例：球面 $S^{n-1}\subset\RR^n$。取 $F:\RR^n\to\RR$，$F(x)=|x|^2$，则 $S^{n-1}=F^{-1}(1)$。$F$ 的微分 $\dd F_x(v)=2x\cdot v$ 在 $|x|=1$ 时满射（非零），故 $1$ 是 正则 值，$S^{n-1}$ 是 $n-1$ 维子流形，切空间
\[
...
正则 水平集 定理是约束系统力学的基础：构形空间 $Q$ 上的约束 $F(q)=0$（$F:Q\to\RR^k$）在 正则 条件下给出约束子流形 $Q_{\mathrm c}=F^{-1}(0)$，其切空间 $T_qQ_{\mathrm c}=\ker\dd F_q$ 是虚位移空间，Lagrange 乘子 $\lambda$ 正是约束力。这一“约束即光滑子流形”的图景在微正则系综（能量约束 $H=E$）与规范约束（Gauss 定律 $\nabla\cdot E=\rho$）中反复出现：正则 条件保证约束面光滑，Liouville 测度与规范轨道空间都有良定义的几何。
\end{derivation}
...
横截条件 $\Leftrightarrow$ $0$ 是 正则 值。$\pi_\perp$ 的微分是到后 $n-r$ 个坐标分量的投影。复合映射的微分
\begin{equation*}
...
应用 正则 值 定理。既然 $0$ 是 $\pi_\perp\circ F$ 的 正则 值，正则 值 定理 给出 $F^{-1}(S)=(\pi_\perp\circ F)^{-1}(0)$ 是 $M$ 的嵌入子流形，余维数 $n-r=\operatorname{codim}_N S$，即\eqnrefs{eq:math-dg-transverse-codimension}。切空间是 $\dd(\pi_\perp\circ F)_p$ 的核，即 $\{v\in T_pM:\dd F_p(v)\in T_{F(p)}S\}$，即\eqnrefs{eq:math-dg-transverse-tangent-space}。

横截 原像 定理 因而不是新的局部正规形，而是把 正则 值 定理 应用于"法向坐标分量"。

例如 $F(t)=(t,t^2)$ 与 $x$ 轴在 $t=0$ 处横截（$\dd F_0=(1,0)$ 与 $T_{(0,0)}S$ 张成 $\RR^2$），$F^{-1}(S)=\{0\}$ 为零维子流形；而 $F(t)=(t,0)$ 与 $x$ 轴非横截，$F^{-1}(S)=\RR$ 为一维，经微扰 $F_\varepsilon(t)=(t,\varepsilon)$ 后横截 原像 为空，体现横截性的稳定性。约束力学中 $Q_{\mathrm c}=\{q:F(q)=0\}$（$F:\RR^{3N}\to\RR^k$）在 $\dd F_q$ 满秩时为 $3N-k$ 维子流形；纤维积 $M\times_NP$ 即 $(F,G)$ 与对角 $\Delta_N$ 的横截 原像，维数 $\dim M+\dim P-\dim N$。一般地，两 $a,b$ 维子流形横截相交给出 $\dim(A\cap B)=a+b-n$，这是"未知量数减约束数"的严格版本。

横截性是 模 空间 理论与一般存在性论证的基础：规范理论的瞬子模空间 $\mathcal M$ 由自对偶方程 $F_A^+=0$ 定义，横截性保证它是光滑流形，维数由 Atiyah--Singer 指标定理给出。其一般形式正是 Sard 定理——横截相交在参数扰动下保持稳定，这一稳定性是各类“泛型对象光滑”结论的共同来源。
\end{derivation}
...
\begin{derivation}[Cartan 演算 从生成元恒等式闭合]
先在函数与一形式上验证
\[
[\mathcal L_X,\iota_Y]=\iota_{[X,Y]}.
\]
...
\end{aligned}
\]
两边都是次数 $-1$ 的导子，所以在整个外代数上一致，得到\eqnrefs{eq:cartan-lie-iota}。再用 分次 Jacobi 恒等式 与
\[
\mathcal L_X=[\dd,\iota_X]_{\rm gr}
\]
可得
