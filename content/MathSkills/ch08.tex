...
例如 $\RR^n$ 中 $\gamma(t)=p+tv$ 直接给出 $v_\gamma=v^i\partial_i$；$S^2$ 北极处曲线 $\gamma(t)=(\sin t,0,\cos t)$ 给出 $\partial_\theta|_p=(1,0,0)\in T_pS^2$；环面 $T^n$ 的坐标曲线给出 $\{\partial_{\theta^j}\}$，Lie 群 $G$ 的左不变向量场 $X(g)=L_{g*}X_e$ 则以曲线速度解释 $\exp$ 映射。

由此 $T_pM$ 在坐标变换下分量按 Jacobi 矩阵协变变换（$v^a=(\partial y^a/\partial x^i)v^i$），维数恰为 $\dim M=n$，余切空间 $T_p^*M=(T_pM)^*$ 由 $\dd x^i$ 张成并满足 $\dd f(v)=v(f)$；局部平凡化 $TM\cong U\times\RR^n$ 的结构群即 $GL(n,\RR)$。此定义可直接推广到 Banach 流形，场构型 $\phi$ 的变分 $\delta\phi$ 正是构形空间上的切向量。
\end{derivation}
...
regular level set 定理是约束系统力学的基础：构形空间 $Q$ 上的约束 $F(q)=0$（$F:Q\to\RR^k$）在 regular 条件下给出约束子流形 $Q_{\mathrm c}=F^{-1}(0)$，其切空间 $T_qQ_{\mathrm c}=\ker\dd F_q$ 是虚位移空间，Lagrange 乘子 $\lambda$ 正是约束力。这一“约束即光滑子流形”的图景在微正则系综（能量约束 $H=E$）与规范约束（Gauss 定律 $\nabla\cdot E=\rho$）中反复出现：regular 条件保证约束面光滑，Liouville 测度与规范轨道空间都有良定义的几何。
\end{derivation}
...
拉回与外微分交换是 de Rham 上同调自然性的基础：$F^*$ 诱导上同调群间的同态 $F^*:H^k_{\mathrm{dR}}(N)\to H^k_{\mathrm{dR}}(M)$。它保证坐标不变的物理结论不随拉回改变，例如电磁学中场强 $F=\dd A$ 在规范变换 $A\mapsto A+\dd\chi$ 下不变，正是 $\dd^2=0$ 与拉回交换性的结合。
\end{derivation}
...
例如 $F(t)=(t,t^2)$ 与 $x$ 轴在 $t=0$ 处横截（$\dd F_0=(1,0)$ 与 $T_{(0,0)}S$ 张成 $\RR^2$），$F^{-1}(S)=\{0\}$ 为零维子流形；而 $F(t)=(t,0)$ 与 $x$ 轴非横截，$F^{-1}(S)=\RR$ 为一维，经微扰 $F_\varepsilon(t)=(t,\varepsilon)$ 后横截 preimage 为空，体现横截性的稳定性。约束力学中 $Q_{\mathrm c}=\{q:F(q)=0\}$（$F:\RR^{3N}\to\RR^k$）在 $\dd F_q$ 满秩时为 $3N-k$ 维子流形；纤维积 $M\times_NP$ 即 $(F,G)$ 与对角 $\Delta_N$ 的横截 preimage，维数 $\dim M+\dim P-\dim N$。一般地，两 $a,b$ 维子流形横截相交给出 $\dim(A\cap B)=a+b-n$，这是"未知量数减约束数"的严格版本。

横截性是 moduli space 理论与一般存在性论证的基础：规范理论的瞬子模空间 $\mathcal M$ 由自对偶方程 $F_A^+=0$ 定义，横截性保证它是光滑流形，维数由 Atiyah--Singer 指标定理给出。其一般形式正是 Sard 定理——横截相交在参数扰动下保持稳定，这一稳定性是各类“泛型对象光滑”结论的共同来源。
\end{derivation}
...
例如 $F(x)=Ax$（$A$ 可逆）把径向场 $X=x^i\partial_i$ 推前为同方向径向场，$\ee^{tI}$ 与 $A$ 对易即给出流共轭；极坐标 $F:(r,\theta)\mapsto(r\cos\theta,r\sin\theta)$ 把 $\partial_\theta$ 推前为旋转场 $-y\partial_x+x\partial_y$，$F(r,\theta+t)$ 正是旋转流。

此共轭性是对称性约化的基础：$G$ 作用下的不变向量场推前到商空间 $M/G$，流的共轭保证约化流与原流相容；Noether 定理即对称流与 Hamilton 流相容 $[X,X_H]=0$ 时给出守恒量。
\end{derivation}
...
例如 $X=\partial_x$、$Y=x\partial_y$ 给出 $[X,Y]=\partial_y$，群交换子 $\Phi_t\Psi_s\Phi_{-t}\Psi_{-s}(x,y)=(x,y+st)$ 的位移 $st$ 恰为 $[X,Y]$ 的流。Heisenberg 群中 $X=\partial_x-\frac12y\partial_z$、$Y=\partial_y+\frac12x\partial_z$ 给出 $[X,Y]=\partial_z$（中心方向），群交换子在 $ts$ 阶产生 $z$ 位移 $st$，量子上对应正则对易 $[\hat x,\hat p]=\ii\hbar$；$SO(3)$ 中两个旋转场的括号即交叉积法则 $[e_z,e_x]=e_y$，是刚体 Euler 方程的几何来源。

故 $G$ 的左不变向量场在恒等元处的括号给出 $\mathfrak g$，群交换子在恒等元附近展开为 $ts[X,Y]$；Hamilton 向量场满足 $[X_H,X_F]=X_{\{H,F\}}$，同一括号在量子层面对应算子交换子、在规范理论中对应场强的非交换部分 $[A_\mu,A_\nu]$。
\end{derivation}
...
此结构在各领域反复出现：收缩流形上闭形式 $\dd\omega=0$ 由 $\mathcal L_X=\dd\iota_X+\iota_X\dd$ 给出同伦算子 $h=\int_0^1\Phi_t^*\iota_X\dd t/t$，使 $\omega=\omega_0+\dd(h\omega)$，即 Poincar\'e 引理；Minkowski 时空上 $F=\dd A$ 的 Bianchi 恒等式 $\dd F=\dd^2A=0$ 给出齐次 Maxwell 方程；不可压 Euler 的涡度 $\omega=\dd v$ 满足 $\partial_t\omega+\mathcal L_v\omega=0$，Kelvin 环量定理即其积分形式。Lie 群上 Maurer--Cartan 方程 $\dd\theta+\frac12[\theta\wedge\theta]=0$ 是 $\dd^2=0$ 的非线性推广，Frobenius 可积条件则等价于消没形式 $\dd\alpha\equiv0\pmod{\mathcal D}$。

故外微分与 Hodge 对偶把电磁场强、Bianchi 恒等式与 Maxwell 方程 $\dd *F=J$ 写成统一语言，其核心 $\dd^2=0$ 与 Cartan 恒等式在 Berry 相位、热力学 Maxwell 关系与涡度守恒中反复出现，根源即外代数上的分次微分结构。
\end{derivation}
