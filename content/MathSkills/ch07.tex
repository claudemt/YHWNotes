...
爆破 择一 的三个典型应用如下。热方程 $\partial_t u=\Delta u-u^3$ 取 $X=L^2(\RR^n)$，能量估计
\[
\frac12\frac{\dd}{\dd t}\|u\|_{L^2}^2
=-\|\nabla u\|_{L^2}^2-\int u^4\,\dd x
\le0
\]
使 $\|u(t)\|_{L^2}\le\|u_0\|_{L^2}$ 一致有界，由 择一 即全局存在。ODE $u'=u^2$、$u(0)=u_0>0$ 的解 $u(t)=u_0/(1-tu_0)$ 在 $T_{\max}=1/u_0$ 处爆破，$\|u(t)\|\to\infty$ 恰与判据一致。波方程 $\dd_t^2u-\Delta u=0$ 取 $X=H^1(\RR^3)\times L^2(\RR^3)$，能量
\[
E(t)=\frac12\int(|\dot u|^2+|\nabla u|^2)\,\dd x
\]
守恒，$\|u(t)\|_X\le C\|u_0\|_X$ 一致有界，亦全局存在。流体力学中，Navier--Stokes 的 $H^s$ 估计 $\frac{\dd}{\dd t}\|u\|_{H^s}\le C\|u\|_{H^s}^{1+2/(s-1)}$ 把 Millennium 问题归结为 $\|u(t)\|_{H^s}$ 是否在有限时间爆破；经典 Yang--Mills 方程的 爆破 分析决定规范场是否形成有限时间奇点。
