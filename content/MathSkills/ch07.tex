...
\]
由上面的唯一性拼接，$u$ 与 $v_n$ 在 $[t_n,T_{\max})$ 上一致，故 $\tilde u$ 是 $[0,t_n+\tau]$ 上的 mild solution，其存在时间超过 $T_{\max}$，与 $T_{\max}$ 的最大性矛盾。因此若 $T_{\max}<\infty$，解不可能保持有界，即\eqnrefs{eq:semilinear-blowup-alternative}。
...
流体力学中，Navier--Stokes 的 $H^s$ 估计 $\frac{\dd}{\dd t}\|u\|_{H^s}\le C\|u\|_{H^s}^{1+2/(s-1)}$ 把 Millennium 问题归结为 $\|u(t)\|_{H^s}$ 是否在有限时间爆破；经典 Yang--Mills 方程的 blow-up 分析决定规范场是否形成有限时间奇点。
\end{derivation}
...
对\eqnrefs{eq:zakharov-water-wave}中的 $G(\eta)$ 及非线性边界条件按 $(\varepsilon,\mu)$ 展开，会得到一族彼此经近恒等变量变换相关的 Boussinesq 系统。若取 distinguished limit
\[
\varepsilon=O(\mu)
\]
