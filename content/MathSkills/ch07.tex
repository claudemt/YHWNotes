...
\end{equation}
满足\eqnrefs{eq:duhamel-semilinear} 的连续函数称为 温和解。若它还落在 $D(A)$ 中并具有足够时间正则性，才可进一步称为 经典解。这个区分在热方程、Navier--Stokes、NLS 等无界生成元问题中很重要：形式上的 $u_t=Au+F(u)$ 并不自动保证 $Au$ 逐点有意义。

局部适定性的标准输入是：在某个半径 $R$ 的球上，$F$ 局部 Lipschitz，且半群满足
\[
\|S(t)\|\le M\ee^{\omega t}.
\]
此时对足够短的时间区间，\eqnrefs{eq:duhamel-semilinear} 的右端定义一个压缩映射，于是存在唯一 温和解。真正的难点因此被分成两部分：线性生成元决定传播估计，非线性项决定 Lipschitz 或更精细的 multilinear 估计。

\begin{derivation}[Duhamel 公式与局部压缩映射]
先假设 $u$ 是 经典解，并固定终点 $t>0$。对参数 $s\in[0,t]$ 定义
\[
w(s):=S(t-s)u(s).
\]
...
局部不动点只说明短时间存在，不能自动推出解能延伸到任意时刻。设\eqnrefs{eq:abstract-semilinear-evolution}在初值 $u_0$ 下的唯一 温和解 定义在最大时间区间
\[
0\le t<T_{\max}.
\]
...
\begin{derivation}[局部适定性怎样给出 爆破 择一]
用反证法证明：若 $T_{\max}<\infty$ 且解有界，则可延拓过 $T_{\max}$，矛盾。

假设有界。设 $T_{\max}<\infty$，但存在常数 $R<\infty$ 使
\[
  \sup_{0\le t<T_{\max}}\|u(t)\|_X\le R.
\]

统一存在时间。取任意序列 $t_n\uparrow T_{\max}$。所有 $u(t_n)$ 位于半径 $R$ 的球内。$F$ 在半径 $R+1$ 的球上具有统一 Lipschitz 常数 $L_{R+1}$ 和界 $M_{R+1}$。局部压缩映射证明给出的存在时间仅依赖这些参数：
\[
  \tau=\tau(R,M_{R+1},\omega,L_{R+1})>0,
\]
与具体初值 $u(t_n)$ 和起始时刻 $t_n$ 无关。

重启并延拓。把 $u(t_n)$ 当作新的初值，从 $t_n$ 重新启动积分方程
\[
  u(t)=S(t-t_n)u(t_n)+\int_{t_n}^t S(t-s)F(u(s))\,\dd s.
\]
局部适定性给出定义在 $[t_n,t_n+\tau]$ 上的唯一 温和解 $v_n(t)$。

唯一性拼接。在重叠区间 $[t_n,T_{\max})$ 上，$u(t)$ 和 $v_n(t)$ 满足同一积分方程且同一初值 $u(t_n)$。由局部唯一性，$u(t)=v_n(t)$ 对所有 $t\in[t_n,T_{\max})$ 成立。

矛盾。取 $n$ 足够大使 $t_n+\tau>T_{\max}$（因 $t_n\to T_{\max}$ 且 $\tau>0$ 固定）。则 $v_n$ 把 $u$ 延拓到 $T_{\max}$ 之后：
\[
...
举例：热方程的全局存在。取 $X=L^2(\RR^n)$，$A=\Delta$，$F(u)=-u^3$。能量估计给出
\[
\frac12\frac{\dd}{\dd t}\|u\|_{L^2}^2
=-\|\nabla u\|_{L^2}^2-\int u^4\,\dd x
\le0,
\]
故 $\|u(t)\|_{L^2}\le\|u_0\|_{L^2}$ 一致有界，爆破 择一 给出全局存在。

举例：非线性爆破。对 ODE $u'=u^2$，$u(0)=u_0>0$，解 $u(t)=u_0/(1-tu_0)$ 在 $T_{\max}=1/u_0$ 爆破，$\|u(t)\|\to\infty$ 符合判据。

举例：波方程的能量守恒。取 $X=H^1(\RR^3)\times L^2(\RR^3)$，方程 $\dd_t^2 u-\Delta u=0$。能量
\[
E(t)=\frac12\int(|\dot u|^2+|\nabla u|^2)\,\dd x
\]
守恒，$\|u(t)\|_X\le C\|u_0\|_X$ 一致有界，全局存在。

流体力学中，Navier--Stokes 的 $H^s$ 估计 $\frac{\dd}{\dd t}\|u\|_{H^s}\le C\|u\|_{H^s}^{1+2/(s-1)}$ 把 Millennium 问题归结为 $\|u(t)\|_{H^s}$ 是否在有限时间爆破；经典 Yang--Mills 方程的 爆破 分析决定规范场是否形成有限时间奇点。
\end{derivation}
...
\end{aligned}
\]
其中第二个等号用了分部积分，边界项因 $D^\alpha u\in H^1\hookrightarrow L^2$ 在无穷远衰减而消失。故只需 $\|\nabla a\|_{L^\infty}$。其余交换子项用 Sobolev 乘积估计得到
\[
\|[D^\alpha,a]\nabla u\|_{L^2}
\le C\bigl(
...
\bigr).
\]
若 $a=a(u)$，Moser 估计再把 $\|a\|_{H^m}$ 和 $\|\nabla a\|_\infty$ 控制到 $u$ 的相应范数上，于是得到\eqnrefs{eq:quasilinear-sobolev-energy}。非整数 $s$ 的证明用 Fourier 乘子 交换子估计完成，结构完全相同。

举例：常系数输运方程。取 $a$ 为常数向量，$\nabla a=0$，交换子项全部消失，能量不等式变为
\[
\frac{\dd}{\dd t}\|u\|_{H^s}\le0,
\]
即 $H^s$ 范数守恒，解 $u(t,x)=u_0(x-at)$ 是 $H^s$ 等距。

举例：Burgers 方程。取 $a(u)=u$，$d=1$，则
\[
...
对不可压 Euler（$a(u)=u$，$d=3$），涡量 $\omega=\nabla\times u$ 满足 $\dot{\|\omega\|}_\infty\le C\|\omega\|_\infty^2$，能量估计化为
\[\frac{\dd}{\dd t}\|u\|_{H^s}\le C\|\omega\|_{L^\infty}\|u\|_{H^s}.\]
这正是 Beale--Kato--Majda 判据的核心：$\int_0^T\|\omega(t)\|_\infty\dd t<\infty$ 保证 $[0,T]$ 上经典存在。Schr\"odinger 方程 $\ii\partial_t\psi=-\Delta\psi+V\psi$ 的 $H^s$ 估计结构相同，但 $A=\ii\Delta$ 反自伴、能量守恒而全局存在。
\end{derivation}
