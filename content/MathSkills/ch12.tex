...
Hodge Laplacian 的椭圆性是 Hodge 定理成立的分析基础：椭圆 + 自伴 + 非负 + 紧致流形 $\Rightarrow$ 谱离散且有限维核。在物理中，这对应规范理论中"规范场方程的解空间有限维"（如 Yang--Mills 瞬子模空间），以及弦理论中 Laplacian 谱决定物理态能级。Weitzenböck 公式把椭圆性 + 曲率正性结合，给出 Bochner 消没定理——曲率通过低阶项直接控制核空间维数。
\end{derivation}

Hodge Laplacian 的主符号恰为 $|\xi|_g^2$（式~\eqnrefs{eq:math-dg-hodge-principal-symbol}），这是其椭圆性与谱离散性的来源。
...
\delta\alpha=-\nabla^i\alpha_i.
\end{equation*}
第一个是外微分的协变分量形式（无挠使 Christoffel 项相消），第二个是余微分的分量约定（散度的负号来自本书非负 Laplacian 约定）。
...
代入并化简。将两个下界代入上面的 Bochner 恒等式（移项后右端 $\ge0$）：
\begin{align*}
 0
...
两类边界条件对应物理中不同的场边界行为：absolute（Neumann 型）对应"法向通量为零"（如绝缘边界），relative（Dirichlet 型）对应"切向场为零"（如固定边界）。在规范理论中，absolute 边界条件保持规范对称，relative 边界条件固定规范。在热方程方法中，两类边界条件对应不同的热核渐近展开，其系数含边界曲率项（如 Gilkey--Greiner 公式），给出带边流形的指标定理与局部指标密度。Atiyah--Patodi--Singer $\eta$ 算子进一步引入非局部边界条件，把 eta 不变量纳入指标公式，是带边流形指标定理的现代形式。
\end{derivation}
