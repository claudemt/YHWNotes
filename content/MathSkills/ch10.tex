...
\begin{derivation}[局部 联络形式 的非齐次 规范 变换]
换局部截面 $s'=sg$ 时，新截面沿 $X$ 的切向量 $s'_*X$ 既含旧截面切向量的右平移，又含 $g$ 自身变化产生的垂直分量。Connection 形式 对水平部分用 $G$-等变性、对垂直部分用 reproduction 性质，两项合起来给出非齐次变换律。

分解新截面切向量。令 $s'=sg$，其中 $g:U\to G$。对 $X\in T_xM$，先计算新截面沿 $X$ 的切向量 $s'_*X$。由乘积截面微分，$s'_*X$ 同时含有 $s_*X$ 经右平移到 $s(x)g(x)$ 的水平部分，以及 $g(x)$ 自身沿 $X$ 变化产生的垂直部分。把后者经右平移回到单位元再经 基本 竖直 场 嵌入，得
\begin{equation*}
s'_*X
...
代入 联络形式。由 $A'(X)=\boldsymbol\omega_{s'(x)}(s'_*X)$ 且 $s'(x)=s(x)g(x)$，代入上式并利用 联络形式 的线性性，
\begin{equation*}
A'(X)
...
处理水平项。对第一项用 $G$-等变性 $\boldsymbol\omega_{sg}\bigl((R_g)_*v\bigr)=\operatorname{Ad}_{g^{-1}}\boldsymbol\omega_s(v)$，取 $v=s_*X$，
\begin{equation*}
\boldsymbol\omega_{sg}\bigl((R_g)_*s_*X\bigr)
=\operatorname{Ad}_{g^{-1}}\boldsymbol\omega_s(s_*X)
=\operatorname{Ad}_{g^{-1}}A(X).
\end{equation*}
Equivariance 是 联络形式 的定义公理之一，几何意义是"右作用把 联络形式 按 伴随 表示拉回"。

处理垂直项。对第二项用 reproduction 性质 $\boldsymbol\omega_p\bigl(\xi_P(p)\bigr)=\xi$，取 $\xi=g^{-1}\dd g(X)$、$p=sg$，
\begin{equation*}
\boldsymbol\omega_{sg}\bigl(\bigl(g^{-1}\dd g(X)\bigr)_P(sg)\bigr)
=g^{-1}\dd g(X).
\end{equation*}
Reproduction 性质 是 联络形式 的另一条定义公理：在 基本 竖直 场 上 联络形式 是恒等映射。
...
\end{equation*}
即\eqnrefs{eq:principal-local-connection-transform}。非齐次项来自"局部截面本身也在变化"，所以任何把 联络 系数 当作普通 张量 的做法都会漏掉这一项。
...
\end{equation*}
由 $\dd^2=0$ 第一项消失。对第二项用 分次 Leibniz 规则 $\dd(\alpha\wedge\beta)=\dd\alpha\wedge\beta+(-1)^{|\alpha|}\alpha\wedge\dd\beta$，其中 $|\omega|=1$，
\begin{equation*}
\dd(\omega\wedge\eta)
...
\item 一次 $A$ 的交叉项：$\nabla_XA_Y-\nabla_YA_X-A_{[X,Y]}$，组合成 covariant 外微分 $(D_\nabla A)(X,Y)$，即 $A$ 沿旧联络的协变外微分。
...
\begin{equation*}
 \widetilde R=R+D_\nabla A+A\wedge A,
\end{equation*}
与\eqnrefs{eq:math-dg-curvature-finite-perturbation}完全相同。曲率对联络"线性化 + 自相互作用"的结构因此不是坐标巧合，而是 联络 仿射 几何 的必然结果：线性项 $D_\nabla A$ 是一阶变分（用于 Chern--Weil 理论和指标定理），二次项 $A\wedge A$ 是非 Abel 自相互作用（与 Yang--Mills 方程的非线性性同源）。
...
几何意义。第一结构方程 $T^a=\dd\theta^a+\omega^a{}_b\wedge\theta^b$ 说挠率是「外微分加联络修正」；第二结构方程 $\Omega^a{}_b=\dd\omega^a{}_b+\omega^a{}_c\wedge\omega^c{}_b$ 说曲率是联络的「外微分加自乘」。二者合起来把挠率和曲率完全用标架 $\{\theta^a\}$ 和联络 $\{\omega^a{}_b\}$ 表达，无需坐标。对 Riemann 流形（挠率 $T=0$），第一方程显式解出 $\omega^a{}_b$，将其表为 $\dd\theta^a$ 的函数，第二方程则给出曲率——这是 Cartan 活动标架法计算曲率的标准流程。
...
\begin{derivation}[曲率的齐次 规范 变换，即 $F'=g^{-1}Fg$]
出发点。规范 变换下 联络形式 的变换律为
\begin{equation*}
 A'=g^{-1}Ag+g^{-1}\dd g.
\end{equation*}
记 $\eta=g^{-1}\dd g$（纯 规范），则 $A'=g^{-1}Ag+\eta$。目标是对 $F'=\dd A'+A'\wedge A'$ 直接计算并证明 $F'=g^{-1}Fg$。

Maurer--Cartan 恒等式。由 $\eta=g^{-1}\dd g$ 是纯 规范，Maurer--Cartan 方程给出
\begin{equation*}
...
\begin{derivation}[Chern--Weil 类 为何与 联络 无关]
...
迹 把 $D_t$ 退化为 $\dd$。对 迹 而言，$D_t$ 与普通 $\dd$ 的差是 对易子 $[A_t,\cdot]$，其 迹 为零（迹 的循环性 $\operatorname{tr}([X,Y])=\operatorname{tr}(XY)-\operatorname{tr}(YX)=0$）。因此
\begin{equation*}
...
典型例子。
\begin{itemize}
\item $U(1)$ 线丛：$k=1$，$\operatorname{tr}(F)=F$ 是闭二形式，$[F/2\pi]\in H^2_{\mathrm{dR}}(M)$ 是第一 Chern 类 $c_1(E)$。两个不同联络的 $F$ 相差 恰当形式，但积分 $\frac{1}{2\pi}\int_M F$ 给出 Chern 数（如磁通量子化）。
...
更一般结论。Chern--Weil 理论给出从 不变 多项式 on $\mathfrak g$ 到 $H_{\mathrm{dR}}^*(M)$ 的同态：每个 $G$-不变多项式 $P$ 给出闭形式 $P(F)$，其 de Rham 类只依赖丛 $E$ 的拓扑。结合 Atiyah--Singer 指标定理，这些特征类积分给出椭圆算子的解析指标——如 Dirac 算子的 $\hat A$ 类、符号丛的 Chern 字符。这是"曲率局部数据决定全局拓扑"的精确机制。
\end{derivation}
...
\begin{derivation}[path-ordered 指数 的迭代构造]
...
\end{equation*}
其中 $j=0$ 项理解为 $I\,V(0)$。在 $A_t$ 关于 $t$ 一致有界的前提下余项趋于零，从而产生 Dyson 级数
\begin{equation*}
