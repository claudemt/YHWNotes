...
\begin{derivation}[局部联络形式的非齐次规范变换]
...
\begin{derivation}[曲率的齐次规范变换，即 $F'=g^{-1}Fg$]
规范变换下联络形式的变换律为
\begin{equation*}
 A'=g^{-1}Ag+g^{-1}\dd g.
\end{equation*}
...
Bianchi 恒等式是曲率的可积条件：$D\Omega=0$ 说曲率 2-形式在外协变微分下闭，类比于 $\dd^2=0$。这是 Chern--Weil 理论的出发点——$\operatorname{tr}(\Omega^k)$ 在 $D$ 下闭，因此定义了不依赖联络选取的示性类。实例：二维曲面上 $D\Omega=0$ 退化为 $\dd K\,\operatorname{vol}=0$ 在常曲率情形自动成立；四维 Yang--Mills 理论中 $D\Omega=0$ 是第二 Bianchi 恒等式的规范理论形式，与 Bianchi 恒等式 $D\Omega=0$ 共同保证 $\nabla_\mu G^{\mu\nu}=0$（Einstein 方程的协变守恒）。
\end{derivation}
...
\begin{derivation}[Chern--Weil 类为何与联络无关]
...
迹把 $D_t$ 退化为 $\dd$。对迹而言，$D_t$ 与普通 $\dd$ 的差是对易子 $[A_t,\cdot]$，其迹为零（迹的循环性 $\operatorname{tr}([X,Y])=\operatorname{tr}(XY)-\operatorname{tr}(YX)=0$）。因此
\begin{equation*}
