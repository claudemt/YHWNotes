...
在离散模型中，非随机拒绝域未必能恰好达到 size $\alpha$。此时在 likelihood-ratio 边界上以概率 $\gamma\in[0,1]$ 拒绝，并选取 $\gamma$ 使
\[
...
\]
其中 $R_i$ 是 $|D_i|$ 的秩，就得到 Wilcoxon signed-rank 的精确原假设分布。若只假设 $\mathbb P(D_i>0)=1/2$ 且无零差值，则正号个数本身仍服从 $\mathrm{Bin}(n,1/2)$，但 signed-rank 所需的符号--绝对值不变性不再自动成立。这两个检验的假设层级不能混为一谈。
...
\]
这说明 score 落在约束法向余切空间。将 $U_n/\sqrt n$ 用 LAN 线性化并乘以 $I_0^{-1}$，得到的参数空间向量正是 $h_\perp^*+o_{\mathbb P}(1)$，故 Score 也给出相同平方长度。最后
\[
...
在已知方差的正态均值检验 $H_0:\mu=0$ 对 $H_1:\mu\neq0$ 中，LR、Wald 与 Score 统计量都等于 $n\bar X_n^2/\sigma^2$，所以三者精确相同。一般模型只保证它们在正则条件下渐近等价；有限样本中，Wald 统计量还会随非线性重参数化改变。
\end{derivation}
...
\begin{align*}
 ...
 \right].
\end{align*}
它们在有限样本中不是同一个函数：Wald 在 MLE 处估计曲率，Score 在原假设处估计曲率，LR 则直接比较两个极大 likelihood。若 $H_0$ 成立，$\widehat p-p_0=O_{\mathbb P}(n^{-1/2})$。对 Bernoulli KL 距离在 $p_0$ 处作 Taylor 展开，
\[
 2\left[
 p\log\frac p{p_0}
...
\]
三者才在一阶上共同趋于 $\chi^2_1$。若样本全为 $0$ 或全为 $1$，则 $\widehat p$ 落在边界，以上 Wald 形式甚至因 $\widehat p(1-\widehat p)=0$ 而失效，而 Score 与 LR 在 $0<p_0<1$ 下仍有良好定义。这进一步说明 Wald 统计量对参数坐标和边界更敏感；“三者正则渐近等价”绝不是有限样本可互换性。
\end{exbox}
...
\]
而不是 $\chi^2_1$。若 $X_i\iid\mathrm{Unif}(0,\theta)$，最大次序统计量以 $n^{-1}$ 而非 $n^{-1/2}$ 速度逼近端点，支撑依赖参数使普通 score/Fisher 展开失效。有限混合模型在成分重合处还会同时出现不可识别与奇异信息矩阵。遇到这些模型时，应先重新确定局部尺度和极限实验，而不是机械查 Wilks 自由度。
...
原始 multinomial 涨落满足 $Z_n\xrightarrow{d}\mathcal N_k(0,I-ss^{\mathsf T})$，其中 $s=(\sqrt{p_{01}},\ldots,\sqrt{p_{0k}})^{\mathsf T}$，故极限随机向量位于 $s^\perp$。又因为 $s^{\mathsf T}B_{p,0}=0$，$\mathcal C(B_{p,0})$ 是 $s^\perp$ 内的 $q$ 维子空间。投影掉它之后，剩余球对称 Gaussian 分量位于维数
\[
 (k-1)-q
\]
的正交补中，于是得到\eqnrefs{eq:math-stat-pearson-fitted-df}。所以“估计 $q$ 个参数就扣掉 $q$ 个自由度”不是计数口诀，而是 score 方程把经验涨落沿模型 Fisher 切空间的 $q$ 个方向正交消去后的几何结果。
\end{derivation}
...
\]
正则 MLE 的 $n^{-1/2}$ 局部化与 $p_j(\eta_0)>0$ 给出 $\max_j|\delta_{j,n}^{\rm fit}|=O_{\mathbb P}(n^{-1/2})$。而
\[
 \sum_jnp_j(\widehat\eta)\delta_{j,n}^{\rm fit}
 =\sum_jN_j-n=0.
\]
对每一项使用同一个 Taylor 展开，有限个 cell 的三阶余项总和为 $o_{\mathbb P}(1)$，因此
\begin{equation*}\mathcal D_{n,\mathrm{fit}}^{2}
