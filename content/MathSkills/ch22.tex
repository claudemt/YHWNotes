...
估计问题在参数空间中寻找一个点或区域，检验问题则比较两个参数集合。有限样本精确检验依赖特殊模型中的枢轴量或离散分布；一般正则模型的大样本检验则由 \chapref{chap:math-stat-estimation-likelihood} 的 局部渐近正态 二次型统一控制。因而本章不按“某某检验公式”分类，而是先定义检验作为一个受第一类错误约束的决策问题，再从 Neyman--Pearson 的有限样本最优性进入 Fisher 几何，随后由同一个局部法向投影推出 LR、Wald、得分、局部功效与 冗余参数 消元，最后把 Pearson 卡方解释成概率单纯形上的同一投影机制。
...
\]
简单原假设时它退化为普通尾概率；复合原假设中的上确界则保证 冗余参数 不能被随意代成有利于拒绝的值。
...
\]
则称 $\phi^*$ 在该复合备择上 一致最优势（一致最优势检验）。Karlin--Rubin 定理说明：在一参数单调 似然-比 族中，检验
\[
 H_0:\theta\le\theta_0,
 \qquad
 H_1:\theta>\theta_0
\]
可以由 $T$ 的同一个右尾拒绝域得到 一致最优势检验 检验。双侧复合问题通常没有 一致最优势检验 检验，这不是技术缺陷，而是不同备择方向对“最有利的拒绝域”存在冲突。
...
\begin{derivation}[Karlin--Rubin 为何把逐点最强升级为 一致最优势检验]
...
令 $\phi^*$ 为 似然-比 检验，阈值为 $c>0$，拒绝域为
\[
 R=\{t:p_{\theta_1}^{(n)}(t)>c\,p_{\theta_0}^{(n)}(t)\},
\]
即 $\phi^*(t)=\mathds{1}_R(t)$（为简洁略去边界随机化）。令 $\phi$ 是另一个满足 $0\le\phi\le1$ 且
\[
...
\]
由水平约束 $\mathbb E_{\theta_0}\phi\le\mathbb E_{\theta_0}\phi^*$，右端因子 $\mathbb E_{\theta_0}(\phi^*-\phi)\ge0$，又 $c>0$，故
\[
...
设 $\theta\in\R^d$，真值为 $\theta_0$，模型满足 \chapref{chap:math-stat-estimation-likelihood} 的 局部渐近正态。为看清最基本结构，一维形式为
...
\end{equation}
这同时包含 Wilks 定理以及 Wald、得分 的一阶等价性。自由度 $r$ 是从完整参数空间到零假设子流形减少的局部维数，而不是由统计量名称决定的经验表项。

\begin{derivation}[局部渐近正态 中三类统计量的显式投影]
局部渐近正态 主项为
\[
 q_n(h)
...
\[
 ...
 H_1:\mu>\mu_0.
\]
由单调 似然比，水平 $\alpha$ 的 一致最优势检验 检验在
\[
...
\[
 \theta=(\vartheta,\eta),
\]
其中 $\vartheta\in\R^r$ 是关注参数，$\eta\in\R^q$ 是 冗余参数。Fisher 信息写成
\[
 I=
 \begin{pmatrix}
 ...
 \end{pmatrix}.
\]
若 $I_{\eta\eta}$ 可逆，关注参数 得分 中可由 冗余 得分 线性预测的部分应当先投影掉。定义有效 得分
...
\]
它正是 $s_\vartheta$ 对 冗余 得分 线性子空间做 $L^2$ 投影后的残差。其协方差为 Schur 补
...
\begin{derivation}[消去 冗余 参数就是 Fisher 正交投影]
...
\]
把它代回 $q_n$，与 $h_\vartheta$ 无关的项并入常数 $C_n$，得到 线形 局部渐近正态：
\begin{align*}
 ...
 h_\vartheta.
\end{align*}
因此 冗余 参数并不是“把协方差矩阵某一块删掉”。必须先从关注 得分 中减去它在 冗余 得分 线性空间上的 $L^2$ 投影：
\[
...
\]
所以 Schur 补 正是投影后剩余的 Fisher 信息。线形 LR、有效 得分 与关注参数的 Wald 协方差之所以一阶一致，根源就在这个同一个投影后的局部高斯实验。
\end{derivation}
...
对含 冗余参数 的正则 似然，这个对偶性最自然地写成 线形-似然 置信集。把参数分块为 $\theta=(\vartheta,\eta)\in\Theta$，其中 $\vartheta\in\R^r$ 是关注参数。关注参数的可行空间与给定 $\vartheta$ 时的 冗余 切片分别记为
\[
 \mathcal V
...
线形-LR 区域与 Wald 区域的一阶等价可以直接从有效信息看出。前文消去 冗余参数 后的 线形 局部渐近正态 二次型为
\[
 q_n^{\rm prof}(h)
...
\begin{align*}
 ...
 +o_{\mathbb P}(1).
\end{align*}
所以 线形 LR 置信集在 $n^{-1/2}$ 局部尺度上正是 Fisher 有效信息诱导的椭球。Wald 区域是在估计点处直接写出同一个椭球；得分 区域则在候选原假设点处用有效 得分 测量法向距离。三者有限样本边界可以不同，但一阶几何相同。

本章的统一图像是：正则 似然 在 $n^{-1/2}$ 局部尺度上是带 Fisher 度量的随机二次型，检验只测量无约束极大点相对零假设切空间的法向距离；冗余参数 是先投影掉的切向方向，Pearson 检验则把完全相同的几何搬到概率单纯形。下一章中，线性模型会把这些抽象的 Fisher 投影变成观测空间中的显式矩阵投影，并在正态误差下给出有限样本精确 $F$ 理论。
