...
主丛局部截面之间的换页函数完全由结构群的右作用决定；associated bundle 上截面的分量变换则由表示 $\rho$ 把这一右作用翻译成 $W$ 上的线性变换。两步合起来给出"换标架"的统一机制。

局部截面与 transition function。设 $s_\alpha:U_\alpha\to P$、$s_\beta:U_\beta\to P$ 是开覆盖 $\{U_\alpha\}$ 上的两个局部截面。在重叠 $U_\alpha\cap U_\beta$ 上，$s_\alpha(p)$ 与 $s_\beta(p)$ 落在同一根 fiber $P_p$ 中。由于结构群 $G$ 在每根 fiber 上的右作用自由且传递，对每个 $p\in U_\alpha\cap U_\beta$ 存在唯一 $g_{\alpha\beta}(p)\in G$ 使
\begin{equation*}
...
cocycle 条件。在三重重叠 $U_\alpha\cap U_\beta\cap U_\gamma$ 上依次换页，
\begin{equation*}
 s_\gamma
...
associated-bundle 截面的分量变换。设 $E=P\times_\rho W$，$\sigma$ 是 $E$ 上一个截面。在 $U_\alpha$ 上把它写成 $\sigma|_{U_\alpha}=[s_\alpha,w_\alpha]$，其中 $w_\alpha:U_\alpha\to W$ 是局部 $W$-值函数。在重叠上同一截面有两种表示
\begin{equation*}
...
典型例子。取 $P=\operatorname{Fr}(M)$、$G=GL(n,\RR)$：
\begin{itemize}
...
物理建模。规范理论中 matter field 是 associated bundle $P\times_\rho W$ 的截面，$W$ 是内部空间（如 $\CC^n$ 上的表示空间），$g_{\alpha\beta}$ 是规范变换。分量变换律 $w_\alpha=\rho(g_{\alpha\beta})w_\beta$ 正是"波函数在重叠区域上的规范变换"的几何来源。
\end{derivation}
...
外微分的内在公式。对任意一形式 $\alpha$，外微分在两个向量场上满足
\begin{equation*}
 \dd\alpha(X,Y)
...
代入余标架。令 $\alpha=\theta^a$、$X=e_b$、$Y=e_c$。由余标架的对偶性 $\theta^a(e_b)=\delta^a{}_b$（\eqnrefs{eq:math-dg-frame-duality}），
\begin{equation*}
 e_b[\theta^a(e_c)]
 =e_b(\delta^a{}_c)=0,
 \qquad
...
代入结构函数。由\eqnrefs{eq:math-dg-synthesis-anholonomy}，$[e_b,e_c]=C^d{}_{bc}\,e_d$，故
\begin{equation*}
 \dd\theta^a(e_b,e_c)
 =-\theta^a(C^d{}_{bc}\,e_d)
 =-C^d{}_{bc}\,\theta^a(e_d)
...
写出二形式。任意二形式都由其在基向量上的值唯一确定。因 $C^a{}_{bc}$ 对 $(b,c)$ 反对称，展开为楔积时需除以 $2$（重复计数修正）：
...
物理意义。非完整系数 $C^a{}_{bc}$ 在刚体力学中表现为旋转标架下的"假想力"系数（Coriolis、离心）；在广义相对论中，选择非完整标架（如 tetrad）时，$C^a{}_{bc}$ 是连接局部 Lorentz 标架与坐标系的运动学数据，与联络系数 $\omega^a{}_b$ 一起进入 Cartan 第一结构方程 $T^a=\dd\theta^a+\omega^a{}_b\wedge\theta^b$。
\end{derivation}
...
方向一：纯 gauge $\Rightarrow$ 平坦。若 $A=g^{-1}\dd g$，由外微分的 Leibniz 规则 $\dd(g^{-1})=-g^{-1}(\dd g)g^{-1}$（对 $g\cdot g^{-1}=I$ 求微分），
\begin{align*}
 \dd A
...
方向二：平坦 $\Rightarrow$ 纯 gauge。固定基点 $x_0$ 与 $g_0\in G$，沿路径 $\gamma:[0,1]\to U$ 解矩阵 ODE
\begin{equation*}
 \frac{\dd g}{\dd t}
...
路径无关性。若把路径作一个小矩形变形（先沿 $s$ 方向再沿 $t$ 方向，与先 $t$ 再 $s$），两种次序所得解的最低非平凡差由曲率
\begin{equation*}
 F_A=\dd A+A\wedge A
\end{equation*}
...
显式小矩形计算。取 $\RR^2$ 上小矩形 $[0,\epsilon]^2$，先 $s$ 后 $t$ 与先 $t$ 后 $s$ 的乘积相差
\begin{equation*}
...
非单连通区域的修正。若 $U$ 不单连通，平坦条件 $F_A=0$ 只保证局部纯 gauge，全局仍可能非平凡。基本群 $\pi_1(U)$ 通过 holonomy 表示
\begin{equation*}
...
合并循环项。对三个循环项 $(a,b,c)$、$(b,c,a)$、$(c,a,b)$ 求和，并利用 $\{e_d\}$ 线性无关得到
...
从 $\dd^2=0$ 重新得到。对\eqnrefs{eq:math-dg-frame-structure-equation}再作用一次 $\dd$，由 $\dd^2\equiv0$ 得
\begin{equation*}
 0=\dd^2\theta^d
...
显式核对三维。取 $n=3$、$(a,b,c)=(1,2,3)$，Jacobi 约束为
\begin{equation*}
...
更一般结论。这一可积条件是 Frobenius 定理与 Newlander--Nirenberg 定理的共同原型：一组分布可积当且仅当 Lie 括号封闭；一组近复结构可积当且仅当 Nijenhuis 张量消失。在规范理论中，Maurer--Cartan 方程 $\dd A+A\wedge A=0$ 是同一思想在主丛上的版本——"局部存在 $g$ 使 $A=g^{-1}\dd g$"的充要条件。
\end{derivation}
