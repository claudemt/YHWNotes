...
\end{equation*}
即\eqnrefs{eq:principal-transition-cocycle}。由 $s_\alpha=s_\alpha g_{\alpha\alpha}$ 及右作用自由性得 $g_{\alpha\alpha}=e$；把 $s_\beta=s_\alpha g_{\alpha\beta}$ 与 $s_\alpha=s_\beta g_{\beta\alpha}$ 合并再用自由性得 $g_{\alpha\beta}=g_{\beta\alpha}^{-1}$。$\{g_{\alpha\beta}\}$ 因此是取值于 $G$ 的 \v Cech $1$-cocycle，主丛的拓扑分类恰由其 \v Cech 上同调类决定。
...
规范理论中 matter field 是 associated bundle $P\times_\rho W$ 的截面，$W$ 是内部空间（如 $\CC^n$ 上的表示空间），$g_{\alpha\beta}$ 是规范变换。分量变换律 $w_\alpha=\rho(g_{\alpha\beta})w_\beta$ 正是"波函数在重叠区域上的规范变换"的几何来源。
\end{derivation}
...
非完整系数 $C^a{}_{bc}$ 在刚体力学中表现为旋转标架下的"假想力"系数（Coriolis、离心）；在广义相对论中，选择非完整标架（如 tetrad）时，$C^a{}_{bc}$ 是连接局部 Lorentz 标架与坐标系的运动学数据，与联络系数 $\omega^a{}_b$ 一起进入 Cartan 第一结构方程 $T^a=\dd\theta^a+\omega^a{}_b\wedge\theta^b$。
\end{derivation}
...
\begin{equation*}
 F_A=\dd A+A\wedge A
\end{equation*}
在矩形顶点处的值控制（显式小矩形计算见下文）。平坦条件 $F_A=0$ 使这个差为零，因此平行积分对可缩路径变形不敏感。单连通性进一步消除 homotopy obstruction：任意两条具有相同端点的路径同伦，故 $g(x)$ 只依赖终点而不依赖所选路径，得到\eqnrefs{eq:mc-pure-gauge}。
