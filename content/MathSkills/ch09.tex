...
\begin{derivation}[局部截面、转移函数 与 伴随-丛 分量]
主丛局部截面之间的换页函数完全由结构群的右作用决定；伴随丛 上截面的分量变换则由表示 $\rho$ 把这一右作用翻译成 $W$ 上的线性变换。两步合起来给出"换标架"的统一机制。

局部截面与 转移函数。设 $s_\alpha:U_\alpha\to P$、$s_\beta:U_\beta\to P$ 是开覆盖 $\{U_\alpha\}$ 上的两个局部截面。在重叠 $U_\alpha\cap U_\beta$ 上，$s_\alpha(p)$ 与 $s_\beta(p)$ 落在同一根 纤维 $P_p$ 中。由于结构群 $G$ 在每根 纤维 上的右作用自由且传递，对每个 $p\in U_\alpha\cap U_\beta$ 存在唯一 $g_{\alpha\beta}(p)\in G$ 使
\begin{equation*}
...
上闭链 条件。在三重重叠 $U_\alpha\cap U_\beta\cap U_\gamma$ 上依次换页，
\begin{equation*}
 s_\gamma
...
伴随-丛 截面的分量变换。设 $E=P\times_\rho W$，$\sigma$ 是 $E$ 上一个截面。在 $U_\alpha$ 上把它写成 $\sigma|_{U_\alpha}=[s_\alpha,w_\alpha]$，其中 $w_\alpha:U_\alpha\to W$ 是局部 $W$-值函数。在重叠上同一截面有两种表示
\begin{equation*}
...
规范理论中 物质 场 是 伴随丛 $P\times_\rho W$ 的截面，$W$ 是内部空间（如 $\CC^n$ 上的表示空间），$g_{\alpha\beta}$ 是规范变换。分量变换律 $w_\alpha=\rho(g_{\alpha\beta})w_\beta$ 正是"波函数在重叠区域上的规范变换"的几何来源。
\end{derivation}
...
非完整系数 $C^a{}_{bc}$ 在刚体力学中表现为旋转标架下的"假想力"系数（Coriolis、离心）；在广义相对论中，选择非完整标架（如 四脚标架）时，$C^a{}_{bc}$ 是连接局部 Lorentz 标架与坐标系的运动学数据，与联络系数 $\omega^a{}_b$ 一起进入 Cartan 第一结构方程 $T^a=\dd\theta^a+\omega^a{}_b\wedge\theta^b$。
\end{derivation}
...
\begin{derivation}[Maurer--Cartan 方程是纯 规范 表示的可积条件，即\eqnrefs{eq:mc-flat-integrability} 对\eqnrefs{eq:mc-pure-gauge} 的充要性]
方向一：纯 规范 $\Rightarrow$ 平坦。若 $A=g^{-1}\dd g$，由外微分的 Leibniz 规则 $\dd(g^{-1})=-g^{-1}(\dd g)g^{-1}$（对 $g\cdot g^{-1}=I$ 求微分），
\begin{align*}
 \dd A
...
方向二：平坦 $\Rightarrow$ 纯 规范。固定基点 $x_0$ 与 $g_0\in G$，沿路径 $\gamma:[0,1]\to U$ 解矩阵 ODE
\begin{equation*}
 \frac{\dd g}{\dd t}
...
非单连通区域的修正。若 $U$ 不单连通，平坦条件 $F_A=0$ 只保证局部纯 规范，全局仍可能非平凡。基本群 $\pi_1(U)$ 通过 和乐 表示
\begin{equation*}
 \mathrm{Hol}_A:\pi_1(U,x_0)\to G,
 \qquad
 [\gamma]\mapsto g_\gamma(1)
\end{equation*}
给出全局信息。两个平坦联络 $A,A'$ 给同一 和乐 当且仅当差一个全局 规范 变换。因此平坦联络的同构类由 $\mathrm{Hom}(\pi_1(U),G)/G$ 参数化（模 空间）。这是规范理论中"真空模空间"的几何原型：即使场强为零，全局拓扑仍可区分不同规范真空。在 Chern--Simons 理论中，这一 模 空间正是三维流形上的拓扑不变量来源，对应 Witten 对 Jones 多项式的规范理论解释。
