...
接下来考察标架丛、主丛与 associated bundle。


更一般地，一个 principal $G$-bundle 是带自由右 $G$-作用的纤维丛
\[
\pi:P\to M,
\]
其每根纤维都是一个 $G$-torsor：不存在优先选定的单位点，但任意两点由唯一群元素联系。局部平凡化给出
\[
...
\label{eq:principal-transition-function}
\end{equation}
并在三重重叠上满足 cocycle condition
\begin{equation}
g_{\alpha\beta}g_{\beta\gamma}=g_{\alpha\gamma}.
\label{eq:principal-transition-cocycle}
\end{equation}
因此 principal bundle 的全局拓扑被局部平凡化之间的粘合函数编码。

给定群表示
\[
\rho:G\to GL(W),
\]
可以由 principal bundle 构造 associated vector bundle
\begin{equation}
E=P\times_\rho W
:=(P\times W)/\sim,
\qquad
(pg,w)\sim(p,\rho(g)w).
\label{eq:associated-bundle}
\end{equation}
标架丛是母对象：取 $P=\operatorname{Fr}(M)$、$G=GL(n,\RR)$，标准表示给回切丛
\[
TM\cong\operatorname{Fr}(M)\times_{GL(n)}\RR^n,
\]
对偶表示给出 $T^*M$，张量表示给出任意 $T^r{}_sM$。因此“张量分量怎样随标架变换”本质上只是 associated-bundle 表示律。

\begin{derivation}[局部截面、transition function 与 associated-bundle 分量]
\emph{思路：}主丛局部截面之间的换页函数完全由结构群的右作用决定；associated bundle 上截面的分量变换则由表示 $\rho$ 把这一右作用翻译成 $W$ 上的线性变换。两步合起来给出"换标架"的统一机制。

\emph{第一步：局部截面与 transition function。}设 $s_\alpha:U_\alpha\to P$、$s_\beta:U_\beta\to P$ 是开覆盖 $\{U_\alpha\}$ 上的两个局部截面。在重叠 $U_\alpha\cap U_\beta$ 上，$s_\alpha(p)$ 与 $s_\beta(p)$ 落在同一根 fiber $P_p$ 中。由于结构群 $G$ 在每根 fiber 上的右作用自由且传递，对每个 $p\in U_\alpha\cap U_\beta$ 存在唯一 $g_{\alpha\beta}(p)\in G$ 使
\begin{equation*}
 s_\beta(p)=s_\alpha(p)\,g_{\alpha\beta}(p),
 \qquad g_{\alpha\beta}:U_\alpha\cap U_\beta\to G.
\end{equation*}
光滑性由局部截面的光滑性与右作用的光滑性保证。$g_{\alpha\beta}$ 称为 \emph{transition function}（或换页函数）。

\emph{第二步：cocycle 条件。}在三重重叠 $U_\alpha\cap U_\beta\cap U_\gamma$ 上依次换页，
\begin{equation*}
 s_\gamma
...
\end{equation*}
即\eqnrefs{eq:principal-transition-cocycle}。同时 $g_{\alpha\alpha}=e$、$g_{\alpha\beta}=g_{\beta\alpha}^{-1}$。$\{g_{\alpha\beta}\}$ 因此是取值于 $G$ 的 \v Cech $1$-cocycle，主丛的拓扑分类恰由其 \v Cech 上同调类决定。

\emph{第三步：associated-bundle 截面的分量变换。}设 $E=P\times_\rho W$，$\sigma$ 是 $E$ 上一个截面。在 $U_\alpha$ 上把它写成 $\sigma|_{U_\alpha}=[s_\alpha,w_\alpha]$，其中 $w_\alpha:U_\alpha\to W$ 是局部 $W$-值函数。在重叠上同一截面有两种表示
\begin{equation*}
...
\emph{第四步：典型例子。}取 $P=\operatorname{Fr}(M)$、$G=GL(n,\RR)$：
\begin{itemize}
...
\end{itemize}
因此所有换标架分量公式——向量、余向量、张量、张量密度——都是同一条 associated-bundle 表示律 $w_\alpha=\rho(g_{\alpha\beta})w_\beta$ 在不同表示 $\rho$ 下的特例。

\emph{物理建模。}规范理论中 matter field 是 associated bundle $P\times_\rho W$ 的截面，$W$ 是内部空间（如 $\CC^n$ 上的表示空间），$g_{\alpha\beta}$ 是规范变换。分量变换律 $w_\alpha=\rho(g_{\alpha\beta})w_\beta$ 正是"波函数在重叠区域上的规范变换"的几何来源。
\end{derivation}
...
\]
完全等价，因此“换标架”就是主丛截面之间的 gauge 变换，而不是坐标变换的同义词。

标架丛上存在不需要任何联络就能定义的典范 $\RR^n$-值一形式，称为 solder form 或 tautological form：
\begin{equation}
\vartheta_u(V)
:=u^{-1}(\pi_*V),
\qquad
V\in T_u\operatorname{Fr}(M).
\label{eq:math-dg-solder-form}
\end{equation}
它把标架丛上的切向量投到底流形，再用当前标架 $u$ 读成 $\RR^n$ 分量。若 $s$ 是由 $e_a$ 给出的局部截面，则
\begin{equation}
 s^*\vartheta
 =\theta^a\mathbf e_a.
\label{eq:math-dg-solder-pullback}
\end{equation}
所以普通余标架并不是凭空出现的局部记号，而是全局 solder form 在局部 gauge 中的表示。由右作用定义还可直接得到
\begin{equation}
R_A^*\vartheta=A^{-1}\vartheta.
\label{eq:math-dg-solder-equivariance}
\end{equation}
...
\emph{思路：}利用外微分在两个向量场上的内在公式，将 $\dd\theta^a(e_b,e_c)$ 用 Lie 括号 $[e_b,e_c]$ 表达，再用结构函数 $C^a{}_{bc}$ 代入。

\emph{第一步：外微分的内在公式。}对任意一形式 $\alpha$，外微分在两个向量场上满足
\begin{equation*}
 \dd\alpha(X,Y)
 =X[\alpha(Y)]-Y[\alpha(X)]-\alpha([X,Y]).
\end{equation*}
这是外微分的不坐标定义，可直接验证：在坐标基下两端都等于 $\partial_i\alpha_j-\partial_j\alpha_i$，由张量性全局成立。第三项中 Lie 括号的出现正是外微分"反对称化方向导数并扣除方向场自身不交换"的几何实质。

\emph{第二步：代入余标架。}令 $\alpha=\theta^a$、$X=e_b$、$Y=e_c$。由余标架的对偶性 $\theta^a(e_b)=\delta^a{}_b$（\eqnrefs{eq:math-dg-frame-duality}），
\begin{equation*}
 e_b[\theta^a(e_c)]
 =e_b(\delta^a{}_c)=0,
 \qquad
...
\emph{第三步：代入结构函数。}由\eqnrefs{eq:math-dg-synthesis-anholonomy}，$[e_b,e_c]=C^d{}_{bc}\,e_d$，故
\begin{equation*}
 \dd\theta^a(e_b,e_c)
 =-\theta^a(C^d{}_{bc}\,e_d)
 =-C^d{}_{bc}\,\theta^a(e_d)
...
\emph{第四步：写出二形式。}任意二形式都由其在基向量上的值唯一确定。因 $C^a{}_{bc}$ 对 $(b,c)$ 反对称，展开为楔积时需除以 $2$（重复计数修正）：
...
\emph{物理意义。}非完整系数 $C^a{}_{bc}$ 在刚体力学中表现为旋转标架下的"假想力"系数（Coriolis、离心）；在广义相对论中，选择非完整标架（如 tetrad）时，$C^a{}_{bc}$ 是连接局部 Lorentz 标架与坐标系的运动学数据，与联络系数 $\omega^a{}_b$ 一起进入 Cartan 第一结构方程 $T^a=\dd\theta^a+\omega^a{}_b\wedge\theta^b$。
\end{derivation}
...
接下来考察非坐标导数、分量公式与局部可积性。


在非坐标标架中，函数的外微分仍然没有任何额外修正：
\begin{equation}
\dd f=e_a(f)\theta^a.
\label{eq:df-anholonomic-frame}
\end{equation}
但是连续作两次 frame derivative 时不再交换：
\begin{equation}
[e_a,e_b]f
=e_a(e_bf)-e_b(e_af)
=C^c{}_{ab}e_cf.
\label{eq:frame-derivative-commutator}
\end{equation}
...
\begin{derivation}[非完整标架中的 Lie bracket 分量]
先回忆对光滑函数 $f,g$ 与向量场 $X,Y$，Lie 括号作为微分算子作用在检验函数 $\varphi$ 上满足
\begin{equation*}
[fX,gY]\varphi
...
一组余标架是否来自局部坐标，可以直接由 exterior derivative 判断。若存在局部函数 $y^a$ 使
\[
\theta^a=\dd y^a,
\]
...
\begin{equation}
C^a{}_{bc}=0\ \forall a,b,c
\quad\Longleftrightarrow\quad
\theta^a\text{ 局部为坐标余标架}.
\label{eq:frame-coordinate-integrability}
\end{equation}
...
在三重重叠上必须有
\begin{equation}
\Lambda_{\alpha\beta}\Lambda_{\beta\gamma}
=\Lambda_{\alpha\gamma}.
\label{eq:frame-transition-cocycle}
\end{equation}
所以换标架矩阵不是彼此独立的局部选择，而是切丛的 transition cocycle。若所有 $\Lambda_{\alpha\beta}$ 都能同时写成
\[
\Lambda_{\alpha\beta}=h_\alpha^{-1}h_\beta,
\]
则可通过局部 gauge $h_\alpha$ 把 transition functions 全部消掉，得到全局标架；这等价于切丛 trivializable。不是每个流形都可平行化，因此“选一组全局 frame”本身可能受拓扑阻碍。

一个固定 tensor type 对应 $GL(n)$ 的一个表示。例如 $(1,1)$ tensor 在换标架下按 conjugation 变换：
\[
T'^a{}_b
=(\Lambda^{-1})^a{}_cT^c{}_d\Lambda^d{}_b,
\]
而 top form 的分量带 determinant character：
\[
\theta'^1\wedge\cdots\wedge\theta'^n
=(\det\Lambda)^{-1}
\theta^1\wedge\cdots\wedge\theta^n.
\]
于是 orientation 可以理解为把标架丛的结构群从 $GL(n,\RR)$ 约化到 $GL^+(n,\RR)$；Riemannian metric 则进一步把结构群约化到 $O(n)$，定向 Riemannian manifold 约化到 $SO(n)$。后续 Spin structure 正是在 $SO(n)$ frame bundle 上再寻求二重覆盖提升。

这一区分在后面十分重要：联络系数同样不是张量，也会在换标架时出现非齐次导数项；挠率与曲率却是张量。判断一个分量公式是否具有几何意义，不能只看指标形状，而要看它的换基律。

\section{非完整性与 Lie 群的原型}

接下来考察左不变标架与 Maurer--Cartan 方程。
...
把 Maurer--Cartan 形式写成 Lie algebra 值一形式更紧凑。对矩阵 Lie 群 $G\subset GL(N)$，左 Maurer--Cartan form 为
\begin{equation}
\Theta=g^{-1}\dd g\in\Omega^1(G;\mathfrak g).
\label{eq:matrix-maurer-cartan-form}
\end{equation}
直接使用
\[
\dd(g^{-1})=-g^{-1}(\dd g)g^{-1}
...
\end{equation}
其中矩阵乘法与 wedge product 同时作用。若取 Lie algebra 基 $E_a$，写
\[
\Theta=\vartheta^aE_a,
\qquad
[E_b,E_c]=c^a{}_{bc}E_a,
...
\begin{derivation}[Maurer--Cartan 方程是纯 gauge 表示的可积条件，即\eqnrefs{eq:mc-flat-integrability} 对\eqnrefs{eq:mc-pure-gauge} 的充要性]
\emph{方向一：纯 gauge $\Rightarrow$ 平坦。}若 $A=g^{-1}\dd g$，由外微分的 Leibniz 规则 $\dd(g^{-1})=-g^{-1}(\dd g)g^{-1}$（对 $g\cdot g^{-1}=I$ 求微分），
\begin{align*}
 \dd A
 ...
 &=-A\wedge A,
\end{align*}
其中 $\dd^2g=0$（外微分的幂零性），第三步用了 $g^{-1}\dd g$ 的矩阵乘法与楔积的兼容性。因此 $\dd A+A\wedge A=0$，即\eqnrefs{eq:mc-flat-integrability} 必要。

\emph{方向二：平坦 $\Rightarrow$ 纯 gauge。}固定基点 $x_0$ 与 $g_0\in G$，沿路径 $\gamma:[0,1]\to U$ 解矩阵 ODE
\begin{equation*}
 \frac{\dd g}{\dd t}
...
\emph{路径无关性。}若把路径作一个小矩形变形（先沿 $s$ 方向再沿 $t$ 方向，与先 $t$ 再 $s$），两种次序所得解的最低非平凡差由曲率
\begin{equation*}
 F_A=\dd A+A\wedge A
\end{equation*}
在矩形顶点处的值控制（这是非 Abel 换位子的标准计算）。平坦条件 $F_A=0$ 使这个差为零，因此平行积分对可缩路径变形不敏感。单连通性进一步消除 homotopy obstruction：任意两条具有相同端点的路径同伦，故 $g(x)$ 只依赖终点而不依赖所选路径，得到\eqnrefs{eq:mc-pure-gauge}。

\emph{显式小矩形计算。}取 $\RR^2$ 上小矩形 $[0,\epsilon]^2$，先 $s$ 后 $t$ 与先 $t$ 后 $s$ 的乘积相差
\begin{equation*}
...
\emph{非单连通区域的修正。}若 $U$ 不单连通，平坦条件 $F_A=0$ 只保证局部纯 gauge，全局仍可能非平凡。基本群 $\pi_1(U)$ 通过 holonomy 表示
\begin{equation*}
 \mathrm{Hol}_A:\pi_1(U,x_0)\to G,
 \qquad
 [\gamma]\mapsto g_\gamma(1)
\end{equation*}
给出全局信息。两个平坦联络 $A,A'$ 给同一 holonomy 当且仅当差一个全局 gauge 变换。因此平坦联络的同构类由 $\mathrm{Hom}(\pi_1(U),G)/G$ 参数化（moduli 空间）。这是规范理论中"真空模空间"的几何原型：即使场强为零，全局拓扑仍可区分不同规范真空。在 Chern--Simons 理论中，这一 moduli 空间正是三维流形上的拓扑不变量来源，对应 Witten 对 Jones 多项式的规范理论解释。
...
\item $G=\RR^n$（平移群）：$A$ 是普通闭一形式，平坦即 $\dd A=0$，纯 gauge $A=\dd f$ 对应势函数 $f$。这是 Hodge 分解中"闭形式 vs 恰当形式"的群论原型。
\end{itemize}

\emph{意义。}下一章中，非零曲率 $F_A\neq0$ 正是这个路径无关性失败的测量：沿不同路径的 holonomy 产生非平凡群元素，即规范场有"磁通"。在物理上，Aharonov--Bohm 效应正是 $U(1)$ 平坦联络在非单连通区域（挖去磁通管）上具有非平凡 holonomy 的体现：局部 $F=0$ 但全局 $A$ 不能写成纯规范，绕磁通管一圈积累相位 $e^{\ii\Phi/\hbar}$。
\end{derivation}
...
\emph{思路：}Lie 括号的 Jacobi 恒等式对结构函数 $C^c{}_{ab}$ 给出一组一阶微分约束；另一方面 $\dd^2=0$ 作用在结构方程上给出同一组约束。两条途径的等价性揭示"代数可积（Lie 代数）"与"微分可积（$\dd^2=0$）"是同一局部可积条件的两种语言。
...
\emph{第二步：合并循环项。}对三个循环项 $(a,b,c)$、$(b,c,a)$、$(c,a,b)$ 求和，并利用 $\{e_d\}$ 线性无关得到
...
\emph{第三步：从 $\dd^2=0$ 重新得到。}对\eqnrefs{eq:math-dg-frame-structure-equation}再作用一次 $\dd$，由 $\dd^2\equiv0$ 得
\begin{equation*}
 0=\dd^2\theta^d
...
\emph{第四步：显式核对三维。}取 $n=3$、$(a,b,c)=(1,2,3)$，Jacobi 约束为
\begin{equation*}
...
\emph{更一般结论。}这一可积条件是 Frobenius 定理与 Newlander--Nirenberg 定理的共同原型：一组分布可积当且仅当 Lie 括号封闭；一组近复结构可积当且仅当 Nijenhuis 张量消失。在规范理论中，Maurer--Cartan 方程 $\dd A+A\wedge A=0$ 是同一思想在主丛上的版本——"局部存在 $g$ 使 $A=g^{-1}\dd g$"的充要条件。
\end{derivation}



接下来考察G-structure：从结构群约化到几何结构。

标架丛 $F(M)$ 的结构群天然是 $GL(n,\RR)$。许多“额外几何结构”都可以统一理解为把这个结构群约化到一个子群 $G\subset GL(n,\RR)$。所谓 $G$-structure，就是一个主 $G$-子丛
\begin{equation}
P_G\subset F(M)
\label{eq:math-dg-g-structure}
\end{equation}
使得每个 fiber 中只保留满足指定几何条件的标架。
...
\end{align*}
这里最后两行的含义分别是：允许的标架必须把固定模型复结构 $J_0$ 或模型 symplectic form $\omega_0$ 搬到当前点的 $J_p$、$\omega_p$。因此 tensor field 与 structure-group reduction 是同一几何信息的两种描述。

若 $G$ 是某个模型张量 $T_0$ 的稳定子群
\[
G=\{A\in GL(n,\RR):A\cdot T_0=T_0\},
\]
则一个 $G$-structure 等价于在 $M$ 上给定一个处处与 $T_0$ 属于同一 $GL(n)$-orbit 的 tensor field $T$。例如 Euclidean 内积的稳定子群是 $O(n)$，所以 Riemannian metric 正是 $O(n)$-structure；标准复结构的稳定子群是 $GL(m,\CC)$，所以 almost complex structure 是 $GL(m,\CC)$-structure。

但“约化结构群”和“结构可积”必须严格区分。一个 almost complex structure 只要求
\[
J^2=-I,
\]
并不自动来自复坐标。其 integrability obstruction 是 Nijenhuis tensor
\begin{equation}
N_J(X,Y)
=[JX,JY]-J[JX,Y]-J[X,JY]-[X,Y].
\label{eq:math-dg-nijenhuis-tensor}
\end{equation}
...
J\text{ 局部来自复坐标图册}.
\]
这与下一章的 torsion、再下一章的 Frobenius obstruction 具有相同哲学：给定 pointwise algebraic structure 只是第一步，真正的局部几何还要检查其一阶微分可积条件。

同样地，一个非退化二形式 $\omega$ 把结构群约化到 $Sp(2m,\RR)$，但只有再满足
\begin{equation}
\dd\omega=0
\label{eq:math-dg-symplectic-closedness}
\end{equation}
时才是 symplectic structure。Darboux theorem 随后说明所有 symplectic forms 局部都可以化为
\[
\omega=\sum_{a=1}^m\dd q^a\wedge\dd p_a.
\]
因此 symplectic 几何没有类似 Riemann 曲率那样的局部零阶不变量；其局部非平凡性首先受 closedness 这一一阶微分条件控制。

G-structure 还解释了“兼容联络”意味着什么。若 $\omega$ 是 $F(M)$ 上的 $\mathfrak{gl}(n)$-值联络一形式，一个联络保持 $G$-structure，当且仅当限制到 $P_G$ 后它取值于 Lie algebra $\mathfrak g$：
\begin{equation}
\omega|_{P_G}\in\Omega^1(P_G;\mathfrak g).
\label{eq:math-dg-g-compatible-connection}
\end{equation}
Riemannian 情形 $G=O(n)$ 时，这就是联络形式反对称；若再要求 torsion-free，便唯一得到 Levi--Civita 联络。其他 $G$-structures 未必存在唯一 torsion-free compatible connection；剩余的不可消去 torsion 成分称为 intrinsic torsion，它刻画该 $G$-structure 偏离“最理想局部模型”的一阶程度。
...
\]
因此标架并非只为写 Cartan 公式服务，它实际上是联结 tensor geometry、gauge geometry 与特殊 holonomy 的共同母空间。

这一章至此仍未回答“如何把一个向量沿曲线平行搬运”以及“怎样比较邻点上的向量”。结构函数只刻画基场的非完整性。下一章引入 affine connection 后，平行移动、挠率与曲率才成为新的几何数据。
