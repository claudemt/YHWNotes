...
\end{equation*}
这里暂不要求 $\Theta$ 是欧氏空间：有限维参数模型取 $\Theta\subseteq\R^d$，而把未知分布函数、密度或回归函数本身作为参数时，$\Theta$ 可以是函数空间。\chapref{chap:math-stat-estimation-likelihood}--\chapref{chap:math-stat-testing} 的 Fisher 信息、二次均值可微 与有限维 局部渐近正态 会在需要微分结构时再明确加入 $\Theta\subseteq\R^d$ 的假设。
...
这个 $Y$ 就定义为
\[
\boxed{Y=\mathbb E(X\mid\mathcal G)}.
\]
...
=a\mathbb E(X\mid\mathcal G)+b\mathbb E(Z\mid\mathcal G).
\]
...
若 $\mathcal H\subseteq\mathcal G\subseteq\mathcal F$，则 塔 性质 为
\begin{equation*}\boxed{
...
凸函数与条件期望还满足 条件 Jensen 不等式。若 $\varphi$ 凸且两边可积，则
\begin{equation*}\varphi\!\left(\mathbb E(X\mid\mathcal G)\right)
\le
\mathbb E\!\left(\varphi(X)\mid\mathcal G\right)
\quad\text{a.s.}\end{equation*}
取 $\varphi(x)=|x|^p$ 可知条件期望在 $L^p$ 中是 收缩：
\[
\|\mathbb E(X\mid\mathcal G)\|_p
\le\|X\|_p,
\qquad 1\le p\le\infty.
\]
因此“丢掉信息”不会增大 $L^p$ 风险；Rao--Blackwell 改进正是这一 收缩 在统计估计上的体现。

当 $X\in L^2$ 时，条件期望具有更强的 Hilbert 空间刻画。对任意 $Z\in L^2(\mathcal G)$，
\[
\mathbb E\left[
\bigl(X-\mathbb E(X\mid\mathcal G)\bigr)Z
\right]=0.
\]
于是
\begin{equation*}\boxed{
...
\end{equation}
第一项是知道 $\mathcal G$ 后仍无法消除的平均不确定性，第二项是由 $\mathcal G$ 所解释的波动；两项都刻画所需的概率结构，而不只是证明的末行。
...
\]
因此构成 鞅。也就是说，条件期望不仅是静态投影，还自然产生随信息增长演化的 鞅。后续若研究依赖数据的极限定理，鞅 差 将成为 iid 求和之外的第二类基本涨落结构。
...
\begin{derivation}[Bayes 公式是同一联合测度的两种 分解]
...
\begin{derivation}[$L^2$ 鞅 收敛 是嵌套投影收敛]
设
\[
M_n=P_nX,
\qquad
...
设 $X_n$ 为 iid 且 $\mathbb E|X_1|^2<\infty$，$\mathcal F_n=\sigma(X_1,\ldots,X_n)$，则 $M_n=\mathbb E(X_1\mid\mathcal F_n)=\bar X_n$（样本均值），$L^2$ 收敛到 $X_1$ 自身——这是 $L^2$ 强大数律的鞅证明。对 Brown 运动 $B_t$ 关于自然滤子 $\mathcal F_t$，$M_t=B_t$ 是鞅，$L^2$ 极限不存在（$B_t$ 不 $L^2$ 收敛），但 Doob 鞅收敛定理给出 a.s. 收敛到 $0$（当 $t\to\infty$ 时 $B_t/t\to0$）。
\end{derivation}
...
正态分布统一写成
\[
 X\sim\mathcal N(\mu,\sigma^2),\qquad \mu\in\R,\ \sigma^2>0,
\]
