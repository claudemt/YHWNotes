...
实例。设 $X_n$ 为 iid 且 $\mathbb E|X_1|^2<\infty$，$\mathcal F_n=\sigma(X_1,\ldots,X_n)$，则 $M_n=\mathbb E(X_1\mid\mathcal F_n)=\bar X_n$（样本均值），$L^2$ 收敛到 $X_1$ 自身——这是 $L^2$ 强大数律的鞅证明。对 Brown 运动 $B_t$ 关于自然滤子 $\mathcal F_t$，$M_t=B_t$ 是鞅，$L^2$ 极限不存在（$B_t$ 不 $L^2$ 收敛），但 Doob 鞅收敛定理给出 a.s. 收敛到 $0$（当 $t\to\infty$ 时 $B_t/t\to0$）。
\end{derivation}
