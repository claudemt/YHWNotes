...
\begin{equation}\label{eq:math-stat-factorization}
 p_\theta^{(n)}(x^n)
 =g_\theta(T(x^n))h(x^n),
\end{equation}
...
\begin{align*}
 \E_{P_\theta}[\ind_B K(T,A)]
 &=\E_Q\!\left[
 g_\theta(T)H\ind_BK(T,A)
 \right]\\
 &=\E_Q\!\left[
 g_\theta(T)m(T)\ind_BK(T,A)
 \right]\\
 &=\E_Q\!\left[
 g_\theta(T)\ind_B
 \E_Q\{\ind_AH\mid T\}
 \right]\\
 &=\E_Q\!\left[
 g_\theta(T)\ind_B\ind_AH
 \right]\\
 &=P_\theta^{(n)}(A\cap B).
\end{align*}
第二步与第四步只用了条件期望的定义，因为 $g_\theta(T)\ind_BK(T,A)$ 与 $g_\theta(T)\ind_B$ 都是 $\sigma(T)$-可测的。于是
\[
 K(T,A)
 =P_\theta^{(n)}(X^n\in A\mid T)
...
\emph{举例：正态位置族的最小充分统计量。}对 \(X_1,\ldots,X_n\iid\Normal(\mu,1)\)，
\[
...
\emph{举例：均匀族的最小充分统计量。}对 \(X_1,\ldots,X_n\iid\Uniform(0,\theta)\)，
\[
p_\theta^{(n)}(x)=\theta^{-n}\ind_{\{x_{(n)}\le\theta\}}\prod_i\ind_{\{x_i\ge0\}},
\]
两样本成比例当且仅当 \(x_{(n)}=y_{(n)}\)，故 \(T=X_{(n)}=\max_i X_i\) 最小充分。这与正态情形对比鲜明：充分统计量维数可远小于样本量，也可等于 1，关键在于 likelihood 对样本的依赖通道。

\emph{更一般结论：Birnbaum 定理与充分性的认识论意义。}Birnbaum 定理断言：若推断方法满足似然原理（likelihood principle）与条件化原理（conditionality principle），则它只依赖似然函数而非整个样本。充分性原则是似然原理的弱形式：充分统计量保留全部似然信息。最小充分统计量因此是"在不违反似然原理前提下最大压缩"——任何进一步压缩都会丢失区分不同似然比例类的能力。这一把充分性从计算工具提升为统计推断基础公理的视角，是经典统计与 Bayesian 统计的共同基石。

\emph{信息论视角：最小充分统计量与互信息。}最小充分统计量 \(T\) 使 \(I(\theta;X^n)=I(\theta;T)\)，即它保留了参数与样本的全部互信息。任何进一步压缩 \(S=f(T)\) 严格损失信息（除非模型退化）。这把充分性从"分布因子分解"等价表述为"信息保持"，是 Fisher 信息几何与充分统计量对偶关系的起点。

\emph{举例：Poisson 族的最小充分统计量。}对 \(X_1,\ldots,X_n\iid\Poisson(\lambda)\)，联合密度
\[
p_\lambda^{(n)}(x)=\ee^{-n\lambda}\lambda^{\sum x_i}\prod_i\frac1{x_i!},
\]
两样本成比例当且仅当 \(\sum x_i=\sum y_i\)，故 \(T=\sum X_i\) 最小充分。注意 \(T\sim\Poisson(n\lambda)\) 仍属同一族，这是指数族的闭包性质：充分统计量分布仍在新指数族中。
...
\begin{equation}\label{eq:math-stat-log-partition-moments}
...
\textbf{定义（完备统计量）.}\enspace 统计量 $T$ 称为完备的，若对任意可测函数 $a$，只要
\[
 \E_\theta[a(T)]=0
...
\textbf{例（Poisson 完备性）.}\enspace 若 $X_i\iid\Poisson(\lambda)$，则
\[
 T=\sum_{i=1}^nX_i\sim\Poisson(n\lambda)
\]
充分。若 $\E_\lambda a(T)=0$ 对所有 $\lambda>0$，则
\[
 0=\ee^{-n\lambda}
...
\emph{举例：正态族 \((\mu,\sigma^2)\) 的完备性。}对 \(\Normal(\mu,\sigma^2)\) 两参数族，自然参数 \(\eta=(\mu/\sigma^2,-1/(2\sigma^2))\) 取值于下半平面 \(\eta_2<0\)，这是 \(\R^2\) 中开集。自然统计量 \(T_n=(\sum_i X_i,\sum_i X_i^2)\) 因此完备。但若固定 \(\sigma^2=1\) 只变 \(\mu\)，参数空间是一维曲线 \(\{(\mu,-1/2):\mu\in\R\}\)，它不是 \(\R^2\) 中开集；不过此时退化为一维满秩指数族，仍完备。

\emph{反例：曲线指数族可能不完备。}考虑密度 \(p_\theta(x)=C(\theta)\exp\{\theta x+\theta^2 x^2\}\) 在 \(\theta\in\R\) 上形成的族，自然参数 \(\eta=(\theta,\theta^2)\) 落在抛物线上，不含开集。可构造非零函数 \(a\) 使 \(\E_\theta a(T)=0\) 对所有 \(\theta\) 成立，故 \(T\) 不完备。这说明完备性不是"族有多大"而是"参数在自然空间中能否张满开集"。

\emph{与热力学配分函数的联系。}指数族密度 \(p_\eta(x)=h(x)\exp\{\eta^{\mathsf T}t(x)-A(\eta)\}\) 中，\(A(\eta)=\log\int h(x)\exp\{\eta^{\mathsf T}t(x)\}\,\dd x\) 正是统计力学的 log-partition function（自由能）。其导数 \(\nabla A(\eta)=\E_\eta t(X)\) 给出能量期望，二阶导 \(\nabla^2 A(\eta)=\Cov_\eta(t)\) 是 Fisher 信息即热容。完备性定理中"Laplace 变换在开集上恒零推出测度零"对应热力学中"配分函数决定全部状态方程"：自由能的解析延拓唯一确定分布族。

\emph{举例：Poisson 族的完备性验证。}对 \(X\sim\Poisson(\lambda)\)，自然参数 \(\eta=\log\lambda\)，自然空间 \(\R\)（开集）。若 \(\E_\lambda a(X)=\sum_x a(x)\ee^{-\lambda}\lambda^x/x!=0\) 对所有 \(\lambda>0\)，乘以 \(\ee^\lambda\) 得 \(\sum_x a(x)\lambda^x/x!=0\)，这是 \(a(x)/x!\) 的幂级数恒为零，故 \(a(x)=0\) 对所有 \(x\)。完备性成立，且证明不需抽象 Laplace 变换，直接用幂级数唯一性。
...
\textbf{定理（Basu 定理）.}\enspace 若 $T$ 对模型充分且完备，$A$ 为辅助统计量，则 $T$ 与 $A$ 独立。


\emph{思路：}对任意有界可测函数 $h(A)$，证明 $\E_\theta\{h(A)\mid T\}=\E_\theta h(A)$，即 $A$ 与 $T$ 独立。充分性使条件期望不依赖 $\theta$，完备性排除条件期望的非平凡波动。

\emph{第一步：充分性分离参数。}取任意有界可测函数 $h(A)$。由于 $T$ 充分，由 Rao--Blackwell 定理，条件期望
\begin{equation*}
 q(T)=\E_\theta\{h(A)\mid T\}
\end{equation*}
不依赖 $\theta$（充分性保证条件分布 $P(A\in\cdot\mid T)$ 与 $\theta$ 无关）。这一步把 \(q(T)\) 从参数中解放出来，使其成为一个纯粹的统计量函数。

\emph{第二步：辅助性给出常数期望。}因为 $A$ 的分布与 $\theta$ 无关（辅助性），存在常数 $c$ 使
\begin{equation*}
 \E_\theta h(A)=c
 \qquad\text{对所有 }\theta.
\end{equation*}
辅助性是关键：它使 \(h(A)\) 的期望在参数变化时保持不动，与第一步的 \(q(T)\) 形成对照——一个不依赖 \(\theta\) 是因为条件机制，另一个是因为边缘机制。

\emph{第三步：塔式性质传递到 $T$。}由条件期望的塔式性质 $\E_\theta\E_\theta\{h(A)\mid T\}=\E_\theta h(A)$，
\begin{equation*}
 \E_\theta\{q(T)-c\}
 =\E_\theta q(T)-c
 =\E_\theta h(A)-c
...
\emph{第四步：完备性迫使退化。}$q(T)-c$ 是 $T$ 的可测函数，且对所有 $\theta$ 期望为零。由 $T$ 的完备性，这迫使
\begin{equation*}
 q(T)=c
...
\emph{第五步：独立性。}代回 $q(T)$ 的定义，
\begin{equation*}
 \E_\theta\{h(A)\mid T\}=c=\E_\theta h(A),
\end{equation*}
即条件期望等于无条件期望。由任意有界可测 $h$ 的任意性，$A$ 与 $T$ 独立。

Basu 定理的逻辑不是"某两个统计量恰好零相关"，而是：充分性先把给定 $T$ 后的条件机制从参数中分离，完备性再排除任何仍随 $T$ 波动但对所有参数平均为零的方向。

\emph{举例：正态样本均值与样本方差的独立性。}对 \(X_1,\ldots,X_n\iid\Normal(\mu,\sigma^2)\)，\(T=(\bar X,\sum(X_i-\bar X)^2)\) 充分且完备（满秩指数族），而 \(A=(X_1-\bar X,\ldots,X_n-\bar X)/S\)（标准化残差方向）是辅助统计量——其分布在 \(\mu,\sigma^2\) 的平移与尺度变换下不变。Basu 定理立即给出 \(A\perp\!\!\!\perp T\)，特别地 \(\bar X\perp\!\!\!\perp S^2\)。这给出了 \(\bar X\) 与 \(S^2\) 独立这一基本事实的群论解释，而不必直接计算联合分布。

\emph{更一般结论：Basu 在指数族中的系统应用。}对满秩指数族，自然统计量 \(T_n\) 充分完备。任何分布不依赖自然参数的统计量都与 \(T_n\) 独立。这一原理在构造 pivot、检验统计量分布无关性以及简化期望计算时反复出现：例如 \(F\) 统计量的分子分母独立性、Student \(t\) 的分子分母独立性，都可以从 Basu 定理一次性推出。

\emph{物理建模：充分统计量与可观测量的分离。}在统计物理中，充分统计量对应宏观可观测量（如总能量、总粒子数），辅助统计量对应微观构型的"形状"信息（如能量分布的标准化模式）。Basu 定理的物理对应是：在正则系综下，宏观可观测量与微观构型形状独立——这正是统计力学中"宏观与微观尺度分离"的数学形式。完备性对应参数可识别性：若宏观量期望固定，则分布唯一确定，没有隐藏自由度。

\emph{举例：位置尺度族的辅助统计量。}对 \(X_1,\ldots,X_n\iid\Normal(\mu,\sigma^2)\)，标准化残差 \(A_i=(X_i-\bar X)/S\) 的分布不依赖 \((\mu,\sigma^2)\)，是辅助统计量。Basu 定理给出 \(A\perp\!\!\!\perp(\bar X,S^2)\)。这把"残差形状与位置尺度参数独立"从直观提升为定理，是回归诊断中残差图可解释性的理论基础：残差形状只反映模型拟合质量，不携带参数信息。同理，Student \(t\) 统计量作为尺度不变商，其分布只依赖自由度而非 \((\mu,\sigma^2)\)，这正是 Basu 定理在 pivot 构造中的体现。这一类"参数无关量"的系统性识别，使经典统计能构造出分布表可查的 pivot，而不必对每个参数值重新计算临界值。

\emph{完备性缺失时的反例。}若 \(T\) 充分但不完备，Basu 定理一般失效。例如对 \(\Uniform(\theta,\theta+1)\) 位置族，\((X_{(1)},X_{(n)})\) 充分但不完备，存在非零函数 \(a\) 使 \(\E_\theta a(T)=0\)。此时辅助统计量可能与 \(T\) 不独立，Basu 的"充分+完备 \(\Rightarrow\) 独立"链条在完备性环节断裂。
\end{derivation}
...
\[
 \Bias_\theta(\widehat g_n)
 =\E_\theta\widehat g_n-g(\theta),
\]
\[
 \MSE_\theta(\widehat g_n)
 =\E_\theta\{\widehat g_n-g(\theta)\}^2
 =\Var_\theta(\widehat g_n)+\Bias_\theta(\widehat g_n)^2.
\]
无偏性是每个固定 $n$ 的精确期望约束；一致性则是序列性质
\[
 \widehat g_n\toP g(\theta).
\]
...
若 $T$ 同时完备，则任意两个 $T$ 的无偏函数几乎处处相同。因此只要存在 $a(T)$ 满足 $\E_\theta a(T)=g(\theta)$，它就是唯一的无偏 $T$-函数，并且在所有无偏估计量中方差最小。这就是 Lehmann--Scheff\'e 定理。

\begin{exbox}[Poisson 均值与正态方差的 UMVU]
对 $X_i\iid\Poisson(\lambda)$，$T=\sum_iX_i$ 充分完备，而
\[
 \E_\lambda(T/n)=\lambda,
\]
所以 $\overline X=T/n$ 是 $\lambda$ 的 UMVU 估计量。

若 $X_i\iid\Normal(\mu,\sigma^2)$ 且两参数未知，则
\[
 T=\left(\sum_iX_i,\sum_iX_i^2\right)
\]
是完整满秩二参数指数族的充分完备统计量。由于 $S^2$ 是 $T$ 的函数且 $\E S^2=\sigma^2$，Lehmann--Scheff\'e 定理给出 $S^2$ 是 $\sigma^2$ 的 UMVU 估计量。
\end{exbox}

\begin{derivation}[Rao--Blackwell 是条件期望投影]
分三步证明：无偏性保持、方差不增、完备性给出唯一性。

\emph{第一步：无偏性保持。}设 \(U\) 是 \(g(\theta)\) 的无偏估计量，\(T\) 是充分统计量。Rao--Blackwell 化简为 \(U^*=\E_\theta(U\mid T)\)。由塔式性质（\(T\) 生成的 \(\sigma\)-代数是全空间的子 \(\sigma\)-代数），
\[
 \E_\theta U^*
 ...
 =g(\theta),
\]
故 \(U^*\) 仍无偏。这里充分性通过保证 \(\E_\theta(U\mid T)\) 不依赖 \(\theta\) 来起作用：否则化简后的量会随参数变化，无法作为统一估计量。

\emph{第二步：方差不增。}由条件方差分解（对任意随机变量 \(U\) 和子 \(\sigma\)-代数 \(\sigma(T)\) 成立），
\[
 \Var_\theta(U)
 =\Var_\theta\{\E(U\mid T)\}
 +\E_\theta\{\Var(U\mid T)\}.
\]
右端第二项 \(\E_\theta\{\Var(U\mid T)\}\ge0\)，因为方差非负。因此
\[
 \Var_\theta(U^*)
 =\Var_\theta\{\E(U\mid T)\}
 \le\Var_\theta(U).
...
在 \(L^2(P_\theta)\) 语言中，\(U^*=\E(U\mid T)\) 就是 \(U\) 到闭子空间 \(L^2(\sigma(T))\) 的正交投影。记 \(U^\perp=U-U^*\)，则 \(\E(U^\perp\mid T)=0\)，即 \(U^\perp\perp L^2(\sigma(T))\)。方差分解就是 Pythagoras 定理：
\[
...
\emph{第三步：完备性给出唯一性。}若 \(T\) 完备且 \(a_1(T),a_2(T)\) 都是 \(g(\theta)\) 的无偏估计，则
\[
 \E_\theta\{a_1(T)-a_2(T)\}=0
\]
对所有 \(\theta\) 成立。完备性定义恰好要求：若 \(\E_\theta h(T)=0\) 对所有 \(\theta\)，则 \(h(T)=0\) a.s.。故 \(a_1(T)=a_2(T)\) a.s.，UMVU 估计唯一。结合第二步的方差下界和第三步的唯一性，Lehmann--Scheff\'e 定理给出：完备充分统计量的无偏函数是唯一 UMVU 估计。

\emph{举例：Poisson 总体的 UMVU 估计。}设 \(X_1,\ldots,X_n\iid\Poisson(\lambda)\)，求 \(g(\lambda)=\ee^{-\lambda}=\Pbb(X_1=0)\) 的 UMVU 估计。原始估计 \(U=\ind_{\{X_1=0\}}\) 无偏但方差大。完备充分统计量 \(T=\sum_i X_i\sim\Poisson(n\lambda)\)。Rao--Blackwell 化简：
\[
U^*=\E(U\mid T)=\Pbb(X_1=0\mid T=t)
=\frac{\Pbb(X_1=0,T=t)}{\Pbb(T=t)}
=\frac{\ee^{-\lambda}\cdot\ee^{-(n-1)\lambda}[(n-1)\lambda]^t/t!}{\ee^{-n\lambda}(n\lambda)^t/t!}
=\left(1-\frac1n\right)^t.
\]
这是 \(\ee^{-\lambda}\) 的 UMVU 估计，方差严格小于 \(\ind_{\{X_1=0\}}\)。它体现了 Rao--Blackwell 把"用第一个观测指示零"改进为"用全体观测通过充分统计量估计零概率"。

\emph{更一般结论：Lehmann--Scheff\'e 与 Cramér--Rao 比较。}当模型满足正则条件且 \(T\) 完备充分时，UMVU 方差同时是 Cramér--Rao 下界 \(g'(\theta)^2/I(\theta)\) 的上界。等号成立当且仅当 \(g'(\theta)/I(\theta)\) 与 score 成比例，即 \(g\) 是指数族自然参数的仿射函数。一般情形下 UMVU 方差严格大于 CRLB，差距来自充分统计量分布的非线性结构。
...
\]
并由
\begin{equation}\label{eq:math-stat-estimating-equation}
 \Psi_n(\widehat\theta_n)=0
\end{equation}
定义估计量。若 $\E_{\theta_0}\psi(X,\theta_0)=0$，定义
\[
 \dot\Psi_n(\theta)
 =\frac1n\sum_{i=1}^n
...
\[
 ...
 A_0(\theta_0)=A.
\]
假设 $A$ 可逆、$\widehat\theta_n\toP\theta_0$，并且在某个固定邻域 $N$ 内满足统一 Jacobian 大数律
\begin{equation}\label{eq:math-stat-estimating-jacobian-uniform}
 \sup_{\vartheta\in N}
 \|\dot\Psi_n(\vartheta)-A_0(\vartheta)\|\toP0,
 \qquad
 A_0(\vartheta)\to A
...
\begin{equation}\label{eq:math-stat-estimating-equation-linearization}
...
\end{equation}
若 $A,B,I$ 均非奇异，取逆后得到
\begin{equation}\label{eq:math-stat-godambe-covariance-bound}
 A^{-1}BA^{-\mathsf T}\succeq I(\theta)^{-1}.
\end{equation}
...
\emph{思路：}在 $L^2(P_\theta)$ 中，将 score $s_\theta$ 投影到估计函数 $\psi$ 张成的子空间。投影的协方差不超过 score 的协方差（即 Fisher 信息），差是正交残差的协方差（半正定）。

\emph{第一步：最佳线性投影。}把随机向量 $s_\theta$ 对 $\psi$ 的各坐标所张成的有限维子空间作线性投影 $C\psi$。最佳线性系数矩阵由最小化 $\E\|s_\theta-C\psi\|^2$ 得到。对目标关于 \(C\) 求导并令其为零：
\[
\frac{\partial}{\partial C}\E\|s_\theta-C\psi\|^2
=-2\E(s_\theta\psi^{\mathsf T})+2C\E(\psi\psi^{\mathsf T})=0,
\]
解出
\begin{equation*}
 C
 =\E(s_\theta\psi^{\mathsf T})
 B^{-1}
 =-A^{\mathsf T}B^{-1},
\end{equation*}
其中 $B=\E(\psi\psi^{\mathsf T})$，最后一步用了\eqnrefs{eq:math-stat-estimating-score-covariance}（即 \(\E(s_\theta\psi^{\mathsf T})=-A^{\mathsf T}\)，来自对 \(\E_\theta\psi=0\) 关于 \(\theta\) 求导并交换积分与微分）。

\emph{第二步：投影的协方差。}
\begin{equation*}
 \E[(C\psi)(C\psi)^{\mathsf T}]
 =C\,B\,C^{\mathsf T}
 =(-A^{\mathsf T}B^{-1})B(-A^{\mathsf T}B^{-1})^{\mathsf T}
 =A^{\mathsf T}B^{-1}A.
\end{equation*}

\emph{第三步：Pythagoras 分解。}残差 $s_\theta-C\psi$ 与投影 $C\psi$ 正交（由最佳投影性，\(\E[(s_\theta-C\psi)\psi^{\mathsf T}]=0\) 推出 \(\E[(s_\theta-C\psi)(C\psi)^{\mathsf T}]=0\)），故协方差作正交分解：
\begin{equation*}
 I(\theta)
 =\E(s_\theta s_\theta^{\mathsf T})
 =A^{\mathsf T}B^{-1}A
 +\E[(s_\theta-C\psi)(s_\theta-C\psi)^{\mathsf T}].
\end{equation*}

\emph{第四步：残差半正定。}对任意 $v\in\R^d$，
\begin{equation*}
 v^{\mathsf T}
 \E[(s_\theta-C\psi)(s_\theta-C\psi)^{\mathsf T}]
 v
 =\E\!\left[\{v^{\mathsf T}(s_\theta-C\psi)\}^2\right]\ge0,
\end{equation*}
所以残差协方差矩阵半正定。移项后得到\eqnrefs{eq:math-stat-godambe-information-bound}：$A^{\mathsf T}B^{-1}A\preceq I(\theta)$。

\emph{第五步：等号条件。}当残差协方差为零时，$s_\theta=C\psi$ 几乎处处成立；score 与估计函数只差一个非奇异线性变换。此时 sandwich 协方差 $A^{-1}BA^{-\mathsf T}=I(\theta)^{-1}$，达到 Cram\'er--Rao 下界。等号成立当且仅当 score 几乎处处落在 $\psi$ 的线性张成空间内；换言之，正则 likelihood score 在这类无偏估计方程中保留了全部一阶 Fisher 信息。

\emph{举例：MLE 的 score 方程达到 CRLB。}取 \(\psi=s_\theta\) 即 likelihood score 本身，则 \(A=\E(s_\theta s_\theta^{\mathsf T})=I(\theta)\)、\(B=I(\theta)\)，sandwich 协方差
\[
...
\emph{更一般结论：sandwich 与 Huber 稳健估计。}对稳健 M 估计 \(\psi(x,\theta)=\psi_c(x-\theta)\)（Huber 截断函数），\(A=-\E\psi'\)、\(B=\E\psi^2\)，渐近方差为 \(\E\psi^2/(\E\psi')^2\)。当误差分布为正态时，取 \(c\to\infty\) 退化为 \(1/I(\theta)=\sigma^2\)；当误差有重尾时，有限 \(c\) 给出比 OLS 更小的渐近方差。Godambe 不等式因此不仅是理论下界，更是估计方程设计的指导原则：在指定 \(\psi\) 类中选使 \(A^{\mathsf T}B^{-1}A\) 最大的估计函数。
...
\]
都满足 $\widehat\theta_n\toP\theta_0$。若进一步 $m$ 在 $\theta_0$ 可微且
\[
 M_0=Dm(\theta_0)
\]
可逆，记
...
\[
 \sqrt n(\widehat\theta_n-\theta_0)
 \toD
 \Normal_d(0,M_0^{-1}\Sigma_hM_0^{-\mathsf T}).
\]
因此矩估计的一致性来自 LLN 与可识别性，渐近分布来自 CLT 与矩映射的局部 Jacobian；这正是一般估计方程公式的具体坐标形式。

以本书统一的 shape--rate 参数化 $X\sim\GammaD(\alpha,\lambda)$ 为例，
\[
...
\begin{equation}\label{eq:math-stat-gamma-moment-estimator}
 \widehat\alpha_{\rm MM}
...
\[
 \overline M_n^{-1}\toP M_0^{-1}.
\]
再由多元 CLT，
\[
 \sqrt n\{P_nh-m(\theta_0)\}
 \toD\Normal_d(0,\Sigma_h),
\]
...
令 $X_i\iid\Unif(0,\theta)$，$\theta>0$。由于 $\E X=\theta/2$，矩估计为
\[
...
\[
 ...
 \toD\Normal\!\left(0,\frac{\theta^2}{3}\right).
\]
另一方面，likelihood 为
\[
 L_n(\theta)
 =\theta^{-n}\ind\{\theta\ge X_{(n)}\},
\]
...
\end{align*}
因此
\begin{equation}\label{eq:math-stat-uniform-endpoint-rate}
 \frac{n(\theta-X_{(n)})}{\theta}
 \toD\operatorname{Exp}(1),
\end{equation}
...
\]
偏差校正量 $((n+1)/n)X_{(n)}$ 对 $\theta$ 无偏。因子分解直接给出 $T=X_{(n)}$ 的充分性。若对所有 $\theta>0$ 都有 $\E_\theta a(T)=0$，则
\[
 0=\frac{n}{\theta^n}
...
\[
 ...
 \ge\sup_{\theta\in\Theta}\ell_n(\theta)-o_{\Pbb}(1)
\]
...
\begin{equation}\label{eq:math-stat-profile-likelihood-definition}
 L_n^*(\vartheta)
...
\]
每观测 Fisher 信息定义为
\begin{equation}\label{eq:math-stat-fisher-information}
 I(\theta)
 =\E_\theta[s_\theta(X)s_\theta(X)^{\mathsf T}].
\end{equation}
若支撑固定且可以把参数微分移入积分，则
\[
 \E_\theta s_\theta(X)=0,
...
\[
 ...
 =\E_\theta[TU_{n,j}(\theta)].
\]
因为 $\E_\theta U_n=0$（对 \(\int p_\theta=1\) 求导即得），向量形式为
\[
 \Cov_\theta(U_n,T)
 =\E_\theta(U_n T)-\E_\theta U_n\,\E_\theta T
 =\nabla g(\theta).
\]
令
\[
...
\emph{举例：正态均值的信息下界。}对 \(X_1,\ldots,X_n\iid\Normal(\mu,\sigma^2)\)（\(\sigma^2\) 已知），单参数 Fisher 信息 \(I(\mu)=1/\sigma^2\)，CRLB 给出
\[
...
\emph{更一般结论：多参数与 nuisance 的下界。}对 \(\theta=(\psi,\lambda)\)（\(\psi\) 兴趣参数，\(\lambda\) nuisance），分块 Fisher 信息
\[
I(\theta)=\begin{pmatrix}I_{\psi\psi}&I_{\psi\lambda}\\I_{\lambda\psi}&I_{\lambda\lambda}\end{pmatrix},
\]
对 \(\psi\) 的有效下界为 Schur 补 \([I_{\psi\psi}-I_{\psi\lambda}I_{\lambda\lambda}^{-1}I_{\lambda\psi}]^{-1}\)，严格大于无 nuisance 时的 \(I_{\psi\psi}^{-1}\)。Nuisance 参数通过 score 的相关性抬高兴趣参数的有效方差，这正是半参数统计中 profile likelihood 与 efficient score \(U_\psi-I_{\psi\lambda}I_{\lambda\lambda}^{-1}U_\lambda\) 的来源。
...
\[
 ...
 \right|\toP0,
\]
而总体准则在 $\theta_0$ 唯一最大，则 argmax 定理给出 $\widehat\theta_n\toP\theta_0$。唯一最大性来自 Kullback--Leibler 分离：
\[
...
\begin{equation}\label{eq:math-stat-qmd}
 \int\left[
 \sqrt{p_{\theta_0+h}}
 -\sqrt{p_{\theta_0}}
 ...
 =o(\|h\|^2).
\end{equation}
QMD 是对概率模型本身的局部光滑性要求，比“某次计算中 Hessian 存在”更稳定。它自动给出 $\E_{\theta_0}s_{\theta_0}=0$，并把
\[
 I_0=I(\theta_0)
 =\E_{\theta_0}[s_{\theta_0}s_{\theta_0}^{\mathsf T}]
\]
识别为局部二次曲率。
...
\begin{equation}\label{eq:math-stat-lamn}
 \ell_n(\theta_0+h/r_n)-\ell_n(\theta_0)
 =h^{\mathsf T}\Delta_n
...
其中 $r_n\to\infty$，并且
\[
 (\Delta_n,\mathcal J_n)\toD(\Delta,\mathcal J),
 \qquad
 \Delta\mid\mathcal J\sim\Normal_d(0,\mathcal J),
\]
则称实验在 $\theta_0$ 处满足局部渐近混合正态性（LAMN）。条件于随机信息 $\mathcal J$ 后，极限实验仍是 Gaussian shift；取消条件以后则成为不同信息水平的混合。普通 iid 正则模型是 $r_n=\sqrt n$ 且 $\mathcal J_n\toP I_0$ 的退化特例。LAMN 常出现在信息量本身受随机路径控制的实验中，因此不能把所有局部二次 likelihood 都强行写成固定 Fisher 矩阵。后面的 Wilks/Wald/Score 与 BvM 结论若要推广到 LAMN，也必须把确定信息椭球相应替换为条件于 $\mathcal J$ 的随机椭球。
...
\]
需要注意，$P_{\theta_0+h_n}$ 可能有 $q_n$ 的质量位于 $\mathcal D_0^c$，所以不能直接把 $\E(1+r_n)^2$ 写成 $1$。在 $\mathcal D_0$ 上积分得到
\[
 \E(1+r_n)^2
...
线性项先分解为
\begin{align*}
 2\sum_{i=1}^n\{r_{n,i}-\E r_n\}
 &=\frac1{\sqrt n}\sum_{i=1}^n a_i
 +2\sum_{i=1}^n\{u_{n,i}-\E u_n\}.
\end{align*}
第二项的方差满足
\[
 \Var\!\left(
...
第三项满足
\[
 \E\sum_{i=1}^nu_{n,i}^2
 =n\E u_n^2\to0,
\]
所以由 Markov 不等式为 $o_{\Pbb}(1)$。交叉项用 Cauchy--Schwarz 控制：
\begin{align*}
 \left|
 \frac1{\sqrt n}\sum_{i=1}^na_i u_{n,i}
 \right|
...
\[
 n\E\!\left[
 r_n^2\ind\{|r_n|>\varepsilon\}
 \right]
 =\E\!\left[
 nr_n^2\ind\{\sqrt n|r_n|>\varepsilon\sqrt n\}
 \right]
 \to0.
\]
于是
\begin{align*}
 ...
 \right)
 &\le n\Pbb(|r_n|>\varepsilon)\\
 &\le\varepsilon^{-2}n\E\!\left[
 r_n^2\ind\{|r_n|>\varepsilon\}
 \right]
 \to0,
\end{align*}
...
\begin{align*}
 ...
 &=h^{\mathsf T}\Delta_n
   ...
   +o_P(1),
\end{align*}
这就是\eqnrefs{eq:math-stat-lan-vector}。这里确定漂移的一半来自局部概率质量归一化所固定的 $2n\E r_n$，另一半来自对数展开中的 $-\sum r_{n,i}^2$；QMD 允许的支撑外质量只有 $q_n=o(n^{-1})$，因此不进入 $O(1)$ 的 LAN 漂移。LAN 的 $1/2$ 系数不是形式 Taylor 展开的记忆项，而是 QMD 与局部概率归一化共同固定的。


\textbf{定理（正则 MLE 的渐近正态性）.}\enspace \label{thm:math-stat-mle-asymptotic-normal}
...
\begin{equation}\label{eq:math-stat-mle-rootn-localization}
 \sqrt n(\widehat\theta_n-\theta_0)=O_{\Pbb}(1),
\end{equation}
同时 LAN 余项在每个有界局部参数集上一致为 $o_{\Pbb}(1)$。则
\begin{equation}\label{eq:math-stat-mle-asymptotic-normal}
 \sqrt n(\widehat\theta_n-\theta_0)
 \toD\Normal_d(0,I_0^{-1}).
\end{equation}
在通常二阶可微的正则模型中，\eqnrefs{eq:math-stat-mle-rootn-localization} 可由 score 的 $O_{\Pbb}(\sqrt n)$ 波动、平均 Hessian 向 $-I_0$ 的一致收敛以及局部严格凹性推出；它不能仅由一致性本身推出。

同一组条件实际上给出比弱收敛更强的一阶随机展开：
\begin{equation}\label{eq:math-stat-mle-score-representation}
 \sqrt n(\widehat\theta_n-\theta_0)
 =I_0^{-1}\Delta_n+o_{\Pbb}(1)
 =I_0^{-1}\frac{U_n(\theta_0)}{\sqrt n}+o_{\Pbb}(1).
\end{equation}
...
\[
 ...
 =h^{\mathsf T}\Delta_n-\frac12h^{\mathsf T}I_0h+o_{\Pbb}(1),
\]
其随机二次主项为
\[
 q_n(h)
...
\[
 \widehat h_n=\sqrt n(\widehat\theta_n-\theta_0)=O_{\Pbb}(1),
\]
即以任意高概率落在某个固定大球内。LAN 余项在这个球上的一致控制，再把真实局部极大点与二次近似极大点之差压到 $o_{\Pbb}(1)$：
\[
 \widehat h_n-h^\star=o_{\Pbb}(1).
\]
代回 $h^\star=I_0^{-1}\Delta_n$，得到正文的渐近线性表示\eqnrefs{eq:math-stat-mle-score-representation}。由\eqnrefs{eq:math-stat-score-clt}即 $\Delta_n\toD\Normal_d(0,I_0)$，连续映射给出 $I_0^{-1}\Delta_n\toD\Normal_d(0,I_0^{-1})$，并用 Slutsky 定理吸收 $o_{\Pbb}(1)$ 项，得到\eqnrefs{eq:math-stat-mle-asymptotic-normal}。因此有效 MLE 的 influence function 正是
\[
 \mathrm{IF}_{\theta_0}^{\rm eff}(x)
 =I_0^{-1}s_{\theta_0}(x).
\]

\textbf{实例。}正态模型 $\Normal(\mu,\sigma^2)$：$\hat\mu_n=\bar X_n$，$I_0=1/\sigma^2$，渐近表示 $\sqrt n(\hat\mu_n-\mu)=\frac1{\sqrt n}\sum(X_i-\mu)/\sigma+o_P(1)$，直接给出 $\sqrt n(\hat\mu_n-\mu)\toD\Normal(0,\sigma^2)$。Logistic 回归：MLE 的 influence function $I_0^{-1}s_\theta(x)$ 把单个观测的 score 向量乘以 Fisher 逆，量化该观测对估计量的影响——Huber 稳健统计用 influence function 的有界性定义稳健性，MLE 的 IF 无界（单个离群点可任意影响 $\hat\theta$），故需 M-估计量稳健化。
...
\]
regularity 使 $T_n^*\toD L$，而 LAN 给出 $\Delta_n\toD\Normal_d(0,I_0)$。两者边缘都紧，因此任取子序列都可再取子序列，使
\[
 (T_n^*,\Delta_n)\toD(T,\Delta),
 \qquad
 \Delta\sim\Normal_d(0,I_0),
 \qquad
 T\sim L.
\]
...
\[
 \varphi_L(t)=\E_{Q_0}\ee^{\ii t^{\mathsf T}T}.
\]
...
则
\begin{align*}
 \E\ee^{\ii t^{\mathsf T}V+\ii s^{\mathsf T}\Delta}
 &=\varphi_L(t)
...
\[
 I_0^{-1}\Delta\sim\Normal_d(0,I_0^{-1}),
\]
从而
\[
 T=I_0^{-1}\Delta+V
\]
是有效 Gaussian 噪声与独立额外噪声之和。由于上述论证对任意联合收敛子序列都成立，regular estimator 的极限分布必满足正文的卷积分解\eqnrefs{eq:math-stat-hajek-convolution}。
\end{derivation}
...
\[
 h\sim\Normal_d(0,\tau^2I_d).
\]
在观测模型 $Y\mid h\sim\Normal_d(h,I_0^{-1})$ 下，后验精度相加（Gaussian--Gaussian 共轭：先验精度 \(\tau^{-2}I_d\) 加观测精度 \(I_0\)），故后验协方差为
\[
 V_\tau
 =(I_0+\tau^{-2}I_d)^{-1},
\]
...
\[
 r_\tau^*
 =\E_h\E_{Y\mid h}\|\delta^*(Y)-h\|^2
 =\E_h\operatorname{tr}(V_\tau)
...
从极限实验回到原始 LAN 实验还需要 Le Cam 的局部实验比较定理：有限个局部参数下的 likelihood-ratio 向量由 LAN 收敛到 Gaussian shift 中相应的 likelihood-ratio 向量，再利用随机化决策规则的紧性与风险下半连续性，把任意原实验估计程序的局部风险下极限转移到极限实验。这个"实验收敛 $\Rightarrow$ 风险下界转移"是局部渐近最小最大定理中独立于上面 Gaussian 风险计算的非平凡部分，不能由一次 Taylor 展开替代。对有界 bowl-shaped loss 不需要额外二阶矩控制；平方损失则先截断，再在局部二阶风险一致可积时解除截断。

\emph{举例：正态位置的最小最大性。}对 \(X_1,\ldots,X_n\iid\Normal(\theta,1)\)，\(I_0=1\)，\(\bar X\) 的局部风险 \(\E\|\sqrt n(\bar X-\theta)\|^2=1=\operatorname{tr}(I_0^{-1})\)。Hodges 估计在 \(\theta=0\) 处达到超效率 \(o(n^{-1})\)，但 Le Cam 三引理保证这种逐点改进不能在邻域上整体压低最小最大常数 \(1\)。这正是 Hájek--Le Cam 下界的内容：正则估计的最大风险 \(\ge\operatorname{tr}(I_0^{-1})\)，超效率只能在零测集上发生。

\emph{更一般结论：Hájek 投影下界与 Le Cam 引理。}对任意 bowl-shaped 损失 \(L\)，极限实验最小最大风险为 \(\E L(Z_0)\)，\(Z_0\sim\Normal_d(0,I_0^{-1})\)。Le Cam 第三引理把原实验的局部风险列下极限与极限实验风险通过 likelihood-ratio 加权联系起来，是局部渐近理论从"Gaussian 计算"过渡到"原实验结论"的桥梁。Bernstein--von Mises 定理可看作这一框架在 Bayes 后验下的对应：后验局部化后渐近 Gaussian，且中心在 MLE，方差为 \(I_0^{-1}/n\)。

\emph{物理建模：量子参数估计的 Heisenberg 下界。}Gaussian shift 最小最大风险 \(\operatorname{tr}(I_0^{-1})\) 与量子参数估计的量子 Cramér--Rao 下界 \(\operatorname{tr}(J_Q^{-1})\)（\(J_Q\) 为量子 Fisher 信息）同构。经典与量子下界的差异在于 \(J_Q\ge J_C\)（量子信息不小于经典信息），因此量子估计原则上可超越经典极限——这是量子计量学的理论基础。最小最大常数对应量子 Heisenberg 不确定关系的统计形式：\(\Delta\theta\cdot\Delta p\ge1/2\) 中的 \(1/2\) 正是单参数 Gaussian shift 的最小最大风险。

\emph{举例：位置参数的常数风险估计。}对 \(X\sim\Normal(\theta,1)\)，\(\delta(X)=X\) 风险恒为 1。James--Stein 估计 \(\delta^{JS}(X)=(1-(p-2)/\|X\|^2)X\) 在 \(p\ge3\) 时最大风险严格小于 1，违反正态均值估计的"显然最优"。这一现象的 Le Cam 解释是：Gaussian shift 极限实验的最小最大风险只在\emph{极限}中达到，有限样本下存在严格改进；但改进只能在 \(p\ge3\) 出现，\(p=1,2\) 时 \(\delta(X)=X\) 仍最小最大。


\textbf{例（Hodges 估计量：逐点超效率为何不违反 Fisher 下界）.}\enspace 在最简单的正态位置模型
\[
 X_i\iid\Normal(\theta,1)
\]
中定义
\[
 \widetilde\theta_n
 ...
 \end{cases}
\]
在固定真值 $\theta=0$ 下，$\overline X_n=O_{\Pbb}(n^{-1/2})$，而 $n^{-1/2}=o(n^{-1/4})$，故
\[
 \Pbb_0\{|\overline X_n|\le n^{-1/4}\}\to1.
\]
因此
\[
 \sqrt n\,\widetilde\theta_n\toP0,
\]
...
MLE 渐近正态性给出 Wald 型置信椭球。若 $I(\widehat\theta_n)\toP I_0$，则
\[
...
\]
若 $\theta^*$ 是总体 KL 准则的唯一内点极值，$\widehat\theta_n\toP\theta^*$，score 在其邻域可微，经验 Hessian 一致收敛到非奇异矩阵 $-A$，并且 $n^{-1/2}\sum_i s_{\theta^*}(X_i)$ 满足协方差为 $B$ 的多元 CLT，则得分方程线性化给出
\[
...
\]
Bayes 恒等式给出
\begin{equation}\label{eq:math-stat-em-kl-decomposition}
 \ell(\theta)-\ell(\theta^{(t)})
 =Q(\theta\mid\theta^{(t)})-Q(\theta^{(t)}\mid\theta^{(t)})
 +D_{\rm KL}\!\left(q_t\Vert p_\theta(\,\cdot\mid x)\right).
\end{equation}
所以只要 M 步使 $Q$ 不下降，就有
\begin{equation}\label{eq:math-stat-em-monotonicity}
...
\label{eq:math-stat-semiparametric-tangent-space}
\end{equation}
...
\dot\Psi_P(s)
=
\E_P\bigl[\phi_{\mathrm{eff}}(X)s(X)\bigr],
\qquad s\in\mathcal T_P.
}
\label{eq:math-stat-pathwise-gradient}
\end{equation}
...
\[
\E\{(s_\psi-B^{{\mathsf T}*}s_\eta)s_\eta^{\mathsf T}\}=0
\quad\Longrightarrow\quad
...
\[
 \begin{aligned}
 I_{\mathrm{eff}}
 &=\E(s_{\mathrm{eff}}s_{\mathrm{eff}}^{\mathsf T})\\
 &=I_{\psi\psi}
...
\emph{举例：正态两参数的 efficient score。}对 \(\Normal(\mu,\sigma^2)\)，兴趣参数 \(\psi=\mu\)，nuisance \(\eta=\sigma^2\)。Score
\[
...
\emph{更一般结论：半参数有效信息。}当 nuisance 是无限维（如分布函数、连接函数），tangent space 是无限维闭子空间，efficient score 仍定义为 \(s_\psi\) 到 nuisance tangent space 正交补的投影，但投影算子一般无显式公式。Cramér--Rao 型下界推广为
\[
\Var(\text{efficient estimator})\ge I_{\rm eff}^{-1}/n,
\]
达到此下界的估计量称为半参数有效。经验似然、双稳健估计与增广逆概率加权估计都是构造 efficient influence function 的具体实现路径。
...
\sum_{i=1}^n\phi(X_i)
+o_P(1),
\label{eq:math-stat-semiparametric-asymptotic-linear}
\end{equation}
且 $\E_P\phi=0$、$\E_P\|\phi\|^2<\infty$，则多元 CLT 给出
\[
\sqrt n\{T_n-\Psi(P)\}
\Longrightarrow
\Normal_q(0,\Var_P\phi).
\]
...
0
=
\E_P[\ind_{\{X\le q_p\}}s(X)]
+f(q_p)\dot q_p(s).
\]
又因 $\E s=0$ 且 $F(q_p)=p$，可写成
\[
\dot q_p(s)
=
\E_P\left[
\frac{p-\ind_{\{X\le q_p\}}}{f(q_p)}s(X)
\right].
\]
因此
\begin{equation}
...
\phi_{q_p}(x)
=
\frac{p-\ind_{\{x\le q_p\}}}{f(q_p)}
}
\label{eq:math-stat-quantile-eif}
\end{equation}
并得到效率界
\[
\Var(\phi_{q_p})
...
\textbf{过渡：Bayes 风险、后验与 Bernstein--von Mises 极限.}\enspace
...
\begin{equation}\label{eq:math-stat-prior-density-transform}
 \pi_\eta(\eta)
 =\pi_\theta\{h(\eta)\}
...
\]
于是
\begin{equation}\label{eq:math-stat-fisher-volume-element}
 \sqrt{\det I_\eta(\eta)}\,\dd\eta
 =\sqrt{\det I_\theta(\theta)}\,\dd\theta.
\end{equation}
这导出 Jeffreys 先验
\begin{equation}\label{eq:math-stat-jeffreys-prior}
...
标量平方损失下，令后验均值为 $m_x=\E(\theta\mid x)$，则
\begin{align*}
 \E\{(a-\theta)^2\mid x\}
 &=\E\{(a-m_x+m_x-\theta)^2\mid x\}\\
 &=(a-m_x)^2+\Var(\theta\mid x),
\end{align*}
交叉项因为 $\E(m_x-\theta\mid x)=0$ 消失，所以唯一极小点是 $a=m_x$。绝对损失
\[
 q_x(a)=\E(|a-\theta|\mid x)
\]
...
\textbf{例（Beta--Bernoulli 与 Normal--Normal）.}\enspace 若 $X_i\iid\Bern(p)$、$p\sim\BetaD(a,b)$，记 $S=\sum_iX_i$，则
\[
 p\mid X^n\sim\BetaD(a+S,b+n-S).
\]
后验均值为
\[
 \E(p\mid X^n)
...
若
\[
 X_i\iid\Normal(\mu,\sigma^2),
 \qquad
 \mu\sim\Normal(m_0,v_0),
\]
...
\[
 \lambda\mid X^n
 \sim\GammaD\!\left(a+\sum_{i=1}^nX_i,b+n\right).
\]
...
\begin{equation}\label{eq:math-stat-bvm-rootn-concentration}
 \Pi\!\left(
...
\[
 ...
 \end{aligned}
\]
第一个项与 $z$ 无关，会在后验归一化常数中消去。因此，只要 LAN 余项在后验实际集中的局部区域上一致为 $o_{\Pbb}(1)$，且
\[
...
\]
之比趋于 $1$。后验集中条件负责说明有界局部区域之外的质量可以忽略：即对任意 \(M_n\to\infty\)（缓增），\(\Pi(\|h\|>M_n\mid X^n)\toP0\)。最后用
\[
 I_0^{-1}\Delta_n
 =\sqrt n(\widehat\theta_n-\theta_0)+o_{\Pbb}(1)
\]
...
\emph{举例：正态均值后验的精确 BvM。}对 \(X_1,\ldots,X_n\iid\Normal(\theta,1)\) 配先验 \(\pi\) 在真值 \(\theta_0\) 连续且严格正，后验
\[
...
\]
后验 \(z\) 密度趋于 \((2\pi)^{-1/2}\ee^{-z^2/2}\)，即 \(\sqrt n(\theta-\bar X)\mid X^n\toD\Normal(0,1)\)。这里 BvM 是精确的，因 likelihood 本身就是 Gaussian。

\emph{更一般结论：Bernstein--von Mises 与非参数 Bayesian。}BvM 在半参数模型中仍可成立，只要先验在参数方向上"充分光滑"且在 nuisance 方向上"足够不集中"。但在纯非参数模型（如先验放在分布函数空间）中，BvM 一般失效：后验收缩速率不再是 \(n^{-1/2}\)，且极限形状依赖先验的光滑度参数。这把 BvM 与频率统计的 \(\sqrt n\) 渐近理论区分开：BvM 是"参数 + 半参数"现象，不是普遍 Bayesian 现象。

\emph{物理建模：BvM 与系综等价。}BvM 定理的物理对应是统计系综等价：在热力学极限下，不同系综（microcanonical、canonical、grand canonical）给出相同的宏观可观测量分布。BvM 中"后验渐近 Gaussian 中心在 MLE"对应"不同先验在热力学极限下给出相同后验"——先验选择（系综选择）在大样本下不重要，因为 likelihood（能量函数）主导后验（概率测度）。这正是统计力学中"边界条件不重要"的 Bayesian 表述。
\end{derivation}
...
\begin{equation}\label{eq:math-stat-bvm-delta}
 \int
 \varphi\!\left(
 ...
 \E\varphi(Z_g),
 \qquad
 Z_g\sim\Normal_k(0,G_0I_0^{-1}G_0^{\mathsf T}).
\end{equation}
...
\]
或渐近等于 $1-\alpha$，就称它为后验 $1-\alpha$ 可信集；这是后验分布内部的概率陈述，不等同于\eqnrefs{eq:math-stat-confidence-set-definition}的重复抽样覆盖定义。记 $q_{d,1-\alpha}$ 为 $\chiSq_d$ 的 $1-\alpha$ 分位数，并定义
\begin{equation}\label{eq:math-stat-bvm-credible-ellipsoid}
 C_n^{\rm B}
 =\left\{
 \theta:
 ...
 \right\}.
\end{equation}
由总变差 BvM 与 $I(\widehat\theta_n)\toP I_0$，其后验可信度满足
\[
