...
以正态位置族为例。对 \(X_1,\ldots,X_n\iid\Normal(\mu,1)\)，
\[
...
支撑依赖参数的均匀族呈现另一种压缩方式。对 \(X_1,\ldots,X_n\iid\Uniform(0,\theta)\)，
\[
...
对 Poisson 样本 \(X_1,\ldots,X_n\iid\Poisson(\lambda)\)，联合密度
\[
...
定义（完备统计量）.\enspace 统计量 $T$ 称为完备的，若对任意可测函数 $a$，只要
\[
 \E_\theta[a(T)]=0
...
例（Poisson 完备性）.\enspace 若 $X_i\iid\Poisson(\lambda)$，则
\[
...
对 \(\Normal(\mu,\sigma^2)\) 两参数族，自然参数 \(\eta=(\mu/\sigma^2,-1/(2\sigma^2))\) 取值于开半平面 \(\eta_2<0\)，所以自然统计量 \(T_n=(\sum_i X_i,\sum_i X_i^2)\) 完备。若固定 \(\sigma^2=1\)，则应改用一维自然参数化；相应的一维自然参数空间仍为开集，故完备性依然成立。
...
\]
这里 \(A\) 正是 log-partition function（自由能）；\(\nabla A(\eta)=\E_\eta t(X)\) 给出能量期望，\(\nabla^2 A(\eta)=\Cov_\eta(t)\) 则同时表示 Fisher 信息与响应系数。完备性定理中“Laplace 变换在开集上恒零推出测度零”对应热力学中“配分函数决定全部状态方程”：自由能的解析延拓唯一确定分布族。

Poisson 族可以直接验证这一点。若 \(X\sim\Poisson(\lambda)\) 且 \(\E_\lambda a(X)=0\) 对所有 \(\lambda>0\)，则乘以 \(\ee^\lambda\) 后有 \(\sum_x a(x)\lambda^x/x!=0\)。幂级数系数唯一性迫使 \(a(x)=0\) 对所有 \(x\)，所以该族完备。
...
定理（Basu 定理）.\enspace 若 $T$ 对模型充分且完备，$A$ 为辅助统计量，则 $T$ 与 $A$ 独立。


对任意有界可测函数 $h(A)$，证明 $\E_\theta\{h(A)\mid T\}=\E_\theta h(A)$，即 $A$ 与 $T$ 独立。充分性使条件期望不依赖 $\theta$，完备性排除条件期望的非平凡波动。
...
塔式性质传递到 $T$。由条件期望的塔式性质 $\E_\theta\E_\theta\{h(A)\mid T\}=\E_\theta h(A)$，
\begin{equation*}
 \E_\theta\{q(T)-c\}
 =\E_\theta q(T)-c
 =\E_\theta h(A)-c
...
完备性缺失时的反例。若 \(T\) 充分但不完备，Basu 定理一般失效。例如对 \(\Uniform(\theta,\theta+1)\) 位置族，\((X_{(1)},X_{(n)})\) 充分但不完备，存在非零函数 \(a\) 使 \(\E_\theta a(T)=0\)。此时辅助统计量可能与 \(T\) 不独立，Basu 的"充分+完备 \(\Rightarrow\) 独立"链条在完备性环节断裂。
...
设 \(U\) 是 \(g(\theta)\) 的无偏估计量，\(T\) 是充分统计量。Rao--Blackwell 化简为 \(U^*=\E_\theta(U\mid T)\)。由塔式性质（\(T\) 生成的 \(\sigma\)-代数是全空间的子 \(\sigma\)-代数），
\[
 \E_\theta U^*
...
Rao--Blackwell 在 Poisson 样本中可以显式计算。若目标是 \(g(\lambda)=\ee^{-\lambda}=\Pbb(X_1=0)\)，则 \(U=\ind_{\{X_1=0\}}\) 无偏，而完备充分统计量为 \(T=\sum_i X_i\sim\Poisson(n\lambda)\)。条件化给出
\[
...
最佳线性投影。把随机向量 $s_\theta$ 对 $\psi$ 的各坐标所张成的有限维子空间作线性投影 $C\psi$。最佳线性系数矩阵由最小化 $\E\|s_\theta-C\psi\|^2$ 得到。对目标关于 \(C\) 求导并令其为零：
\[
...
Pythagoras 分解。残差 $s_\theta-C\psi$ 与投影 $C\psi$ 正交（由最佳投影性，\(\E[(s_\theta-C\psi)\psi^{\mathsf T}]=0\) 推出 \(\E[(s_\theta-C\psi)(C\psi)^{\mathsf T}]=0\)），故协方差作正交分解：
\begin{equation*}
 I(\theta)
...
取估计函数 \(\psi=s_\theta\) 为 likelihood score 本身，则 \(A=\E(s_\theta s_\theta^{\mathsf T})=I(\theta)\)、\(B=I(\theta)\)，sandwich 协方差
\[
...
对位置模型中的标量 M 估计，若使用截断估计函数 $\psi_c(X-\theta)$，上述 sandwich 公式化为 $\E\psi_c^2/\{\E\psi_c'\}^2$。截断降低异常值的影响，但也会改变正态模型下的效率；因此 $c$ 控制的是稳健性与模型内效率之间的权衡。
...
定理（正则 MLE 的渐近正态性）.\enspace \label{thm:math-stat-mle-asymptotic-normal}
...
在正态位置模型 \(X_i\iid\Normal(\theta,1)\) 中，\(I_0=1\)，而 \(\bar X\) 的局部风险 \(\E\{\sqrt n(\bar X-\theta)\}^2=1\)，恰好达到极限实验的最小最大常数。Hodges 估计虽能在 \(\theta=0\) 处超效率，却不能在一个局部邻域上整体压低这个常数。

对 \(X\sim\Normal_p(\theta,I_p)\)，通常估计量 \(\delta_0(X)=X\) 的风险恒为 \(p\)。当 \(p\ge3\) 时，James--Stein 估计
\[
...
对 \(\Normal(\mu,\sigma^2)\)，若兴趣参数为 \(\psi=\mu\)、nuisance 参数为 \(\eta=\sigma^2\)，则 score 为
\[
...
例（Beta--Bernoulli 与 Normal--Normal）.\enspace 若 $X_i\iid\Bern(p)$、$p\sim\BetaD(a,b)$，记 $S=\sum_iX_i$，则
\[
