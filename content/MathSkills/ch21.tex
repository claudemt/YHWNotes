...
\[
 ...
 =g_\theta\{T(x)\}H(x).
\]
为把所有事件 $A$ 同时组织成一个条件概率核，这里使用标准 Borel 样本空间上的 正则 条件 law。记 $Q_t(\dd x)$ 为 $Q$ 下给定 $T=t$ 的一个 正则 条件 分布，并定义
\[
...
\]
两样本 $x,y$ 的 似然 成比例当且仅当 $\sum_i x_i=\sum_i y_i$，故 $T(x)=\sum_i X_i$（等价地 $\bar X$）最小充分。注意原始样本 $(X_1,\ldots,X_n)$ 充分但非最小；顺序统计量在非参数族中最小充分，但在参数族中通常过度。
...
\]
是完整满秩二参数指数族的充分完备统计量。由于 $S^2$ 是 $T$ 的函数且 $\mathbb E S^2=\sigma^2$，Lehmann--Scheff\'e 定理给出 $S^2$ 是 $\sigma^2$ 的 一致最小方差无偏 估计量。
\end{exbox}
...
\]
这是 $\ee^{-\lambda}$ 的 一致最小方差无偏 估计，方差严格小于 $\mathds{1}_{\{X_1=0\}}$。它体现了 Rao--Blackwell 把"用第一个观测指示零"改进为"用全体观测通过充分统计量估计零概率"。
...
在正确设定且可微的参数模型中，得分 之所以特殊，还可以直接从所有无偏估计方程中比较出来。设 $\theta\in\R^d$，$\psi(X,\theta)\in\R^d$ 满足
\[
...
等号条件。当残差协方差为零时，$s_\theta=C\psi$ 几乎处处成立；得分 与估计函数只差一个非奇异线性变换。此时 三明治 协方差 $A^{-1}BA^{-\mathsf T}=I(\theta)^{-1}$，达到 Cram\'er--Rao 下界。等号成立当且仅当 得分 几乎处处落在 $\psi$ 的线性张成空间内；换言之，正则 似然 得分 在这类无偏估计方程中保留了全部一阶 Fisher 信息。

取估计函数 $\psi=s_\theta$ 为 似然 得分 本身，则 $A=\mathbb E(s_\theta s_\theta^{\mathsf T})=I(\theta)$、$B=I(\theta)$，三明治 协方差
\[
...
\]
对 $\theta$ 几乎处处求导得到 $a(\theta)\theta^{n-1}=0$，故 $a=0$ 几乎处处，$T$ 完备。Lehmann--Scheff'e 定理于是说明 $((n+1)/n)X_{(n)}$ 是 $\theta$ 的 一致最小方差无偏 估计量。这个例子同时分开了有限样本无偏最优、渐近收敛速率与正则 似然 几何。
\end{exbox}
...
若 $\theta\in\R^d$ 且 对数密度 可微，定义单观测 得分、样本 得分 与 观测信息
\[
...
\]
所以 $nI(\theta)$ 是样本 Fisher 信息。它既是 得分 协方差，也是平均 对数似然 的局部负 Hessian；这正是后面 Fisher 几何出现的原因。
...
若参数分为兴趣参数 $\psi$ 与 冗余 参数 $\lambda$，则分块 Fisher 信息
\[
...
对 iid 二次均值可微 模型，若 $I_0$ 有限，则对固定 $h\in\R^d$，局部参数
\[
...
正态模型 $\mathcal N(\mu,\sigma^2)$：$\hat\mu_n=\bar X_n$，$I_0=1/\sigma^2$，渐近表示 $\sqrt n(\hat\mu_n-\mu)=\frac1{\sqrt n}\sum(X_i-\mu)/\sigma+o_P(1)$，直接给出 $\sqrt n(\hat\mu_n-\mu)\xrightarrow{d}\mathcal N(0,\sigma^2)$。Logistic 回归：极大似然估计 的 影响函数 $I_0^{-1}s_\theta(x)$ 把单个观测的 得分 向量乘以 Fisher 逆，量化该观测对估计量的影响——Huber 稳健统计用 影响函数 的有界性定义稳健性，极大似然估计 的 IF 无界（单个离群点可任意影响 $\hat\theta$），故需 M-估计量稳健化。


“达到 $I_0^{-1}$”必须说明比较的是哪一类估计量。若只在单个参数点比较方差，可以人为构造在该点异常快、却在 $n^{-1/2}$ 邻域中行为不稳定的估计序列，因此 Fisher 逆 不是对任意估计量逐点成立的渐近方差下界。正确的比较对象是对局部备择稳定的 \emph{正则估计量}。称 $T_n$ 在 $\theta_0$ 处 正则，若存在不依赖固定 $h$ 的极限分布 $L$，使对每个 $h\in\R^d$，在
\[
 \theta_{n,h}=\theta_0+\frac h{\sqrt n}
\]
下都有
...
\[
 \Cov(L)\succeq I_0^{-1}.
\]
有效估计量对应 $M$ 退化在 $0$；极大似然估计 在前述正则条件下正是这一情形。这个结论比有限样本 Cram\'er--Rao 更精确地说明了“渐近效率”的含义：下界约束的是整个 $n^{-1/2}$ 局部实验中稳定的估计程序，而不是只在某一个参数点观察到的方差。
...
\begin{derivation}[Gaussian 位移 极限中的最小最大风险]
...
有限维 局部渐近正态 把局部模型压缩成 $d$ 维 Gaussian 位移；但许多统计问题同时包含有限维目标参数与无限维 冗余。例如密度本身未知而只关心均值、分位数、回归函数某个线性泛函，或缺失机制/误差分布作为无限维未知量。这时 Fisher 信息矩阵不再足够，必须把``所有可行局部扰动''组织成 Hilbert 空间中的 切空间。
...
\end{equation}
这里最后一个不等式按半正定矩阵理解。于是半参数效率界不是另一个独立的 Cram\'er--Rao 公式，而是 切-空间 正交投影的直接结果。
...
\[
 ...
 \end{pmatrix}.
\]
冗余 切空间 是 $s_\eta$ 各线性组合张成的有限维子空间 $\mathcal T_\eta=\{B^{\mathsf T}s_\eta:B\in\R^{r\times q}\}$。把 $s_\psi$ 正交投影到其正交补 $\mathcal T_\eta^\perp$：找 $B^*$ 使 $s_\psi-B^{{\mathsf T}*}s_\eta\perp\mathcal T_\eta$，即
\[
...
对 $\mathcal N(\mu,\sigma^2)$，若兴趣参数为 $\psi=\mu$、冗余 参数为 $\eta=\sigma^2$，则 得分 为
\[
...
\end{equation*}
它在光滑一一重参数化下表示同一个先验测度。Jeffreys 构造只解决坐标不变性，并不保证先验可归一化、后验必为 真，也不保证在边界或奇异模型中具有良好频率性质；这些仍需逐模型检查。
...
\[
 ...
 \right|\to0.
\]
这些条件是便于直接证明下式的一组充分条件；在更一般的 BvM 定理中，扩张球上的控制可由局部检验、后验收缩与模型特有的经验过程论证替代，但不能从固定球 局部渐近正态 自动推出。令
\[
...
\]
先验在该尺度上趋于常数，而 似然 的二次项完成平方后中心为 $I_0^{-1}\Delta_n$。结合 极大似然估计 的表示\eqnrefs{eq:math-stat-mle-score-representation}，典型 Bernstein--von Mises 结论是
...
\]
后验 $z$ 密度趋于 $(2\pi)^{-1/2}\ee^{-z^2/2}$，即 $\sqrt n(\theta-\bar X)\mid X^n\xrightarrow{d}\mathcal N(0,1)$。这里 BvM 是精确的，因 似然 本身就是 Gaussian。
