...
\begin{align*}
 ...
 &=P_\theta^{(n)}(A\cap B).
\end{align*}
第二步与第四步只用了条件期望的定义，因为 $g_\theta(T)\mathds{1}_BK(T,A)$ 与 $g_\theta(T)\mathds{1}_B$ 都是 $\sigma(T)$-可测的。于是
\[
 K(T,A)
 =P_\theta^{(n)}(X^n\in A\mid T)
...
例（Poisson 完备性）.\enspace 若 $X_i\iid\mathrm{Poisson}(\lambda)$，则
\[
...
\]
对所有 \(\theta\) 成立。完备性定义恰好要求：若 \(\mathbb E_\theta h(T)=0\) 对所有 \(\theta\)，则 \(h(T)=0\) a.s.。故 \(a_1(T)=a_2(T)\) a.s.，UMVU 估计唯一。结合第二步的方差下界和第三步的唯一性，Lehmann--Scheff\'e 定理给出：完备充分统计量的无偏函数是唯一 UMVU 估计。
\end{derivation}
...
例（Beta--Bernoulli 与 Normal--Normal）.\enspace 若 $X_i\iid\mathrm{Bernoulli}(p)$、$p\sim\mathrm{Beta}(a,b)$，记 $S=\sum_iX_i$，则
\[
