...
推广与物理建模。对分段 \(C^1\) 函数 \(f\) 在超曲面 \(\Sigma\) 处有跳跃 \([f]_\Sigma\)，分布导数满足
\[
...
把复合算子写成双重积分核，做一次变量替换分离出一个「快振荡相位」，再把符号沿位置 Taylor 展开，并用振荡相位把位置多项式转成频率导数。
...
先用 parametrix 恒等式把解 \(u\) 分解为「正则部分 \(Qf\)」与「光滑误差 \(Ru\)」，再借 \(Q\) 的负阶符号提升 Sobolev 指数，最后用 bootstrap 归纳与 wavefront 论证闭合正则性。
...
bootstrap 归纳。若 \(f\) 光滑，先取 \(s\) 足够小使 \(u\in H^s\)（任何分布局部地属于某个 \(H^s\)），第一步给 \(u\in H^{s+m}\)；把新的 \(u\in H^{s+m}\) 代回原方程 \(Pu=f\) 再应用第一步，得 \(u\in H^{s+2m}\)；依此类推，对任意 \(k\) 有 \(u\in H^{s+km}\)。由 Sobolev 嵌入 \(H^\infty\subset C^\infty\)，最终 \(u\) 光滑。
