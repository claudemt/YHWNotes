...
分布的支集只回答``对象在哪里不为零''，却不能区分一个点附近究竟是光滑函数、跳跃还是 δ 型奇性。为描述正则性，先引入标准 磨光子。取
\[
...
\]
则称 $T$ 在 $(x_0,\xi_0)$ 处 microlocally 光滑。所有不满足这一条件的点组成波前集
\[
\operatorname{WF}(T)
...
波前集真正强大的地方在于研究偏微分算子的奇性传播：椭圆算子不会制造新的波前方向，双曲算子则沿特征流传播奇性。完整理论需要 伪微分 算符、符号 演算 与 Hamilton 流。本书在这里先固定定义和乘积判据；后续若需要传播定理，再从这一母结构继续展开，而不把 microlocal analysis 退化成一张术语表。
\end{note}
...
再用单位分解把一般检验函数拆成各零点邻域中的部分并求和，就得到上式。绝对值不能省略，因为 δ 分布在变量反向时仍应保持正的测度权重。
...
\]
因此 分布 导数 不是重新定义了一个与经典微分无关的运算，而是经典导数的唯一连续延拓。
\end{derivation}

例：Heaviside 函数产生 δ 分布。取 $n=1$、$H(x)=\mathbf 1_{x>0}$，对任意 $\phi\in C_c^\infty(\RR)$，$\langle H',\phi\rangle=-\int_0^\infty\phi'(x)\,\dd x=\phi(0)=\langle\delta_0,\phi\rangle$，故 $H'=\delta_0$——弱导数记录跳跃奇点的机制。对分段 $C^1$ 函数在超曲面 $\Sigma$ 处有跳跃 $[f]_\Sigma$，分布导数满足 $\partial_jT_f=T_{\partial_jf}+[f]_\Sigma\,n_j\,\delta_\Sigma$，其中 $n_j$ 为法向分量。量子力学中一维 δ 势阱 $V(x)=-\lambda\delta(x)$ 的 Schrödinger 方程正是在分布意义下求解：$\psi$ 连续但 $\psi'$ 在 $x=0$ 处跳跃 $\psi'(0^+)-\psi'(0^-)=-2m\lambda\psi(0)/\hbar^2$，即分布导数把 δ 奇性翻译为导数跳跃条件。高阶情形 $\partial^\alpha T_f=T_{\partial^\alpha f}$ 由归纳法直接得到。
...
\begin{derivation}[tempered 分布 上 Fourier 微分律]
...
例：$H^1(\RR)$ 与 $W^{1,2}(\RR)$。对 $u\in H^1(\RR)$，Plancherel 给出 $\|u\|_{H^1}^2=\int(1+k^2)|\widehat u(k)|^2\,\dd k=\|u\|_{L^2}^2+\|u'\|_{L^2}^2$，即 $H^1$ 范数恰是 $L^2$ 范数与导数 $L^2$ 范数的平方和；$u(x)=\ee^{-|x|}$ 时 $\widehat u(k)=2/(1+k^2)$，故 $u\in H^1$ 但 $u\notin H^2$。量子力学中，$H^1(\RR^3)$ 是氢原子 Hamiltonian 的形式域，$\psi\in H^1$ 保证动能 $\int|\nabla\psi|^2$ 有限；Sobolev 嵌入 $H^1(\RR^3)\hookrightarrow L^6(\RR^3)$ 则给出 Hartree 非线性项 $\int|\psi|^4$ 的良定义性，这是多体 Schrödinger 方程能谱分析的基础。
...
Sokhotski--Plemelj 公式进一步把主值部分与 δ 奇性统一到同一个正则化极限中。
...
从全空间常系数基本解继续向变系数问题推进时，普通 Fourier 乘子 已经不够。真正需要的是“允许 乘子 同时依赖位置”的伪微分算子。它把前面的 Sobolev 权重、波前 集合 与椭圆正则性统一到同一套 符号 演算 中。

设
\[
\langle\xi\rangle=(1+|\xi|^2)^{1/2}.
\]
对实数 $m$，Kohn--Nirenberg 符号类 $S^m_{1,0}(\RR^n_x\times\RR^n_\xi)$ 由所有光滑函数 $a(x,\xi)$ 组成，并满足对任意 multi-指标 $\alpha,\beta$ 都存在 $C_{\alpha\beta}$ 使
\begin{equation*}\boxed{
...
对 Schwartz 函数该式可先按振荡积分理解。若 $a$ 与 $x$ 无关，就退化为 Fourier 乘子；若
\[
...
\]
因此 differential 算符 只是 符号 在 $\xi$ 上为多项式的特殊伪微分算子。

若两个 $m$ 阶符号之差属于 $S^{m-1}$，它们具有相同 主符号。于是 主符号 应理解为商类
\begin{equation*}\sigma_m(P)\in S^m/S^{m-1},\end{equation*}
而不是依赖某个低阶写法的具体函数。对微分算子，上式正恢复最高阶齐次多项式
\[
p_m(x,\xi)
=\sum_{|\alpha|=m}a_\alpha(x)(\ii\xi)^\alpha.
\]
高频极限只看 主符号，这正是椭圆性、特征集与奇性传播全部由最高阶部分控制的原因。
...
在 真-支集 或适当局部化条件下连续。$m>0$ 的算子损失 $m$ 阶 Sobolev 正则性，$m<0$ 的算子则获得 $|m|$ 阶正则性；$S^{-\infty}:=\cap_mS^m$ 对应 光滑化 算符，把任意 分布 局部送到 $C^\infty$。
...
\end{equation}
“渐近”表示截断到 $|\alpha|<N$ 后的余项降低 $N$ 个 符号 阶。零阶项只是 $ab$，因此
\[
\sigma_{m_1+m_2}(AB)
...
分离快相位。令 $\theta=\xi-\eta$，即 $\xi=\eta+\theta$。总相位变为
\[
...
沿位置 Taylor 展开。因为 $\ee^{\ii(x-y)\cdot\theta}$ 对 $y$ 是快振荡的，$y$ 远离 $x$ 时正负相位相消（稳定相位原理），主导贡献来自 $y\approx x$。故把 $b(y,\eta)$ 在 $y=x$ 处展开：
\[
b(y,\eta)
\sim\sum_\alpha
...
多项式换成频率导数。位置多项式乘上振荡指数可改写为对 $\theta$ 的导数：
\[
...
分部积分移位。把 $\partial_\theta^\alpha$ 从指数上分部积分移到 $a(x,\eta+\theta)$ 上（边界项因符号速降而消失，并注意每次分部积分产生一个负号）：
\begin{equation*}
...
对 $y$、$z$ 积分并置 $\theta=0$。对 $z$ 积分给出 $\hat u(\eta)$，对 $y$ 积分给出 $(2\pi)^n\delta(\theta)$，于是令 $\theta=0$、$\partial_\theta^\alpha a(x,\eta+\theta)|_{\theta=0}=\partial_\xi^\alpha a(x,\eta)$，得到
\[
\sum_\alpha
\frac1{\alpha!}
\left(\frac1\ii\right)^{|\alpha|}
...
\partial_x^\alpha b(x,\eta),
\]
即\eqnrefs{eq:math-dist-symbol-composition}。所以 符号 演算 本质上是“振荡相位把位置 Taylor 系数转成频率导数”。

一阶项与 Poisson 括号。取 $|\alpha|=1$，一阶修正为
\[
...
\begin{equation*}[A,B]\in\Psi^{m_1+m_2-1},\end{equation*}
而其 主符号 为
\begin{equation}
\boxed{
...
\right)
\]
是余切丛上的 正则 Poisson 括号。于是“量子交换子降低一阶并趋向 经典 Poisson 括号”并不是半经典理论里突然出现的类比，而已经内嵌在 寻常 伪微分 符号 演算 中。

现在回到正则性。若 $P\in\Psi^m$ 的 主符号 $p_m(x,\xi)$ 在某个 conic 区域满足
\begin{equation*}|p_m(x,\xi)|\ge c|\xi|^m
\qquad(|\xi|\gg1),\end{equation*}
就称 $P$ 在该区域 microlocally 椭圆。若处处满足，则称 椭圆。其 特征 集合 为
\begin{equation*}\operatorname{Char}(P)
:=\{(x,\xi)\in T^*M\setminus0:p_m(x,\xi)=0\}.\end{equation*}
椭圆算子正是 $\operatorname{Char}(P)=\varnothing$ 的情形。

椭圆性的核心不是一个不等式本身，而是可以逐阶构造 拟逆。先取
\[
q_{-m}^{(0)}(x,\xi)
...
这就是 拟逆：它未必是全局精确逆，但误差已经 光滑 到对 波前 集合 完全不可见。

\begin{derivation}[拟逆 怎样推出椭圆正则性]
解的分裂。设 $Pu=f$。把 $QP=I-R$ 左作用到方程上，得
\begin{equation*}
  QPu=QP u = (I-R)u = Qf,
\end{equation*}
移项即
\begin{equation*}
  u=Qf+Ru.
\end{equation*}
这一步把未知解 $u$ 拆成两项：$Qf$ 由已知数据 $f$ 经 拟逆 $Q$ 决定，$Ru$ 是「光滑到不可见」的误差项。

$Q$ 的 Sobolev 提升。$Q\in\Psi^{-m}$ 的符号 $q\in S^{-m}$ 满足
\begin{equation*}
  |q(x,\xi)|\le C(1+|\xi|)^{-m},
\end{equation*}
...
\begin{equation*}
  ...
  \in L^2,
\end{equation*}
即 $Q u\in H^{s+m}$。直观上，每个 $m$ 阶负号相当于「抹平」了 $m$ 阶导数，因此 拟逆 把正则指数整体抬高 $m$。误差算子 $R\in\Psi^{-\infty}$ 则把任意分布映成光滑函数，不损失任何正则性。故若 $f\in H^s_{\mathrm{loc}}$，则
\begin{equation}
...
自助法 归纳。若 $f$ 光滑，先取 $s$ 足够小使 $u\in H^s$（任何分布局部地属于某个 $H^s$），正则性提升一次即给 $u\in H^{s+m}$；把新的 $u\in H^{s+m}$ 代回原方程 $Pu=f$ 再应用提升，得 $u\in H^{s+2m}$；依此类推，对任意 $k$ 有 $u\in H^{s+km}$。由 Sobolev 嵌入 $H^\infty\subset C^\infty$，最终 $u$ 光滑。

波前 集合 的精确化。从 $u=Qf+Ru$，因 $Ru$ 光滑，其波前集为空，故
\begin{equation*}
  \operatorname{WF}(u)
  =\operatorname{WF}(Qf)
  \subseteq\operatorname{WF}(f)\cup\operatorname{Char}(P),
\end{equation*}
其中右端补上 $\operatorname{Char}(P)$ 是因为 $Q$ 只在 $\operatorname{Char}(P)$ 之外才是真正的逆；在特征集上 $Q$ 不再控制 $P$ 的方向。另一方面，任何 伪微分 算符 都不会创造原来不存在的波前方向：
\[
...
\end{equation}
这正把本章前面的 波前 定义闭合成一个可计算的 PDE 定理：椭圆区域不改变解的奇异方向。
\end{derivation}

例：Laplace 算子。取 $P=-\Delta$（$m=2$），它是处处椭圆算子，$\operatorname{Char}(-\Delta)=\varnothing$，于是 $\operatorname{WF}(u)=\operatorname{WF}(\Delta u)$：若 $\Delta u=f$ 在某区域光滑，则 $u$ 在该区域光滑。这正是位势论中「调和函数必解析、Poisson 方程右端光滑则解光滑」的严格微局域表述；对一般 $m$ 阶椭圆算子，$u$ 比 $f$ 多 $m$ 阶导数，且奇异方向在椭圆区域精确保持。

椭圆正则性由此定量给出：$Pu\in H^{s+m}$ 局部蕴含 $u\in H^{s+m}_{\mathrm{loc}}$（\eqnrefs{eq:math-dist-elliptic-sobolev-regularity}），波前集在椭圆区域满足 $\operatorname{WF}(u)=\operatorname{WF}(Pu)$（\eqnrefs{eq:math-dist-wavefront-elliptic-regularity}）。

当 $P$ 不椭圆时，真正困难只剩在 $\operatorname{Char}(P)$ 上。若 主符号 $p$ 为实值且满足 real-主-型 的非退化条件，则 Hamilton 矢量场
\begin{equation*}H_p
=\sum_j
\left(
...
\right)\end{equation*}
在 特征 集合 上生成 bicharacteristics。propagation of 奇点 定理 的核心结论是：若 $Pu=f$，那么在 $\operatorname{WF}(f)$ 之外，$\operatorname{WF}(u)\cap\operatorname{Char}(P)$ 必沿 $H_p$ 的 积分曲线 传播。椭圆算子没有 特征 directions，所以奇性被完全压制；双曲算子则允许奇性沿特征流传播。
...
\tau^2=c^2|\xi|^2,
\]
正对应有限传播速度的光锥方向。于是“波前沿光线传播”在微局部语言中就是 Hamiltonian 流 of the 主符号；这同时解释了几何光学、经典 Hamilton 流 与双曲 PDE 奇性传播为何共享同一余切丛结构。
