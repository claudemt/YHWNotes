...
\begin{derivation}[tempered 分布上的 Fourier 微分律]
目标是由分布导数定义推出 tempered 分布上的 Fourier 微分律。设 $T\in\mathcal S'(\RR^n)$。按本章约定
\[
\langle\mathcal FT,\phi\rangle
=\langle T,\mathcal F\phi\rangle.
\]
...
令 $\theta=\xi-\eta$（即 $\xi=\eta+\theta$）以分离快相位。总相位变为
\[
...
因为 $\ee^{\ii(x-y)\cdot\theta}$ 对 $y$ 快振荡，$y$ 远离 $x$ 时正负相位相消（稳定相位原理），主导贡献来自 $y\approx x$，故把 $b(y,\eta)$ 在 $y=x$ 处展开：
\[
b(y,\eta)
\sim\sum_\alpha
...
把 $\partial_\theta^\alpha$ 从指数上分部积分移到 $a(x,\eta+\theta)$ 上（边界项因符号速降而消失，每次分部积分产生一个负号）：
\begin{equation*}
...
对 $z$、$y$ 积分后令 $\theta=0$：对 $z$ 积分给出 $\hat u(\eta)$，对 $y$ 积分给出 $(2\pi)^n\delta(\theta)$，且 $\partial_\theta^\alpha a(x,\eta+\theta)|_{\theta=0}=\partial_\xi^\alpha a(x,\eta)$，于是
\[
\sum_\alpha
\frac1{\alpha!}
\left(\frac1\ii\right)^{|\alpha|}
...
\partial_x^\alpha b(x,\eta),
\]
即式\eqnrefs{eq:math-dist-symbol-composition}。符号演算的本质，就是振荡相位把位置 Taylor 系数转成频率导数。
...
\begin{derivation}[拟逆怎样推出椭圆正则性]
设 $Pu=f$。把 $QP=I-R$ 左作用于方程，得
\begin{equation*}
  QPu=QP u = (I-R)u = Qf,
\end{equation*}
移项即
\begin{equation*}
  u=Qf+Ru.
\end{equation*}
这把未知解 $u$ 拆成两项：$Qf$ 由已知数据 $f$ 经拟逆 $Q$ 决定，$Ru$ 是「光滑到不可见」的误差项。
...
\begin{equation*}
  ...
  \in L^2,
\end{equation*}
即 $Q u\in H^{s+m}$：每个 $m$ 阶负符号相当于抹平 $m$ 阶导数，故拟逆把正则指数整体抬高 $m$。误差算子 $R\in\Psi^{-\infty}$ 把任意分布映成光滑函数，不损失正则性。故若 $f\in H^s_{\mathrm{loc}}$，则
\begin{equation}
...
若 $f$ 光滑，先取 $s$ 足够小使 $u\in H^s$（任何分布局部地属于某个 $H^s$），提升一次即得 $u\in H^{s+m}$；把它代回原方程 $Pu=f$ 再提升，得 $u\in H^{s+2m}$；反复迭代，对任意 $k$ 有 $u\in H^{s+km}$。由 Sobolev 嵌入 $H^\infty\subset C^\infty$，最终 $u$ 光滑。

再精确到波前集：因 $Ru$ 光滑、波前集为空，由 $u=Qf+Ru$ 得
\begin{equation*}
  \operatorname{WF}(u)
  =\operatorname{WF}(Qf)
  \subseteq\operatorname{WF}(f)\cup\operatorname{Char}(P),
\end{equation*}
右端补 $\operatorname{Char}(P)$，是因为 $Q$ 只在 $\operatorname{Char}(P)$ 之外才是真正的逆，在特征集上 $Q$ 不再控制 $P$ 的方向。另一方面，伪微分算符不会凭空创造波前方向：
\[
...
\end{equation}
这把本章波前集的定义闭合成可计算的 PDE 定理：椭圆区域不改变解的奇异方向。
\end{derivation}
