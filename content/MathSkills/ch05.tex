...
举例：Heaviside 函数产生 delta 分布。取 $n=1$、$H(x)=\mathbf 1_{x>0}$。对任意 $\phi\in C_c^\infty(\RR)$，
\[
\langle H',\phi\rangle
...
于是 Fourier 变换在缓增分布上把微分与乘子互换：$\mathcal F[D^\alpha T]=(\ii k)^\alpha\mathcal FT$（式~\eqnrefs{eq:tempered-fourier-derivative}），逆形式见式~\eqnrefs{eq:tempered-fourier-multiplication}。
...
bootstrap 归纳。若 $f$ 光滑，先取 $s$ 足够小使 $u\in H^s$（任何分布局部地属于某个 $H^s$），正则性提升一次即给 $u\in H^{s+m}$；把新的 $u\in H^{s+m}$ 代回原方程 $Pu=f$ 再应用提升，得 $u\in H^{s+2m}$；依此类推，对任意 $k$ 有 $u\in H^{s+km}$。由 Sobolev 嵌入 $H^\infty\subset C^\infty$，最终 $u$ 光滑。

wavefront set 的精确化。从 $u=Qf+Ru$，因 $Ru$ 光滑，其波前集为空，故
\begin{equation*}
  \operatorname{WF}(u)
  =\operatorname{WF}(Qf)
...
椭圆正则性由此定量给出：$Pu\in H^{s+m}$ 局部蕴含 $u\in H^{s+m}_{\mathrm{loc}}$（式~\eqnrefs{eq:math-dist-elliptic-sobolev-regularity}），波前集在椭圆区域满足 $\operatorname{WF}(u)=\operatorname{WF}(Pu)$（式~\eqnrefs{eq:math-dist-wavefront-elliptic-regularity}）。
