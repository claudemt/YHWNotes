...
\begin{align*}
 ...
 \bigl(\partial_tT_t+\mathcal L_{V_t}T_t\bigr),
\end{align*}
即\eqnrefs{eq:math-dg-transport-identity}。$\partial_t$ 比较同一坐标点上的显式时间变化，$\mathcal L_{V_t}$ 补上由流动本身造成的搬运变化——这是真正的几何版材料导数。
\end{derivation}
...
Ricci 流中的特例。Ricci 流 $\partial_tg=-2\operatorname{Ric}$ 给出 $\dot g=-2\operatorname{Ric}$，代入得
\begin{equation*}
\partial_t\Gamma^k{}_{ij}
=-g^{k\ell}\left(
...
\right).
\end{equation*}
利用 收缩 Bianchi 恒等式 $\nabla^\ell\operatorname{Ric}_{\ell j}=\tfrac12\nabla_j\mathcal R$ 与 deTurck 技巧，可把此式改写为
\begin{equation*}
\partial_t\Gamma^k{}_{ij}
=-g^{k\ell}\nabla_\ell\operatorname{Ric}_{ij}+\nabla_i(g^{k\ell}\operatorname{Ric}_{\ell j}),
\end{equation*}
第一项是 Ricci 张量的梯度，第二项可吸收为微分同胚作用。这就是 Ricci--deTurck 流的来源，它抛物化 Ricci 流，是证明短时间存在性的关键。

Hodge 星变分的预备。Hodge 星同时依赖 $g^{-1}$ 与体积形式 $\operatorname{vol}_g$，两者都通过 $\dot g$ 变化。把 $\dot g$ 升指标得自伴 自同态 $\mathcal V$，则
\begin{equation*}
\partial_t\operatorname{vol}_g=\tfrac12\operatorname{tr}\mathcal V\,\operatorname{vol}_g,
\qquad
\partial_tg^{-1}=-\mathcal V\circ g^{-1},
\end{equation*}
这两条与联络变分公式联合给出 Hodge 星的完整变分，是研究 调和形式 在度量演化下行为的起点。
\end{derivation}
...
意义。标量曲率变分公式 $\partial_t\mathcal R=\nabla^i\nabla^j\dot g_{ij}-\Delta_{\mathrm{LB}}(\operatorname{tr}_g\dot g)-\langle\dot g,\operatorname{Ric}\rangle_g$ 是几何流理论的基础。代入 Ricci 流 $\dot g=-2\operatorname{Ric}$ 得 $\partial_t\mathcal R=\Delta\mathcal R+2|\operatorname{Ric}|^2$，由此推出标量曲率沿 Ricci 流 的极小值原理。代入 Einstein 方程的度规微扰 $\dot g=h$ 则给出广义相对论中曲率线性化——引力波方程的出发点。

特例：共形变分。取 $\dot g=2u\,g$（纯共形），则 $\operatorname{tr}_g\dot g=2nu$、$\nabla^i\nabla^j\dot g_{ij}=2\nabla^i\nabla_i u=2\Delta u$，代入得 $\partial_t\mathcal R=2\Delta u-2n\Delta u-2nu\mathcal R=-2(n-1)\Delta u-2nu\mathcal R$。在二维 ($n=2$) 化为 $\partial_t\mathcal R=-2\Delta u-4u\mathcal R$，即 Gauss 曲率在共形变分下的演化，是 Yamabe 问题的核心。
\end{derivation}
...
Einstein 常数的符号分类。Einstein 方程 $\operatorname{Ric}=cg$ 中常数 $c$ 的符号给出三类典型几何：
\begin{itemize}
...
Kähler--Einstein 与 Calabi--Yau 定理。在 Kähler 流形上，Einstein 条件 $\operatorname{Ric}=cg$ 等价于第一 Chern 类 $c_1(M)=c[\omega]$。当 $c_1=0$ 时 Calabi--Yau 定理保证 Ricci 平坦度量存在；当 $c_1>0$ 时存在性等价于 K-稳定性（Yau--Tian--Donaldson 猜想）；当 $c_1<0$ 时 Aubin--Yau 定理给出唯一 Kähler--Einstein 度量。三类情形的统一是几何不变量理论的核心问题。

Ricci 流的静态点。Einstein 度量 $\operatorname{Ric}=cg$ 在 Ricci 流 $\partial_tg=-2\operatorname{Ric}$ 下演化为 $g(t)=(1-2ct)g$，是缩放固定点。因此 Einstein 度量是 Ricci 流的"理想终点"：Hamilton 的 Ricci 流程序把一般度量通过流推向 Einstein 度量，给出 Thurston 几何化猜想的解析证明路径。Perelman 的熵泛函单调性正是这一程序的关键工具。

Einstein 真空方程。广义相对论中 Einstein--Hilbert 作用量
\begin{equation*}
S_{\mathrm{EH}}[g]=\int_M\mathcal R\,\operatorname{vol}_g
\end{equation*}
的变分给出真空 Einstein 方程 $\operatorname{Ric}-\tfrac12\mathcal Rg=0$，取迹得 $\mathcal R=0$，故 $\operatorname{Ric}=0$。固定体积约束的变分问题对应宇宙常数 $\Lambda$ 的 Einstein 方程 $\operatorname{Ric}=\Lambda g$，描述 de Sitter 与 anti-de Sitter 时空。Einstein 度量因此既是 Riemann 几何的极值点，也是广义相对论的基本场方程解。
\end{derivation}
...
意义。Hodge 星变分公式 $\dot{*}\alpha=*(\frac12\operatorname{tr}_g\dot g\,\alpha-\mathcal V^{[k]}\alpha)$ 度量了度规变化对 Hodge 对偶的影响。这在度规流理论中不可或缺：Ricci 流 下 Hodge 星随之演化，调和形式随之变化，影响上同调类的几何实现。实例：共形变分 $\dot g=2u\,g$ 下 $\operatorname{tr}_g\dot g=2nu$、$\mathcal V=0$，$\dot{*}\alpha=nu\,*\alpha$——Hodge 星按体积变化缩放，与共形因子 $e^{nu}$ 一致。
\end{derivation}
...
三类 孤子 的分类。根据 $\lambda$ 的符号，孤子 分为三类：
\begin{itemize}
\item \emph{Shrinker}（$\lambda>0$）：流在 $t\to1/(2\lambda)$ 时收缩到一点，是 Ricci 流奇点的 爆破 模型。典型例子是 Gauss 收缩子 $g_{\mathrm{euc}}$、$X=-\tfrac12\nabla|x|^2$，对应 Gaussian 稳定分布。
\item \emph{Steady}（$\lambda=0$）：流仅由微分同胚驱动，体积不变。典型例子是 cigar 孤子 $g=\frac{\dd x^2+\dd y^2}{1+x^2+y^2}$，对应稳态热核。
\item \emph{Expander}（$\lambda<0$）：流对所有 $t<0$ 存在，从 $t\to-\infty$ 的一点展开。典型例子是 expanding sphere，对应热核扩散。
\end{itemize}

Gradient 孤子 的简化。当 $X=\nabla f$ 为梯度场时，孤子 方程简化为
\begin{equation*}
\operatorname{Ric}+\nabla^2f=\lambda g.
\end{equation*}
取迹得 $\mathcal R+\Delta f=n\lambda$。对\eqnrefs{eq:math-dg-gradient-ricci-soliton}取散度，用 收缩 Bianchi 恒等式 $\nabla^i\operatorname{Ric}_{ij}=\tfrac12\nabla_j\mathcal R$ 与 Hessian 散度公式
\begin{equation*}
\operatorname{div}(\nabla^2f)_j=\nabla_j\Delta f+\operatorname{Ric}_{ij}\nabla^if,
\end{equation*}
得到
\begin{equation*}
\nabla_j\mathcal R+\nabla_j\Delta f+\operatorname{Ric}_{ij}\nabla^if=n\lambda\nabla_jf,
\end{equation*}
结合迹方程给出 $\nabla\mathcal R=2\operatorname{Ric}(\nabla f,\cdot)$，积分得 first 积分
\begin{equation*}
\mathcal R+|\nabla f|^2-2\lambda f=C.
\end{equation*}
这是 Perelman $\mathcal W$ 泛函的临界点方程。

Perelman 熵与 孤子。Perelman $\mathcal W$ 泛函
\begin{equation*}
\mathcal W[g,f,\tau]
=\int_M\left[\tau(\mathcal R+|\nabla f|^2)-f-n\right]\frac{\ee^{-f}}{(4\pi\tau)^{n/2}}\operatorname{vol}_g
\end{equation*}
在 Ricci 流耦合 $\partial_tf=-\Delta f-\mathcal R+\frac n{2\tau}$、$\partial_t\tau=-1$ 下单调不增，临界点恰为 梯度 收缩子。因此 收缩子 是 Ricci 流的"熵极大点"，对应统计力学中的高斯分布。

举例：Gauss 收缩子 与雪茄 孤子。在 $\RR^n$ 上取 $g_{\mathrm{euc}}$、$f=\tfrac{|x|^2}{4\tau}$、$\lambda=\tfrac1{2\tau}$，直接验证
\begin{equation*}
\operatorname{Ric}=0,\qquad
\nabla^2f=\frac{1}{2\tau}g_{\mathrm{euc}},
\end{equation*}
满足 收缩子 方程。这给出 Ricci 流从一点展开的标准 Euclidean 解，对应热方程基本解。二维雪茄 孤子 $g=\frac{\dd x^2+\dd y^2}{1+x^2+y^2}$、$X=-2(x\partial_x+y\partial_y)$、$\lambda=0$ 是 稳态孤子，曲率 $\mathcal R=\frac{4}{1+r^2}$ 在无穷远趋于零，是二维 Ricci 流分类的基本模型。

黑洞几何与 孤子。Bryant 孤子 是三维 Ricci 流中非紧 收缩子 的典型例子，其渐近行为是圆柱形，对应正曲率 Ricci 流的奇点模型。在广义相对论中，梯度 Ricci 孤子 方程与静态 Einstein 方程通过 共形 变换相关，给出某些静态时空的几何描述，例如 Schwarzschild 几何在适当坐标下满足 孤子 型方程。
\end{derivation}
