...
一般 affine connection 与长度、角度没有必然关系。若希望几何中存在“距离的无穷小版本”，必须额外指定度量；若进一步要求平行移动保持这个度量且没有挠率，联络就不再任意。Levi--Civita 定理说明：一个非退化对称度量唯一决定满足这两个条件的联络。本章先证明这一唯一性与存在性，再由同一联络定义测地线以及 Riemann、Ricci 与标量曲率，为后面的 Killing 对称、Hodge 理论和几何演化建立共同母结构。
...
\]
且随 \(p\) 光滑变化。若只要求非退化而允许固定符号差，则称 pseudo-Riemann 度量。局部坐标中
\begin{equation}\label{eq:math-dg-metric-local}
...
度量给出向量与一形式之间的自然同构
\begin{equation}\label{eq:math-dg-musical-isomorphisms}
X^\flat:=g(X,\cdot),
\qquad
...
g(e_a,e_b)=\eta_{ab}
\]
且 \(\eta_{ab}\) 为固定对角矩阵，则称为局部正交归一标架。Riemann 情形 \(\eta=I\)；pseudo-Riemann 情形 \(\eta\) 包含正负号。正交标架并不等于坐标标架：前者是度量条件，后者是 Lie 括号条件。

\section{Levi--Civita 定理与 Koszul 公式}

联络与度量相容指
\begin{equation}\label{eq:math-dg-metric-compatibility}
\nabla g=0,
\end{equation}
也就是
\[
...
给定任意 Riemann 或 pseudo-Riemann 度量 \(g\)，存在唯一 affine connection \(\nabla\) 同时满足
...
反过来，以\eqnrefs{eq:math-dg-koszul-formula}定义 \(\nabla_XY\)。右端对 \(Z\) 是函数线性的，故由非退化性确实唯一对应一个向量场。直接检查 \(X,Y\) 上的线性与 Leibniz 规则可知它是 affine connection。将 \(X,Y\) 互换并相减，右端化为 \(2g([X,Y],Z)\)，故 \(T=0\)；把 Koszul 公式适当置换后相加，则恢复
\[
Xg(Y,Z)=g(\nabla_XY,Z)+g(Y,\nabla_XZ),
\]
故 \(\nabla g=0\)。存在性也得到证明。

\textbf{意义。}Koszul 公式是唯一性定理：给定度量 $g$，Levi--Civita 联络（无挠+度量相容）被 $g$ 唯一决定，无需选择坐标或标架。实例：欧氏度量 $g=\delta_{ij}\dd x^i\dd x^j$ 下，所有 $\partial_k g_{ij}=0$，Koszul 公式给出 $\Gamma^k_{ij}=0$（平直联络）。球面度量 $g=R^2(\dd\theta^2+\sin^2\theta\,\dd\varphi^2)$ 下，代入得 $\Gamma^\theta_{\varphi\varphi}=-\sin\theta\cos\theta$、$\Gamma^\varphi_{\theta\varphi}=\cot\theta$，其余为零——这就是球面测地线方程的联络系数。
\end{derivation}

在坐标基 \(\partial_i\) 中 Lie 括号消失，把
\(\nabla_{\partial_i}\partial_j=\Gamma^k{}_{ij}\partial_k\) 代入 Koszul 公式可得
\begin{equation}\label{eq:math-dg-christoffel-metric}
\boxed{
\Gamma^k{}_{ij}
=\frac12g^{k\ell}
\left(
...
\]
则
\begin{equation}\label{eq:math-dg-levi-civita-noncoordinate}
\begin{aligned}
2\Gamma_{abc}
...
\begin{equation}\label{eq:math-dg-geodesic-equation}
...
\end{equation}
标量曲率再作一次迹：
\begin{equation}\label{eq:math-dg-scalar-curvature}
\mathcal R:=\operatorname{tr}_g\Ric.
\end{equation}
Riemann 曲率保留二维方向之间的完整信息，Ricci 曲率是其一次收缩，标量曲率则只保留总迹。后面 Ricci flow 的母方程会直接使用\eqnrefs{eq:math-dg-ricci-definition}。

接下来考察截面曲率、曲率算子与常曲率模型。

Ricci 与标量曲率都是收缩后的信息；若要保留一张二维切平面本身的弯曲强度，应先定义截面曲率。对点 \(p\in M\) 处由线性无关向量 \(u,v\) 张成的二维平面 \(\sigma\subset T_pM\)，定义
\begin{equation}\label{eq:math-dg-sectional-curvature}
K(\sigma)
...
\begin{equation}\label{eq:math-dg-curvature-operator}
\left\langle \mathcal R_p(u\wedge v),w\wedge z\right\rangle
:=R(u,v,z,w).
\end{equation}
...
\xi=u\wedge v,
\]
恰是曲率算子在 decomposable bivector 上的 Rayleigh quotient。这个观点把 comparison geometry 与前面谱理论的语言直接接起来：截面曲率上下界就是曲率算子在所有简单二向量方向上的二次型界。
...
\]
则
\begin{equation}\label{eq:math-dg-ricci-sectional-sum}
\Ric(v,v)
=\sum_{a=2}^{n}K(v,e_a).
\end{equation}
所以 Ricci 曲率不是某个单独二维平面的曲率，而是所有包含 \(v\) 的正交二维平面的平均型总和；标量曲率则进一步满足
\begin{equation}\label{eq:math-dg-scalar-sectional-sum}
\mathcal R
...
\end{equation}
反之，\eqnrefs{eq:math-dg-space-form-curvature} 给出常截面曲率 \(K\)。Euclidean 空间、半径 \(a\) 的圆球以及常曲率双曲空间分别对应 \(K=0\)、\(K=a^{-2}\) 与 \(K=-a^{-2}\)。它们不是三个互不相干的例子，而是所有 comparison theorem 的标准参照模型。
...
\begin{equation}\label{eq:math-dg-exponential-map}
\exp_p(v):=\gamma_v(1).
\end{equation}
由测地线方程对初值的光滑依赖，\(\exp_p\) 在零向量附近光滑，并且
\[
(\dd\exp_p)_0=\IdMap_{T_pM}.
\]
所以逆函数定理说明 \(\exp_p\) 在零向量附近给出一组 normal coordinates。这里“normal”不是额外坐标假设，而是由所有从 \(p\) 发出的测地线同时构造出的坐标。
...
\emph{设置。}$F(s,r)$ 是测地线变分：对每个固定 $r$，$s\mapsto F(s,r)$ 都是测地线。沿中心测地线 $\gamma(s)=F(s,0)$ 定义切场 $T=\partial_sF|_{r=0}$ 和变分场 $J=\partial_rF|_{r=0}$。因混合偏导交换，$[T,J]=0$。

\emph{第一步：测地线方程。}由于每条 $r=\mathrm{const.}$ 曲线都是测地线，
\begin{equation*}
 \nabla_TT=0.
\end{equation*}
这是测地线的内蕴定义，等价于 $\dd^2 x^k/\dd s^2+\Gamma^k{}_{ij}\dot x^i\dot x^j=0$。

\emph{第二步：无挠性交换联络。}Levi--Civita 联络无挠，故 $\nabla_TJ-\nabla_JT=[T,J]=0$，即
\begin{equation*}
 \nabla_TJ=\nabla_JT.
\end{equation*}
于是
...
\emph{第四步：整理符号。}由曲率算子的反对称性 $R(T,J)T=-R(J,T)T$，
\begin{equation*}
 \nabla_T\nabla_TJ=-R(J,T)T,
\end{equation*}
移项即得\eqnrefs{eq:math-dg-jacobi-equation}。

\emph{第五步：典型例子——球面与双曲面。}
\begin{itemize}
\item 单位球面 $S^n$（截面曲率 $+1$）：沿大圆测地线取单位正交初始 Jacobi 场 $J(0)=0$、$\nabla_TJ(0)=e$，方程 $\nabla_T^2 J+J=0$，解
\begin{equation*}
 J(s)=\sin(s)\,e(s),
\end{equation*}
$s=\pi$ 处 $J(\pi)=0$，即对径点为共轭点。所有大圆在 antipode 处汇聚。
...
接下来考察Gauss 引理、normal coordinates 与局部度量展开。
...
\emph{思路：}取从 $p$ 出发的二维测地线变分，沿径向参数 $t$ 求内积 $g(T,J)$ 的导数。利用测地线方程 $\nabla_TT=0$、无挠性 $\nabla_TJ=\nabla_JT$ 与速度平方沿测地线守恒，把导数化为 $\tfrac12 J[g(T,T)]$，再对 $s$ 求导后积分得 Gauss 引理。
...
\emph{第二步：交换联络。}由于 \([T,J]=0\)（混合偏导交换）且 Levi--Civita 联络无挠，\(\nabla_TJ=\nabla_JT\)。

\emph{第三步：求内积导数。}沿 $t$ 方向求
\begin{equation*}
\frac{\dd}{\dd t}g(T,J)
=g(\nabla_TT,J)+g(T,\nabla_TJ).
\end{equation*}
...
\emph{第四步：用速度守恒。}每条径向测地线速度平方恒定，并且初速度为 \(v+s w\)，故
\begin{equation*}
J[g(T,T)]
...
\emph{第五步：积分得 Gauss 引理。}在 \(s=0\) 处积分 \(t\in[0,1]\)：
\begin{equation*}
g(T(1),J(1))-g(T(0),J(0))
...
\emph{更一般结论。}Gauss 引理是 Rauch 比较定理的局部基石：它保证指数映射在径向方向不改变长度，因此比较定理只需控制角向方向的伸缩——这正是 Jacobi 场比较的内容。在广义相对论中，Gauss 引理保证 normal coordinates 中径向光锥的几何性质，是 causal structure 分析的基础工具。
\end{derivation}

在 \(p\) 的 normal coordinates \((x^1,\ldots,x^n)\) 中，坐标线 \(t\mapsto tx\) 是从 \(p\) 发出的测地线。由测地线方程在 \(t=0\) 对任意初速度成立，得到
\begin{equation}\label{eq:math-dg-normal-coordinate-first-order}
 g_{ij}(p)=\delta_{ij},
\qquad
\Gamma^k{}_{ij}(p)=0,
\qquad
\partial_k g_{ij}(p)=0.
\end{equation}
因此 normal coordinates 消去的是度量的一阶变化，而不是曲率。真正的几何信息从二阶 Taylor 系数开始出现。与本书曲率约定相容的展开为
\begin{equation}\label{eq:math-dg-normal-metric-expansion}
 g_{ij}(x)
 =\delta_{ij}
...
\begin{equation}\label{eq:math-dg-normal-volume-expansion}
 \sqrt{\det g(x)}
 =1-\frac16\Ric_{k\ell}(p)x^k x^\ell
 +O(|x|^3).
\end{equation}
...
\begin{equation}\label{eq:math-dg-small-ball-volume}
\operatorname{Vol}_g(B_r(p))
=\omega_n r^n
\left[
...
接下来考察cut locus、injectivity radius 与距离函数的光滑区域。

normal coordinates 只能在指数映射保持一一且微分可逆的区域内工作。固定 \(p\)，对单位向量 \(v\in T_pM\) 定义 cut time
\begin{equation}\label{eq:math-dg-cut-time}
 c(v):=\sup\left\{t>0:
 d\bigl(p,\exp_p(sv)\bigr)=s
 \text{ for every }0\le s\le t
 \right\}.
\end{equation}
若 \(c(v)<\infty\)，则 \(\exp_p(c(v)v)\) 称为沿 \(v\) 的 cut point；所有这类点组成 \(\operatorname{Cut}(p)\)。injectivity radius 定义为
\begin{equation}\label{eq:math-dg-injectivity-radius}
 \operatorname{inj}(p):=\sup\{r>0:
...
\]
光滑，并且
\begin{equation}\label{eq:math-dg-distance-gradient}
 |\nabla r_p|=1,
\qquad
\nabla r_p(q)=\dot\gamma_{pq}(r_p(q)),
\end{equation}
其中 \(\gamma_{pq}\) 是唯一的单位速最短测地线。第一式是 Riemannian eikonal equation。它说明距离函数虽是全局 Lipschitz 对象，但在 cut locus 之外服从一阶 Hamilton--Jacobi 方程；奇性恰在最短测地线开始多值或指数映射退化的地方产生。

\begin{theorem}[最短性与 cut/conjugate 障碍]\label{thm:math-dg-minimizing-cut-conjugate}
...
这一区分非常重要：Jacobi 场和 index form 检测的是指数映射的微分退化，是“局部二阶”障碍；cut locus 还记录不同测地线之间的全局竞争。圆球上南极同时是北极的共轭点与 cut point；平坦圆柱上某些 cut point 却可由拓扑导致，而局部曲率仍为零。

接下来考察二阶变分、Jacobi 零模与比较几何。


测地线还具有一个等价的变分刻画。设 $\gamma:[0,L]\to M$ 为分段光滑曲线，能量泛函定义为
\begin{equation}
E[\gamma]
:=\frac12\int_0^L g(\dot\gamma,\dot\gamma)\,\dd s.
\label{eq:math-dg-geodesic-energy}
\end{equation}
对固定端点变分 $F(s,r)$，记变分场
\[
...
\begin{derivation}[从能量变分到 index form]
对二维变分 $F(s,r)$ 记
\[
T=\partial_sF,
\qquad
...
Jacobi 场正是 index form 的零模。若 $J(0)=J(L)=0$ 并满足 Jacobi 方程，则对任意固定端点变分场 $W$，分部积分得到
\[
I(J,W)
=-\int_0^L
 g\bigl(\nabla_T\nabla_TJ+R(J,T)T,W\bigr)\,\dd s=0.
\]
因此共轭点出现时，能量 Hessian 退化。更进一步，Morse index theorem 说明一条测地线段的 index form 负方向数等于区间内共轭点的重数总和。于是“共轭点前局部极小、越过共轭点后出现下降方向”不是几何图像上的猜测，而是二阶变分的谱性质。这里说的是固定端点的局部极小性；全局最短性还会受到 cut locus 和不同测地线竞争的影响。
...
\end{equation}
因此正曲率使径向测地线比 Euclidean 情形更快聚焦，负曲率则使其更快分离。$S_K$ 并不只是三个特例的记忆公式；它是 comparison geometry 中所有 radial estimate 的母函数。
...
\ge (n-1)k\,g(v,v),
\qquad k>0.
\label{eq:math-dg-ricci-lower-bound}
\end{equation}
Bonnet--Myers 定理给出
\begin{equation}
\boxed{
...
\begin{derivation}[Bonnet--Myers 如何从 index form 推出直径上界]
反设存在两点距离
\[
L>\frac{\pi}{\sqrt k}.
\]
...
\]
由于 $\gamma$ 是最短测地线，在没有越过 cut point 的区间上 index form 必须非负：
\[
I(V_i,V_i)\ge0.
\]
直接计算
...
\textbf{实例。}半径 $R$ 的球面 $\mathbb S^n(R)$：截面曲率 $K=1/R^2$，Ricci 曲率 $\operatorname{Ric}=(n-1)/R^2$，Bonnet--Myers 给出直径 $\le\pi R$，恰好等于球面直径——球面是等号情形。平坦环面 $T^n$：$\operatorname{Ric}=0$，定理不适用（$k=0$），直径确实无上界。双曲空间 $\mathbb H^n$：$\operatorname{Ric}=-(n-1)<0$，定理也不适用，测地线无限延伸。正曲率流形有限直径是整体微分几何的核心结论：它把局部曲率信息直接转化为全局紧致性，是 Cheeger--Gromoll 理论的起点。
\end{derivation}

这一证明揭示了 comparison geometry 的主线：曲率下界本身并不直接``看见''全局直径；它先进入二阶变分的 index form，再通过所有横向变分方向求和变成 Ricci 曲率，最后与最短性矛盾。局部张量信息由此转化为全局拓扑与度量结论。
...
\begin{equation}
\Delta_{\mathrm{LB}}f:=\operatorname{tr}_g(\nabla^2f)=\nabla^i\nabla_i f
=-\Delta_H f.
\label{eq:math-dg-lb-vs-hodge-laplacian}
\end{equation}
沿一条径向单位速测地线 $\gamma$，在法向空间 $T^\perp\gamma$ 上定义形算子
\begin{equation}
A_r(X):=\nabla_X\partial_r.
\label{eq:math-dg-radial-shape-operator}
\end{equation}
...
\qquad
R_{\partial_r}(X):=R(X,\partial_r)\partial_r.}
\label{eq:math-dg-radial-riccati}
\end{equation}
...
便得到
\begin{equation}
H'(r)+|A_r|^2+\Ric(\partial_r,\partial_r)=0.
\label{eq:math-dg-radial-riccati-trace}
\end{equation}
若 $\Ric\ge(n-1)k g$，由 Cauchy--Schwarz
\[
|A_r|^2\ge\frac{H(r)^2}{n-1},
\]
于是
\begin{equation}
H'(r)+\frac{H(r)^2}{n-1}+(n-1)k\le0.
\label{eq:math-dg-radial-riccati-inequality}
\end{equation}
...
\]
标量 Riccati 比较因此给出 Laplacian comparison：
\begin{equation}
\boxed{
...
\end{equation}
在 cut locus 之外逐点成立；在更一般的全局表述中可理解为分布或 barrier 意义的不等式。$k=0$ 时它退化为
\[
...
从逻辑上看，Rauch、Bonnet--Myers、Laplacian comparison 与 Bishop--Gromov 并不是四个彼此无关的定理：它们都从同一个 Jacobi 方程出发。Rauch 比较 Jacobi 场的长度；Bonnet--Myers 把 Jacobi 信息送入 index form；Laplacian comparison 取 Riccati 方程的迹；Bishop--Gromov 再把迹积分为指数映射的 Jacobian。比较几何的“局部曲率 $\Rightarrow$ 全局度量结论”由此形成一条连续的推导链。
