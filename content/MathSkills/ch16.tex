...
物理学的核心信条之一是：对称性决定守恒律。一个系统如果在空间平移下不变，就拥有动量守恒；如果在时间平移下不变，就拥有能量守恒；如果在旋转下不变，就拥有角动量守恒。Noether 定理把这一直觉提升为精确的数学命题：每一个连续对称性对应一个守恒流。在非相对论力学中，这只涉及 Galilei 群的三维平移和三维旋转。一旦进入相对论，时空统一为四维 Minkowski 流形，对称群变为 Poincaré 群 \(ISO(1,d-1)\)，它同时包含 Lorentz 变换（旋转和 boost）与四维平移。Poincaré 群的结构比 Galilei 群更丰富，也比纯旋转群更复杂——它是一个半直积，平移子群与 Lorentz 子群之间有非平凡的交织。
...
\begin{equation}\label{eq:ms-poincare-metric}
\eta_{\mu\nu}=\operatorname{diag}(-,+,\ldots,+),\qquad \mu,\nu=0,\ldots,d-1.
\end{equation}
这里选"mostly plus"符号约定，即时间分量负、空间分量正。这个选取在广义相对论和粒子物理中都很常见；它的好处是空间度规正定，三维 Euclidean 几何的公式可以直接继承。升降指标均由 \(\eta_{\mu\nu}\) 完成，例如 \(V^\mu=\eta^{\mu\nu}V_\nu\)，\(V_0=-V^0\)，\(V_i=V^i\)。全文使用自然单位 \(\hbar=c=1\)。
...
\end{equation}
其中 \(P^\mu\) 是平移生成元（物理上是四动量算符），\(M^{\mu\nu}=-M^{\nu\mu}\) 是 Lorentz 生成元（物理上是角动量算符，包含空间旋转和 boost）。三个细节值得解释。第一，因子 \(i\) 使 \(P^\mu\) 和 \(M^{\mu\nu}\) 成为 Hermite 算符，从而对应物理可观测量。第二，\(\omega_{\mu\nu}\) 的反对称性 \(\omega_{\mu\nu}=-\omega_{\nu\mu}\) 保证只有 \(d(d-1)/2\) 个独立 Lorentz 参数（\(d=4\) 时是 6 个：3 个旋转加 3 个 boost），而求和中的 \(1/2\) 补偿了每对 \((\mu,\nu)\) 被计算两次。第三，平移参数 \(\epsilon^\mu\) 是一个 \(d\)-矢量，没有反对称约束。

\section{Poincaré 群及其 Lie 代数}

接下来考察Minkowski 时空的等距变换。

物理上，Poincaré 变换是保持 Minkowski 间隔 \(ds^2=\eta_{\mu\nu}dx^\mu dx^\nu\) 不变的最一般仿射变换。它由一个 Lorentz 矩阵 \(\Lambda\) 和一个平移矢量 \(a\) 组成：
\begin{equation}\label{eq:ms-poincare-transform}
x'^\mu=\Lambda^\mu{}_\nu x^\nu+a^\mu.
\end{equation}
Lorentz 矩阵必须满足
...
\end{equation}
即 \(\eta_{\rho\sigma}\Lambda^\rho{}_\mu\Lambda^\sigma{}_\nu=\eta_{\mu\nu}\)。这个条件的几何意义是：\(\Lambda\) 保持 Minkowski 内积不变。物理上，它保证光锥结构不变——如果两个事件由光信号连接（\(ds^2=0\)），变换后仍然如此。
...
逆元公式在推导 Lie bracket 时会用到。令 \((\Lambda,a)^{-1}=(\Lambda',a')\)，要求 \((\Lambda',a')(\Lambda,a)=(I,0)\)。由群乘法~\eqnrefs{eq:ms-poincare-semidirect} 得 \((\Lambda'\Lambda,\;\Lambda'a+a')=(I,0)\)。第一分量给出 \(\Lambda'=\Lambda^{-1}\)；代入第二分量给出 \(a'=-\Lambda^{-1}a\)。因此
...
有了群结构，下一步问：为什么要把群变成 Lie 代数？原因是群乘法是非线性的，而 Lie 代数的交换关系是线性的结构常数，后者远比前者容易操作。无穷小生成元的交换关系 \([P,P]\)、\([M,P]\)、\([M,M]\) 完全编码了群的局域结构，而且可以用来构造 Casimir 不变量、分类不可约表示、验证场方程的协变性。本节不直接列出这三组公式，而是从群乘法和 Lie bracket 的定义出发，用一个统一的推导一次性得到全部三组关系。
...
\end{equation}
即
\begin{equation}\label{eq:ms-poincare-omega-antisym}
\boxed{\omega_{\mu\nu}=-\omega_{\nu\mu}.}
\end{equation}
\(d\times d\) 反对称矩阵有 \(d(d-1)/2\) 个独立分量，这就是 Lorentz 群的维数。在 \(d=4\) 时，6 个生成元分为 3 个旋转 \(J_i=\frac{1}{2}\epsilon_{ijk}M^{jk}\) 和 3 个 boost \(K_i=M^{0i}\)。

接下来考察半直积 Lie bracket。

现在进入核心推导。Poincaré Lie 代数的元素写成 \(X=(\omega,a)\)，其中 \(\omega\in\mathfrak{so}(1,d-1)\) 是反对称矩阵，\(a\in\mathbb{R}^{1,d-1}\) 是平移矢量。Lie bracket 的一般定义是通过群交换子：
\begin{equation}\label{eq:ms-poincare-bracket-def}
g_1(t)g_2(s)g_1(t)^{-1}g_2(s)^{-1}=I+ts\,[X_1,X_2]+O(t^2,s^2,t^2s,ts^2),
\end{equation}
其中 \(g_1(t)=(e^{t\omega_1},ta_1)\) 和 \(g_2(s)=(e^{s\omega_2},sa_2)\) 是两条一参数子群。群交换子的几何意义是：先沿 \(g_1\) 走一小段 \(t\)，再沿 \(g_2\) 走一小段 \(s\)，然后沿 \(g_1\) 退回，再沿 \(g_2\) 退回。如果群是 Abel 的，这四步完全抵消；非抵消的部分到 \(ts\) 阶正是 Lie bracket，它测量两个生成元"不交换"的程度。

\begin{derivation}[从群乘法导出半直积 Lie bracket]
\emph{思路：}逐次使用群乘法~\eqnrefs{eq:ms-poincare-semidirect} 与逆元公式~\eqnrefs{eq:ms-poincare-inverse}，把四元群乘积~\eqnrefs{eq:ms-poincare-bracket-def} 展开到 \(ts\) 阶。关键是跟踪每一步中 Lorentz 部分如何作用于平移部分。

\emph{第一步：计算 \(g_1(t)g_2(s)\)。}用群乘法，Lorentz 部分相乘、平移部分被第一个 Lorentz 作用后再加：
\begin{equation*}
...
\emph{第二步：求 \(g_1(t)^{-1}\)。}由逆元公式~\eqnrefs{eq:ms-poincare-inverse}，
\begin{equation*}
g_1(t)^{-1}=\big(e^{-t\omega_1},\;-e^{-t\omega_1}(ta_1)\big)+O(t^2).
\end{equation*}

\emph{第三步：计算 \(g_1(t)g_2(s)\cdot g_1(t)^{-1}\)。}把第一步和第二步的结果再用群乘法合并。Lorentz 部分：
\begin{equation*}
...
\emph{第四步：展开到 \(ts\) 阶。}Lorentz 部分：利用 Baker--Campbell--Hausdorff 或直接展开矩阵指数，
\begin{equation*}
e^{t\omega_1}e^{s\omega_2}e^{-t\omega_1}=(I+t\omega_1)(I+s\omega_2)(I-t\omega_1)+O(t^2,s^2)=I+s\omega_2+O(s^2).
\end{equation*}
\(t\) 的一阶项被共轭消去了——这是预期的，因为共轭 \(\Lambda\omega_2\Lambda^{-1}\) 到 \(t=0\) 就是 \(\omega_2\) 本身。\(ts\) 阶的修正要到第四步右乘 \(e^{-s\omega_2}\) 后才出现。
...
\emph{第五步：右乘 \(g_2(s)^{-1}\)。}由逆元公式，\(g_2(s)^{-1}=(e^{-s\omega_2},-e^{-s\omega_2}sa_2)\)。将第四步结果记为 \((I+s\omega_2,\;ts(\omega_1 a_2-\omega_2 a_1))\)，再乘以 \(g_2(s)^{-1}\)。Lorentz 部分：
\begin{equation*}
...
\emph{第六步：读出 Lie bracket。}群交换子到 \(ts\) 阶为
\begin{equation*}
...
这条统一的 bracket 公式~\eqnrefs{eq:ms-poincare-unified-bracket} 是整个 Poincaré 代数的压缩形式。它的两个分量分别给出：Lorentz 部分的矩阵交换子 \([\omega_1,\omega_2]\)（纯 Lorentz 子代数），以及平移部分的 \(\omega_1 a_2-\omega_2 a_1\)（Lorentz 对平移的交叉作用）。下面三小节只需在~\eqnrefs{eq:ms-poincare-unified-bracket} 中代入不同的特例，即可一次性得到全部三组交换关系。

接下来考察由统一 bracket 推出 \([P,P]=0\)。

纯平移对应 \(\omega_1=\omega_2=0\)。代入~\eqnrefs{eq:ms-poincare-unified-bracket}：
\begin{equation*}
[(0,a_1),(0,a_2)]=\big([0,0],\;0\cdot a_2-0\cdot a_1\big)=(0,0).
\end{equation*}
两个纯平移的 bracket 为零，因为 Lorentz 部分恒为零、交叉项中 \(\omega=0\) 也为零。翻译成生成元语言：
\begin{equation}\label{eq:ms-poincare-PP}
\boxed{[P_\mu,P_\nu]=0.}
\end{equation}
这反映平移子群 \(\mathbb{R}^{1,d-1}\) 是 Abel 群——先沿 \(x^\mu\) 方向平移再沿 \(x^\nu\) 方向平移，与顺序无关。

接下来考察由统一 bracket 推出 \([M,P]\)。
...
\end{equation*}
结果是一个纯平移元素，平移矢量被 \(\omega\) 作用。物理上，这说 Lorentz 变换会把一个平移矢量旋转或 boost 到另一个方向。在生成元语言中，Lorentz 生成元对平移生成元的伴随作用正是矢量表示。为了写出显式公式，需要把矩阵 \(\omega\) 用反对称参数 \(\omega_{\rho\sigma}\) 和矢量表示的 Lorentz 生成元 \(J_V^{\rho\sigma}\) 展开。矢量表示的定义是：对任意矢量 \(a\)，\((\omega a)^\mu=\omega^\mu{}_\nu a^\nu\)。由 \(\omega^\mu{}_\nu=\eta^{\mu\alpha}\omega_{\alpha\nu}\) 和 \(\omega_{\alpha\nu}\) 的反对称性，
\begin{equation*}
...
\emph{第二步：展开四项并对 \(\gamma\) 求和。}利用 \(\delta^\nu{}_\gamma\eta^{\rho\gamma}=\eta^{\rho\nu}\) 等：
\begin{equation*}
...
\emph{第四步：相减并整理。}把含 \(\delta^\sigma{}_\beta\) 的项合并：
\begin{equation*}
(-\eta^{\mu\alpha}\eta^{\rho\nu}+\eta^{\nu\alpha}\eta^{\rho\mu})\delta^\sigma{}_\beta.
\end{equation*}
...
接下来考察Poincaré Lie 代数总结。

式~\eqnrefs{eq:ms-poincare-PP}、\eqnrefs{eq:ms-poincare-MP}、\eqnrefs{eq:ms-poincare-MM} 合起来构成 Poincaré Lie 代数。三条关系全部由统一的半直积 Lie bracket~\eqnrefs{eq:ms-poincare-unified-bracket} 一次性导出，这正是统一推导的优势：不需要分别猜三组公式的右端，一个群结构给出全部。

物理上，这三条关系有清晰的含义。\([P,P]=0\) 说不同方向的平移互不干扰。\([M,P]\neq 0\) 说 Lorentz 变换会把平移方向旋转到新方向——这就是为什么能量（\(P^0\)）和动量（\(P^i\)）在 boost 下会混合。\([M,M]\neq 0\) 说两个旋转的合成一般不是简单相加——例如绕 \(x\) 轴旋转再绕 \(y\) 轴旋转，与反过来做，结果差一个绕 \(z\) 轴的旋转。
...
\emph{第二步：反对称投影。}关键观察：\(\omega_{\alpha\nu}\) 反对称，而 \(x^\nu\partial^\alpha\) 可以分解为对称和反对称两部分：
\begin{equation*}
...
\emph{第三步：提出因子并定义轨道角动量。}把哑指标 \((\alpha,\nu)\) 换回 \((\mu,\nu)\)：
\begin{equation*}
...
\begin{equation}\label{eq:ms-poincare-M-decomp}
\boxed{M^{\mu\nu}_{\text{field}}=L^{\mu\nu}+\Sigma^{\mu\nu}.}
\end{equation}
...
\begin{equation}\label{eq:ms-poincare-Sigma-vector}
\boxed{
...
\begin{equation}\label{eq:ms-poincare-Sigma-tensor}
\boxed{
\begin{aligned}
...
\begin{equation}\label{eq:ms-poincare-clifford}
...
\emph{思路：}把 \(D(\Lambda)=I-\frac{i}{2}\omega_{\mu\nu}\Sigma^{\mu\nu}\) 代入协变条件~\eqnrefs{eq:ms-poincare-clifford-cov}，比较 \(\omega\) 的一阶项，解出 \(\Sigma^{\mu\nu}\)。

\emph{第一步：展开左边。}把 \(D\) 和 \(D^{-1}\) 代入~\eqnrefs{eq:ms-poincare-clifford-cov} 左边：
\begin{equation*}
...
\emph{第二步：展开右边。}\(\Lambda^\rho{}_\sigma=\delta^\rho{}_\sigma+\omega^\rho{}_\sigma\)，故
\begin{equation*}
...
\emph{第三步：比较 \(\omega_{\mu\nu}\) 系数。}左右两边相等给出
\begin{equation}\label{eq:ms-poincare-sigma-gamma-comm}
-\frac{i}{2}[\Sigma^{\mu\nu},\gamma^\rho]=\eta^{\mu\rho}\gamma^\nu-\eta^{\nu\rho}\gamma^\mu.
\end{equation}

\emph{第四步：验证 \(\Sigma^{\mu\nu}=\frac{i}{4}[\gamma^\mu,\gamma^\nu]\) 满足此式。}先算一个辅助恒等式。利用 Clifford 代数 \(\{\gamma^\mu,\gamma^\nu\}=2\eta^{\mu\nu}\)：
\begin{align*}
[\gamma^\mu\gamma^\nu,\gamma^\rho]
...
\begin{equation}\label{eq:ms-poincare-M-dirac}
\boxed{
...
这张表是后续自旋分类的基础。注意 Dirac 旋量是可约表示——它由左手和右手 Weyl 旋量直和而成。质量为零时这两个分量解耦，各自独立描述不同螺旋度的粒子；质量不为零时质量项把它们耦合，这就是下一章要详细讨论的 chirality-helicity 关系。
...
\begin{equation}\label{eq:ms-poincare-C1}
\boxed{C_1=P^2=P_\mu P^\mu,}
\end{equation}
对质量为 \(m\) 的粒子给出 \(P^2=-m^2\)（本文"mostly plus"度规下）。第二 Casimir 由 Pauli--Lubanski 向量
\begin{equation}\label{eq:ms-poincare-W-def}
W^\mu=\frac{1}{2}\epsilon^{\mu\nu\rho\sigma}P_\nu M_{\rho\sigma}
\end{equation}
构造：
\begin{equation}\label{eq:ms-poincare-C2}
\boxed{C_2=W^2=W_\mu W^\mu.}
\end{equation}
对 massive spin-\(s\) 粒子，\(W^2=m^2 s(s+1)\)。无质量粒子则由 little group 分类——massive 的 little group 是 \(SO(3)\)，massless 的是 \(ISO(2)\)，两者的表示结构截然不同。这些细节留到下一章系统讨论。

\section{最小作用量原理与 Euler--Lagrange 方程}

有了场的变换律，下一步问：什么样的场是物理允许的？经典场论的回答是作用量驻定。考虑一阶局域场论
\begin{equation}
S[\Phi]=\int d^dx\,\mathcal{L}(\Phi^A,\partial_\mu\Phi^A),
\end{equation}
其中拉氏密度 \(\mathcal{L}\) 依赖于场和它的一阶导数。物理场是使 \(\delta S=0\)（对边界消失的任意变分）的场。

\begin{derivation}[Euler--Lagrange 方程的推导]
\emph{思路：}对作用量变分，分部积分把 \(\delta\Phi\) 的导数移走，使变分只以 \(\delta\Phi^A\)（而非 \(\partial_\mu\delta\Phi^A\)）的形式出现。然后利用 \(\delta\Phi^A\) 的任意性读出方程。

\emph{第一步：写出变分。}场的任意变分 \(\Phi^A\to\Phi^A+\delta\Phi^A\) 引起
\begin{equation*}
\delta S=\int d^dx\left[\frac{\partial\mathcal{L}}{\partial\Phi^A}\delta\Phi^A+\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\partial_\mu\delta\Phi^A\right].
\end{equation*}
...
\emph{第三步：分部积分。}第二项中 \(\partial_\mu\delta\Phi^A\) 是变分的导数。用 Leibniz 法则把它移到 \(\Pi_A^\mu\) 上：
\begin{equation*}
...
\emph{第四步：代入并丢掉边界项。}
\begin{equation*}
\delta S=\int d^dx\left[\left(\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\Pi_A^\mu\right)\delta\Phi^A+\partial_\mu(\Pi_A^\mu\delta\Phi^A)\right].
\end{equation*}
若变分 \(\delta\Phi^A\) 在边界上消失（或场在无穷远衰减足够快），散度项不贡献。

\emph{第五步：利用 \(\delta\Phi^A\) 任意性。}剩余积分中 \(\delta\Phi^A\) 任意，要使 \(\delta S=0\) 对所有 \(\delta\Phi^A\) 成立，系数必须为零：
...
\emph{高阶导数 Lagrangian.}\enspace 若 $\mathcal{L}=\mathcal{L}(\Phi^A,\partial_\mu\Phi^A,\partial_\mu\partial_\nu\Phi^A,\dots)$ 含高阶导数，Euler--Lagrange 方程推广为 $\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}+\partial_\mu\partial_\nu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\partial_\nu\Phi^A)}-\cdots=0$（Ostrogradsky 方程）。对二阶导数 Lagrangian，Ostrogradsky 框架给出 Hamiltonian 形式，但一般有 Ostrogradsky 不稳定性（Hamiltonian 无下界），除非 Lagrangian 是全导数或退化的。有效场论中高阶导数项（如 $\Box\Phi$ 修正）作为微扰处理，不引入 Ostrogradsky 不稳定，是有效理论的典型处理方式。

\emph{Noether 定理的联系.}\enspace Euler--Lagrange 方程与 Noether 定理同源：作用量在连续对称变换下的不变性 $\delta S=0$ 结合 Euler--Lagrange 方程给出守恒流 $\partial_\mu j^\mu=0$。平移对称性给出能动量守恒 $\partial_\mu T^{\mu\nu}=0$，Lorentz 对称性给出角动量守恒 $\partial_\lambda J^{\lambda\mu\nu}=0$，内禀对称性（如 $U(1)$）给出荷守恒 $\partial_\mu j^\mu=0$。Noether 定理把对称性（几何性质）转化为守恒律（动力学约束），是 Euler--Lagrange 方程的对称性推论。

\emph{约束系统与 Dirac--Bergmann.}\enspace 若 Lagrangian 是退化的（Hessian $\frac{\partial^2\mathcal{L}}{\partial\dot\Phi^A\partial\dot\Phi^B}$ 不满秩），Euler--Lagrange 方程含约束。Dirac--Bergmann 算法系统处理约束：(1) 初级约束 $\phi_m(\Phi,\pi)=0$（来自 Legendre 变换退化）；(2) 自洽条件 $\dot\phi_m=\{\phi_m,H\}\approx0$ 给出次级约束；(3) 反复迭代直到约束代数封闭。约束分为第一类（与所有约束 Poisson 对易，生成规范变换）和第二类（需用 Dirac 括号修正）。规范理论（如 Maxwell、Yang--Mills）的规范对称性来自第一类约束，是 Euler--Lagrange 方程在退化系统中的推广。

\emph{作用量变分的边界项.}\enspace 变分 $\delta S=\int\left[\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\right]\delta\Phi^A\,d^4x+\int\partial_\mu\left(\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\delta\Phi^A\right)d^4x$ 的边界项 $\theta^\mu=\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\delta\Phi^A$ 是 Noether 流的变分形式。在无穷远边界（$\delta\Phi|_{\partial\Omega}=0$）边界项消失，但在有限区域或渐近对称下，边界项给出荷的定义 $Q=\int_\Sigma\theta^0\,d^3x$。在引力理论中，边界项的处理更微妙（Gibbons--Hawking 边界项），是作用量变分良定义的关键。
\end{derivation}
...
\end{equation}
其中 \(E_A=\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\Pi_A^\mu\) 是 EL 表达式（\(E_A=0\) 即运动方程），\(\Theta^\mu=\Pi_A^\mu\delta\Phi^A\) 是 Symplectic potential。这个分解的物理意义：变分的效果分为两部分——一部分正比于运动方程（on-shell 为零），另一部分是全散度（边界项）。

现在假设某种连续变换使拉氏量的变分是一个全散度：
\begin{equation}
\delta\mathcal{L}=\partial_\mu K^\mu.
\end{equation}
这种变换称为"准对称"（quasi-symmetry）。例如平移变换使 \(\delta\mathcal{L}=\epsilon^\nu\partial_\nu\mathcal{L}=\partial_\mu(\epsilon^\mu\mathcal{L})\)，故 \(K^\mu=\epsilon^\mu\mathcal{L}\)。把~\eqnrefs{eq:ms-poincare-delta-L} 与准对称条件结合：
\begin{equation*}
...
\end{equation}
则 off-shell 恒等式为
\begin{equation}\label{eq:ms-poincare-noether-offshell}
\boxed{\partial_\mu J^\mu=-E_A\,\delta\Phi^A.}
\end{equation}
在运动方程成立时（on-shell，\(E_A=0\)），右端为零：
\begin{equation}\label{eq:ms-poincare-noether}
\boxed{\partial_\mu J^\mu=0\quad\text{(on-shell)}.}
\end{equation}
这就是 Noether 第一定理：每一个连续准对称性给出一个 on-shell 守恒流。
...
\emph{第三步：定义 canonical 能动量张量。}因为 \(\epsilon^\nu\) 任意，方括号中的量对每个 \(\nu\) 独立守恒。定义 canonical 能动量张量
...
\emph{第四步：守恒律。}由 Noether 定理~\eqnrefs{eq:ms-poincare-noether}，\(\partial_\mu J^\mu=0\) on-shell。代入 \(J^\mu=\epsilon^\nu T^\mu{}_\nu\)：
\begin{equation*}
...
\emph{Belinfante 对称化.}\enspace 一般 $T^{\mu\nu}$ 不对称（$T^{\mu\nu}\ne T^{\nu\mu}$），但可加全导数项（Belinfante 修正）$\Theta^{\mu\nu}=T^{\mu\nu}+\partial_\lambda S^{\lambda\mu\nu}$（$S^{\lambda\mu\nu}=-S^{\mu\lambda\nu}$）使 $\Theta^{\mu\nu}=\Theta^{\nu\mu}$ 且 $\partial_\mu\Theta^{\mu\nu}=0$。对称能动量张量 $\Theta^{\mu\nu}$ 是引力源（Einstein 方程 $G_{\mu\nu}=8\pi G\,\Theta_{\mu\nu}$），且给出角动量流 $J^{\lambda\mu\nu}=x^\mu\Theta^{\lambda\nu}-x^\nu\Theta^{\lambda\mu}$（无需自旋修正项）。Belinfante 修正把自旋流吸收进能动量张量，是 Einstein--Cartan 理论与 Einstein 理论等价的桥梁。

\emph{度量变分定义.}\enspace 能动量张量可从作用量对度量的变分定义：$T_{\mu\nu}=\frac{-2}{\sqrt{-g}}\frac{\delta S}{\delta g^{\mu\nu}}$。这一定义自动给出对称的 $T_{\mu\nu}$，且在弯曲时空推广为 $\nabla_\mu T^{\mu\nu}=0$（协变守恒）。度量变分定义与 Noether 定义在平直时空一致（差 Belinfante 修正），但在弯曲时空只有度量变分定义给出正确的引力源。对标量场 $\mathcal{L}=\frac12 g^{\mu\nu}\partial_\mu\phi\partial_\nu\phi-\frac12m^2\phi^2$，$T_{\mu\nu}=\partial_\mu\phi\partial_\nu\phi-\frac12 g_{\mu\nu}(\partial\phi)^2+g_{\mu\nu}\frac12 m^2\phi^2$，直接验证 $\nabla_\mu T^{\mu\nu}=0$（用 Klein--Gordon 方程）。
\end{derivation}
...
\begin{equation*}
\frac{d}{dt}\int d^{d-1}x\,T^{0\nu}=-\int d^{d-1}x\,\partial_i T^{i\nu}.
\end{equation*}
若场在空间无穷远衰减足够快，右面积分为零（Gauss 定理），故
\begin{equation}\label{eq:ms-poincare-P-charge}
\boxed{P^\nu=\int d^{d-1}x\,T^{0\nu}}
\end{equation}
...
\emph{思路：}把 Lorentz 变分代入 Noether 流，分离出轨道部分（含 \(x\) 和 \(T\)）和自旋部分（含 \(\Sigma\)），分别定义轨道角动量流和自旋流。
...
\emph{第二步：识别能动量张量。}第一括号正是 \(T^\lambda{}_\rho\)（见~\eqnrefs{eq:ms-poincare-T-can}）。

\emph{第三步：轨道部分改写。}利用 \(\omega_{\mu\nu}\) 反对称性，把 \(\omega^\rho{}_\sigma x^\sigma T^\lambda{}_\rho\) 改写为 \(\omega_{\mu\nu}\) 的缩并：
\begin{equation*}
...
总角动量流守恒 \(\partial_\lambda J^{\lambda\mu\nu}=0\) 展开后给出 canonical 能动量张量的一个重要性质。展开：
\begin{equation*}
...
\emph{思路：}利用反对称关系~\eqnrefs{eq:ms-poincare-T-asym} 与 Belinfante correction~\eqnrefs{eq:ms-poincare-B-def} 的指标性质，分三步验证：(1) \(T_B^{\mu\nu}=T_B^{\nu\mu}\)（对称性），(2) \(\partial_\mu T_B^{\mu\nu}=0\)（守恒性），(3) 角动量流可用 \(T_B\) 单独表示（自旋吸收）。

\emph{第一步：对称性——计算 \(T_B^{\mu\nu}-T_B^{\nu\mu}\)。}由定义~\eqnrefs{eq:ms-poincare-TB-def}：
\begin{equation*}
...
\emph{第二步：守恒性。}由~\eqnrefs{eq:ms-poincare-TB-def}，\(\partial_\mu T_B^{\mu\nu}=\partial_\mu T_{\text{can}}^{\mu\nu}+\partial_\mu\partial_\lambda B^{\lambda\mu\nu}\)。第一项为零（canonical 守恒~\eqnrefs{eq:ms-poincare-T-conservation}）。第二项中 \(B^{\lambda\mu\nu}\) 对前两指标反对称：\(B^{\lambda\mu\nu}=-B^{\mu\lambda\nu}\)。验证：由定义
\begin{equation*}
...
\emph{第三步：角动量流改写。}用 \(T_B\) 改写总"纯轨道"角动量流：
\begin{equation*}
J_B^{\lambda\mu\nu}=x^\mu T_B^{\lambda\nu}-x^\nu T_B^{\lambda\mu}.
\end{equation*}
...
\item \textbf{Lie 代数}：统一的半直积 Lie bracket~\eqnrefs{eq:ms-poincare-unified-bracket} 一次性给出全部三组交换关系。\([P,P]=0\)（平移 Abel），\([M,P]\sim P\)（Lorentz 旋转平移），\([M,M]\sim M\)（Lorentz 子代数）。
...
\item \textbf{Noether 定理}：连续准对称性 \(\delta\mathcal{L}=\partial_\mu K^\mu\) 给出守恒流 \(J^\mu=\Pi_A^\mu\delta\Phi^A-K^\mu\)，on-shell 满足 \(\partial_\mu J^\mu=0\)。
\item \textbf{守恒律}：平移对称性给出能动量守恒 \(\partial_\mu T^{\mu\nu}=0\)，Lorentz 对称性给出角动量守恒 \(\partial_\lambda(L^{\lambda\mu\nu}+S^{\lambda\mu\nu})=0\)。
\item \textbf{Belinfante 改进}：通过添加 superpotential 散度把 canonical 能动量张量对称化，自旋流被吸收进对称的 \(T_B^{\mu\nu}\) 中。
\end{enumerate}

下一章把这套结构推广到曲率时空：平直的 \(\partial_\mu\) 被协变导数 \(D_\mu=\partial_\mu-\frac{i}{2}\omega_{\mu ab}\Sigma^{ab}\) 取代，全局 Poincaré 对称被 diffeomorphism covariance 与局域 Lorentz 对称取代，并系统讨论自旋与螺旋度的 Wigner 分类。
