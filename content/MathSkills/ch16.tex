...
逐次使用群乘法~\eqnrefs{eq:ms-poincare-semidirect} 与逆元公式~\eqnrefs{eq:ms-poincare-inverse}，把四元群乘积~\eqnrefs{eq:ms-poincare-bracket-def} 展开到 \(ts\) 阶。关键是跟踪每一步中 Lorentz 部分如何作用于平移部分。
...
由逆元公式~\eqnrefs{eq:ms-poincare-inverse}，
\begin{equation*}
...
Symplectic potential。平移变分 \(\delta\Phi^A=\epsilon^\nu\partial_\nu\Phi^A\) 代入 \(\Theta^\mu=\Pi_A^\mu\delta\Phi^A\)：
\begin{equation*}
...
守恒律。由 Noether 定理~\eqnrefs{eq:ms-poincare-noether}，\(\partial_\mu J^\mu=0\) on-shell。代入 \(J^\mu=\epsilon^\nu T^\mu{}_\nu\)：
\begin{equation*}
...
利用反对称关系~\eqnrefs{eq:ms-poincare-T-asym} 与 Belinfante correction~\eqnrefs{eq:ms-poincare-B-def} 的指标性质，分三步验证：(1) \(T_B^{\mu\nu}=T_B^{\nu\mu}\)（对称性），(2) \(\partial_\mu T_B^{\mu\nu}=0\)（守恒性），(3) 角动量流可用 \(T_B\) 单独表示（自旋吸收）。
