...
若 Lagrangian 含高阶导数 $\mathcal{L}=\mathcal{L}(\Phi^A,\partial_\mu\Phi^A,\partial_\mu\partial_\nu\Phi^A,\dots)$，Euler--Lagrange 方程推广为 $\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}+\partial_\mu\partial_\nu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\partial_\nu\Phi^A)}-\cdots=0$（Ostrogradsky 方程）。对二阶导数 Lagrangian，Ostrogradsky 框架给出 Hamiltonian 形式，但一般有 Ostrogradsky 不稳定性（Hamiltonian 无下界），除非 Lagrangian 是全导数或退化的。有效场论中高阶导数项（如 $\Box\Phi$ 修正）作为微扰处理，不引入 Ostrogradsky 不稳定，是有效理论的典型处理方式。

Euler--Lagrange 方程与 Noether 定理同源：作用量在连续对称变换下的不变性 $\delta S=0$ 结合 Euler--Lagrange 方程给出守恒流 $\partial_\mu j^\mu=0$。平移对称性给出能动量守恒 $\partial_\mu T^{\mu\nu}=0$，Lorentz 对称性给出角动量守恒 $\partial_\lambda J^{\lambda\mu\nu}=0$，内禀对称性（如 $U(1)$）给出荷守恒 $\partial_\mu j^\mu=0$。Noether 定理把对称性（几何性质）转化为守恒律（动力学约束），是 Euler--Lagrange 方程的对称性推论。

若 Lagrangian 是退化的（Hessian $\frac{\partial^2\mathcal{L}}{\partial\dot\Phi^A\partial\dot\Phi^B}$ 不满秩），Euler--Lagrange 方程含约束。Dirac--Bergmann 算法系统处理约束：(1) 初级约束 $\phi_m(\Phi,\pi)=0$（来自 Legendre 变换退化）；(2) 自洽条件 $\dot\phi_m=\{\phi_m,H\}\approx0$ 给出次级约束；(3) 反复迭代直到约束代数封闭。约束分为第一类（与所有约束 Poisson 对易，生成规范变换）和第二类（需用 Dirac 括号修正）。规范理论（如 Maxwell、Yang--Mills）的规范对称性来自第一类约束，是 Euler--Lagrange 方程在退化系统中的推广。

变分 $\delta S=\int\left[\frac{\partial\mathcal{L}}{\partial\Phi^A}-\partial_\mu\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\right]\delta\Phi^A\,\dd^4x+\int\partial_\mu\left(\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\delta\Phi^A\right)\dd^4x$ 的边界项 $\theta^\mu=\frac{\partial\mathcal{L}}{\partial(\partial_\mu\Phi^A)}\delta\Phi^A$ 是 Noether 流的变分形式。在无穷远边界（$\delta\Phi|_{\partial\Omega}=0$）边界项消失，但在有限区域或渐近对称下，边界项给出荷的定义 $Q=\int_\Sigma\theta^0\,\dd^3x$。在引力理论中，边界项的处理更微妙（Gibbons--Hawking 边界项），是作用量变分良定义的关键。
\end{derivation}
...
总角动量 $J_i=\frac12\epsilon_{ijk}\int J^{0jk}\,\dd^3x$ 满足 $SO(3)$ 李代数 $[J_i,J_j]=i\epsilon_{ijk}J_k$，Casimir $J^2=J_iJ_i$ 的本征值 $j(j+1)$ 标记不可约表示。对单粒子态，$j$ 是总角动量量子数（轨道 $l$ + 自旋 $s$ 的耦合）。Poincar\'e 群的 Casimir 算符 $P^2=m^2$（质量）和 $W^2=-m^2s(s+1)$（自旋，$W_\mu=\frac12\epsilon_{\mu\nu\rho\sigma}J^{\nu\rho}P^\sigma$ 为 Pauli--Lubanski 矢量）完全标记粒子态，是 Wigner 分类的数学基础。
\end{derivation}

角动量流由此分解：轨道流 $L^{\lambda\mu\nu}=x^\mu T^{\lambda\nu}-x^\nu T^{\lambda\mu}$（式~\eqnrefs{eq:ms-poincare-L-current}）、自旋流 $S^{\lambda\mu\nu}=-i\Pi_A^\lambda(\Sigma^{\mu\nu})^A{}_B\Phi^B$（式~\eqnrefs{eq:ms-poincare-S-current}），总角动量流 $J^{\lambda\mu\nu}=L^{\lambda\mu\nu}+S^{\lambda\mu\nu}$ 满足 $\partial_\lambda J^{\lambda\mu\nu}=0$（式~\eqnrefs{eq:ms-poincare-J-total}、\eqnrefs{eq:ms-poincare-J-conservation}）。
