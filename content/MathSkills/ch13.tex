...
对秩 $k$ 归纳。$k=1$ 时，取非零局部截面 $X_1$。流-box 定理给出坐标使
\[
X_1=\partial_{y^1},
\]
故积分曲线就是一维叶。

设结论对秩 $k-1$ 成立。取 $p$ 附近非零截面 $X_1\in\Gamma(\mathcal D)$，再次用 流-box 坐标令
\[
X_1=\partial_{y^1}.
\]
令横截面
...
\]
是 $\Sigma$ 上秩 $k-1$ 分布。若 $Y,Z$ 是 $\mathcal E$ 的截面，则它们既切于 $\Sigma$ 又属于 $\mathcal D$。切于固定子流形的向量场在 Lie 括号下封闭，而 $\mathcal D$ 假设 对合的，因此
\[
...
现在令 $\Phi_t$ 为 $X_1$ 的局部流。关键是证明它保持分布 $\mathcal D$。取局部标架 $X_1,\ldots,X_k$ 张成 $\mathcal D$。对合的 条件给出
\[
[X_1,X_a]=B^b{}_aX_b
\]
...
$\mathbb R^3$ 中的分布 $\mathcal D=\operatorname{span}\{\partial_x,-y\partial_z+\partial_y\}$：$[\partial_x,-y\partial_z+\partial_y]=0\in\mathcal D$，对合，积分叶为平面 $y=\text{const}$（每个叶是 $xz$ 平面）。反例 $\mathcal D=\operatorname{span}\{\partial_x,x\partial_z+\partial_y\}$：$[\partial_x,x\partial_z+\partial_y]=\partial_z\notin\mathcal D$，不对合，不存在通过每点的二维积分曲面——这是接触几何的典型例子。Frobenius 定理在控制论中给出可达性条件：向量场张成的分布对合当且仅当系统可沿积分叶任意移动。
\end{derivation}
...
\begin{derivation}[接触 分布 为什么一步 Lie 括号就生成全部切空间]
...
\end{equation}
接触 分布 因而是 step-two 括号-生成 分布：虽然单个允许速度永远限制在 $\mathcal H$ 中，但通过"先沿 $X$、再沿 $Y$、再反向返回"的小回路，可以在二阶产生横向位移。
\end{derivation}
...
\begin{derivation}[接触 Hamiltonian 如何提升为 辛 Hamiltonian]
在 辛化 上令
\[
\widetilde H(s,x)=\ee^sH(x).
\]
把 Hamiltonian 矢量场 分解为
\[
...
\end{equation*}
即 $\widetilde H$ 是沿 $s$ 方向的 $1$ 次齐次函数。对应地，辛 形式 $\omega=\dd(\ee^s\alpha)$ 在 $r_\lambda$ 下满足 $r_\lambda^*\omega=\lambda\,\omega$，因此 Hamiltonian 流保持这一齐次结构：若 $\phi_t$ 是 $\widetilde H$ 的流，则 $r_\lambda\circ\phi_t\circ r_{\lambda^{-1}}$ 也是 $\widetilde H$ 的流，由唯一性得 $r_\lambda\circ\phi_t=\phi_t\circ r_\lambda$。

Reeb 流作为特例。取 $H\equiv1$，则 $R_\alpha H=0$ 且 $X_H=R_\alpha$（由 接触 Hamiltonian 的定义方程 $\iota_{X_H}\dd\alpha=\dd H-(R_\alpha H)\alpha=0$ 与 $\alpha(X_H)=H=1$）。此时
\begin{equation*}
...
接触 Hamilton 系统。经典耗散系统的 Hamilton 化需要接触几何。对带摩擦的一维振子 $\ddot q+\gamma\dot q+\omega^2q=0$，取接触形式 $\alpha=\dd z-p\,\dd q$ 与 Hamiltonian $H=\tfrac12p^2+\tfrac12\omega^2q^2+\gamma z$，则 $R_\alpha H=\gamma$，接触 Hamiltonian 流给出原耗散方程，而能量沿流的变化为
\begin{equation*}
\frac{\dd H}{\dd t}=R_\alpha H\cdot H=\gamma H,
\end{equation*}
正确反映能量耗散（或注入）。提升到 辛化 后，$\widetilde H=\ee^sH$ 的 辛 流在 $s$ 方向以速率 $-\gamma$ 演化，把耗散编码为 辛 框架下的额外自由度。
...
\]
因此提升曲线始终停留在对应 纤维 之上。因为 $F$ 真，紧小球 $\overline U$ 的原像 $F^{-1}(\overline U)$ 紧，水平 ODE 的解不会在有限时间逃向无穷远；缩小 $U$ 后，所有这些 ODE 在 $t\in[0,1]$ 上统一存在。定义
\[
...
真性 的必要性。考虑 $F:\RR^2\setminus\{0\}\to\RR$，$F(x,y)=x$。这是 淹没 但不 真：纤维 $F^{-1}(0)=\{(0,y):y\neq0\}$ 不紧。取 $b_0=0$、$b_n\to0^+$，从 $(0,1)$ 出发沿 $b_n$ 方向水平提升，当 $b_n\to0$ 时提升曲线可能逃向无穷远，不存在统一局部平凡化。真性 通过紧性排除这种逃逸。

纤维丛结构的进一步推论。当 $F:M\to B$ 是 真 淹没 且 $B$ 连通时，所有 纤维 微分同胚。结合 long 正合序列 of 同伦群，
\begin{equation*}
\cdots\to\pi_1(F)\to\pi_1(M)\to\pi_1(B)\to\pi_0(F)\to\cdots
\end{equation*}
给出纤维丛的同伦分类。Ehresmann 定理因此是 纤维化 在光滑范畴的版本：真 淹没 自动是 locally trivial 纤维化。
...
\end{equation*}
给出。$S^3$ 紧、$F$ 淹没，故自动 真，Ehresmann 给出 $S^3\to S^2$ 是 $S^1$-纤维丛。水平分布对应 Hopf 联络，其曲率是 $S^2$ 上面积形式的拉回，正是 Berry 几何相的数学来源。

与叶状结构的对比。Ehresmann 的纤维丛结构是"被动"的：从一个 真 淹没 推出纤维自动形成局部平凡族。叶状结构理论则是"主动"的：从一个可积分布出发构造叶，但叶空间可能具有非流形拓扑（例如 Reeb 叶状结构中叶空间含区间与 Cantor 集混合）。两者代表可积性在全局层面的两种不同表现。
...
一般结论：Ehresmann 纤维化定理。若 $F:M\to B$ 是 真 淹没，则 $F$ 是 locally trivial 纤维化。结合 $B$ 连通条件，所有 纤维 微分同胚；进一步若 $B$ 单连通，纤维丛的整体结构由任一 纤维 的同伦型决定。这把纤维丛分类问题化为同伦论问题，是 特征类 理论的出发点。Ehresmann 定理因此是微分拓扑中"真性 换全局平凡性"的典型范式，与 Sard 定理、横截性定理同属 Morse 理论的工具族。

Berry 联络与 Ehresmann。在量子绝热系统中，参数空间 $B$ 上的 Hilbert 丛 $\mathcal H\to B$ 的投影是 淹没，Ehresmann 给出局部平凡化。Berry 联络是水平分布的选择，曲率即 Berry 曲率。当参数空间含简并点时 真性 可能失效，对应量子相变，这是 Ehresmann 条件在物理上的具体意义。
\end{derivation}
...
和乐 的具体计算。对 $U(1)$ 联络在圆 $S^1$ 上的平行移动，沿闭路 $\gamma$ 的 和乐 为
\begin{equation*}
\operatorname{Hol}(\gamma)=\exp\left(i\oint_\gamma\omega\right)
=\exp\left(i\int_D\Omega\right),
\end{equation*}
最后一步用到 Stokes 定理。曲率 $\Omega$ 因此是 和乐 的"密度"：把闭路包围的曲率积分即得 和乐 角度。非 Abelian 情形下此式变为非交换 Wilson 圈
\begin{equation*}
W(\gamma)=\mathcal P\exp\left(\oint_\gamma\omega\right),
\end{equation*}
其中 $\mathcal P$ 表示 path-ordering，反映非交换性使局部贡献不能简单交换。

但 $\Omega=0$ 仍只是局部可积性。固定 $p_0\in P$ 后，沿基流形路径作水平提升定义平行移动。若基空间存在非平凡闭路，平坦联络仍可能具有非平凡 和乐 表示
\[
 \operatorname{Hol}:\pi_1(B,b_0)\to G.
\]
只有当这些闭路 和乐 全部平凡时，从 $p_0$ 出发的水平叶才会在每个基点上选出唯一一点，从而成为全局水平截面。若 $B$ 单连通，则 $\pi_1(B)=0$，平坦性使平行移动与路径无关，于是该局部 Frobenius 结构可以沿整个连通基底拼成全局水平截面。由此可见：曲率控制局部可积性，和乐 控制全局拼接；前者是微分条件，后者是拓扑条件，两者共同决定联络的可积性结构。

分类空间与特征类。曲率与 和乐 的分离对应特征类的两层结构：第一 Chern 类 $c_1\in H^2(B,\ZZ)$ 通过 de Rham 表示由曲率积分给出（局部信息），但其整体挠部分由 和乐 决定（全局信息）。Chern--Weil 理论把曲率不变多项式映射到上同调类，给出 $c_k$、$p_k$、Euler 类等的微分代表；而 和乐 的离散部分则对应 挠率 类，需要其他工具（如 $\hat A$-亏格、指数定理）才能完全捕捉。
\end{derivation}
...
取局部联络形式 $A=A_i\,\dd x^i$，沿 $X$、$Y$ 方向各走长度 $\varepsilon$ 的小矩形。平行移动在每条边上由 path-ordered 指数 给出。保留到二阶，四条边的乘积为
\[
\begin{aligned}
U_{\partial\square}
={}&
...
=(\dd A+A\wedge A)(X,Y).
\]
因此曲率不是平行移动误差的比喻，而是无穷小闭路 和乐 的首个非平凡系数。
...
Ambrose--Singer 定理的极限形式。把无穷小回路公式积分到有限回路，得到 和乐 沿任意闭路 $\gamma$ 的表达式
\begin{equation*}
...
\end{equation*}
其中 $\mathcal S$ 表示 surface-ordering。把 $\Sigma$ 分成 $N^2$ 个面积为 $1/N^2$ 的小方格并取 $N\to\infty$，每个小方格贡献 $I-\frac1{N^2}F+O(N^{-3})$，乘积收敛到上式。这是 Ambrose--Singer 定理的显式版本：和乐 由曲率沿曲面取值的非交换积分完全决定。

Berry 相位与 Aharonov--Bohm。在量子力学中，参数空间上的绝热演化给出 $U(1)$ 联络（Berry 联络）$\mathcal A=i\langle\psi|\dd\psi\rangle$，曲率 $\mathcal F=\dd\mathcal A$ 是 Berry 曲率。沿参数空间闭路演化一周后波函数获得相位 $\exp(i\oint\mathcal A)$，这正是无穷小回路公式在 $U(1)$ 情形的应用。Aharonov--Bohm 效应中电子绕磁通量管的相位差则是非平凡 和乐 的典型例子：联络局部平坦（磁场为零区域）但全局 和乐 非平凡，体现曲率与 和乐 的全局-局部分离。
\end{derivation}
