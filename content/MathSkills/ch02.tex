...
例如 $\Cl_2(\RR)$ 中取 $v=e_1$，$\rho_{e_1}(x)=x-2x_1e_1$ 即关于 $e_2$ 轴的反射；两个反射复合给出转 $\pi$ 的旋转。$\Cl_3(\RR)$ 中 $\rho_{e_1}$ 是关于 $e_2e_3$ 平面的反射，而 rotor $R=\exp(-\theta e_1e_2/2)$ 经 $x\mapsto RxR^{-1}$ 实现转 $\theta$ 的旋转，半角正来自两个反射的复合。在 $\Cl_{1,3}(\RR)$ 中取 $v=\gamma^0$，$\rho_{\gamma^0}$ 反号空间分量而保持时间分量，正是宇称的代数实现；Pin 群与 Spin 群由此分别通过 twisted conjugation 实现 O(1,3) 与其双覆盖 SO(1,3)。

此公式要求 $v$ 可逆（$Q(v)\ne0$）：退化二次型上 $v^{-1}$ 不存在，需限制到非迷向锥外；复 Clifford 代数则所有非零向量可逆，公式全局成立。Cartan--Dieudonn\'e 定理进一步指出任意正交变换可表为不超过 $n$ 个反射之积，故 Clifford 可逆元经 twisted conjugation 生成整个 $O(V,Q)$。
\end{derivation}
...
举例：$\Spin(2)\cong U(1)$ 经 $\rho(e^{i\theta/2})$ 给出 $SO(2)$ 中转 $\theta$ 的旋转，核为 $\pm1$——即转 $2\pi$ 在 $\Spin(2)$ 中回到 $-1$，须转 $4\pi$ 才回到恒等。$\Spin(3)\cong SU(2)\cong S^3$ 经 $q\vec x q^{-1}$ 作用于纯虚四元数，核为 $q=\pm1$，这是自旋 $1/2$ 粒子转 $2\pi$ 出负号的代数根源。$\Spin(4)\cong SU(2)\times SU(2)$，核为 $\{(1,1),(-1,-1)\}$，故 $SO(4)\cong(SU(2)\times SU(2))/\mathbb Z_2$。

核 $\{\pm1\}$ 完全由 Clifford 代数的中心与 norm 决定，与矩阵实现无关：$\Spin(n)$（$n\ge3$）单连通，故是 $SO(n)$ 的万有覆盖，所有 $SO(n)$ 的投影表示提升为 $\Spin(n)$ 的真实表示。物理上，$\Spin$ 在旋量表示中于 $-1$ 处取负号，正是 Fermi 统计的几何来源；而 Spin 结构的存在性 $w_2(TM)=0$ 是 Dirac 算子定义与 Atiyah--Singer 指标公式的前提。对带不定度规的 $\Spin(p,q)$ 证明相同，只是群非紧且连通分支可能更多；Pin 群的核则推广为 $\{\pm1,\pm\omega\}$（含体积元）。
\end{derivation}
...
整体上，每点切空间的 bivector 拼成 $\Lambda^2TM\cong\mathfrak{so}(T_pM)$，给出标架丛的 Spin 结构。
\end{derivation}
...
自由费米子 Fock 空间。在量子场论中，自由 Dirac 场的 Fock 空间即由此构造：$a_a^\dagger$ 为产生算子、$a_a$ 为湮灭算子，真空 $|0\rangle$ 为无粒子态。多费米子态
\[
a_{i_1}^\dagger\cdots a_{i_k}^\dagger|0\rangle
\]
由 Pauli 不相容原理（CAR 的 $\{a_a^\dagger,a_b^\dagger\}=0$）保证每模式最多占一个粒子。旋量场的反对易关系不是额外公理，而是 Clifford 表示在 Fock 模型下的自然结果。

同一 Fock 构造把玻色与费米振子合并即得超 Clifford 代数的超旋量表示。
\end{derivation}
...
举例：$n=2$ 时 $\omega_\CC=i e_1e_2$ 可对角化为 $\operatorname{diag}(1,-1)$，$S^\pm$ 各一维，是 $\Spin(2)\cong U(1)$ 的两个不同权表示。$n=4$ 时 $\omega_\CC=i\gamma^0\gamma^1\gamma^2\gamma^3=\gamma_5$，$P_\pm=\frac12(1\pm\gamma_5)$，四维 Dirac 旋量 $S=\CC^4$ 分解为各二维的 $S^\pm$。

完整 Clifford 作用把 $S^+$ 映到 $S^-$，故向量耦合 $\bar\psi\gamma^\mu\psi$ 保持手征，而质量项 $m\bar\psi\psi=m(\bar\psi_L\psi_R+\bar\psi_R\psi_L)$ 必须混合左右手——这正是弱相互作用 V--A 结构与 Majorana 质量项的代数根源。奇数维 $n=2m+1$ 时体积元属于中心，旋量空间不再有这种 Weyl 分解。

Weyl 分解的存在性随维数 mod 8 周期变化：复情形周期为 $2$（偶维有、奇维无），实情形周期为 $8$（不可约模类型随 $(p-q)\bmod8$ 在 $\RR,\CC,\mathbb H$ 间循环）。这正是实 Clifford 代数 Bott 周期性的表示论体现，也是复 K--群 $\widetilde K(S^n)$ 生成元与 Atiyah--Singer 指标 $\operatorname{ind}D^+=\dim\ker D^+-\dim\ker D^-$ 的代数来源。
\end{derivation}
...
Dirac 矩阵的完备性与流--流相互作用。在 QED 中，电子--光子顶角 $\Gamma^\mu=\gamma^\mu$ 的完备性保证任意 Lorentz 协变顶角可写为
\[
...
对一般偶维 $n=2m$，复 Clifford 基由 $\Lambda^k V$（$k=0,\ldots,2m$）给出，共 $2^{2m}$ 个元素，与 $\dim M_{2^m}(\CC)=2^{2m}$ 一致。对实 Clifford 代数，需区分 $\Cl_{p,q}(\RR)$ 的矩阵类型（由\eqnrefs{eq:real-clifford-classification}），基元数仍为 $2^n$ 但矩阵规模与实/复/四元数结构随 $(p-q)\bmod 8$ 周期变化（Bott 周期性）。在 K--理论与指标定理中，Clifford 基的完备性是构造 Dirac 算子与计算解析指数的代数前提。
\end{derivation}
