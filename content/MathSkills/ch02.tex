...
验证 正则反交换关系。对 decomposable $k$-形式
\[
...
\Cl_{1,0}\cong\RR\oplus\RR,
\]
以及 张量-积 的分次版本反复递推，就得到\eqnrefs{eq:real-clifford-classification}。走完八步后 division-代数 类型回到原点，而矩阵阶数累积出因子 $16$，于是得到\eqnrefs{eq:clifford-bott-eight-periodicity}。这里真正周期的是 Morita 类型；矩阵阶数负责补偿总维数的增长。
\end{derivation}
...
例如 $\Cl_2(\RR)$ 中取 $v=e_1$，$\rho_{e_1}(x)=x-2x_1e_1$ 即关于 $e_2$ 轴的反射；两个反射复合给出转 $\pi$ 的旋转。$\Cl_3(\RR)$ 中 $\rho_{e_1}$ 是关于 $e_2e_3$ 平面的反射，而 rotor $R=\exp(-\theta e_1e_2/2)$ 经 $x\mapsto RxR^{-1}$ 实现转 $\theta$ 的旋转，半角正来自两个反射的复合。在 $\Cl_{1,3}(\RR)$ 中取 $v=\gamma^0$，$\rho_{\gamma^0}$ 反号空间分量而保持时间分量，正是宇称的代数实现；Pin 群与 Spin 群由此分别通过 扭曲 共轭 实现 O(1,3) 与其双覆盖 SO(1,3)。
...
核 $\{\pm1\}$ 完全由 Clifford 代数的中心与 范数 决定，与矩阵实现无关：$\Spin(n)$（$n\ge3$）单连通，故是 $SO(n)$ 的万有覆盖，所有 $SO(n)$ 的投影表示提升为 $\Spin(n)$ 的真实表示。物理上，$\Spin$ 在旋量表示中于 $-1$ 处取负号，正是 Fermi 统计的几何来源；而 Spin 结构的存在性 $w_2(TM)=0$ 是 Dirac 算子定义与 Atiyah--Singer 指标公式的前提。对带不定度规的 $\Spin(p,q)$ 证明相同，只是群非紧且连通分支可能更多；Pin 群的核则推广为 $\{\pm1,\pm\omega\}$（含体积元）。
\end{derivation}
...
举例：$\mathfrak{spin}(3)$ 与角动量。取 $V=\RR^3$ Euclidean，正交基 $e_1,e_2,e_3$。双矢 基 $e_2e_3,e_3e_1,e_1e_2$ 对应
\[
...
Clifford 偶子代数由偶数个 产生/湮灭 算符 生成，保持 宇称；奇部分则交换 宇称。具体地：偶元素如 $a_a^\dagger a_b$ 把 $\Lambda^k W^*$ 映到自身（一次 create 一次 annihilate，宇称 不变）；奇元素如 $a_a^\dagger$ 把 $\Lambda^k W^*$ 映到 $\Lambda^{k+1} W^*$（宇称 翻转）。于是\eqnrefs{eq:spinor-fock-chirality} 不是额外强加的手征分解，而是 Clifford $\mathbb Z_2$ 分次在不可约模上的自然实现。
...
举例：$m=2$ 即四维复 Clifford。$S=\Lambda^\bullet W^*=\operatorname{span}\{1,\xi^1,\xi^2,\xi^1\wedge\xi^2\}$，$\dim S=4$。Fock 基按 宇称 分组：
\[
...
a_{i_1}^\dagger\cdots a_{i_k}^\dagger|0\rangle
\]
由 Pauli 不相容原理（正则反交换关系 的 $\{a_a^\dagger,a_b^\dagger\}=0$）保证每模式最多占一个粒子。旋量场的反对易关系不是额外公理，而是 Clifford 表示在 Fock 模型下的自然结果。
