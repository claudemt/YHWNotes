...
先用次数过滤把 Clifford 乘法的「低阶标量收缩」压掉，让 associated graded 只剩反对易关系而成为外代数；再用过滤的维数守恒把满射升级为同构；最后在最外层验证 Chevalley 作用满足 CAR。

过滤与 associated graded。把张量代数 $T(V)$ 的次数过滤传到商代数
\[
\Cl(V,Q)
...
外代数 universal property 给出满射。外代数 \(\Lambda V\) 正是「由 \(V\) 生成、且元素反对易」的最一般代数。既然 \(\operatorname{gr}\Cl\) 中向量乘积反对易，就存在唯一代数同态
\[
...
举例：\(\Cl_2(\RR)\) 中的平面反射。取 \(V=\RR^2\)，\(Q(x)=x_1^2+x_2^2\)，正交基 \(e_1,e_2\) 满足 \(e_1^2=e_2^2=1\)、\(e_1e_2=-e_2e_1\)。取 \(v=e_1\)，对任意 \(x=x_1e_1+x_2e_2\)，\(B(e_1,x)=x_1\)、\(Q(e_1)=1\)，故
\[
\rho_{e_1}(x)=x-2x_1e_1=-x_1e_1+x_2e_2,
\]
即关于 \(e_2\) 轴的反射。类似地 \(\rho_{e_2}\) 是关于 \(e_1\) 轴的反射。两者复合 \(\rho_{e_1}\rho_{e_2}\) 给出绕原点转 \(\pi\) 的旋转。

举例：\(\Cl_3(\RR)\) 中的空间反射与旋转。取 \(V=\RR^3\) Euclidean，\(v=e_1\)，\(\rho_{e_1}\) 是关于 \(e_2e_3\) 平面的反射。两个反射复合 \(\rho_{e_1}\rho_{e_2}\) 是绕 \(e_3\) 轴转 \(\pi\) 的旋转。更一般地，rotor \(R=\exp(-\theta e_1e_2/2)=\cos(\theta/2)-e_1e_2\sin(\theta/2)\) 通过 \(x\mapsto RxR^{-1}\) 实现绕 \(e_3\) 轴转 \(\theta\) 的旋转，半角正来自两个反射的复合。

物理应用：宇称与时间反演的 Clifford 实现。在相对论 Clifford 代数 \(\Cl_{1,3}(\RR)\) 中，取 \(v=\gamma^0\)（时间方向），\(\rho_{\gamma^0}\) 把空间分量反向而保持时间分量，正是宇称变换的代数实现。取 \(v=\gamma^1\)，相应反射翻转 \(x^1\) 与时间方向，与其它反射复合可生成 Lorentz 群的离散子群。Pin 群通过 twisted conjugation 实现 O(1,3)，Spin 群实现 SO(1,3) 的双覆盖，这是旋量表示下 Lorentz 变换严格代数化的母结构。

推广方向。对退化二次型，\(v^{-1}\) 不存在，反射公式失效，需用广义 Clifford 作用或限制到非迷向锥外。对复 Clifford 代数 \(\Cl_n(\CC)\)，所有非零向量可逆，反射公式全局成立。在 Spin 几何中，此反射公式推广为 Clifford 作用在切丛上的 gauge 变换，是 Dirac 算子协变性的代数基础。Cartan--Dieudonn\'e 定理进一步断言任意正交变换可写为不超过 \(n\) 个反射之积，因此 Clifford 可逆元通过 twisted conjugation 生成整个 \(O(V,Q)\)。
\end{derivation}
...
Clifford norm 限定 \(\lambda=\pm1\)。Spin 群元素满足 Clifford norm \(N(a)=a\tilde a=\pm1\)（其中 \(\tilde{}\) 是主对合，把每个生成元 \(e_i\mapsto -e_i\)）。对 \(a=\lambda\cdot 1\)，\(\tilde a=\lambda\cdot 1\)（标量被主对合保持），故
\[
 N(a)=\lambda^2=1,
\]
...
举例：\(\Spin(2)\to SO(2)\)。\(\Spin(2)\cong U(1)=\{e^{i\theta/2}\}\)，映射 \(\rho(e^{i\theta/2})\) 给出 \(SO(2)\) 中转 \(\theta\) 角的旋转。核为 \(e^{i\theta/2}=\pm1\) 即 \(\theta=0,2\pi\)，对应 \(SO(2)\) 中恒等。故 \(\Spin(2)\) 是 \(SO(2)\) 的二重覆盖：转 \(2\pi\) 在 \(\Spin(2)\) 中回到 \(-1\) 而非 \(1\)，需转 \(4\pi\) 才回到 \(1\)。

举例：\(\Spin(3)\to SO(3)\)。\(\Spin(3)\cong SU(2)=\{a+b\mathbf i+c\mathbf j+d\mathbf k:a^2+b^2+c^2+d^2=1\}\cong S^3\)，映射 \(\rho(q)(\vec x)=q\vec x q^{-1}\)（把 \(\vec x\) 视为纯虚四元数）。核为 \(q=\pm1\)。这是量子力学中自旋 \(1/2\) 粒子转 \(2\pi\) 出负号的代数根源：物理上的 \(2\pi\) 旋转在 \(\Spin(3)\) 中对应 \(-1\)，而非恒等。

举例：\(\Spin(4)\to SO(4)\)。\(\Spin(4)\cong SU(2)\times SU(2)\)，核为 \(\{(1,1),(-1,-1)\}\)。这给出 \(SO(4)\cong (SU(2)\times SU(2))/\mathbb Z_2\)，是四维 Riemann 几何与 Yang--Mills 瞬子理论的代数基础。

物理应用：自旋统计定理的代数前提。旋量场在 \(\Spin\) 群的核 \(\{\pm1\}\) 中的 \(-1\) 表示下获得负号，正是 Fermi 统计的几何来源。在弯曲时空量子场论中，Spin 结构的存在性（即 \(w_2(TM)=0\)）是定义旋量场的前提；不同 Spin 结构对应不同量子场论。此推导把“旋量表示是 SO 的投影表示”严格归结为 Clifford 代数中心与 Clifford norm 的代数事实。

推广方向。对带不定度规的 \(\Spin(p,q)\)，证明相同，但群非紧且可能有更多连通分支。对 Pin 群，核推广为 \(\{\pm1,\pm\omega\}\)（含体积元）。在 String 群与 Fivebrane 群的高阶推广中，二重覆盖升级为高阶 gerbe 结构，是 M 理论与弦论中 spin--string--fivebrane 阶塔的代数起点。

数学联系：与 Weyl 群的精确序列。对紧致连通 Lie 群 \(G\)，有精确序列
\[
...
总结。Spin 二重覆盖的核 \(\{\pm1\}\) 完全由 Clifford 代数的代数结构（中心 + norm）决定，不依赖任何具体矩阵实现。此代数事实是旋量表示、Dirac 算子与指标定理的基石，在物理中体现为自旋 \(1/2\) 粒子的 \(4\pi\) 周期性与 Fermi 统计。在规范理论中，Spin 结构的存在性是定义 Dirac 算子与构造作用量 \(\int\bar\psi i\gamma^\mu D_\mu\psi\,\dd^4x\) 的前提，不同 Spin 结构给出不同量子场论。此二重覆盖在物理中是不可消除的：\(4\pi\) 旋转才回到恒等，\(2\pi\) 旋转给出 \(-1\)，这是旋量与张量表示的本质区别。

尾声。此核的计算是 Spin 几何的代数起点，后续 Dirac 算子、旋量丛与指标定理都建立在此二重覆盖之上。
\end{derivation}
...
利用 Clifford 关系 $vw=-wv+2B(v,w)$ 逐步展开 $[uv,w]$，证明 bivector $u\wedge v$ 的 Clifford 交换子恰好是 $\mathfrak{so}(V,B)$ 的标准生成元。

计算 $[uv,w]$。由 Clifford 关系 $vw=-wv+2B(v,w)$ 和 $wv=-vw+2B(v,w)$，
\begin{align*}
 [uv,w]
 &=uvw-wuv\\
...
举例：\(\mathfrak{spin}(3)\) 与角动量。取 $V=\RR^3$ Euclidean，正交基 $e_1,e_2,e_3$。bivector 基 $e_2e_3,e_3e_1,e_1e_2$ 对应
\[
...
推广方向。对伪 Riemann 流形，每点切空间的 bivector 拼成 $\Lambda^2 TM$，对应 $\mathfrak{so}(T_pM)$，整体给出标架丛的 Spin 结构。在 K\"ahler 流形上，复结构把 $\mathfrak{so}(2n)$ 约化为 $\mathfrak u(n)$，bivector 分解为 $(2,0)+(0,2)+(1,1)$ 部分，对应 holomorphic、anti-holomorphic 与 central 部分。在广义几何中，$B$-field 变换把 bivector 推广为 $H\in\Lambda^2 T^*\oplus\Lambda^2 T$，是 Courant algebrot 与 generalized spinor 的母结构。
\end{derivation}
...
举例：$m=1$ 即二维复 Clifford。$W=\operatorname{span}\{w_1\}$，$S=\Lambda^\bullet W^*=\operatorname{span}\{1,\xi^1\}$，$\dim S=2$。$a_1=\iota_{w_1}$、$a_1^\dagger=\varepsilon(\xi^1)$ 在基 $\{1,\xi^1\}$ 下表示为
\[
...
举例：$m=2$ 即四维复 Clifford。$S=\Lambda^\bullet W^*=\operatorname{span}\{1,\xi^1,\xi^2,\xi^1\wedge\xi^2\}$，$\dim S=4$。Fock 基按 parity 分组：
\[
...
物理应用：自由费米子 Fock 空间。在量子场论中，自由 Dirac 场的 Fock 空间即由此构造：$a_a^\dagger$ 为产生算子、$a_a$ 为湮灭算子，真空 $|0\rangle$ 为无粒子态。多费米子态
\[
a_{i_1}^\dagger\cdots a_{i_k}^\dagger|0\rangle
\]
由 Pauli 不相容原理（CAR 的 $\{a_a^\dagger,a_b^\dagger\}=0$）保证每模式最多占一个粒子。旋量场的反对易关系不是额外公理，而是 Clifford 表示在 Fock 模型下的自然结果。

推广方向。对超对称理论，bosonic 与 fermionic 振子共同构成超 Clifford 代数，Fock 模型给出超旋量表示。在顶点算子代数中，Fock 空间由 Heisenberg 代数作用于真空生成，是自由玻色子/费米子 VOA 的标准构造。在几何 Langlands 纲领中，旋量 Fock 模型推广为 opers 与 Hecke eigensheaf，连接表示论与几何。
\end{derivation}
...
举例：$n=2$ 的 Weyl 分解。$\omega_\CC=i e_1e_2$（取 $\omega_\CC^2=1$），$P_\pm=\frac12(1\pm i e_1e_2)$。在二维复旋量表示 $S=\CC^2$ 上，$\omega_\CC$ 可对角化为 $\operatorname{diag}(1,-1)$，$P_+=\operatorname{diag}(1,0)$、$P_-=\operatorname{diag}(0,1)$。$S^+$ 与 $S^-$ 各一维，是 $\Spin(2)\cong U(1)$ 的两个不同权（$+1$ 与 $-1$）表示。

举例：$n=4$ 的 Weyl 分解。$\omega_\CC=i\gamma^0\gamma^1\gamma^2\gamma^3=\gamma_5$（物理常用 $\gamma_5$ 记号），$P_\pm=\frac12(1\pm\gamma_5)$。在四维 Dirac 旋量 $S=\CC^4$ 上，$S^\pm$ 各二维。Weyl 旋量 $\psi_L\in S^+$、$\psi_R\in S^-$ 在弱相互作用中分别参与左手与右手耦合，标准模型 V--A 结构的代数根源即此分解。

物理应用：手征费米子与质量项。完整 Clifford 作用把 $S^+$ 映到 $S^-$，故向量--旋量耦合 $\bar\psi\gamma^\mu\psi$ 保持手征（$\bar\psi_L\gamma^\mu\psi_L$、$\bar\psi_R\gamma^\mu\psi_R$ 分别合法），而质量项 $m\bar\psi\psi=m(\bar\psi_L\psi_R+\bar\psi_R\psi_L)$ 必须混合左右手。这是弱相互作用破缺手征对称性、中微子质量问题的代数起点。在 QED 与 QCD 中手征对称性 $U(1)_L\times U(1)_R$（或 $SU(N)_L\times SU(N)_R$）的破缺模式都由此结构决定。

推广方向。对奇数维 $n=2m+1$，体积元属于中心，复 Clifford 代数分解为两个矩阵代数之积，但旋量空间不再有 Weyl 分解。在超对称理论中，超旋量在超 Clifford 代数上的分解给出超 Poincaré 表示的 R--对称性。在 K--理论中，Weyl 分解对应于拓扑 K--群的 Bott 周期性，是指标定理与 Dirac 算子解析指数的代数基础。

数学联系：与 Bott 周期性。Weyl 分解的存在性随维数 mod 8 周期变化，正是实 Clifford 代数的 Bott 周期性在表示论上的体现。复情形周期为 2：偶维有 Weyl 分解，奇维无。实情形周期为 8：\(\Cl_{p,q}\) 的不可约模类型随 \((p-q)\bmod8\) 在 \(\RR,\CC,\mathbb H\) 间循环，决定实旋量、Majorana 条件与 symplectic Majorana 旋量的存在性。此周期性是拓扑 K--理论 \(\widetilde{KO}(S^n)\) 的算子代数实现，是 Atiyah--Bott--Shapiro 构造与指标定理的关键环节。

物理应用：标准模型中的手征费米子。标准模型中左旋与右旋费米子分别属于 \(SU(2)_L\) 双重态与单态，Weyl 分解正是此分配的代数基础。弱相互作用顶角 \(\bar\psi_L\gamma^\mu(1-\gamma_5)\psi_L\) 仅耦合左旋 Weyl 旋量，右旋不参与，V--A 结构由 chirality 投影 \(P_-=\frac12(1-\gamma_5)\) 实现。中微子质量问题（Majorana 质量项需混合左右手）与 CP 破缺的代数根源都在此 Weyl 分解结构中。在格点 QCD 中，Wilson 费米子项通过显式破缺手征对称性避免 doubling，连续极限下恢复 Weyl 分解，是格点规范理论离散化与连续对称性协调的关键技术。在超对称理论中，Weyl 旋量是构造超 Poincaré 代数的基本对象，手征超场即 Weyl 旋量的超对称推广，其自由度与 Weyl 分解的代数结构精确对应。在弦论中，世界面费米子分为左移与右移 Weyl 旋量，Type II 弦/弦的区别即左右旋量 Weyl 分解的相对手征性：Type IIA 左右相反（非手征），Type IIB 左右相同（手征）。这给出弦论中类型分类的代数起源，与 Bott 周期性在 10 维（\(10\bmod8=2\)）的复 Clifford 分类直接相关。

总结。Weyl 分解的代数根源是体积元与向量的反交换关系，仅在偶维给出非平凡手征投影。此结构贯穿粒子物理（手征费米子、弱相互作用）、弦论（Type IIA/IIB 分类）与指标定理（Bott 周期性），是 Clifford 代数偶维表示论的核心结论。在几何 Langlands 纲领中，Weyl 旋量对应于 Hecke eigensheaf 的局部结构，手征分解与几何 Satake 等价精确对应。在凝聚态物理中，拓扑绝缘体的 \(\mathbb Z_2\) 分类依赖于 time-reversal 对称与 Weyl 分解的交互，是三维拓扑绝缘体与二维拓扑超导体分类的代数基础。在 K--理论中，Weyl 分解给出复 K--群 \(\widetilde K(S^n)\) 的生成元，Bott 周期性 \(\widetilde K(S^{n+2})\cong\widetilde K(S^n)\) 即此分解在 suspension 下的稳定性。在指标定理中，Weyl 分解把 Dirac 算子分为 \(D^+:S^+\to S^-\) 与 \(D^-:S^-\to S^+\)，指标 \(\operatorname{ind}D^+=\dim\ker D^+-\dim\ker D^-\) 即此分解的解析体现。

尾声。Weyl 分解是偶维 Clifford 代数的标志性质，把旋量表示从单一不可约分解为两个 Spin 不可约，是手征物理与指标理论的代数根源。此分解在物理中不可消除：左右手费米子耦合不同是弱相互作用的实验事实。在数学中，Weyl 分解给出偶维旋量丛 \(S=S^+\oplus S^-\) 的整体分解，是 Dirac 算子 \(D=\begin{pmatrix}0&D^-\\D^+&0\end{pmatrix}\) 非对角结构的代数来源，直接导致 \(\operatorname{ind}D^+\) 的定义。

后记。Weyl 分解在偶维 Clifford 代数中是自然的，但在奇维消失，此差异是 Bott 周期性的代数体现，也是拓扑绝缘体 \(\mathbb Z_2\) 分类的根源。
\end{derivation}
...
举例：Fierz 恒等式。考虑旋量双线性 $\bar u\gamma^\mu v\,\bar w\gamma_\mu z$。把 $|v\rangle\langle w|$ 视为 $4\times4$ 矩阵并按 Clifford 基展开，再代入第二个双线性，得到 Fierz 恒等式
\[
...
物理应用：Dirac 矩阵的完备性与流--流相互作用。在 QED 中，电子--光子顶角 $\Gamma^\mu=\gamma^\mu$ 的完备性保证任意 Lorentz 协变顶角可写为
\[
...
推广方向。对一般偶维 $n=2m$，复 Clifford 基由 $\Lambda^k V$（$k=0,\ldots,2m$）给出，共 $2^{2m}$ 个元素，与 $\dim M_{2^m}(\CC)=2^{2m}$ 一致。对实 Clifford 代数，需区分 $\Cl_{p,q}(\RR)$ 的矩阵类型（由\eqnrefs{eq:real-clifford-classification}），基元数仍为 $2^n$ 但矩阵规模与实/复/四元数结构随 $(p-q)\bmod 8$ 周期变化（Bott 周期性）。在 K--理论与指标定理中，Clifford 基的完备性是构造 Dirac 算子与计算解析指数的代数前提。
\end{derivation}
