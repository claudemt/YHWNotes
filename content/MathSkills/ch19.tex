...
实例。Cauchy 分布（密度 $1/(\pi(1+x^2))$）：$\mathbb E|X_1|=\infty$，弱大数律不适用——事实上 $\bar X_n$ 仍服从 Cauchy 分布，不收敛。Pareto 分布（密度 $\alpha x^{-\alpha-1}$，$x\ge1$）：$\alpha>1$ 时 $\mathbb E X_1<\infty$，$\bar X_n\xrightarrow{\mathbb P}\alpha/(\alpha-1)$；$\alpha\le1$ 时均值发散。截断技巧的关键在于：有限期望保证截断均值与原均值之差在截断水平趋于无穷时消失，而截断后随机变量方差有界，可用 Chebyshev 控制。
...
实例。经验分布函数 $F_n(t)=\frac1n\sum\mathbf 1_{X_i\le t}$：取 $\mathcal F=\{\mathbf 1_{(-\infty,t]}:t\in\mathbb R\}$，ULLN 给出 $\sup_t|F_n(t)-F(t)|\xrightarrow{\mathbb P}0$——这就是 Glivenko--Cantelli 定理。核密度估计 $\hat f_n(x)=\frac1{nh_n}\sum K((x-X_i)/h_n)$：对函数类 $\{K_h(x-\cdot):x\in\mathbb R\}$ 施加 ULLN 给出逐点一致收敛，是非参数统计的基础。
