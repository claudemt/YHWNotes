...
\end{equation}
与 Chebyshev 的多项式尾界相比，它揭示 有界 独立 和 的指数集中。
...
\]
Hoeffding 引理 对 $Y_i\in[a_i-\mathbb E X_i,b_i-\mathbb E X_i]$ 给出
\[
...
=\ee^{L(\lambda)},
\]
其中 $L(\lambda)=\lambda\mathbb E Y_i+\log\{(1-p_i)+p_i\ee^{\lambda(b_i-a_i)}\}$。中心化使 $\mathbb E Y_i=0$，再利用 $\log u\le u-1$ 与 $L''(\lambda)\le(b_i-a_i)^2/4$，对 $L$ 在 $0$ 处 Taylor 展开即得 Hoeffding 引理。
所以
\[
P(S_n\ge t)
\le
...
\]
当 $t\ll nv/M$ 时退化为 $\exp(-t^2/2nv)$，与 中心极限定理 的 Gaussian 中心区一致；当 $t$ 进入大偏差区时转为 $\exp(-3t/2M)$，体现单项幅度控制。
...
独立平均只是遍历平均的一种最简单结构。对 鞅 差 $D_k$ 满足
\[
...
\[
 ...
 <2\varepsilon
\]
以概率趋于 $1$ 成立。再令 $\varepsilon\downarrow0$，得到\eqnrefs{eq:math-stat-ulln}。这个证明清楚区分了两层机制：有限网格上的随机误差由普通 大数律 控制，网格之间的连续参数误差由可积振荡包络控制。
...
\]
的近似最大化序列都有 $\widehat\theta_n\xrightarrow{\mathbb P}\theta_0$。这就是后面 $M$-估计和 极大似然估计 一致性的基本 argmax 逻辑：先保证经验目标对总体目标的一致逼近，再利用总体目标的可识别极值。
...
\bigr)
\]
是 iid 向量平均的 $\sqrt n$ 缩放，因此由多元 中心极限定理 收敛到均值为零的 Gaussian 矢量。协方差可直接算出
\[
\begin{aligned}
\Cov\bigl(
...
\end{equation}
这正是标准 Brown 桥 $B(t)$ 经 $t=F(x)$ 复合后的协方差。

Donsker 定理把有限维 中心极限定理 升级成函数空间弱收敛：在适当的有界函数空间中，
\begin{equation}
\boxed{
\alpha_n
\Longrightarrow
...
\end{equation}
其中 $B$ 是 $[0,1]$ 上标准 Brown 桥，满足
\[
B(0)=B(1)=0,
\qquad
\Cov(B(s),B(t))=s\wedge t-st.
\]
这里真正困难的部分不是有限维正态性，而是 tightness：必须排除随机函数把质量逃到越来越细的尺度。也正因为如此，泛函 中心极限定理 不能由“对每个 $x$ 都应用普通 中心极限定理”机械得到。
...
\sup_{0\le t\le1}|B(t)|.
\]
这就是 Kolmogorov--Smirnov 统计量具有 分布-自由 极限的原因：极限中原本的未知 $F$ 被单调时间变换吸收到 $t=F(x)$，只剩同一个 Brown 桥。
...
\]
若 $\mathbb G_n$ 在 $\ell^\infty(\mathcal G)$ 中弱收敛到 tight Gaussian 过程 $\mathbb G_P$，就称 $\mathcal G$ 为 $P$-Donsker 类。其 协方差 为
\[
\Cov(\mathbb G_Pf,\mathbb G_Pg)
=P(fg)-Pf\,Pg.
\]
单个函数的 中心极限定理 只对应 $\mathcal G=\{g\}$；有限维多元 中心极限定理 对应有限函数类；真正的 经验过程 理论 研究无限函数类在 熵、covering number 或 VC 维条件下何时仍能获得统一 Gaussian 极限。

\begin{derivation}[Brown 桥 协方差 从 empirical CDF 直接得到]
记
\[
...
Brown 桥 的零端点结构。由 $F(-\infty)=0$、$F(+\infty)=1$，协方差 $\sigma(x,y)=F(x\wedge y)-F(x)F(y)$ 在两端满足
\[
\sigma(-\infty,y)=0,
\qquad
...
\qquad 0\le s,t\le1,
\]
即标准 Brown 桥 的协方差。对分位数泛函 $T(F)=F^{-1}(p)$，其 Hadamard 导数为
\[
...
\]
方差为 $p(1-p)/f^2(F^{-1}(p))$，即 Bahadur 量化的渐近方差。可见 Brown 桥 协方差同时控制经验 CDF 与经验分位数的渐近涨落。


泛函 Δ 方法 进一步把\eqnrefs{eq:math-stat-donsker-cdf}传递给依赖整个分布的统计泛函。若 $T$ 在 $F$ 处具有 Hadamard 导数 $\dot T_F$，则形式上
\[
\sqrt n\{T(F_n)-T(F)\}
\Longrightarrow
\dot T_F(B\circ F).
\]
普通有限维 Δ 方法因此只是这一函数空间线性化的特例。分位数、累积分布函数泛函、某些 $M$-估计量和半参数 影响函数 都可以放在这一统一框架中。
...
独立同分布不是 中心极限定理 的逻辑必要条件。对三角阵 $\{X_{n,k}:1\le k\le k_n\}$，设每行独立且中心化，记
\[
...
\]
它排除了任何单个变量占据总方差的有限比例。反过来，在 Feller 条件下，Lindeberg 条件对三角阵 中心极限定理 接近精确条件；Lindeberg--Feller 定理因此比“有限二阶矩 + iid”更接近中心极限定理真正的结构核心。
...
若 $X_i\in\R^m$ iid，$\mathbb E X_i=\mu$，协方差矩阵 $\Sigma$ 有限，则多元 中心极限定理 为
...
中心极限定理刻画线性平均；一般统计量则要先用 Δ 方法或渐近线性表示化为这种平均。
...
\end{equation}
则 $\mathrm{IF}_{\theta_0}$ 称为一阶影响函数。若其二阶矩有限，则 iid 向量 $\mathrm{IF}_{\theta_0}(X_i)$ 的均值为零、协方差为 $V_{\rm IF}$，多元 中心极限定理 给出
\[
 \frac1{\sqrt n}\sum_{i=1}^n\mathrm{IF}_{\theta_0}(X_i)
...
\begin{equation}\label{eq:math-stat-quantile-bahadur}
 ...
 +o_{\mathbb P}(1).
\end{equation}
对半直线指示函数这一 VC 类，\eqnrefs{eq:math-stat-local-empirical-equicontinuity}确实成立；但它是经验过程的局部增量定理，而不是普通一维 Taylor 公式的自动后果。把这项输入单独列出，可以清楚区分两个层次：分位数渐近正态性只需局部计数 中心极限定理，而 Bahadur 线性化还要求随机分布函数在收缩邻域内足够平滑。
...
本章建立了后续统计理论真正反复使用的四个接口：统一 大数律 控制经验准则，一维与多元 中心极限定理 控制随机一阶项，Δ 方法传播光滑变换，渐近线性表示把复杂估计量还原为 iid 平均。下一章暂时离开 $n\to\infty$，研究正态模型中为什么可以在每一个有限 $n$ 下由正交投影得到更强的精确抽样分布。

\section{CLT 的数值验证与应用实例}\label{sec:clt-examples}
Berry--Esseen 定理给出 中心极限定理 的收敛速率上界：
\[
\sup_x\left|F_n(x)-\Phi(x)\right|\le\frac{C\,\mathbb E|X-\mu|^3}{\sigma^3\sqrt n}
\]
其中 $C\le0.4748$（Shevtsova 2011）。这给出有限样本下正态近似的最大偏差。

\begin{exbox}[均匀分布的 中心极限定理 收敛]
...
对标准差 $S=\sqrt{S^2}$，Δ 方法（$g(x)=\sqrt{x}$，$g'(\sigma^2)=1/(2\sigma)$）：
\[
...
\begin{exbox}[比值估计的 Δ 方法]
估计比值 $\theta=\mu_X/\mu_Y$（如平均收入/平均家庭规模）。Δ 方法给出
\[
