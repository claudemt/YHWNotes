...
谱理论首先研究的是 Hilbert 空间中的闭算子，而不是某一类可解微分方程。一个形式微分表达式只有在 Hilbert 空间、定义域和边界数据同时确定以后，才成为真正可以讨论自伴性、预解式、谱测度与传播的算子。本章因此先建立闭对称算子的普适母理论：伴随与 亏量 空间 描述“缺少多少自伴边界数据”，Cayley--von Neumann 理论给出全部自伴扩张，边界三元组 与 Krein 预解式 公式把扩张参数直接连接到 预解式。随后才把 Sturm--Liouville 表达式作为这一抽象结构的第一个完整实现，并由 Weyl 极限点型/极限圆型 理论处理奇异端点。

一旦自伴算子 $A$ 确定，后半章不再依赖它来自二阶 ODE：投影值谱测度、Borel 泛函演算、预解式、Riesz 投影、Stone 公式、闭二次型、Friedrichs 扩张、极小极大 原理、谱型与长期传播都属于一般自伴算子理论。Green 核只是 预解式 在坐标表示中的积分核；离散模态求和只是谱积分在纯点谱上的特例；可解的有限区间、半直线和全线模型则统一放在母理论之后。这样所有特殊计算都沿“抽象精确结构 $\to$ 微分算子实现 $\to$ 可解参数模型”的单向链条出现。
...
\]
固定任意 $J=[c,d]\Subset I$。若 $u\in\Dom(S_J)$，把它在 $J$ 外延拓为零；由于 $u$ 与 $u^{[1]}$ 在 $c,d$ 同时为零，零延拓后的函数及其准导数都局部绝对连续，而且不会在拼接点产生 δ 项，所以该延拓属于 $\Dom(T_0)$。因此
\[
...
前面的 $T_{\min}\subset T_{\max}=T_{\min}^*$ 是 Sturm--Liouville 表达式给出的具体算子对。为了不把“自伴边界条件”误认为二阶 ODE 的专有技巧，先暂时忘掉微分来源。设 $S$ 是任意复 Hilbert 空间 $\mathcal H$ 上稠密定义的闭对称算子。对 $z\in\CC\setminus\RR$ 定义 缺陷 空间
\[
...
$(S+\ii I)u_n\to y$，则上式说明 $u_n$ 在 图 范数 中 Cauchy；$S$ 闭性给出 $u_n\to u\in\Dom(S)$ 且 $(S+\ii I)u=y$，所以值域闭。
...
\end{aligned}
\]
因此一个包含 $\Dom(S)$ 的限制要对称，其 缺陷 坐标必须落在该不定型的零子空间。若令 $u_-=Uu_+$，则边界型变成
\[
...
von Neumann 参数最适合计数全部扩张，但微分算子计算更需要“边界坐标 $\leftrightarrow$ 预解式”的形式。设仍有 $n_+(S)=n_-(S)=N$。一个 寻常 边界三元组 是 Hilbert 空间 $\mathcal G$（$\dim\mathcal G=N$）与两个满射边界映射
\[
...
\end{equation}
“满射”指 $u\mapsto(\Gamma_0u,\Gamma_1u)$ 从 $\Dom(S^*)$ 映满 $\mathcal G\oplus\mathcal G$。equal 亏指数 不只保证这种边界坐标存在，而且可以从 von Neumann 分解显式构造。
...
\]
而 $\Gamma_0u=0$ 等价于 $y=-x$，即 缺陷 坐标满足
\[
 u_-=-J_-^{-1}J_+u_+.
\]
右端的 $-J_-^{-1}J_+$ 是从 $\mathcal N_+$ 到 $\mathcal N_-$ 的酉映射，所以 von Neumann 定理立即说明
$A_0=S^*|_{\ker\Gamma_0}$ 自伴。于是 寻常 边界三元组 不是独立于 亏量 理论 的额外假设，而是 equal 亏指数 的一组辛坐标。
...
\end{equation}
因此 算符-取值 Weyl 函数 本身就是 Nevanlinna/Herglotz 函数；后面的标量 $m(z)$ 正性只是边界空间一维时的特例。
...
\end{align*}
最后一步把内积改写为 $\gamma(\zeta)^*\gamma(z)$ 的矩阵元。右边由 边界三元组 的
Green 映射定义直接展开为
\begin{align*}
...
\end{align*}
因为 $g,h$ 在 $\mathcal G$ 中任意，左右两边作为 $\mathcal G$ 上的 半双线性 形式 相等，
...
$\Im m(E+\ii0)>0$ 标志绝对连续谱的存在性；算符-取值 $M(z)$ 的正定性
则保证多通道散射中每个通道的谱测度非负。正性不等式
$\langle\Im M(z)g,g\rangle>0$ 还排除隐含谱（hidden 谱）：边界数据 $g\ne0$
必产生非零谱质量，这是 边界三元组 参数化自伴扩张的解析基础。


若 $\Theta-M(z)$ 可逆，则 Krein 预解式 公式 为
\begin{equation}\label{eq:math-sl-krein-resolvent-abstract}
 (A_\Theta-zI)^{-1}
 =(A_0-zI)^{-1}
 +\gamma(z)(\Theta-M(z))^{-1}\gamma(\overline z)^*.
\end{equation}
因此有限 亏指数 $N$ 时，不同自伴扩张的 预解式 差秩至多为 $N$。后面一维半线上的秩一公式只是 $N=1$ 的情形。
...
\]
解出 $g$ 并代回 $u$，便得到\eqnrefs{eq:math-sl-krein-resolvent-abstract}。整个推导只使用闭对称算子、边界三元组 和 预解式，不依赖任何微分方程结构。
...
\]
是满射。这个结论不能在低正则系数下用任意光滑 截断 的启发构造替代证明；下面的推导会直接用一阶系统和 $L^2_w$ 源项证明任意四个端点数据都可实现。正因为边界迹没有隐藏约束，自伴扩张才等价于在四维边界空间中选择这个辛型的最大零子空间。这里所谓 Lagrangian 子空间，是指边界型在其上恒为零、且复维数恰为总边界空间一半的子空间；“最大零性”正是这一条件的等价表述。结合最小域的内在刻画与迹映射满射性，还立即得到
...
正则端点的边界迹可以直接用函数值和准导数表示；奇异端点则必须改用 Weyl 分类与 亏量 空间 描述扩张自由度。下面把这一端点问题并入同一个自伴实现框架。
...
\[
 ...
 \end{cases}
\]
因此 极限圆型 端点不通过平方可积性删去局部解方向，而 极限点型 端点恰好删去一个局部方向。这个局部维数如何转化为自伴边界条件的个数，将在 亏量 空间 中由算子定义域严格完成。
\end{theorem}
...
这里还可得到后面计算 亏指数 所需的一条定量事实。对极限点 $m(z)$ 定义
\[
 Q_r(m):=
 \frac{\Im m}{\Im z}
...
$L^2(w(x)w(t)\dd x\dd t)$，故对应 Volterra 算子在 $L^2_w$ 上有界。Volterra 迭代积分被有序单纯形限制，其 Neumann--Volterra 级数对任意固定 $z-z_0$ 收敛，于是任意自由项 $a u_0+b v_0\in L^2_w$ 都产生唯一的 $L^2_w$ 解 $y$。因此某个非实参数若为 极限圆型，则整个上半平面都为 极限圆型；实系数下复共轭把结论传到下半平面。反之，若另一参数是 极限圆型，交换 $z,z_0$ 得到相同结论。故分类与所选非实参数无关，而非 极限圆型 的情形只能是前面已经得到的一维 极限点型 情形。


Weyl 分类首先回答局部谱方程还剩多少平方可积解。要把这个信息变成“端点需要多少个自伴边界条件”，还必须进入最小算子与最大算子的定义域商空间；这正是 亏指数 的作用。

这一端点分类可等价地组织为 亏量 空间 与扩张自由度。
...
\[
 ...
 n_-:=\dim\mathcal N_{-\ii}.
\]
由于系数实，复共轭把 $\mathcal N_{\ii}$ 反线性同构到 $\mathcal N_{-\ii}$，故 $n_+=n_-$。一个正则端点和一个 极限圆型 奇异端点都保留一个边界自由度；极限点型 端点不保留边界自由度。下面的定理把这句话变成精确的算子维数计数。

\begin{theorem}[Weyl 分类与自伴扩张自由度]\label{thm:math-sl-weyl-extension-count}
把正则端点和 极限圆型 奇异端点统称为\emph{边界活动端点}，令其个数为 $N\in\{0,1,2\}$。则
\[
 n_+=n_-=N.
\]
因此两端均为 极限点型 时 $T_{\min}$ 本质自伴；只有一个边界活动端点时，自伴实现需要一个实边界条件；两个端点都活动时，需要两个独立但允许相互耦合的自伴边界关系。
\end{theorem}


固定 $z\in\CC\setminus\RR$。方程 $\tau y=zy$ 的全局解空间由任一正则内点处的两个初值
$(y(x_0),y^{[1]}(x_0))$ 唯一决定，因此为二维。一个正则或 极限圆型 端点不对这两个
初值增加平方可积约束；一个 极限点型 端点则要求解的初值落在一条一维 Weyl 子空间中，
...
情形二：恰有一个 极限点型 端点。全局初值必须落在该端点的一维 Weyl 子空间中，
而另一活动端点不再增加约束，故
\[
 \dim\mathcal N_z=1.
\]
...
Weyl 解的常数倍。上一推导已经证明 极限点型 Weyl 解满足自括号极限为零，所以
\[
 [y,y](a)=[y,y](b)=0.
\]
...
三个情形合并即得 $\dim\mathcal N_z=N$。复共轭把上、下半平面的 缺陷 空间互换
...
奇异，但三维整体仍本质自伴（$N=0$），因 亏指数 由无穷远行为决定。
...
von Neumann 分解还把 Weyl 分类与端点 Lagrange 边界型严格接起来。这个连接对 极限点型 端点尤其重要，因为它说明“无需边界条件”不是把某个函数值条件省略掉，而是该端点的边界辛空间本身已经退化为零。

\begin{theorem}[极限点型 端点的边界型消失]\label{thm:math-sl-limit-point-boundary-form}
若 $c$ 是 极限点型 端点，则对任意 $u,v\in\Dom(T_{\max})$ 都有
\[
 [u,v](c)=0.
\]
若 $c$ 是正则或 极限圆型 端点，则该端点贡献一个二维复边界辛空间，自伴实现必须在其中选择一条一维 Lagrangian 直线；两个活动端点同时存在时，也允许在两端的直和边界空间中选择 耦合 Lagrangian 子空间。
\end{theorem}
...
\]
这就证明了 极限点型 端点的完整边界型消失。

若 $b$ 为 极限圆型，则同一区间上两个端点都活动，故
\[
 \dim_\CC\bigl(\Dom(S_{\max})/\Dom(S_{\min})\bigr)=4.
\]
扣除正则端点 $x_0$ 的两个边界坐标后，$b$ 仍留下二维复边界空间。Lagrange 边界型在这个二维空间上非退化；自伴定义域必须选择其中的一维最大零子空间。这正是“一个 极限圆型 端点贡献一个实自伴边界条件”的算子论含义。
在 极限圆型 情形，还可以选取参考解把抽象边界型显式坐标化。

在正则或 极限圆型 端点 $c$，上一节得到的二维边界辛空间可以用一对实参考函数 $u_c,v_c\in\Dom(T_{\max})$ 坐标化，使
\[
 [u_c,v_c](c)=1.
\]
定义
...
\[
 ...
 \qquad \theta\in[0,\pi).
\]
在 极限圆型 奇异端点，这才是普通 Robin 条件的正确替代物。若两个端点都活动，则把两端边界坐标组成四维边界空间，可以像正则二端点情形一样选择 耦合 Lagrangian 子空间。相反，刚得到的 极限点型 边界型消失定理说明该端点已经没有非平凡边界坐标，因此不能再附加一个独立的自伴端点条件。

这一组结论把三种语言完全对应起来：Weyl 分类计算局部平方可积解的维数，亏指数 计算最小算子的自伴扩张自由度，Lagrange 边界型则把这些自由度写成具体端点坐标。半直线 Schrödinger 型算子通常只在有限端点指定一个条件，而不在 极限点型 无穷远再写“波函数等于零”，原因就在这里。
...
投影值测度不是形式上的“带算子积分号”。记 $\mathfrak B(\RR)$ 为实轴的 Borel $\sigma$-代数。一个投影值测度（投影值测度, 投影值测度）是映射
\[
...
\[
 (A+\ii I)^{-1*}=(A-\ii I)^{-1},
\]
且所有 预解式 都是 $A$ 的 Borel 函数而彼此对易。因此
\[
\begin{aligned}
 U_A^*U_A
...
\end{equation*}
其中若 $f$ 有界，右端本来就是紧闭集；对无界连续 $f$，公式按相应闭算子函数演算理解。后面的 预解式 范数、酉传播保范数与热半群衰减率都只是\eqnrefs{eq:math-sl-functional-calculus-norm} 的直接代入。

抽象 投影值测度 还可以进一步化成真正的“乘法算子模型”。固定 $u\in\mathcal H$，定义其循环子空间
...
\]
因此孤立谱对应 预解式 的孤立奇点，而连续谱则表现为无法解析延拓穿过实轴的边界行为；Riesz 与 Stone 公式正好分别抽取这两种信息。
...
\]
因此“Weyl $m$ 函数是 Herglotz 函数”并不是一维微分方程偶然出现的解析性质，而是一般自伴 预解式 的标量压缩所必然具有的性质。反过来，Poisson 核极限可以从 $F_u$ 恢复 $\mu_u$，再通过 极化 恢复全部复测度 $\mu_{u,v}$，最终恢复 投影值测度。
...
连续谱不能用孤立极点包围，但可以从实轴两侧的 预解式 边界值恢复。若 $\alpha<\beta$ 且端点不是 $E_A$ 的原子，则 Stone 公式为
...
\end{equation}
若端点本身是本征值，右端极限还各包含端点原子投影的一半。Stone 公式说明 预解式 在谱附近的边界行为已经携带完整谱测度，而不是只携带本征值位置。
...
\end{equation*}
这里 $G_z$ 是关于 Hilbert 空间测度 $w(\xi)\dd\xi$ 的 预解式 核。把 $A=w^{-1}L$ 与
\[
 Ly:=-(py')'+qy
\]
代回去，可知核必须满足分布方程
...
\end{aligned}
\end{equation}
这个公式没有假设边界条件分离。其第一项是固定左端初值后的因果型 Volterra 核，第二项是为满足全局自伴边界关系而加入的有限秩修正。还需要逐项核验它确实是 预解式 核。对固定 $\xi$，有限秩修正关于 $x$ 是齐次解，因此不会改变 δ 源；$K_z$ 的 Heaviside 跳变给出
\[
 G_z(\xi+0,\xi)=G_z(\xi-0,\xi),
 \qquad
 G_z^{[1]}(\xi+0,\xi)-G_z^{[1]}(\xi-0,\xi)=-1.
...
左右解乘积公式只适用于边界条件在两个端点分离的情形；一般 耦合 自伴条件仍有 预解式 核，但要用二维基本解矩阵和边界矩阵联立求系数。先考虑分离自伴实现。固定 $z\in\rho(A)$，选取 $\phi_a(\cdot,z)$ 满足左端的自伴条件，选取 $\psi_b(\cdot,z)$ 满足右端的自伴条件。在正则端点上，若使用\eqnrefs{eq:math-sl-separated-bc}，可以规范化为
\begin{align}
 \phi_a(a,z)&=\sin\alpha,
...
\begin{equation}\label{eq:math-sl-green-regular-formula}
 ...
 \end{cases}
\end{equation}
该公式同时适用于正则端点、极限圆型 端点和 极限点型 端点，只要 $\phi_a,\psi_b$ 分别采用相应的边界适配解。
\end{theorem}
...
\begin{theorem}[半直线 预解式 核]\label{thm:math-sl-green-halfline}
对 $z\in\rho(A)$，定义
\[
...
$m$ 函数还把“改变左端自伴边界条件”直接转化成 预解式 的有限秩变化。仍在正则--极限点型 半直线上，令 $A_D$ 表示左端 Dirichlet 条件 $y(a)=0$，令 $A_h$ 表示 Robin 条件
\[
 y^{[1]}(a)=h y(a),
 \qquad h\in\RR.
\]
...
\end{equation*}
右端是可分离核，所以两个 预解式 的差至多秩一。这是一维 Krein 预解式 公式最直接的形式：亏指数 等于一时，自伴扩张只有一个边界自由度，相应的 预解式 变化也只有一个秩一通道。它把端点参数、Weyl $m$ 函数、离散极点与谱测度放在同一解析对象中。


\section{闭二次型、Friedrichs 扩张与谱稳定性}

自伴扩张的定义域方法适合追踪边界自由度，但许多 Hamiltonian、椭圆算子和变分问题首先以能量二次型出现。为了不把 Rayleigh 商局限在正则 Sturm--Liouville 区间，先建立闭半有界 形式 的一般理论。设 $\mathfrak a$ 是稠密定义的对称 半双线性 形式，定义域记为 $\mathcal Q(\mathfrak a)\subset\mathcal H$，并假设存在 $m\in\RR$ 使
\[
 \mathfrak a[u,u]\ge m\norm{u}^2.
\]
取任意 $\alpha>-m$，定义 形式 内积
...
\end{equation}
若 $\mathcal Q(\mathfrak a)$ 在此范数下完备，则称 $\mathfrak a$ 为闭 形式。不同的 $\alpha>-m$ 给出等价范数。

第一表示定理说明：每个稠密、闭、下半有界对称 形式 唯一对应一个下半有界自伴算子 $A$，满足
...
\]
并由谱演算定义相应闭 形式。因此“能量有限”的 形式定义域 通常严格大于算子定义域；例如二阶椭圆算子的 形式定义域 只要求一阶弱导数，而算子域还要满足二阶方程与边界条件。
...
\begin{derivation}[极小极大 原理与 形式 单调性]
...
紧 预解式 也有一个不依赖具体 Green 核的 形式 判据。仍取 $\alpha>-m$，把 $\mathcal Q(\mathfrak a)$ 配以 $\norm{\cdot}_{\mathfrak a_\alpha}$。则
\begin{equation}\label{eq:math-sl-compact-form-embedding}
 ...
 \text{ 的自然嵌入紧}.
\end{equation}
这条等价比“某个具体 Green 核恰好平方可积”更普适：Sobolev 的 Rellich 紧嵌入、受限区域椭圆算子、高阶算子和量子 Hamiltonian 都可以通过同一个 形式-定义域 判据获得离散谱。


预解式 $(A+\alpha I)^{-1}$ 可分解为嵌入算子 $J$ 与其伴随 $J^*$ 的复合
$JJ^*$。紧算子的复合仍紧，故 $J$ 紧当且仅当 $JJ^*$ 紧。这一因式分解把算子层的紧性
完全转化为 形式-定义域 到 $\mathcal H$ 的嵌入紧性。

嵌入算子及其伴随。沿用第一表示定理证明中的连续嵌入
\begin{equation*}
 J:\mathcal Q(\mathfrak a_\alpha)\to\mathcal H.
\end{equation*}
$J$ 是恒等嵌入（形式定义域 中的元素视为 $\mathcal H$ 中的元素），连续性由 形式 范数
...
\end{equation*}
定义。$J^*$ 将 $\mathcal H$ 中的向量映射到 形式定义域 中，是 预解式 的 形式-定义域
版本：对固定 $f$，$J^*f$ 是 形式定义域 中唯一满足该恒等式的向量，存在性由
$\mathcal Q(\mathfrak a_\alpha)$ 上 Riesz 表示保证。

预解式 的因式分解。与\eqnrefs{eq:math-sl-form-riesz-resolvent} 比较可知
\begin{equation*}
 J^*f=(A+\alpha I)^{-1}f
\end{equation*}
作为 形式-定义域 中的向量，而再嵌入 $\mathcal H$ 后得到算子恒等式
\begin{equation*}
 (A+\alpha I)^{-1}=JJ^*.
\end{equation*}
验证：对任意 $f,g\in\mathcal H$，
$\langle JJ^*f,g\rangle_{\mathcal H}=\langle J^*f,J^*g\rangle_{\mathfrak a_\alpha}
=\langle f,JJ^*g\rangle_{\mathcal H}$，且 $JJ^*f$ 满足 预解式 的定义恒等式。
这是关键因式分解：预解式 是嵌入与其伴随的复合。
...
\begin{equation*}
 J=(JJ^*)^{1/2}W
\end{equation*}
其中 $W$ 是从 形式 Hilbert 空间到 $\overline{\Ran J}$ 的部分等距映射。有界算子 $W$ 与
...
高阶算子和量子 Hamiltonian 都可以通过同一个 形式-定义域 判据获得离散谱。

Rellich 嵌入应用。取 $A=-\Delta_D$ 于有界 $\Omega\subset\RR^d$，形式定义域
$H^1_0(\Omega)$。Rellich 定理给出 $H^1_0(\Omega)\hookrightarrow L^2(\Omega)$ 紧（当
$\Omega$ 有界且 Lipschitz），故 $(-\Delta_D+\alpha)^{-1}$ 紧，Dirichlet 拉普拉斯算子
...
\begin{equation}\label{eq:math-sl-kato-relative-bound}
 ...
 \qquad u\in\Dom(A),
\end{equation}
则 $A+B$ 在同一算子域 $\Dom(A)$ 上自伴。若 $A$ 还下半有界，则 $A+B$ 也下半有界；这不是额外假设，而可由同一个 相对-束缚 估计在足够负的实 预解式 参数上证明。
...
$\Ran(T-\ii y)=\Ran(T+\ii y)=\mathcal H$，则其两个 亏量 空间 都为零，故 $T$ 自伴。于是 $A+B$ 自伴。
...
形式 层允许更奇异的扰动。设 $\mathfrak a$ 闭且下界为 $m$，$\mathfrak b$ 是定义在同一 形式定义域 上的对称 形式，并满足
\begin{equation}\label{eq:math-sl-klmn-bound}
 |\mathfrak b[u,u]|
 \le a\bigl(\mathfrak a[u,u]-m\norm{u}^2\bigr)+c\norm{u}^2,
 \qquad 0\le a<1.
\end{equation}
则 $\mathfrak a+\mathfrak b$ 仍闭且下半有界，因此由第一表示定理唯一生成自伴算子。这是 KLMN 原理的基本形式。它允许某些势能只相对于动能 形式 有界，而未必把 $\Dom(A)$ 映回 $\mathcal H$。


分三步证明 $\mathfrak a+\mathfrak b$ 闭且下半有界。核心思想是：相对界 $a<1$ 保证
扰动 形式 $\mathfrak b$ 不改变 形式定义域 的拓扑，即新 形式 范数 与原 形式 范数
等价，从而完备性传递。
...
形式 范数 的双向控制。取 $\alpha>c/(1-a)$（此选择保证
...
\[
 ...
 \end{aligned}
\]
左端是新 形式 范数 $\norm{u}_{\mathfrak a+\mathfrak b+\alpha}^2$，右端涉及原 形式
...
形式定义域 $H^1(\RR^3)$。Coulomb 势 $V(r)=-Z/r$ 对应
...
（$V$ 不把 $\Dom(-\Delta)=H^2$ 映回 $L^2$），但 形式 层 KLMN 仍能处理。

KLMN 允许势能比算子域更奇异，只要相对动能 形式 有界。这覆盖了量子力学
...
\]
若两个自伴算子 $A,B$ 在某个共同 预解式 点 $z_0\in\rho(A)\cap\rho(B)$ 满足
...
\end{equation}
这比“$A-B$ 紧”更一般，因为 $A$、$B$ 甚至可以有不同定义域。Krein 公\eqnrefs{eq:math-sl-krein-resolvent-abstract} 表明有限 亏指数 下任意两个自伴扩张自动满足有限秩 预解式 差，因此改变有限维边界通道不会改变本质谱。

\begin{derivation}[紧 预解式 扰动下的本质谱不变性]
...
Krein 公式与边界扰动。有限 亏指数 $N$ 下，Krein 公式
...
故改变有限维边界条件不改变本质谱。实例：半直线上 Dirichlet 与 Robin 拉普拉斯算子
...
若对某个 $z_0\in\rho(A)$，$R(z_0)=(A-z_0I)^{-1}$ 是紧算子，就称 $A$ 具有 紧 预解式；预解式 恒等式说明此时对每个 $z\in\rho(A)$ 都紧。更一般地有：
...
\end{equation}
因此 紧 预解式 自动推出 $\sigma_{\mathrm{ess}}(A)=\varnothing$。正则有限区间的 Sturm--Liouville 算子正属于这一类；在半直线或全线上，离散束缚态与绝对连续散射谱可以同时存在，某些更复杂系数还可能产生奇异连续部分。


固定 $z_0\in\rho(A)$，记 $K=R(z_0)$。由 预解式 是 $A$ 的 Borel 函数
...
正向：紧 预解式 $\Rightarrow$ 纯离散谱。考虑谱映射
\begin{equation*}
 \sigma(K)\setminus\{0\}
...
反向：纯离散谱 $\Rightarrow$ 紧 预解式。反过来，假设 $\sigma(A)$ 只由
...
$\lambda_n=2n+1$，$|\lambda_n|\to\infty$，故 预解式 紧。验证：$R(z_0)$ 的
本征值 $1/(2n+1-z_0)\to0$，有限秩逼近 $R(z_0)P_N$ 的误差
$\sup_{n>N}1/|2n+1-z_0|\to0$。

有界区域 拉普拉斯算子。取 $A=-\Delta_D$ 于有界 $\Omega\subset\RR^d$。
Rellich 紧嵌入给出 预解式 紧，本征值满足 Weyl 渐近律
$N(\lambda)\sim(2\pi)^{-d}\omega_d\,|\Omega|\,\lambda^{d/2}$
（$\omega_d$ 单位球体积），即 $\lambda_n\sim C\,n^{2/d}\to\infty$。

紧 预解式 $\Leftrightarrow$ 纯离散谱 $\Leftrightarrow$ 全部态为束缚态。
量子系统中，离散本征值对应束缚态（波函数平方可积，粒子局域化），本征值趋于无穷
反映能量量子化无上限。连续谱（如自由粒子 $[0,\infty)$）对应散射态，预解式 非紧。
紧 预解式 判据把"离散谱"这一谱性质转化为"预解式 紧"这一算子性质，后者可通过
Rellich 嵌入等几何工具验证，无需先解本征方程。


Green/预解式 语言正好区分这些情况。孤立本征值对应 预解式 的有限秩极点；绝对连续谱通常由 $R(\lambda\pm\ii0)$ 的非平凡边界值和 Stone 公式描述；奇异连续谱没有孤立极点，却仍由谱测度的奇异连续部分保留。

当一般理论下降到正则有限区间时，紧 预解式 把谱积分化为离散级数。
...
在有限正则区间上，任意自伴 Sturm--Liouville 实现 $A$ 都下有界，并且其 预解式 是紧算子。因此谱由实的孤立本征值组成，每个本征值重数有限，没有有限聚点；按重数排列后可写成
\[
...
这里“完备”不是二阶常微分方程局部解空间为二维的后果，而是 紧 预解式 与自伴谱定理的后果；“单重”则额外使用分离边界条件。若采用周期等 耦合 自伴条件，可以出现二重本征值，所以两种结论必须分开。
...
再证明 预解式 紧性。取任意 $z_0\in\rho(A)$。有限正则区间上，基本矩阵\eqnrefs{eq:math-sl-fundamental-matrix} 连续；一般 耦合 Green 核\eqnrefs{eq:math-sl-green-coupled-matrix} 因而在两个闭三角区域上有界，并在对角线连续。因此 $G_{z_0}(x,\xi)$ 在 $[a,b]^2$ 上有界。又因为 $w\in L^1(a,b)$，
\[
 \int_a^b\int_a^b
...
\]
并由 预解式 恒等式可知 $R(z_0)$ 与 $R(\overline{z_0})$ 对易，所以 $R(z_0)$ 是紧正常算子。紧正常算子的谱定理给出由其非零本征向量与零空间共同构成的完备正交分解。这里 $R(z_0)=(A-z_0)^{-1}$ 是单射，所以 $\ker R(z_0)=\{0\}$；同时 $\Ran R(z_0)=\Dom(A)$ 在 $\mathcal H$ 中稠密。因此不需要额外的零本征空间，非零本征向量本身已经完备。若
\[
...
\]
反过来，$AX=\lambda X$ 蕴含 $R(z_0)X=(\lambda-z_0)^{-1}X$。因此 $A$ 与 $R(z_0)$ 拥有同一组本征向量，紧 预解式 的谱定理便给出 $A$ 的离散谱和完备性。由于前面已经证明 $A$ 下有界，本征值序列不可能向 $-\infty$ 逃逸，所以唯一可能的无界聚集方向是 $+\infty$。

若边界条件分离，固定一个 $\lambda$ 后，满足左端边界条件的齐次解空间是一维。两个本征函数都必须属于这同一维空间，所以它们线性相关；右端条件只筛选允许的 $\lambda$，不能增加第二个线性无关本征函数。这才给出分离情形的单重性。

在纯离散情形，谱表示还可直接读出 预解式 极点、留数与变分结构。

正则紧区间的谱测度纯原子，因此 预解式 核的谱表示为
...
\end{equation}
这正是 Riesz 投影\eqnrefs{eq:math-sl-riesz-projection} 的坐标核版本。Green 核的极点并不是额外的“特征方程技巧”，而是离散谱投影在 预解式 中的解析表现。
...
\end{equation}
若该自伴实现半有界，则最低本征值由相应闭二次型的 极小极大 原理得到。一般 Robin 或 耦合 条件需要把与该自伴扩张对应的边界二次型一起纳入，不能无条件沿用\eqnrefs{eq:math-sl-rayleigh-dirichlet}。
...
Stone 定理给出生成元定义域的精确等价：$u\in\Dom(A)$ 当且仅当 $u$ 与 $Au$ 都落在相应 形式 域中（\eqnrefs{eq:math-sl-stone-generator-domain}）。
...
\]
并以弱意义满足二阶方程；$\Dom(A^{1/2})$ 取 图 范数。对足够正则的强解，取内积并使用 $A=A^*\ge0$，可得到不依赖谱型的能量恒等式
...
时间演化与 预解式 之间还可通过 Laplace 变换建立精确联系。
...
算符-束 公式把时间演化统一为 预解式 在复参数
$z=-P(s)$ 处的取值。时间阶数（一阶 Schrödinger、二阶波动、阻尼 Klein--Gordon 等）
只影响标量 $P(s)$，空间谱理论不变。这说明 预解式 是连接时间动力学与空间谱结构
...
\end{equation*}
这说明“时间 Laplace 变换”和“空间 Green 函数”其实通过同一个 预解式 直接相接。

对常阻尼二阶问题，若 $F$ 与由初值产生的解具有足以保证 Laplace 变换存在的指数界，则在满足后述 预解式 条件的右半平面同理得到
...
\end{aligned}
\end{equation}
因此积分核不是新的对象，而是同一 预解式 核在复参数 $z=-s^2-bs$ 处的取值：
...
非齐次边界数据也先从抽象定义域处理。设 $S$ 是闭对称算子，$\mathcal X=\Dom(S^*)$ 配 图 范数，并设
\[
...
弦端位移驱动），提升 将边界控制能量转化为内部源，再用 预解式 或半群公式求解。
...
强 预解式 收敛 首先保证有界连续函数演算以及传播子的强收敛。
...
\end{equation}
则称 $A_n$ 强 预解式 收敛到 $A$，记作 $A_n\xrightarrow{\mathrm{s.r.}}A$。由 预解恒等式 与
...
\begin{equation}\label{eq:math-sl-strong-resolvent-unitary}
 ...
 \longrightarrow0.
\end{equation}
因此 强 预解式 是自伴生成元收敛与有限时间量子/波相位传播收敛的自然拓扑。
...
\[
 ...
 \longrightarrow0.
\]
因此 强 预解式 收敛 实际给出所有有界连续函数演算的逐向量强收敛，而不仅是 $C_0$ 函数演算。
...
强 预解式 并不保证孤立的近似本征值必须靠近 $\sigma(A)$。一个最简单的无限维反例是：在 $\ell^2(\NN)$ 中令 $P_n$ 为到第 $n$ 个标准基向量 $e_n$ 的秩一投影，取
\[
 A_n=P_n,
 \qquad A=0.
\]
...
\[
 \sigma(A_n)=\{0,1\}
\]
中的本征值 $1$ 对所有 $n$ 都存在，却不属于 $\sigma(A)=\{0\}$。相应本征向量 $e_n$ 弱趋于零，谱质量沿无限维方向“逃走”。因此 强 预解式 适合传播与连续谱投影的强收敛，却不足以单独排除这种谱污染。

更强的 范数 预解式 收敛 还能稳定控制孤立谱簇及其重数。

若更强地有
\begin{equation}\label{eq:math-sl-norm-resolvent-def}
 \norm{(A_n-z_0I)^{-1}-(A-z_0I)^{-1}}\longrightarrow0
\end{equation}
对某个 $z_0\in\CC\setminus\RR$ 成立，则称 范数 预解式 收敛。这里括号中的范数是算子范数。由算子范数收敛对每个固定向量立即得到强收敛，所以它蕴含 强 预解式，但多出的“一致性”正是控制孤立谱位置所需的信息。

令 $K\subset\rho(A)$ 为紧集。范数 预解式 收敛 蕴含：对充分大的 $n$，
\begin{equation}\label{eq:math-sl-norm-resolvent-set-stability}
 K\subset\rho(A_n),
 \qquad
 \sup_{z\in K}\norm{R_n(z)-R(z)}\to0.
\end{equation}
因此 极限 预解式 集中的紧区域最终不会出现 $A_n$ 的谱点。若 $\Gamma\subset\rho(A)$ 是包围孤立有限重数谱簇 $\Delta$ 的闭曲线，则相应 Riesz 投影满足
...
特别地，在闭曲线 $\Gamma\subset\rho(A)$ 上有一致 预解式 收敛，于是由
Riesz 投影的围道积分表示，
\[
 \begin{aligned}
 \norm{P_n-P}
...
范数 预解式 排除了 强 预解式 反例中那种固定在 预解式 区域内的伪本征值
（谱污染）。
...
稳定（单重保持）。但 强 预解式 不足以保证：反例中 $A_n$ 有本征值 $1$ 对所有
$n$，而 $1\in\rho(A)$，范数 预解式 排除此谱污染。

物理与数值意义。范数 预解式 收敛 是谱近似理论的黄金标准：它保证
近似算子（有限维截断、网格离散、正则化）的孤立本征值位置和重数都收敛到真值。
这为数值谱计算（有限元、有限差分、Galerkin 方法）提供严格基础：只要近似在 范数
预解式 意义下收敛，孤立能级的计算就稳定可靠。强 预解式 仅保证传播子
强收敛（量子演化连续依赖），但不足以控制离散谱——谱质量可能"逃逸"到无穷维方向，
对应数值伪模。范数 预解式 通过一致估计排除这种逃逸，是谱计算收敛性的正确拓扑。

\section{一般理论之后的可解模型}

本节的模型只用于检验前面建立的母结构。每个模型都先明确自伴定义域，再从一般 预解式 或谱演算公式中取特例，而不反过来由特殊解推断一般理论。
...
\end{exbox}
\begin{exbox}[半直线常势算子的 预解式 核]\label{ex:math-sl-halfline-resolvent}
在 $L^2(0,\infty)$ 上取
\[
 A=-\frac{\dd^2}{\dd x^2}+\nu^2,
 \qquad \nu>0,
...
\]
因此时间变换后的空间问题没有产生新 Green 理论，它只是对同一个 预解式 在参数 $z=-s$ 处取值。
\end{exbox}
...
同一个 Borel 函数演算在纯点谱与绝对连续谱中只改变“谱坐标”的写法。若 $A$ 具有 紧 预解式，\eqnrefs{eq:math-sl-duhamel-semigroup} 与\eqnrefs{eq:math-sl-abstract-second-order-formal} 分别成为
...
本章最终得到一条闭合的算子链。系数与权函数先确定微分表达式；最小/最大算子和 Lagrange 边界型再确定允许的自伴扩张；Weyl 分类判断奇异端点还留下多少边界自由度。自伴算子确定以后，谱测度统一产生 Borel 函数演算、预解式 与含时传播，Green 核只是 预解式 的坐标积分核。有限正则区间的特征函数级数与全线 Fourier 积分因此不是两套理论，而是同一谱积分在不同谱型下的具体实现。
