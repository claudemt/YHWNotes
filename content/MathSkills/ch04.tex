...
\begin{equation}\label{eq:math-sl-weighted-space}
 \mathcal H=L^2(I,w\dd x),\qquad
...
\begin{equation}\label{eq:math-sl-homogeneous-spectral}
 -(py')'+qy=zwy,
\end{equation}
其中 $z\in\CC$ 用作一般复谱参数，而 $\lambda\in\RR$ 留给自伴算子的实谱值。

接下来考察最大算子、最小算子与 Lagrange 边界型。
...
微分表达式允许的最大自然定义域为
\begin{equation}\label{eq:math-sl-maximal-domain}
\begin{aligned}
 \Dom(T_{\max})
...
\begin{equation}\label{eq:math-sl-minimal-operator}
 T_{\min}:=\overline{T_0}.
\end{equation}
...
\begin{equation}\label{eq:math-sl-bracket}
 [u,v](x):=u(x)\overline{v^{[1]}(x)}-u^{[1]}(x)\overline{v(x)}.
\end{equation}
...
\begin{derivation}[自伴扩张的完整理论：从微分表达式经 deficiency indices 到边界条件分类]
...
\begin{equation}\label{eq:math-sl-preminimal-dense}
 \overline{\Dom(T_0)}^{\mathcal H}=\mathcal H.
\end{equation}
...
接下来考察闭对称算子的 Cayley--von Neumann 母结构。
...
\]
并定义
\begin{equation}\label{eq:math-sl-boundary-triplet-explicit}
 \Gamma_0u=x+y,
 \qquad
 \Gamma_1u=\ii(x-y).
\end{equation}
...
\begin{equation}\label{eq:math-sl-gamma-weyl-operator}
 \gamma(z):=(\Gamma_0|_{\mathcal N_z})^{-1}:\mathcal G\to\mathcal H,
 \qquad
...
\emph{解析性。}由 resolvent 在 $\rho(A_0)$ 上算子范数解析以及
...
\emph{一维特例。}取 $S=-\dd^2/\dd x^2$ 于 $L^2(0,\infty)$，$0$ 端正则、$\infty$ 端
...
\emph{物理意义。}在量子散射中，$m(z)$ 的虚部正比于谱测度的密度（Stone 公式），
...
\begin{equation}\label{eq:math-sl-regular-minimal-domain}
 \Dom(T_{\min})
 =\{y\in\Dom(T_{\max}):
...
先证明正文使用的完整迹映射满射性。令 $M_0(x)$ 是齐次一阶系统
\[
 Y'=A_0(x)Y,
 \qquad
...
\]
即
\begin{equation}\label{eq:math-sl-weyl-circle-identity}
 \frac{\Im m_r}{\Im z}
...
\emph{情形一：两个端点都活动。}没有局部方向被平方可积性删去，所以
\[
 \dim\mathcal N_z=2.
\]
...
\emph{情形二：恰有一个 limit-point 端点。}全局初值必须落在该端点的一维 Weyl 子空间中，
而另一活动端点不再增加约束，故
\[
 \dim\mathcal N_z=1.
\]
...
\emph{情形三：两端均为 limit-point。}两个一维平方可积条件原则上可能相交，也可能不相交，
...
\emph{物理意义。}deficiency index $N$ 等于"缺少的边界数据个数"。$N=0$ 时算子本质自伴，
...
接下来考察投影值测度与 Borel functional calculus。
...
\begin{equation}\label{eq:math-sl-spectral-measure}
 A=\int_{\RR}\lambda\,\dd E_A(\lambda),
 \qquad
...
\begin{equation}\label{eq:math-sl-spectrum-support-pvm}
 \lambda_0\in\sigma(A)
...
\begin{equation}\label{eq:math-sl-eigenprojection-pvm}
 \Ran E_A(\{\lambda_0\})=\ker(A-\lambda_0I).
\end{equation}
...
\begin{derivation}[谱定理的完整推导：从 Cayley 变换经 PVM 到谱测度与投影算符]
...
\begin{equation}\label{eq:math-sl-continuous-spectral-mapping}
...
\emph{物理意义。}谱分解 $\mathcal H=\bigoplus_jL^2(\RR,\dd\mu_{u_j})$ 把抽象自伴算子
...
接下来考察resolvent、Riesz 投影与 Stone 公式。
...
\begin{equation}\label{eq:math-sl-stieltjes-vector-weak}
...
\emph{Herglotz 表示。}$F_u$ 在上半平面 Herglotz，故有唯一积分表示
\[
...
\emph{物理意义。}$\mu_u$ 是态 $u$ 下可观测量 $A$ 的谱分布：$\mu_u(B)=\norm{E_A(B)u}^2$
...
接下来考察resolvent 的 Green 核与 Sturm--Liouville 特化。

先固定积分核相对于带权测度的归一化。

回到由 $\tau$ 产生的一个自伴实现 $A$。算子方程
\begin{equation}\label{eq:math-sl-forced-operator}
 (A-zI)y=g,
 \qquad z\in\rho(A),
\end{equation}
的唯一解是 $y=R(z)g$。对一维 Sturm--Liouville 算子，resolvent 可以写成积分算子
\begin{equation}\label{eq:math-sl-resolvent-eq}
 [R(z)g](x)
...
若原问题写成不除权函数的形式
\begin{equation}\label{eq:math-sl-forced-bvp}
 Ly=zwy+f,
\end{equation}
则 $g=f/w$，所以 $w$ 在积分中抵消：
\begin{equation}\label{eq:math-sl-green-representation}
 y(x)=\int_I G_z(x,\xi)f(\xi)\dd\xi.
\end{equation}
...
\]
于是定义边界特征矩阵
\begin{equation}\label{eq:math-sl-boundary-characteristic-matrix}
 B_{C,D}(z):=C G_0(z)+D G_1(z).
\end{equation}
有
...
\begin{derivation}[Green 函数与谱密度的完整理论：从基本矩阵经 Lagrange 恒等式到 Herglotz 性]
...
\begin{equation}\label{eq:math-sl-bilinear-wronskian}
 \mathcal W[u,v](x):=u^{[1]}(x)v(x)-u(x)v^{[1]}(x).
\end{equation}
...
记
\begin{equation}\label{eq:math-sl-omega-def}
 \omega(z):=\mathcal W[\phi_a,\psi_b]
 =\phi_a^{[1]}\psi_b-\phi_a\psi_b^{[1]}.
\end{equation}
若两端边界条件分离，则
\begin{equation}\label{eq:math-sl-eigen-iff-omega-zero}
 z\text{ 是该边界问题的本征值}
 \quad\Longleftrightarrow\quad
 \omega(z)=0.
\end{equation}
...
\]
则
\begin{equation}\label{eq:math-sl-green-halfline}
 G_z(x,\xi)=\frac{1}{\omega_a(z)}
 \begin{cases}
...
\end{equation}
在相应的谱变换下，
\begin{equation}\label{eq:math-sl-weyl-transform}
 \widehat f(\lambda)
...
\begin{equation}\label{eq:math-sl-m-robin-spectrum}
 m(z)=h,
\end{equation}
而 Dirichlet 与 Robin Green 核之差为
\begin{equation}\label{eq:math-sl-krein-rank-one-kernel}
 G_{h,z}(x,\xi)-G_{D,z}(x,\xi)
...
\begin{equation}\label{eq:math-sl-first-representation-theorem}
 \Dom(A)\subset\mathcal Q(\mathfrak a),
 \qquad
...
\begin{derivation}[二次型理论：从闭 form 构造自伴算子与 KLMN 定理]
...
\emph{Dirichlet Laplacian 实例。}取 $S=-\Delta$ 于 $C_c^\infty(\Omega)\subset L^2(\Omega)$，
...
\emph{Neumann 比较。}同一 form 不要求零迹的完备化给出 $H^1(\Omega)$，对应 Neumann
...
\emph{物理意义。}能量 form $\mathfrak s[u,u]$ 是态 $u$ 的期望能量。Friedrichs extension
...
\begin{derivation}[谱的几何与分析：min--max 原理、紧嵌入与 Kato--Rellich 扰动]
...
\emph{上界：取试探子空间。}取 $L_k=\operatorname{span}\{e_1,\ldots,e_k\}$。对任意
...
\emph{下界：任意子空间必遇高能方向。}反过来，任取 $k$ 维子空间
...
\emph{form 单调性。}若 $\mathfrak a_1\le\mathfrak a_2$（同一 form domain 上逐点
...
\emph{谐振子实例。}取 $A=-\dd^2/\dd x^2+x^2$ 于 $L^2(\RR)$（谐振子 Hamiltonian）。
...
\emph{物理意义。}min--max 原理把第 $k$ 个能级表述为"在最优 $k$ 维试探空间中的最高能量"。
...
\emph{思路：}resolvent $(A+\alpha I)^{-1}$ 可分解为嵌入算子 $J$ 与其伴随 $J^*$ 的复合
$JJ^*$。紧算子的复合仍紧，故 $J$ 紧当且仅当 $JJ^*$ 紧。这一因式分解把算子层的紧性
完全转化为 form-domain 到 $\mathcal H$ 的嵌入紧性。

\emph{第一步：嵌入算子及其伴随。}沿用第一表示定理证明中的连续嵌入
\begin{equation*}
...
\emph{第二步：resolvent 的因式分解。}与\eqnrefs{eq:math-sl-form-riesz-resolvent} 比较可知
\begin{equation*}
 J^*f=(A+\alpha I)^{-1}f
\end{equation*}
...
\emph{第三步：方向一 $J$ 紧 $\Rightarrow$ $JJ^*$ 紧。}若 $J$ 紧，则 $J^*$ 紧（紧算子的
伴随仍紧：若 $f_n\rightharpoonup0$ 则 $J^*f_n\rightharpoonup0$ 且由紧性强收敛到 $0$），
故 $JJ^*$ 紧（紧算子与有界算子的复合紧：$JJ^*$ 把有界集映为预紧集），即
$(A+\alpha I)^{-1}$ 紧。

\emph{第四步：方向二 $JJ^*$ 紧 $\Rightarrow$ $J$ 紧。}反过来，若 $JJ^*$ 紧，则正算子
...
\emph{结论。}两个方向合并即证明\eqnrefs{eq:math-sl-compact-form-embedding}。这条等价比
"某个具体 Green 核恰好平方可积"更普适：Sobolev 的 Rellich 紧嵌入、受限区域椭圆算子、
高阶算子和量子 Hamiltonian 都可以通过同一个 form-domain 判据获得离散谱。

\emph{Rellich 嵌入应用。}取 $A=-\Delta_D$ 于有界 $\Omega\subset\RR^d$，form domain
...
\emph{物理意义。}紧嵌入 $\mathcal Q(\mathfrak a)\hookrightarrow\mathcal H$ 意味着
...
\emph{第一步：平移到 \(m=0\)。}若 \(\mathfrak a\) 的下界 \(m\neq0\)，定义
...
\emph{第二步：form norm 的双向控制。}取 \(\alpha>c/(1-a)\)（此选择保证
...
\emph{第三步：完备性与表示定理。}\(\mathfrak a\) 闭故 \(\mathcal Q(\mathfrak a)\) 在
form norm \(\norm{\cdot}_{\mathfrak a+\alpha}\) 下完备。由第二步的范数等价，
...
\emph{Coulomb 势应用。}取 $A=-\Delta$ 于 $L^2(\RR^3)$，$\mathfrak a[u,u]=\int|\nabla u|^2$，
...
\emph{物理意义。}KLMN 允许势能比算子域更奇异，只要相对动能 form 有界。这覆盖了量子力学
...
\begin{derivation}[本质谱理论与谱分解：Fredholm 扰动、Weyl 判据与离散谱]
...
\emph{Weyl 定理应用。}若 $B=A+K$ 其中 $K$ 紧（特别地有限秩），取共同 $z_0$ 使
...
\emph{Krein 公式与边界扰动。}有限 deficiency index $N$ 下，Krein 公式
...
\begin{equation}\label{eq:math-sl-selfadjoint-no-residual}
 \sigma_{\mathrm r}(A)=\varnothing.
\end{equation}
...
\begin{equation}\label{eq:math-sl-pp-eigenvector-closure}
...
\emph{正向：紧 resolvent $\Rightarrow$ 纯离散谱。}考虑谱映射
\begin{equation*}
 \sigma(K)\setminus\{0\}
...
\emph{反向：纯离散谱 $\Rightarrow$ 紧 resolvent。}反过来，假设 $\sigma(A)$ 只由
...
\emph{物理意义。}紧 resolvent $\Leftrightarrow$ 纯离散谱 $\Leftrightarrow$ 全部态为束缚态。
...
\begin{equation}\label{eq:math-sl-operator-eigenproblem}
 AX=\lambda X,
 \qquad X\in\Dom(A),
\end{equation}
等价于
\begin{equation}\label{eq:math-sl-regular-eig}
 -(pX')'+qX=\lambda wX.
\end{equation}
若强调具体微分实现，也可记
\begin{equation}\label{eq:math-sl-A-regular}
 AX=\tau X=\frac{1}{w}\bigl[-(pX')'+qX\bigr]
\end{equation}
并把自伴边界条件纳入 $\Dom(A)$。
...
\begin{equation}\label{eq:math-sl-orthogonality}
 \langle X_m,X_n\rangle_w
 =\int_a^bX_m(x)\overline{X_n(x)}w(x)\dd x=0
 \qquad(m\neq n).
\end{equation}
若把本征函数按任意非零范数保留，则任意 $f\in L^2_w$ 都有
\begin{equation}\label{eq:math-sl-SL-expansion}
 f=\sum_{n=1}^\infty
...
\begin{equation}\label{eq:math-sl-spectral-green}
 G_z(x,\xi)
 =\sum_n
...
\begin{equation}\label{eq:math-sl-fredholm-regular}
 \int_a^b f(x)\overline{X_n(x)}\dd x=0.
\end{equation}
...
\begin{derivation}[时间演化理论：从 Stone 定理经谱方程到 RAGE 逸散定理]
...
\begin{equation}\label{eq:math-sl-second-order-abstract}
 u_{tt}+Au=F(t),\qquad u(0)=u_0,\quad u_t(0)=u_1.
\end{equation}
...
\begin{equation}\label{eq:math-sl-wave-energy-balance}
 \frac{\dd}{\dd t}\,
...
\begin{equation}\label{eq:math-sl-damped-energy-balance}
 \frac{\dd}{\dd t}\,
 \frac12\left(\norm{u_t}^2+\norm{A^{1/2}u}^2\right)
...
\]
则
\begin{equation}\label{eq:math-sl-Phi-Psi-explicit}
 \Psi_\lambda(t)=\frac{\ee^{r_+t}-\ee^{r_-t}}{r_+-r_-},
 \qquad
...
\begin{equation}\label{eq:math-sl-expand-all}
 u(t)=\sum_n a_n(t)X_n,
 \qquad
 F(t)=\sum_nF_n(t)X_n
\end{equation}
代入谱演算，只是把投影值测度的原子积分写成求和。对一阶问题得到
\begin{equation}\label{eq:math-sl-mode-first-order}
 a_n'+\lambda_n a_n=F_n(t),
\end{equation}
从而
\begin{equation}\label{eq:math-sl-mode-first-order-solution}
 a_n(t)=\ee^{-\lambda_n t}a_n(0)
 +\int_0^t\ee^{-\lambda_n(t-s)}F_n(s)\dd s,
\end{equation}
以及
\begin{equation}\label{eq:math-sl-first-order-series-solution}
 u(t)=\sum_n
...
对阻尼二阶问题，逐模方程为
\begin{equation}\label{eq:math-sl-mode-second-order}
 a_n''+ba_n'+\lambda_n a_n=F_n(t),
\end{equation}
其解为
\begin{equation}\label{eq:math-sl-mode-second-order-solution}
 a_n(t)=\Phi_{\lambda_n}(t)u_{0,n}
 +\Psi_{\lambda_n}(t)u_{1,n}
 +\int_0^t\Psi_{\lambda_n}(t-s)F_n(s)\dd s,
\end{equation}
于是
\begin{equation}\label{eq:math-sl-second-order-series-solution}
 u(t)=\sum_n
...
\begin{equation}\label{eq:math-sl-ac-weak-decay}
 \langle\ee^{-\ii tA}u,v\rangle
...
\begin{equation}\label{eq:math-sl-ac-decay-rate}
 |\langle\ee^{-\ii tA}u,v\rangle|
 \le |t|^{-m}\norm{g_{u,v}^{(m)}}_{L^1},
 \qquad t\ne0.
\end{equation}
...
\begin{equation}\label{eq:math-sl-semigroup-gap}
 \norm{\ee^{-tA}(I-P_0)}
...
\begin{equation}\label{eq:math-sl-semigroup-resolvent-laplace}
 (A+sI)^{-1}
...
\emph{一阶特例。}取 $P(s)=s$（$m=1$，$a_1=1$，$a_0=0$），对应
...
\emph{波动方程特例。}取 $P(s)=s^2$（$m=2$，$a_2=1$，$a_1=a_0=0$），对应
...
\emph{常阻尼特例。}取 $P(s)=s^2+bs$（$m=2$，$a_2=1$，$a_1=b$，$a_0=0$），对应
...
\emph{物理意义。}operator-pencil 公式把时间演化统一为 resolvent 在复参数
...
\]
于是
\begin{equation}\label{eq:math-sl-laplace-first}
 (sI+A)\widehat u(s)=u_0+\widehat F(s),
\end{equation}
所以
...
\begin{equation}\label{eq:math-sl-laplace-first-kernel}
 \widehat u(x,s)=\int_I
...
\begin{equation}\label{eq:math-sl-laplace-second}
 (s^2+bs+A)\widehat u(s)=(s+b)u_0+u_1+\widehat F(s).
\end{equation}
只要
\[
 -s^2-bs\in\rho(A),
...
\begin{equation}\label{eq:math-sl-laplace-second-kernel}
 \widehat u(x,s)=\int_I G_{-s^2-bs}(x,\xi)
...
\begin{derivation}[算子收敛与谱稳定性：从 strong/norm resolvent 到函数演算与 Riesz 投影]
...
\emph{限制映射的双射性。}若 $x\in\mathcal X_1$ 且 $\mathcal Bx=0$，则
...
\emph{非齐次 Robin 实例。}取 $S=-\dd^2/\dd x^2$ 于 $(0,\ell)$，Dirichlet 扩张
...
\emph{物理意义。}bounded lifting 把非齐次边界控制转化为体源项，使问题回到固定自伴
...
\begin{equation}\label{eq:math-sl-nonhom-bc-general}
 Ly=zwy+f
\end{equation}
与分离边界数据
\begin{equation}\label{eq:math-sl-nonhom-bc-general-2}
 B_ay=g_a,\qquad B_by=g_b.
\end{equation}
选取一个足够正则的 lifting $h_b$，满足
\begin{equation}\label{eq:math-sl-lifting-problem}
 B_ah_b=g_a,\qquad B_bh_b=g_b,
\end{equation}
并令
\begin{equation}\label{eq:math-sl-y-v-plus-w}
 y=v+h_b.
\end{equation}
此时 $v$ 满足齐次自伴边界条件，而
\begin{equation}\label{eq:math-sl-v-equation-after-lift}
 Lv=zwv+\widetilde f,
 \qquad
...
\emph{有限差分实例。}取 $A=-\dd^2/\dd x^2$ 于 $L^2(0,\pi)$ Dirichlet，本征值
...
\emph{物理与数值意义。}norm resolvent convergence 是谱近似理论的黄金标准：它保证
...
\begin{example}[常系数分离边界的特征方程]\label{ex:math-sl-regular-robin-eigen}
...
\begin{equation}\label{eq:math-sl-general-char-constant}
 \det\begin{pmatrix}
 a_1&a_2p_0\\
...
\begin{example}[同一问题的 Green 核]\label{ex:math-sl-green-robin-constant}
...
接下来考察半直线常势：Weyl 解与束缚态极点。

\begin{example}[半直线常势算子的 resolvent 核]\label{ex:math-sl-halfline-resolvent}
在 $L^2(0,\infty)$ 上取
\[
 A=-\frac{\dd^2}{\dd x^2}+\nu^2,
 \qquad \nu>0,
...
\begin{example}[Laplace--Green 方法作用于半直线演化]\label{ex:math-sl-laplace-green-halfline}
...
\begin{example}[全空间自由算子的 Green 核]\label{ex:math-sl-line-free-green}
取
\[
 A=-\frac{\dd^2}{\dd x^2}+\nu^2
 \quad\text{on }L^2(\RR),
...
\begin{equation}\label{eq:math-sl-whole-line-green-explicit}
 G_z(x,\xi)=\frac{1}{2\kappa(z)}\ee^{-\kappa(z)|x-\xi|}.
\end{equation}
...
\begin{equation}\label{eq:math-sl-free-line-spectral-density}
 \frac{\dd\mu_f}{\dd\lambda}(\lambda)
 =\frac{
...
\begin{equation}\label{eq:math-sl-fourier-first}
 \widehat u(t,k)=\ee^{-t\mathfrak a(k)}\widehat u_0(k)
...
\begin{equation}\label{eq:math-sl-fourier-second}
 \widehat u(t,k)=\Phi_{\mathfrak a(k)}(t)\widehat u_0(k)
...
\begin{example}[纯点谱与 Fourier 谱的同一传播子]
...
\begin{equation}\label{eq:math-sl-example-first-general-solution}
 u(t)=\sum_n\left[
 \ee^{-\lambda_nt}u_{0,n}
 +\int_0^t\ee^{-\lambda_n(t-s)}F_n(s)\dd s
 \right]X_n,
\end{equation}
以及
\begin{equation}\label{eq:math-sl-example-second-general-solution}
 u(t)=\sum_n\left[
...
\begin{equation}\label{eq:math-sl-fourier-example-first}
 \widehat u(t,k)=\ee^{-t\mathfrak a(k)}\widehat u_0(k)
 +\int_0^t\ee^{-(t-s)\mathfrak a(k)}\widehat F(s,k)\dd s,
\end{equation}
并在物理空间成为
\begin{equation}\label{eq:math-sl-fourier-example-first-physical}
 u(x,t)=\frac{1}{\sqrt{2\pi}}\int_{\RR}\ee^{\ii xk}
 \left[\ee^{-t\mathfrak a(k)}\widehat u_0(k)
 +\int_0^t\ee^{-(t-s)\mathfrak a(k)}\widehat F(s,k)\dd s\right]\dd k.
\end{equation}
二阶传播则为
\begin{equation}\label{eq:math-sl-fourier-example-second}
 \widehat u(t,k)=\Phi_{\mathfrak a(k)}(t)\widehat u_0(k)
 +\Psi_{\mathfrak a(k)}(t)\widehat u_1(k)
 +\int_0^t\Psi_{\mathfrak a(k)}(t-s)\widehat F(s,k)\dd s.
\end{equation}
...
\begin{example}[含时非齐次边界条件的齐次化]\label{ex:math-sl-time-lifting-template}
...
\label{eq:selfadjoint-resolvent-bound}
\end{equation}
...
\]
立即得到 \cref{eq:schmidt-decomposition}。
