...
物理意义。在量子散射中，$m(z)$ 的虚部正比于谱测度的密度（Stone 公式），
...
物理意义。deficiency index $N$ 等于"缺少的边界数据个数"。$N=0$ 时算子本质自伴，
...
物理意义。谱分解 $\mathcal H=\bigoplus_jL^2(\RR,\dd\mu_{u_j})$ 把抽象自伴算子
...
物理意义。$\mu_u$ 是态 $u$ 下可观测量 $A$ 的谱分布：$\mu_u(B)=\norm{E_A(B)u}^2$
...
Dirichlet Laplacian 实例。取 $S=-\Delta$ 于 $C_c^\infty(\Omega)\subset L^2(\Omega)$，
...
物理意义。能量 form $\mathfrak s[u,u]$ 是态 $u$ 的期望能量。Friedrichs extension
...
谐振子实例。取 $A=-\dd^2/\dd x^2+x^2$ 于 $L^2(\RR)$（谐振子 Hamiltonian）。
...
物理意义。min--max 原理把第 $k$ 个能级表述为"在最优 $k$ 维试探空间中的最高能量"。
...
物理意义。紧嵌入 $\mathcal Q(\mathfrak a)\hookrightarrow\mathcal H$ 意味着
...
物理意义。KLMN 允许势能比算子域更奇异，只要相对动能 form 有界。这覆盖了量子力学
...
物理意义。本质谱对应散射态：波包不被囚禁，传播到无穷远。Weyl 定理保证
...
谐振子实例。取 $A=-\dd^2/\dd x^2+x^2$ 于 $L^2(\RR)$（谐振子）。本征值
...
物理意义。紧 resolvent $\Leftrightarrow$ 纯离散谱 $\Leftrightarrow$ 全部态为束缚态。
...
物理意义。operator-pencil 公式把时间演化统一为 resolvent 在复参数
...
非齐次 Robin 实例。取 $S=-\dd^2/\dd x^2$ 于 $(0,\ell)$，Dirichlet 扩张
...
物理意义。bounded lifting 把非齐次边界控制转化为体源项，使问题回到固定自伴
...
有限差分实例。取 $A=-\dd^2/\dd x^2$ 于 $L^2(0,\pi)$ Dirichlet，本征值
