...
由此，自旋联络 与 Yang--Mills 势在规范群、变换、场强（曲率 $R_{\mu\nu}$ 对应 $F_{\mu\nu}$）各层平行；差异在于引力另有标架 $e_\mu^a$ 这一 soldering 形式，且 $\omega$ 由无挠条件 $T^a=de^a+\omega^a{}_b\wedge e^b=0$ 唯一确定为 Levi--Civita 自旋联络。沿闭合曲线的 Wilson 线 $W(C)=\mathcal{P}\exp\oint_C\omega_\mu\dd x^\mu$ 给出 Lorentz 和乐，曲率即其无穷小生成元。
\end{derivation}
...
因 $\nabla_\mu$ 含联络项，此式一般不能直接积分为守恒荷；需 Killing 矢量 $K^\mu$（$\nabla_{(\mu}K_{\nu)}=0$）使 $J_K^\mu=T^{\mu\nu}K_\nu$ 散度为零，才得守恒荷 $Q_K=\int_\Sigma T^{\mu\nu}K_\nu\,\dd\Sigma_\mu$：Minkowski 的 10 个 Killing 矢量给出能量-动量与角动量守恒。引力能量不能局部化，只能在渐近平直极限定义 ADM 质量。
\end{derivation}
