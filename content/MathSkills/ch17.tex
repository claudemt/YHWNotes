...
\begin{derivation}[从协变条件导出 自旋联络 的变换律]
...
\end{equation}
写成 Lorentz 矩阵形式即 $\omega'_\mu=\Lambda\,\omega_\mu\Lambda^{-1}-(\partial_\mu\Lambda)\Lambda^{-1}$：第一项是伴随共轭，第二项 $(\partial_\mu\Lambda)\Lambda^{-1}$ 是非齐次项，补偿局域 标架 的变化。这与电磁规范势 $A'_\mu=\Lambda A_\mu\Lambda^{-1}-(\partial_\mu\Lambda)\Lambda^{-1}$ 同构——自旋联络 正是 Lorentz 规范群 $SO(1,3)$ 的规范势。

由此，自旋联络 与 Yang--Mills 势在规范群、势、变换、场强（曲率 $R_{\mu\nu}$ 对应 $F_{\mu\nu}$）、协变导数各层完全平行，可借用 Wilson 圈、规范固定、BRST 等全套规范理论工具；差异在于引力另有 四脚标架 $e_\mu^a$ 这一 soldering 形式。Tetrad 与 自旋联络 由无挠条件 $T^a=de^a+\omega^a{}_b\wedge e^b=0$ 联系：给定 $e^a{}_\mu$，$\omega$ 被唯一确定为 Levi--Civita 自旋联络。沿闭合曲线 $C$，$W(C)=\mathcal{P}\exp\oint_C\omega_\mu\dd x^\mu$ 给出 Lorentz 和乐；对无穷小回路 $W\approx1+\frac12R_{\mu\nu}\sigma^{\mu\nu}$，曲率即 和乐 的无穷小生成元。
\end{derivation}
...
\begin{derivation}[从 微分同胚 不变性 导出协变守恒律]
...
自由度计数：$A^\mu$ 的 4 个 Lorentz 分量被一个标量约束 $\partial_\mu A^\mu=0$ 降为 3 个独立极化 $\lambda=-1,0,+1$，即 有质量 自旋-1 的 小群 $SO(3)$ 的 $2s+1=3$ 个态；$\lambda=\pm1$ 为横向、$\lambda=0$ 为纵向。当 $m=0$ 时本推导失效（需 $m\neq0$），此时 规范 冗余 $A_\mu\to A_\mu+\partial_\mu\alpha$ 额外消除纵向自由度，只剩 $\lambda=\pm1$；$m\to0$ 纵向极化被 规范 吸收，即电弱破缺中 $W/Z$ "吃掉" Goldstone 玻色子的运动学机制。

Stueckelberg 机制把 Proca 改写为形式规范不变的
\begin{equation*}
\mathcal{L}_{\mathrm{St}}=-\tfrac14F_{\mu\nu}F^{\mu\nu}+\tfrac12m^2(A_\mu-\partial_\mu\phi)(A^\mu-\partial^\mu\phi),
\end{equation*}
在 $A_\mu\to A_\mu+\partial_\mu\alpha,\phi\to\phi+\alpha$ 下不变；幺正 规范 $\phi=0$ 恢复 Proca，$\phi$ 即被吃掉的 Goldstone 模。动量空间 Proca 传播子
\begin{equation*}
D_{\mu\nu}(p)=\frac{-i}{p^2-m^2+i\epsilon}\left(g_{\mu\nu}-\frac{p_\mu p_\nu}{m^2}\right)
\end{equation*}
括号内正是极化完备 $\sum_\lambda\epsilon_\mu^{(\lambda)}\epsilon_\nu^{(\lambda)*}$，且 $p_\mu p_\nu/m^2$ 项在高能不随 $p^2$ 增长，是 有质量 矢量场可重整的关键。对一般动量 $p^\mu=(E,0,0,p)$，极化矢量可取 $\epsilon^{(\pm1)}=\frac1{\sqrt2}(0,1,\pm i,0)$、$\epsilon^{(0)}=\frac1m(p,0,0,E)$，验证 $p^\mu\epsilon_\mu^{(\lambda)}=0$；纵向 $\epsilon^{(0)}\sim p_\mu/m$ 在高能主导，正是等效定理 $A(W_LW_L\to W_LW_L)\simeq A(\phi\phi\to\phi\phi)$ 的运动学根源。$R_\xi$ 规范族 $D^{(\xi)}_{\mu\nu}(p)=\frac{-i}{p^2-m^2+i\epsilon}\bigl(g_{\mu\nu}-(1-\xi)\frac{p_\mu p_\nu}{p^2-\xi m^2}\bigr)$ 在不同 $\xi$ 下物理振幅相同（Ward 恒等式保证），$\xi=1$ 时传播子最简。
\end{derivation}
