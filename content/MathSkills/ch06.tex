...
目标是用 Atkinson 观点统一二择一与稳定性：$T$ 为 Fredholm 当且仅当它模紧算子可逆，且该性质对紧扰动稳定。设 $T$ Fredholm。把定义域分解为
\[
X=\ker T\oplus X_1,
\]
再把值域空间分解为
...
反过来，若存在 $S$ 满足\eqnrefs{eq:math-fredholm-parametrix-mod-compact}，则 $ST=I+K_1$、$TS=I+K_2$ 都是恒等式加紧算子。前面的紧性论证保证它们的核有限维且值域闭，进而推出 $T$ 的核与余核都有限维，故 $T$ Fredholm。
...
\]
所以 $T+C$ 仍 Fredholm。指标不变性由路径 $T+tC$ 在 Fredholm 集合中的局部常值性得到：$t\mapsto\operatorname{ind}(T+tC)$ 是连续整值函数，故为常数。于是“有限维缺陷对紧扰动稳定”不再是积分核问题的特殊现象，而是 Fredholm 范畴的一般拓扑性质。
\end{derivation}
...
目标是解释解析 Fredholm 定理中奇性为何只剩有限维。固定一个可能不可逆点 $z_*$。因 $A(z_*)$ Fredholm 指标为 $0$，可作有限维缺陷分解
\[
X=N\oplus X_1,
\qquad
X=R\oplus Y_0,
...
\dim N=\dim Y_0<\infty.
\]
在 $X_1\to R$ 这块，$A(z_*)$ 可逆；解析扰动后，在 $z_*$ 的小邻域仍可逆。于是通过块高斯消元，把 $A(z)$ 的可逆性归约为一个有限维矩阵
\[
M(z):N\to Y_0
\]
...
\]
因此只产生有限阶极点。把消元块重新乘回，就得到 $A(z)^{-1}$ 的 Laurent 主部只作用在有限维缺陷空间上，即有限秩。

若局部行列式在每个邻域都恒为零，解析延拓迫使 $A(z)$ 在整个连通域都不可逆，即第一种情形。故无限维解析问题的全部非可逆奇性，最终仍压缩成有限维解析行列式。
\end{derivation}
...
目标是把一般正则化滤波器的误差分解为噪声放大与正则化偏差两项。由 $y=Kx^\dagger$ 且 $A=K^*K$，有
\[
\begin{aligned}
x_\alpha^\delta-x^\dagger
...
\end{aligned}
\]
其中 $R_\alpha:=g_\alpha(A)K^*$ 为残差算子，$r_\alpha(\lambda):=1-\lambda g_\alpha(\lambda)$ 为残差滤波函数。由于 $K=U A^{1/2}$ 是极分解（$U$ 为部分等距，$\ker U=\ker K$），
\[
R_\alpha
=g_\alpha(A)A^{1/2}U^*,
\]
...
此谱滤波框架在量子测量、统计力学（热浴 $\ee^{-\beta H}$ 对应 $\alpha\sim1/\beta$）与 CT/MRI 滤波反投影中同构出现。
\end{derivation}
