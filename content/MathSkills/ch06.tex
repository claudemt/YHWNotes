...
例：Hilbert--Schmidt 算子是紧算子。设 $K:L^2(X)\to L^2(Y)$ 的核 $k(x,y)$ 满足 $\|K\|_{\mathrm{HS}}^2=\int_{X\times Y}|k(x,y)|^2\,\dd x\,\dd y<\infty$，则 $K$ 是紧算子，且 $\|K\|\le\|K\|_{\mathrm{HS}}$。对 $T=I+K$，Atkinson 定理给出 $\operatorname{ind}T=0$，即 Fredholm 二择一：$Tx=y$ 可解当且仅当 $y\perp\ker K^*$。Fredholm 指标在量子场论中描述 拓扑 扇区：Dirac 算子 $D_A$ 的指标由 Atiyah--Singer 定理给出，是规范场拓扑荷（瞬子数）的局部表达式；Atkinson 的 拟逆 $S$ 正是 Green 函数 $D_A^{-1}$ 在非零谱上的正则化，紧扰动 $C$ 对应规范场的连续形变，指标不变性保证物理 扇区 在连续形变下守恒。
...
例：紧算子的预解式。取 $A(z)=I-zK$，$K$ 为 Hilbert--Schmidt 积分算子，核 $k(x,y)\in L^2([0,1]^2)$。设 $K$ 有非零本征值 $\mu_1,\mu_2,\ldots$，则 $(I-zK)^{-1}$ 在 $z=1/\mu_j$ 处有极点，主 部分 为 $\sum_{k=1}^{m_j}P_k/(z-1/\mu_j)^k$，其中 $m_j$ 为 $\mu_j$ 的代数重数、$P_k$ 为有限秩投影——这正是量子散射理论中 共振 的数学结构。特别地，$K=(-\dd^2/\dd x^2+\omega^2)^{-1}$（Dirichlet，$L^2[0,\pi]$）本征值 $\mu_n=1/(n^2+\omega^2)$，$(I-zK)^{-1}$ 在 $z=n^2+\omega^2$ 处有单极点，主部为投影到 $\sin(nx)$ 的秩一算子，由此给出热核 $\ee^{-tH}=\sum\ee^{-t(n^2+\omega^2)}P_n$；反之 Volterra 算子 $(Kf)(x)=\int_0^xK(x,t)f(t)\dd t$ 谱仅为 $\{0\}$，$(I-zK)^{-1}$ 在全平面解析，是定理第一种情形的实例。物理上，紧扰动 $H=H_0+V$ 的预解式在复平面亚纯，极点即束缚态（实轴）与共振态（下半平面）；统计力学中 转移矩阵 的极点给出配分函数的相变零点（Yang--Lee）。
