...
推广与物理建模。Fredholm 指标在量子场论中描述 topological sector：Dirac 算子 $D$ 在规范场 $A$ 下的指标
\[
...
举例：Sturm--Liouville 算子。取 $K=(-\dd^2/\dd x^2+\omega^2)^{-1}$ 在 $L^2([0,\pi])$ 上（Dirichlet 边界），本征值 $\mu_n=1/(n^2+\omega^2)$。则 $(I-zK)^{-1}$ 在 $z=n^2+\omega^2$ 处有单极点，principal part 为投影到 $\sin(nx)$ 的秩一算子。这给出热核
\[
\ee^{-tH}=\sum_{n=1}^\infty\ee^{-t(n^2+\omega^2)}P_n
\]
的算子理论推导。

举例：Volterra 积分算子。取 $(Kf)(x)=\int_0^x K(x,t)f(t)\,\dd t$，则 $K$ 是紧算子且谱 $\sigma(K)=\{0\}$（Volterra 算子无非零本征值）。$(I-zK)^{-1}$ 在整个复平面解析，Neumann 级数对所有 $z$ 收敛，这是 analytic Fredholm theorem 第一种情形的反例（$A(0)=I$ 可逆但无极点）。

推广与物理建模。analytic Fredholm theorem 是量子共振态分类的理论基础：设 $H=H_0+V$，$V$ 为紧扰动（如有限力程势），则预解式 $(H-z)^{-1}$ 在复 $z$ 平面上亚纯，极点对应束缚态（实轴上）与共振态（下半平面）。在统计力学中，transfer matrix 的本征值给出配分函数的极点，analytic Fredholm 定理保证相变点离散分布，这是 Yang--Lee 零点定理的算子理论版本。在量子场论中，$S$ 矩阵的极点对应粒子态，Resonance 的有限维结构保证散射振幅的 Lorentz 不变分解。在固体物理中，紧算子的离散谱给出能带结构，analytic Fredholm 定理保证能带在 Brillouin 区内解析，仅在能带交叉处有 Riemann 面分支奇性。在光学中，共振腔的模态对应紧算子的本征值，analytic Fredholm 定理保证共振频率离散分布，Q 因子由极点虚部决定。
\end{derivation}
...
两项的物理意义。第一项 $R_\alpha(y^\delta-y)$ 是噪声放大项，范数 $\|R_\alpha\|\delta$ 随 $\alpha\to0$ 发散（因 $g_\alpha(\lambda)\to1/\lambda$）；第二项 $r_\alpha(A)x^\dagger$ 是正则化偏差，随 $\alpha\to0$ 趋零。最优 $\alpha$ 平衡二者，给出正文中的收敛率。

举例：Tikhonov 滤波器。取 $g_\alpha(\lambda)=1/(\lambda+\alpha)$，则
\[
...
举例：谱截断正则化。取 $g_\alpha(\lambda)=\lambda^{-1}\mathbf 1_{\lambda\ge\alpha}$，则 $r_\alpha(\lambda)=\mathbf 1_{\lambda<\alpha}$。$\|R_\alpha\|=1/\sqrt\alpha$，偏差 $\|r_\alpha(A)x^\dagger\|\le\alpha^\nu\|w\|$，收敛率与 Tikhonov 相同，但谱截断对高频分量硬归零，适合信号去噪。

举例：Landweber 迭代。取 $g_k(\lambda)=\lambda^{-1}(1-(1-\lambda)^k)$（$\alpha=1/k$），则 $r_k(\lambda)=(1-\lambda)^k$。$\|R_k\|\le\sqrt{k}$，偏差 $\|r_k(A)x^\dagger\|\le k^{-\nu}\|w\|$，平衡 $\delta\sqrt{k}=k^{-\nu}\|w\|$ 给出 $k\asymp\delta^{-2/(2\nu+1)}$，与 Tikhonov 同阶但无需显式谱分解。

举例：iterated Tikhonov。取 $g_\alpha^{(m)}(\lambda)=\lambda^{-1}(1-(\alpha/(\lambda+\alpha))^m)$（$m$ 次 Tikhonov 迭代），qualification 为 $m$，对 $\nu>m$ 仍给出 $\delta^{2\nu/(2\nu+1)}$ 收敛率，突破普通 Tikhonov 的 saturation 限制。

举例：显式谱截断。对紧算子 $K$ 有奇异值分解 $K=\sum_j s_j\langle\cdot,u_j\rangle v_j$，谱截断正则化
\[
x_\alpha^\delta=\sum_{s_j\ge\alpha}\frac{\langle y^\delta,v_j\rangle}{s_j}u_j
\]
直接丢弃小奇异值分量，$\|R_\alpha\|=1/\alpha$（注意无 $\sqrt{}$），偏差 $\alpha^\nu\|w\|$，收敛率 $\delta^{2\nu/(2\nu+1)}$ 与 Tikhonov 同阶。

推广与物理建模。谱滤波框架统一了众多正则化方法：谱截断 $g_\alpha(\lambda)=\lambda^{-1}\mathbf 1_{\lambda\ge\alpha}$、Landweber 迭代 $g_\alpha(\lambda)=\lambda^{-1}(1-(1-\lambda)^k)$（$\alpha=1/k$）、iterated Tikhonov 等。在量子逆问题中，紧算子 $K$ 描述有限分辨率的测量过程，$g_\alpha$ 是分辨率函数，$\alpha$ 对应探测器噪声水平。统计力学中，滤波器 $g_\alpha$ 与热浴正则化 $\ee^{-\beta H}$ 结构同构，$\alpha\sim1/\beta$，正则化参数选择对应温度调谐。在图像重建中，紧算子 $K$ 为模糊核（如 Gaussian 卷积），Tikhonov 正则化对应 Tikhonov--Miller 算法，平衡分辨率与噪声放大。在地球物理反演中，重力异常反演是紧算子方程，谱滤波给出 Bouguer 异常的多尺度分解，正则化参数对应探测深度。在医学成像（CT、MRI）中，Radon 变换是紧算子，滤波反投影算法是谱滤波的离散实现，正则化参数控制图像分辨率与噪声的权衡。
\end{derivation}
