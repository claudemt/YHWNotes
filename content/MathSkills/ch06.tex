...
接下来考察Volterra 第二类方程：为什么不需要小耦合条件。
...
\label{eq:volterra-factorial-bound}
\end{equation}
于是
\[
...
\begin{equation}
\boxed{
y=\sum_{n=0}^\infty \lambda^nV^nf}
\label{eq:volterra-neumann-all-lambda}
\end{equation}
...
接下来考察Green 函数的显式例子：Dirichlet 二阶边值问题。
...
\label{eq:dirichlet-green-second-derivative}
\end{equation}
于是
\[
...
\begin{equation}
\boxed{f=\sum_{n=0}^{\infty}\lambda^nK^n\phi}.
...
若把算符幂重新写成积分核，还能得到经典的 Fredholm resolvent kernel。定义迭代核
...
把级数向前错一项还得到 resolvent identity
...
因此 Neumann 级数、逐次逼近和 resolvent kernel 不是三套不同方法，而是同一个逆算符 \((I-\lambda K)^{-1}\) 的三种写法。
...
\begin{equation}
\boxed{\langle g|\phi\rangle=0}.
\label{eq:fredholm-orthogonality}
\end{equation}
...
\begin{derivation}[Fredholm alternative 的闭值域与有限秩逼近]
...
\label{eq:math-fredholm-operator-definition}
\end{equation}
则称 $T$ 为 Fredholm operator，并定义 index
\begin{equation}
\boxed{
\operatorname{ind}T
:=\dim\ker T-\dim\operatorname{coker}T.
}
\label{eq:math-fredholm-index}
\end{equation}
...
\begin{equation}
\operatorname{ind}T
=\dim\ker T-\dim\ker T^\dagger.
\label{eq:math-fredholm-index-hilbert}
\end{equation}
Fredholm alternative 的真正本质正是：$I-\lambda K$ 不仅值域闭，而且是 index 为零的 Fredholm operator。

为什么“identity + compact”天然进入 Fredholm 类，由 Atkinson theorem 精确回答。

\begin{theorem}[Atkinson theorem]\label{thm:math-fredholm-atkinson}
对 Banach 空间上的有界算子 $T$，下列条件等价：
\begin{enumerate}[label=(\roman*)]
\item $T$ 是 Fredholm operator；
\item 存在有界算子 $S$ 使
\begin{equation}
ST-I\in\mathcal K,
\qquad
...
因此 Fredholm 性只取决于算子在 Calkin algebra
\[
\mathcal B(X)/\mathcal K(X)
\]
中的像是否可逆。换言之，“模掉 compact error 后可逆”就是 Fredholm 的内在定义。对
\[
T=I-K,
\qquad K\text{ compact},
\]
...
Atkinson theorem 还立即给出 compact perturbation stability：若 $T$ Fredholm、$C$ compact，则
\begin{equation}
T+C\text{ 仍 Fredholm},
\qquad
\boxed{\operatorname{ind}(T+C)=\operatorname{ind}T.}
\label{eq:math-fredholm-compact-index-invariance}
\end{equation}
index 是整数且在 Fredholm 算子集合上局部常数，因此沿连续 Fredholm 路径不会变化。特别地，考虑
\[
T_t=I-t\lambda K,
\qquad0\le t\le1.
\]
每个 $T_t$ 都是 identity 加 compact，故 Fredholm；而
\[
...
\label{eq:math-fredholm-i-minus-k-index-zero}
\end{equation}
这条整数不变量正是 Fredholm 二择一的最短结构证明：若
\[
\ker(I-\lambda K)=\{0\},
\]
则 index 为零迫使 cokernel 维数也为零；又因值域闭，所以值域就是整个空间。
...
反过来，若存在 $S$ 满足\eqnrefs{eq:math-fredholm-parametrix-mod-compact}，则 $ST=I+K_1$、$TS=I+K_2$ 都是 identity 加 compact。前面的紧性论证保证它们的核有限维且值域闭；进一步可推出 $T$ 的核与 cokernel 都有限维。因此 $T$ Fredholm。

现在若加 compact perturbation $C$，同一个 $S$ 满足
\[
...
\]
所以 $T+C$ 仍 Fredholm。index 不变性则由路径 $T+tC$ 与 Fredholm 集合中的局部常值性得到：$t\mapsto\operatorname{ind}(T+tC)$ 是连续整值函数，故为常数。由此，“有限维 defect 对 compact perturbation 稳定”不再是积分核问题的特殊现象，而是 Fredholm category 的一般拓扑性质。

\emph{举例：Hilbert--Schmidt 算子是紧算子。}设 $K:L^2(X)\to L^2(Y)$ 的核 $k(x,y)$ 满足
\[
...
\emph{推广与物理建模。}Fredholm 指标在量子场论中描述 topological sector：Dirac 算子 $D$ 在规范场 $A$ 下的指标
\[
\operatorname{ind}D_A=\dim\ker D_A-\dim\ker D_A^\dagger
\]
由 Atiyah--Singer 定理给出，是规范场拓扑荷（瞬子数）的局部表达式。Atkinson 的 parametrix $S$ 正是 Green 函数 $D_A^{-1}$ 在非零谱上的正则化，紧扰动 $C$ 对应规范场的连续形变，指标不变性保证物理 sector 在连续形变下守恒。
\end{derivation}

compact operator 的谱结构也可以由这个观点直接读出。若 $K$ compact，则对任意 $\mu\neq0$，
\[
K-\mu I=-\mu\left(I-\mu^{-1}K\right)
\]
是 Fredholm index $0$。若它不可逆，Fredholm alternative 强迫非零 kernel，因此 $\mu$ 必为 eigenvalue。再结合 compactness 可证明：每个非零 eigenvalue 的代数重数有限，而且非零谱只能离散分布，唯一可能的聚点是 $0$。这就是 Riesz--Schauder theory 的核心结论：
\begin{equation}
\boxed{
\sigma(K)\setminus\{0\}
=\{\text{有限重数的孤立本征值}\},
...
\label{eq:math-riesz-schauder-spectrum}
\end{equation}
因此紧算子虽然作用在无限维空间上，但其非零谱仍保留有限维矩阵的离散本征值图像。

这个结构还能沿复参数解析延拓。设 $U\subset\CC$ 连通开，$K(z)$ 是取值于 compact operators 的 analytic family，并令
\[
A(z)=I-K(z).
\]
analytic Fredholm theorem 断言：只有两种可能。
\begin{enumerate}[label=(\roman*)]
\item 对每个 $z\in U$，$A(z)$ 都不可逆；
\item 存在至少一个 $z_0$ 使 $A(z_0)$ 可逆，此时不可逆点在 $U$ 中离散，并且
\begin{equation}
A(z)^{-1}
\label{eq:math-analytic-fredholm-inverse}
\end{equation}
是 operator-valued meromorphic function；每个极点的 principal part 都是有限秩算子。
\end{enumerate}

\begin{derivation}[analytic Fredholm theorem 为什么只剩有限维奇性]
固定一个可能不可逆点 $z_*$。因为 $A(z_*)$ Fredholm index $0$，可作有限维 defect 分解
\[
X=N\oplus X_1,
\qquad
X=R\oplus Y_0,
...
\dim N=\dim Y_0<\infty.
\]
在 $X_1\to R$ 这块，$A(z_*)$ 可逆；解析扰动后，在 $z_*$ 的小邻域仍可逆。于是通过 block Gaussian elimination，可以把 $A(z)$ 的可逆性归约为一个有限维矩阵
\[
M(z):N\to Y_0
\]
...
\det M(z)
\]
是普通复解析函数。它若不恒为零，其零点只能离散；有限维逆矩阵由 adjugate/determinant 表示
\[
M(z)^{-1}=\frac{\operatorname{adj}M(z)}{\det M(z)},
\]
因此只产生有限阶极点。把消元块重新乘回，就得到 $A(z)^{-1}$ 的 Laurent principal part 只作用在有限维 defect spaces 上，也就是有限秩。

若局部 determinant 在每个邻域都恒为零，解析延拓迫使 $A(z)$ 在整个连通域都不可逆，于是得到第一种情形。故无限维解析问题的全部非可逆奇性，最终仍被压缩成有限维解析 determinant。

\emph{举例：紧算子的预解式。}取 $A(z)=I-zK$，$K$ 为 Hilbert--Schmidt 积分算子，核 $k(x,y)\in L^2([0,1]^2)$。设 $K$ 有非零本征值 $\mu_1,\mu_2,\ldots$，则
\[
(I-zK)^{-1}
\]
在 $z=1/\mu_j$ 处有极点，principal part 为
\[
\sum_{k=1}^{m_j}\frac{P_k}{(z-1/\mu_j)^k},
\]
其中 $m_j$ 为 $\mu_j$ 的代数重数，$P_k$ 为有限秩投影。这正是量子散射理论中 resonance 的数学结构。

\emph{举例：Sturm--Liouville 算子。}取 $K=(-\dd^2/\dd x^2+\omega^2)^{-1}$ 在 $L^2([0,\pi])$ 上（Dirichlet 边界），本征值 $\mu_n=1/(n^2+\omega^2)$。则 $(I-zK)^{-1}$ 在 $z=n^2+\omega^2$ 处有单极点，principal part 为投影到 $\sin(nx)$ 的秩一算子。这给出热核
\[
\ee^{-tH}=\sum_{n=1}^\infty\ee^{-t(n^2+\omega^2)}P_n
\]
的算子理论推导。

\emph{举例：Volterra 积分算子。}取 $(Kf)(x)=\int_0^x K(x,t)f(t)\,\dd t$，则 $K$ 是紧算子且谱 $\sigma(K)=\{0\}$（Volterra 算子无非零本征值）。$(I-zK)^{-1}$ 在整个复平面解析，Neumann 级数对所有 $z$ 收敛，这是 analytic Fredholm theorem 第一种情形的反例（$A(0)=I$ 可逆但无极点）。

\emph{推广与物理建模。}analytic Fredholm theorem 是量子共振态分类的理论基础：设 $H=H_0+V$，$V$ 为紧扰动（如有限力程势），则预解式 $(H-z)^{-1}$ 在复 $z$ 平面上亚纯，极点对应束缚态（实轴上）与共振态（下半平面）。在统计力学中，transfer matrix 的本征值给出配分函数的极点，analytic Fredholm 定理保证相变点离散分布，这是 Yang--Lee 零点定理的算子理论版本。在量子场论中，$S$ 矩阵的极点对应粒子态，Resonance 的有限维结构保证散射振幅的 Lorentz 不变分解。在固体物理中，紧算子的离散谱给出能带结构，analytic Fredholm 定理保证能带在 Brillouin 区内解析，仅在能带交叉处有 Riemann 面分支奇性。在光学中，共振腔的模态对应紧算子的本征值，analytic Fredholm 定理保证共振频率离散分布，Q 因子由极点虚部决定。
\end{derivation}

对固定 compact operator $K$，取
\[
A(z)=I-zK.
\]
因为 $A(0)=I$ 可逆，analytic Fredholm theorem 必落在第二种情形。因此
\begin{equation}
\boxed{
(I-zK)^{-1}
\text{ 关于 }z\text{ 亚纯},
}
\label{eq:math-compact-resolvent-meromorphic-z}
\end{equation}
其极点恰位于
\[
z=\mu^{-1},
...
\]
这把 Neumann 级数的局部收敛区域升级成全复平面的亚纯 continuation：Neumann 级数只在 $|z|\|K\|<1$ 直接收敛，但 resolvent 本身可以越过这一圆盘继续解析，直到遇到由非零本征值决定的离散极点。积分方程里的“共振参数”因此就是 operator-valued meromorphic resolvent 的极点。
...
抽象的 Fredholm alternative 最容易在共振参数处显得难懂。下面用两个可以完全算完的例子，把``齐次解出现''、``非齐次右端必须正交''与``解不唯一''三件事放在同一张计算链上。
...
第二类方程为
\begin{equation}
f(x)-\lambda\int_0^1 f(y)\,\dd y=\phi(x).
\label{eq:rank-one-fredholm-example}
\end{equation}
记
\[
c=\int_0^1f(y)\,\dd y,
...
\label{eq:rank-one-fredholm-solution}
\end{equation}
这里的预解核甚至可以直接读出：
\[
...
\qquad c\in\mathbb C
\]
全部都是解；若不满足，则根本无解。一个无限维积分方程在这个退化核上把 Fredholm alternative 的三种情况完整展示出来了。
...
\]
正是 Dirichlet Laplacian $L=-\dd^2/\dd x^2$ 的逆 $K=L^{-1}$。
...
Dirichlet Laplacian 的正交归一本征函数是
\[
...
\frac{2}{\pi n^2}
\sin(nx)\sin(ny),
\label{eq:dirichlet-green-eigen-expansion}
\end{equation}
...
\label{eq:sturm-liouville-resolvent-expansion}
\end{equation}
...
+C\varphi_m(x).
\]
至此可以把抽象 Fredholm alternative 翻译成一句非常具体的话：\textbf{当参数碰到齐次本征频率时，外力不能含有该共振模态；若没有该模态，特解存在，但还可以任意叠加齐次本征函数。}
...
当 \(\lambda=1/\mu_n\) 时分母为零，这正是 Fredholm alternative 中齐次方程出现非零解的谱论版本。
...
=\sum_n\frac{s_n}{s_n^2+\alpha}g_n^\delta\psi_n.
\]
这说明正则化不是在形式逆上任意加一个“小量”，而是由一个 coercive 变分问题产生的谱滤波。若数据误差满足 \(\|g^\delta-g\|\le\delta\)，要获得一致收敛还需令 \(\alpha=\alpha(\delta)\to0\) 且噪声放大受控，例如典型条件 \(\delta/\sqrt{\alpha}\to0\)。
\end{derivation}
...
\label{eq:fredholm-resolvent-spectral}
\end{equation}
其中 \(P_0\) 投影到 \(\ker K\)。每个因子
\[
\frac1{1-\lambda\mu_n}
\]
在 \(\lambda=1/\mu_n\) 处有一阶极点。于是 Fredholm resolvent 的奇异性与齐次方程出现非零解完全对应。
...
\label{eq:tikhonov-error-decomposition}
\end{equation}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.76\textwidth]{content/MathSkills/figures/mathematical-physics/generated/tikhonov_filter.pdf}
...
前面的奇异系统已经暴露了第一类方程的本质困难：在 $s_n\to0$ 的方向上，形式逆包含 $1/s_n$。Tikhonov 只是处理这一困难的一种方法。为了比较不同算法，应先把它们写成同一个 functional-calculus 形式。
...
\]
因此算法稳定性不是由名称决定，而是由这一 sup norm 精确决定。

接下来考察Tikhonov、谱截断与 Landweber 的共同结构。
...
\]
因此``迭代次数''本身就是正则化参数。若无限迭代到噪声被完全拟合，稳定性反而会丢失；这就是 iterative regularization 中 early stopping 的数学来源。

接下来考察source condition、收敛阶与饱和现象。

仅知道 $x^\dagger\in X$ 时，通常只能证明 $x_\alpha^\delta\to x^\dagger$，却不能得到统一收敛速率。要得到 rate，需要描述真实解相对于 $K^*K$ 的光滑性。一个标准 source condition 是
\[
\boxed{
x^\dagger=A^\nu w,
\qquad \nu>0,
...
\right)}.
\]
这不是经验拟合，而是 stability 与 approximation 两个误差项的最优阶平衡。

普通 Tikhonov 的上述简单估计在 $\nu>1$ 时不再继续提高阶数，这体现了 qualification/saturation：一个正则化方法能利用多少阶的 source smoothness 是有限的。若真实解更光滑而算法已经饱和，需要 iterated Tikhonov、谱截断或其他更高 qualification 的滤波器，不能指望把同一个 $\alpha$ 公式无限外推。
...
\end{aligned}
\]
其中 $R_\alpha:=g_\alpha(A)K^*$ 为残差算子，$r_\alpha(\lambda):=1-\lambda g_\alpha(\lambda)$ 为残差滤波函数。由于 $K=U A^{1/2}$ 是 polar decomposition（$U$ 为部分等距，$\ker U=\ker K$），
\[
R_\alpha
=g_\alpha(A)A^{1/2}U^*,
\]
...
\sqrt\lambda\,|g_\alpha(\lambda)|.
\]
在 $\overline{\operatorname{Ran}K}$ 上 $U^*$ 为部分等距，故该上界实际取等，得到正文中的精确公式。

\emph{两项的物理意义。}第一项 $R_\alpha(y^\delta-y)$ 是噪声放大项，范数 $\|R_\alpha\|\delta$ 随 $\alpha\to0$ 发散（因 $g_\alpha(\lambda)\to1/\lambda$）；第二项 $r_\alpha(A)x^\dagger$ 是正则化偏差，随 $\alpha\to0$ 趋零。最优 $\alpha$ 平衡二者，给出正文中的收敛率。

\emph{举例：Tikhonov 滤波器。}取 $g_\alpha(\lambda)=1/(\lambda+\alpha)$，则
\[
\|R_\alpha\|=\sup_{\lambda\ge0}\frac{\sqrt\lambda}{\lambda+\alpha}=\frac{1}{2\sqrt\alpha},
\qquad
\|r_\alpha(A)\|=\sup_{\lambda\ge0}\frac{\alpha}{\lambda+\alpha}=1.
\]
对 source condition $x^\dagger=A^\nu w$，$\|r_\alpha(A)x^\dagger\|\le\alpha^\nu\|w\|$，平衡 $\delta/(2\sqrt\alpha)=\alpha^\nu\|w\|$ 给出 $\alpha\asymp\delta^{2/(2\nu+1)}$。

\emph{举例：谱截断正则化。}取 $g_\alpha(\lambda)=\lambda^{-1}\mathbf 1_{\lambda\ge\alpha}$，则 $r_\alpha(\lambda)=\mathbf 1_{\lambda<\alpha}$。$\|R_\alpha\|=1/\sqrt\alpha$，偏差 $\|r_\alpha(A)x^\dagger\|\le\alpha^\nu\|w\|$，收敛率与 Tikhonov 相同，但谱截断对高频分量硬归零，适合信号去噪。

\emph{举例：Landweber 迭代。}取 $g_k(\lambda)=\lambda^{-1}(1-(1-\lambda)^k)$（$\alpha=1/k$），则 $r_k(\lambda)=(1-\lambda)^k$。$\|R_k\|\le\sqrt{k}$，偏差 $\|r_k(A)x^\dagger\|\le k^{-\nu}\|w\|$，平衡 $\delta\sqrt{k}=k^{-\nu}\|w\|$ 给出 $k\asymp\delta^{-2/(2\nu+1)}$，与 Tikhonov 同阶但无需显式谱分解。

\emph{举例：iterated Tikhonov。}取 $g_\alpha^{(m)}(\lambda)=\lambda^{-1}(1-(\alpha/(\lambda+\alpha))^m)$（$m$ 次 Tikhonov 迭代），qualification 为 $m$，对 $\nu>m$ 仍给出 $\delta^{2\nu/(2\nu+1)}$ 收敛率，突破普通 Tikhonov 的 saturation 限制。

\emph{举例：显式谱截断。}对紧算子 $K$ 有奇异值分解 $K=\sum_j s_j\langle\cdot,u_j\rangle v_j$，谱截断正则化
\[
x_\alpha^\delta=\sum_{s_j\ge\alpha}\frac{\langle y^\delta,v_j\rangle}{s_j}u_j
\]
直接丢弃小奇异值分量，$\|R_\alpha\|=1/\alpha$（注意无 $\sqrt{}$），偏差 $\alpha^\nu\|w\|$，收敛率 $\delta^{2\nu/(2\nu+1)}$ 与 Tikhonov 同阶。

\emph{推广与物理建模。}谱滤波框架统一了众多正则化方法：谱截断 $g_\alpha(\lambda)=\lambda^{-1}\mathbf 1_{\lambda\ge\alpha}$、Landweber 迭代 $g_\alpha(\lambda)=\lambda^{-1}(1-(1-\lambda)^k)$（$\alpha=1/k$）、iterated Tikhonov 等。在量子逆问题中，紧算子 $K$ 描述有限分辨率的测量过程，$g_\alpha$ 是分辨率函数，$\alpha$ 对应探测器噪声水平。统计力学中，滤波器 $g_\alpha$ 与热浴正则化 $\ee^{-\beta H}$ 结构同构，$\alpha\sim1/\beta$，正则化参数选择对应温度调谐。在图像重建中，紧算子 $K$ 为模糊核（如 Gaussian 卷积），Tikhonov 正则化对应 Tikhonov--Miller 算法，平衡分辨率与噪声放大。在地球物理反演中，重力异常反演是紧算子方程，谱滤波给出 Bouguer 异常的多尺度分解，正则化参数对应探测深度。在医学成像（CT、MRI）中，Radon 变换是紧算子，滤波反投影算法是谱滤波的离散实现，正则化参数控制图像分辨率与噪声的权衡。
\end{derivation}

接下来考察参数选择与 discrepancy principle。

若噪声水平 $\delta$ 已知，参数不能只根据图像``看起来最平滑''来选。Morozov discrepancy principle 要求选取 $\alpha=\alpha(\delta,y^\delta)$ 使
\[
\boxed{
...
=-r_\alpha(KK^*)y^\delta
\]
（在 $\ker K^*$ 方向还保留不可拟合分量）。因此 discrepancy principle 实际上是在控制 residual function 对数据谱的作用。它把参数选择从``算法外部的经验旋钮''变成算子理论内部的一条停止准则。
...
\begin{equation}
\widehat{\mathcal H f}(k)
=-\ii\,\operatorname{sgn}(k)\widehat f(k).
\label{eq:hilbert-fourier-multiplier}
\end{equation}
...
\begin{equation}
\|\mathcal H\|_{L^2\to L^2}=1.
\label{eq:hilbert-l2-isometry}
\end{equation}
...
\end{equation}
结合 \cref{eq:hilbert-skew-adjoint,eq:hilbert-square}，还可核对
\[
