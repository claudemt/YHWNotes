...
Gaussian 条件分布由此完整给出：给定 $Y=y$，$X\mid Y=y$ 仍为 Gaussian，均值只改线性预测项，协方差为 Schur 补 $\Sigma_{XX\cdot Y}$（式~\eqnrefs{eq:math-stat-gaussian-conditional}、\eqnrefs{eq:math-stat-gaussian-schur-complement}）。
