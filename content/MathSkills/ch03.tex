...
物理上，Hilbert 空间上每个连续泛函 $F$ 对应一个 bra $\langle\phi|$，Riesz 表示给出 $|\phi\rangle$ 使 $F(|\psi\rangle)=\langle\phi|\psi\rangle$；可观测量期望 $\langle\psi|A|\psi\rangle$ 即 $A|\psi\rangle$ 定义的泛函，其代表向量为 $A^\dagger|\psi\rangle$。注意一般 Banach 空间的对偶 $X^*$ 不能由 $X$ 自身表示（仅 $L^2$ 自对偶），这是 Hilbert 空间的特殊结构；rigged Hilbert space 中 $\Phi^*$ 严格大于 $\mathcal H$，广义本征向量属于 $\Phi'\setminus\mathcal H$。更一般地，coercive 双线性型的 Lax--Milgram 定理（$a(u,v)=F(v)$ 唯一可解）正是 $a(u,v)=\langle u,v\rangle$ 时的 Riesz 表示。
\end{derivation}
...
例如对角紧算子 $Kx=(x_n/n)$ 虽 $\ker K=\{0\}$，但 $\|Ke_n\|=1/n\to0$ 而 $\|e_n\|=1$，无 coercive 下界，其值域 $\{y:\sum n^2|y_n|^2<\infty\}$ 不闭——这正是第一类 Fredholm 方程 $Kx=y$ 不适定的谱论根源。反之 $T=-\Delta+1$ 满足 $\|Tf\|\ge\|f\|$，值域 $L^2$ 闭（实际满射），故 $-\Delta u+u=f$ 对任意 $f\in L^2$ 有唯一解 $u\in H^2$。

对闭稠定算子定理仍成立，只需用图范数处理定义域。它在两个方向上反复出现：Fredholm 算子（$\ker T$、$\operatorname{coker}T$ 有限维且值域闭）的指标 $\operatorname{ind}T=\dim\ker T-\dim\operatorname{coker}T$ 在紧扰动下不变，是 Atiyah--Singer 指标定理的算子代数母结构；椭圆算子 $L$ 的 coercive 估计 $\|u\|_{H^2}\le C(\|Lu\|+\|u\|)$ 则结合闭值域给出 Green 函数 $G=L^{-1}$ 在 $(\ker L)^\perp$ 上的有界性。逆问题中奇异值衰减过快导致反演不适定，Tikhonov 正则化正是用 $\sigma_n^{-1}\mapsto\sigma_n/(\sigma_n^2+\alpha)$ 控制这一发散。
\end{derivation}
...
框架上，有限维情形即 $K=V\Sigma U^\dagger$；Hilbert--Schmidt 算子（$\sum\sigma_n^2<\infty$）的 SVD 与积分核 $k=\sum\sigma_n u_nv_n^*$ 在 $L^2$ 中收敛，trace class（$\sum\sigma_n<\infty$）则迹绝对收敛，一般有界算子需用谱测度与极分解的连续版本。SVD 还直接给出 Moore--Penrose 伪逆 $K^+=\sum\sigma_n^{-1}|u_n\rangle\langle v_n|$，闭值域时有界、否则无界。物理上，密度算子 $\rho$ 的 SVD 即谱分解 $\rho=\sum p_n|\psi_n\rangle\langle\psi_n|$，$p_n$ 为处于纯态 $|\psi_n\rangle$ 的概率，von Neumann 熵 $S=-\sum p_n\log p_n$ 量化混合度。
\end{derivation}
...
将 rank-one 算子重新识别为张量。每个 rank-one 算子 $|u_n\rangle\langle v_n|$ 对应于简单张量 $u_n\otimes v_n$（通过上面将向量识别为算子的逆映射）。逐项转换，
\begin{equation*}
 \Psi=\sum_n s_n\,u_n\otimes v_n.
\end{equation*}
...
\]
嵌入 $H_0^1(0,L)\hookrightarrow L^2(0,L)$ 是紧的，因此该 Dirichlet Laplacian 具有紧预解，\eqnrefs{eq:rayleigh-ritz-first} 具体化为
...
这也说明所谓“变分近似”究竟受什么控制：不是把微分方程随意简化，而是在精确 min--max 公式中把无限维允许空间 $Q(A)$ 限制到有限维试探子空间 $V_N$。由\eqnrefs{eq:courant-fischer-form}得到的 Ritz 本征值满足
\[
