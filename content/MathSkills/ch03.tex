...
例：$\mathcal H=\ell^2(\NN)$、$M=\operatorname{span}\{e_1,e_2\}$ 时，$x=(x_1,x_2,x_3,\ldots)$ 的最近点为 $m_0=(x_1,x_2,0,\ldots)$，残差 $(0,0,x_3,\ldots)\in M^\perp$；$L^2[0,1]$ 中取 $M$ 为前三 Fourier 模时，最近点即三阶 Fourier 截断，残差与 $M$ 正交。量子力学中，闭子空间 $M$ 对应一个是/否测量，$\psi=m_0+\psi_\perp$（$m_0=P_M\psi$）给出测量后坍缩态 $m_0/\|m_0\|$，概率 $\|m_0\|^2$——Born 法则的几何母结构即此分解。一般 Banach 空间虽也有最近点（需 $M$ 闭且空间自反），但无内积使正交分解失效；投影 $P_M$ 在 von Neumann 代数中推广为自伴幂等算子，正交分解升级为谱投影分解。
...
例：$L^2[0,1]$ 上 $F(f)=\int_0^1 f g^*\dd x$ 的代表向量即 $f_F=g$；$\ell^2$ 上 $F(x)=\sum a_nx_n$ 的代表向量为 $f_F=(a_n^*)$。取 $\mathcal H=H^1_0[0,1]$（内积 $\int u'v'^*\dd x$），$F(v)=\int f v^*\dd x$ 的 Riesz 代表 $u$ 满足 $\int u'v'^*=\int f v^*$，正是 $-u''=f$ 的弱解——Riesz 定理由此直接给出 Poisson 方程弱解存在性。物理上，Hilbert 空间上每个连续泛函 $F$ 对应一个 bra $\langle\phi|$，Riesz 表示给出 $|\phi\rangle$ 使 $F(|\psi\rangle)=\langle\phi|\psi\rangle$；可观测量期望 $\langle\psi|A|\psi\rangle$ 即 $A|\psi\rangle$ 定义的泛函，其代表向量为 $A^\dagger|\psi\rangle$。一般 Banach 空间的对偶 $X^*$ 不能由 $X$ 自身表示（仅 $L^2$ 自对偶），这是 Hilbert 空间的特殊结构；rigged Hilbert 空间 中 $\Phi^*$ 严格大于 $\mathcal H$，广义本征向量属于 $\Phi'\setminus\mathcal H$。更一般地，强制 双线性型的 Lax--Milgram 定理（$a(u,v)=F(v)$ 唯一可解）正是 $a(u,v)=\langle u,v\rangle$ 时的 Riesz 表示。
...
例：Bell 态 $|\Phi^+\rangle=(|00\rangle+|11\rangle)/\sqrt2$ 对应 $s_1=s_2=1/\sqrt2$、Schmidt 秩 $2$，为最大纠缠；可分态 $|0\rangle\otimes(|0\rangle+|1\rangle)/\sqrt2$ 对应 $s_1=1$、秩 $1$，为零纠缠。由此二体纠缠获得完整刻画：Schmidt 秩为 $1$ 当且仅当可分，纠缠熵 $S=-\sum s_n^2\log s_n^2$ 是 LOCC 不变量；Schmidt 系数在 LOCC 下单调递减（Nielsen 定理）。对多体态可逐对应用但一般无简洁形式，张量网络（PEPS、MERA）是其数值推广；混合态则需 negativity、concurrence 等更复杂不变量。
