...
方差分析、线性回归、协方差分析和许多实验设计共享同一个母结构：响应向量的均值被限制在设计矩阵的列空间中，估计等价于把观测投影到这个均值子空间，检验则比较两个嵌套子空间之间新增的方向。为了遵守“先一般精确结构、再特殊化”的顺序，本章先允许已知形状的非球形协方差 $\sigma^2V$，通过 白化 把一般 广义最小二乘 化成欧氏投影；随后把 $V=I_n$ 作为经典 最小二乘/方差分析 的特殊情形。正态性只在需要有限样本精确 $t/F$ 分布时加入，Gauss--Markov 与最小二乘几何本身并不需要正态假设。
...
\]
只依赖可识别的均值向量。方差分析 中的组均值差、回归中的线性对比与秩亏设计下的可识别效应都应以这种可估函数表述。
...
\begin{derivation}[秩亏设计下的 白化 与 Gauss--Markov]
在 白化 模型
\[
 Y^*=X^*\beta+\varepsilon^*,
 \qquad
...
一元线性回归 $y_i=\beta_0+\beta_1 x_i+\varepsilon_i$：$p=2$，约束 $\beta_1=0$ 给 $q=1$。白化 把残差标准化后，$\hat\beta_1$ 的标准化平方除以残差平方和给出 $F_{1,n-2}$——这就是回归 $F$ 检验。多元回归中约束 $C\vb\beta=\vb c$（$C$ 为 $q\times p$）给出 $F_{q,n-p}$，白化 把一般协方差 $\sigma^2(X^\top X)^{-1}$ 化为单位阵，使约束检验化为标准正交投影——这是所有回归推断的统一机制。
...
\]
所以 $F$ 统计量变成非中心 $F$，功效由子空间维数、残差自由度与非中心参数共同决定。它与 \chapref{chap:math-stat-testing} 局部渐近正态 局部备择中的非中心卡方具有同一“法向信号平方长度”解释，只是这里由于 Gaussian 线性 model 的特殊结构而在有限样本下精确成立。
...
=\|Z\|.
\end{equation*}
将 $\|Z\|^2$ 拆为处理效应投影与误差投影两部分。处理效应投影 $\|Z\|^2$ 自由度 $g-1$，误差投影 $\mathrm{SSE}/\sigma^2$ 自由度 $\nu$，且二者正交。由 Cochran 定理，
\begin{equation*}
\|Z\|^2\sim\chi^2_{g-1},
\qquad
...
设三组样本量均为 $n$，误差自由度 $\nu=3(n-1)$。对 两两 对比 $L=\mu_1-\mu_2$，Scheff'e 半宽为
\[
\sqrt{2F_{1-\alpha,2,\nu}\,\widehat\sigma^2\cdot\frac2n}
=2\sqrt{\frac{F_{1-\alpha,2,\nu}\widehat\sigma^2}{n}}.
\]
对 Tukey 检验，两两 半宽为 $q_{1-\alpha,3,\nu}/\sqrt2\cdot\sqrt{\widehat\sigma^2/n}$。Scheff'e 对 两两 比较稍宽，但对一般对比（如 $\mu_1-(\mu_2+\mu_3)/2$）紧；Tukey 对 两两 紧但对一般对比不适用。两者覆盖不同对比族，选择由科学问题决定。
...
\[
 ...
 =\frac{e_i}{1-h_{ii}}.
\]
因此一个点只有在“残差大”与“杠杆大”共同出现时才会显著改变参数估计。以拟合向量变化的平方长度标准化，Cook 距离 为
...
\]
在这些方向上被放大。标准化设计后的 条件数 描述整体最坏方向，而单个系数的 方差 inflation 因子（VIF）可以由 FWL 精确推出。
...
\]
在 $s_j\to0$ 时不再发散。这与逆问题章中的 谱 filtering 完全同构。\figref{fig:ridge-spectrum} 画出了谱收缩因子 $\lambda_i/(\lambda_i+\alpha)$ 随特征值的变化：小特征值被显著压缩，大特征值几乎不收缩。
...
\begin{derivation}[岭 的 bias--方差 分解与谱收缩]
在固定设计精确线性模型
\[
Y=X\beta+\varepsilon,
\qquad
...
\]
当 $s_j$ 很小时，最小二乘 的方差 $\sigma^2/s_j^2$ 会爆炸，而 岭 把它压到有限量；代价就是\eqnrefs{eq:ridge-bias} 的系统偏差。正则化的数学本质因此不是“让拟合更平滑”的经验技巧，而是在不可识别或弱识别方向上主动进行 bias--方差 tradeoff。
\end{derivation}
...
=X^{\mathsf T}Y.
\]
这正是一般 Tikhonov 正规化 在有限维线性模型中的版本。若 $L$ 有非平凡核，则这些方向不被惩罚；例如回归中常把截距从 惩罚 中排除。
...
\begin{derivation}[随机设计 最小二乘 是总体线性投影的经验估计]
展开总体风险，
\[
 \mathcal R_{\rm lin}(b)
...
\[
 ...
 (X^{\mathsf T}X)^{-1}.
\]
若 $\Omega=\sigma^2V$ 且 $V$ 已知，就应回到本章开头的 广义最小二乘。若异方差形式未知，把第 $i$ 行设计向量记为 $x_i\in\R^p$，并把固定设计看成一个三角阵列。条件于设计后假设误差独立且
\[
...
最基本的 heteroscedasticity-相合 协方差估计为
...
最小二乘（忽略异方差）：$\hat\beta=0.126$，$s_{\beta}=0.003$。

WLS（权重 $w_i=1/x_i^2$）：$\hat\beta=0.125$，$s_{\beta}=0.002$。

WLS 的斜率估计略低，标准误更小——因为低浓度点（精度高）获得更大权重。若用 最小二乘 的标准误做推断，置信区间偏宽，检验功效偏低。
\end{exbox}
\begin{exbox}[高杠杆点的识别]
