...
\[
 \|Y^*-X^*\beta\|^2.
\]
因此一般 $V$ 下的全部几何，都可以先 whitening 再在欧氏空间中处理。若原误差正态，$\varepsilon^*/\sigma\sim\mathcal N_n(0,I_n)$，于是 \chapref{chap:math-stat-gaussian-sampling} 的 Gaussian projection 定理可直接应用。
...
一元线性回归 $y_i=\beta_0+\beta_1 x_i+\varepsilon_i$：$p=2$，约束 $\beta_1=0$ 给 $q=1$。whitening 把残差标准化后，$\hat\beta_1$ 的标准化平方除以残差平方和给出 $F_{1,n-2}$——这就是回归 $F$ 检验。多元回归中约束 $C\vb\beta=\vb c$（$C$ 为 $q\times p$）给出 $F_{q,n-p}$，whitening 把一般协方差 $\sigma^2(X^\top X)^{-1}$ 化为单位阵，使约束检验化为标准正交投影——这是所有回归推断的统一机制。
...
\]
在 $s_j\to0$ 时不再发散。这与逆问题章中的 spectral filtering 完全同构。\figref{fig:ridge-spectrum} 画出了谱收缩因子 $\lambda_i/(\lambda_i+\alpha)$ 随特征值的变化：小特征值被显著压缩，大特征值几乎不收缩。
...
Ridge 估计的协方差由此为 $\operatorname{Cov}(\widehat\beta_\lambda\mid X)=\sigma^2(X^{\mathsf T}X+\lambda I)^{-1}X^{\mathsf T}X(X^{\mathsf T}X+\lambda I)^{-1}$（式~\eqnrefs{eq:ridge-covariance}）。
...
\]
则迭代期望给出 $\mathbb E[x_i(Y_i-x_i^{\mathsf T}\beta_0)]=0$，由 $G_X\succ0$ 得 $\beta^*=\beta_0$。若条件均值非线性，OLS 仍可能一致估计 $\beta^*$，但这个 $\beta^*$ 只是最佳线性逼近；把它解释为结构效应或因果效应需要额外假设。
...
\]
右端第一因子趋于 $G_X^{-1}$，第二因子由多元 CLT 收敛到 $\mathcal N_p(0,B_X)$（其中 $B_X=\mathbb E(u^2xx^{\mathsf T})$），Slutsky 定理遂给出\eqnrefs{eq:math-stat-random-design-sandwich}。这条推导同时解释了 HC 协方差的来源：它只是把 $G_X$ 与 $B_X$ 分别换成样本 Gram 矩阵和残差外积的经验版本。
