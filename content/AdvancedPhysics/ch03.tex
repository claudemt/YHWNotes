...
上述构造依赖出发边 \(e_1\) 与每次离开时的边选择，故分解一般不唯一。以 \(\theta\)-图（两顶点间三条平行边）为例：\(\mathscr A\) 取全部三条边时，可分解为 \(\{e_1,e_2\}\mathbin{\triangle}\{e_3\}\) 或 \(\{e_1,e_3\}\mathbin{\triangle}\{e_2\}\) 等多种方式。但所有分解的对称差总和在 \(\mathbb F_2\) 上恒等于 \(\mathscr A\)，故 cycle space 元素本身良定，分解非唯一性不影响线性结构。
...
\end{equation*}
结合 \(Z_1\subseteq\mathcal C^{*\perp}\) 即得 \(Z_1=\mathcal C^{*\perp}\)，同理 \(\mathcal C^*=Z_1^\perp\)。但 \(Z_1\cap\mathcal C^*\) 在 \(\mathbb F_2\) 上可以非零，例如四边形的整圈同时是 cycle，也是交错顶点集产生的 cut。若需要唯一坐标，可选生成树 \(T\)，用基本回路给出 \(Z_1\) 的基，并以树边张成空间作为一个依赖于 \(T\) 的代数补空间；这个补空间一般不是 cut space。

图 Laplacian 的定义与基本性质。定义 \(\mathsf L_G=\mathsf I_G\mathsf I_G^{\mathsf T}\)，这是 \(|\mathsf V|\times|\mathsf V|\) 对称矩阵。对无自环图，显式为
\begin{equation}
 (\mathsf L_G)_{xy}=
 \begin{cases}
 \deg(x),&x=y,\\
...
推广到加权图与有向图。对加权图（边权 \(w_{xy}\)），Laplacian 推广为 \((\mathsf L_w)_{xy}=\delta_{xy}\sum_z w_{xz}-w_{xy}\)，谱性质类似但本征值含权。对有向图，定义非对称 Laplacian \(\mathsf L_{\rm dir}=\mathsf D_{\rm out}-\mathsf A\)（出度减邻接），本征值可复，实部控制收敛速率。这些推广在加权 Ising 模型（非均匀耦合）和马尔可夫链 Monte Carlo（转移率的谱分析）中有直接应用。
\end{derivation}
...
\begin{derivation}[Matrix--Tree 定理的 Cauchy--Binet 证明]
...
对顶点子集 \(S\subseteq V\)，cut \(\delta(S)=\{e=\{u,v\}:u\in S,\,v\notin S\}\) 是跨越 \(S\) 与 \(V\setminus S\) 的边集。所有 cut 的 \(\mathbb F_2\)-线性包络满足
\[
...
\end{equation*}
闭链进入 \(S\) 的次数等于离开次数，故穿越 cut 的总次数为偶。维数关系 \(\dim Z_1+\dim B_1^*=|E|\) 于是确认二者互为正交补。需要注意，在 \(\mathbb F_2\) 上可能有非零向量同时属于 cycle space 与 cut space，因此一般不能把这句话加强成正交直和。
