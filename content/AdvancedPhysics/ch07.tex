...
\end{equation*}
奇数点函数恒为零。玻色子版本把 Pfaffian 换成 Hafnian（正号配对求和），行列式公式不再成立——这是费米子与玻色子统计在路径积分中表现为 Grassmann 变量与普通变量的根本原因。
\end{derivation}
...
\end{equation*}
其中求和遍历所有偶子图（每个顶点度数为偶）。方格图上偶子图等价于闭环集合——每个顶点度数为 0 或 2，因此 \(E'\) 分解为不相交的闭合多边形。这是高温展开的核心：配分函数的非平凡部分是所有闭环的带权求和。
