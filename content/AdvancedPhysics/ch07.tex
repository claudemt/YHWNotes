...
先讨论基本对易子。由\eqnrefs{eq:phys-sm-clifford-algebra}（\(\{\chi_a,\chi_b\}=2\delta_{ab}\)），
\begin{equation*}
 [\chi_a\chi_b,\chi_c]
...
随后讨论二次型生成元的作用。代入\eqnrefs{eq:phys-sm-clifford-generator}（\(\mathcal Q(\mathsf A)=-\frac12\sum_{a,b}\mathsf A_{ab}\chi_a\chi_b\)），
\begin{equation*}
 \begin{aligned}
...
接着讨论Lie 代数同态。考虑 \([\mathcal Q(\mathsf A),\mathcal Q(\mathsf B)]-\mathcal Q([\mathsf A,\mathsf B])\)。对任意 \(\chi_c\)，
\begin{equation*}
...
接着讨论Ising 任意子的熔接规则。Majorana 零模的代数 \(\Gamma_i^2=1\)，\(\{\Gamma_i,\Gamma_j\}=2\delta_{ij}\) 定义 Ising 任意子理论。三个任意子类型：\(\mathbf 1\)（真空）、\(\sigma\)（Majorana 零模）、\(\psi\)（费米子）。熔接规则
\begin{equation*}
...
进一步讨论实验实现。Majorana 零模在拓扑超导纳米线中实验实现：InSb/InAs 纳瑟夫森结阵列在磁场下进入拓扑相，两端出现零偏导电导峰（Majorana 零模的签名）。Ising 任意子的编织操作可通过交换纳米线段实现，其非 Abel 统计使拓扑量子计算免受局部退相干——这是 Majorana 方案的根本优势。但实验验证仍面临困难：零模与非拓扑低能态的混合、有限长度效应、无序导致的零模劈裂等，使 Majorana 零模的明确实验鉴定至今仍是凝聚态物理的前沿问题。

Majorana 零模的简并子空间天然定义拓扑量子纠错码：环面 \(g=1\) 上 \(2^2=4\) 维简并空间编码 1 个逻辑量子比特（\(4=2^2\)，物理 2 比特，逻辑 1 比特，1 比特纠错）。亏格 \(g\) 曲面编码 \(g\) 个逻辑量子比特（\(2^{2g}\) 维物理空间），码距 \(d=g\)（需 \(g\) 个局域操作才能翻转逻辑态）。这与表面码（surface code）同构——Ising TQFT 的拓扑序就是 \(\mathbb Z_2\) 表面码的物理实现，是容错量子计算最有前景的方案之一。

Majorana 零模的拓扑不变量是 \(\mathbb Z_2\) 指标（Kitaev 链的 winding number 模 2）：\(\nu=\mathrm{sgn}[\epsilon(0)]\cdot\mathrm{sgn}[\epsilon(\pi)]\in\{+1,-1\}\)，\(\nu=-1\) 为拓扑相（有零模），\(\nu=+1\) 为平庸相（无零模）。这一指标在能隙不闭合的形变下不变，是拓扑相分类的 1D 示例。推广到 2D（Ising 任意子）给出 chiral central charge \(c=1/2\)（mod 8）作为拓扑不变量，与 modular T 矩阵的迹 \(\ee^{2\pi\ii c/24}\) 相关。

有限长度 \(L\) 的 Kitaev 链中，两端 Majorana 零模有指数小的耦合 \(\delta E\sim\ee^{-L/\xi}\)，使简并劈裂。这一劈裂是拓扑保护的定量度量：\(\delta E\) 随 \(L\) 指数衰减，衰减率 \(1/\xi\) 由体谱隙决定。在 Ising 模型中，有限环上 Majorana 零模的劈裂对应关联长度的倒数 \(\xi^{-1}\)，临界点 \(\xi\to\infty\) 时劈裂消失——这是有限尺寸标度 \(E\sim1/N\) 的微观起源。

Ising 任意子的编织表示由 \(\sigma\) 任意子的 conformal block 给出。两个 \(\sigma\) 任意子的编织矩阵 \(B=\frac{1}{\sqrt2}\begin{pmatrix}1&\ee^{-\ii\pi/4}\\\ee^{\ii\pi/4}&-1\end{pmatrix}\) 满足 Yang--Baxter 方程 \(B_1B_2B_1=B_2B_1B_2\)（三任意子编织的一致性）。\(B\) 的本征值 \(\ee^{\pm\ii\pi/8}\) 给出拓扑相位，是 Ising 任意子统计角的精确值。编织群的表示（Burau 表示在 \(t=\ee^{\ii\pi/4}\) 的特化）是拓扑量子计算的门集——\(B\) 生成 Clifford 群，但不含 \(\pi/8\) 相位门，因此 Ising 任意子不通用（需补充非拓扑门）。

Ising 任意子是 2D 拓扑序的最简单非 Abel 例子。其他常见任意子理论：(1) Fibonacci 任意子（\(\tau\times\tau=1+\tau\)），通用（可实现任意量子门），但实验实现更困难；(2) Abelian 任意子（如 \(\mathbb Z_n\) 玻色液体），编织只给相位（Abel 统计），不足以拓扑量子计算；(3) SU(2)\(_k\) Chern--Simons 任意子（\(k=2\) 即 Ising，\(k=3\) 即 Fibonacci）。Ising 任意子在可实验性和计算通用性之间取平衡：拓扑超导纳米线已接近实现，但需额外资源达到通用。

Ising 拓扑相变在 Kitaev 链中由质量项 \(m\) 驱动：\(m<0\) 拓扑相（有零模），\(m>0\) 平庸相（无零模），\(m=0\) 临界点（零模变为体零模，谱隙闭合）。这一相变属于 1D Ising 普适类（\(c=1/2\)），与 2D 经典 Ising 的热相变对偶——Kitaev 链的量子相变等价于 2D Ising 的行-行转移矩阵。拓扑不变量 \(\nu\) 在相变处不定义（能隙闭合），但可通过临界指数（\(\nu=1\)，\(\beta=1/8\)）描述相变的标度行为。

Majorana 零模的实验候选系统包括：(1) 超导-半导体纳米线（InSb/Al，InAs/Al），需满足拓扑条件 \(V_Z>\sqrt{\mu^2+\Delta^2}\)（Zeeman 能超过化学势和配对能）；(2) 拓扑绝缘体-超导异质结（FeTe\(_{0.55}\)Se\(_{0.45}\)），本征拓扑超导；(3) 分数量子霍尔边缘（\(\nu=5/2\)），Ising 任意子作为 \(\nu=5/2\) Laughlin 态的准粒子。2012--2024 年的实验进展包括零偏导峰观测、库仑阻塞谱测量等，但零模与非拓扑态的混合、无序效应、有限长度效应使"确凿"鉴定仍待实现——需要编织实验（非 Abel 统计的直接观测）作为最终判据。拓扑超导的判据可总结为
\begin{equation*}
...
先讨论Pfaffian 的定义。对 \(2n\times2n\) 反对称矩阵 \(A\)（\(A^T=-A\)），Pfaffian 定义为
\begin{equation*}
...
随后讨论Ising 配分函数的图展开。方格图 \(G=(V,E)\) 上 Ising 配分函数
\begin{equation*}
...
先讨论离散复结构。方格图 \(G\) 的对偶图 \(G^*\) 和 medial graph \(G_M\)（旋转 \(45^\circ\)）共同定义离散复结构。每条边 \(e\in E(G)\) 对应对偶边 \(e^*\in E(G^*)\)，medial 边 \(e_M\) 连接 \(e\) 和 \(e^*\) 的中点。离散导数在 medial graph 上定义：函数 \(F:V(G_M)\to\mathbb C\) 的离散导数 \(\partial_M F\) 用相邻 medial 顶点的差商给出。

随后讨论s-holomorphic 条件。Ising 观察值 \(F(e_M)\)（定义在 medial 边上）满足 s-holomorphic 条件：对每个 medial 顶点 \(z\)，
\begin{equation*}
...
再讨论临界 s-holomorphic 与连续极限。临界 Ising（\(\sinh(2K_c)=1\)）的 s-holomorphic 函数在网格细化 \(a\to0\) 时收敛到全纯函数：\(\operatorname{proj}_\ell F\to\operatorname{Re}(f)\) 或 \(\operatorname{Im}(f)\)（视 \(\ell\) 方向），因此 \(F\to f\) 全纯。这一收敛是指数快的（远离临界点衰减），给出临界关联函数的严格连续极限。特别地，自旋关联 \(\mathbb E[\sigma_{z_1}\cdots\sigma_{z_n}]\) 的离散 holomorphicity 保证其收敛到 CFT 关联函数 \(\langle\sigma(z_1)\cdots\sigma(z_n)\rangle_{\mathrm{CFT}}\)。

接着讨论Smirnov 的收敛定理。Smirnov (2010) 证明：临界方格 Ising 的自旋关联在网格细化下收敛到 \(c=1/2\) Ising CFT 的相应关联函数，收敛是局部一致的。这给出了从格点模型到 CFT 的严格收敛证明（而非仅形式匹配），是 2D Ising 精确可解性最深刻的严格结果。s-holomorphicity 是这一证明的核心工具：它把离散 Ising 观察量嵌入到离散复分析框架中，使连续极限成为全纯函数的收敛问题。这一方法已推广到其他临界格点模型（如 FK-Ising、loop O(1)），是现代统计力学严格收敛理论的基石。
\end{derivation}
