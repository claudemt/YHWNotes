...
\emph{举例：Kitaev 链的 Majorana 表示。}考虑 Kitaev 链 Hamiltonian
\begin{equation*}
...
\emph{Wick 定理的费米子版本。}对满足 CAR 的费米子，Gaussian 态下的 \(2n\) 点函数由 Pfaffian 给出：
\begin{equation*}
...
\emph{策略：}先直接验证 Majorana 双线性与单线性的对易子，再代入二次型生成元得到线性作用，最后用不可约性证明 Lie 代数同态。

\emph{第一步：基本对易子。}由\eqnrefs{eq:phys-sm-clifford-algebra}（\(\{\chi_a,\chi_b\}=2\delta_{ab}\)），
\begin{equation*}
 [\chi_a\chi_b,\chi_c]
...
\emph{第二步：二次型生成元的作用。}代入\eqnrefs{eq:phys-sm-clifford-generator}（\(\mathcal Q(\mathsf A)=-\frac12\sum_{a,b}\mathsf A_{ab}\chi_a\chi_b\)），
\begin{equation*}
 \begin{aligned}
...
\emph{第四步：Lie 代数同态。}考虑 \([\mathcal Q(\mathsf A),\mathcal Q(\mathsf B)]-\mathcal Q([\mathsf A,\mathsf B])\)。对任意 \(\chi_c\)，
\begin{equation*}
...
\emph{Clifford 代数的结构。}\(\mathrm{Cl}_{2N}\) 作为代数同构于 \(\mathrm{Mat}_{2^N}(\mathbb C)\)，其偶子代数 \(\mathrm{Cl}_{2N}^{\rm even}\cong\mathrm{Mat}_{2^{N-1}}(\mathbb C)\oplus\mathrm{Mat}_{2^{N-1}}(\mathbb C)\)。Majorana 双线性张成 \(\mathfrak{so}(2N)\) 的旋量表示，\(\ee^{\mathcal Q(\mathsf A)}\) 即 spin 表示下的 \(\mathrm{Spin}(2N)\) 元素。此同构保证 transfer matrix 的 Gaussian 形式可对角化为 \(2N\times2N\) 的单粒子问题，把 \(2^N\) 维 Fock 谱降为 \(N\) 维 Bogoliubov 谱——这是自由费米子可解性的代数根源。

\emph{具体例子：\(N=1\)（两个 Majorana 算子）。}\(\chi_1^2=\chi_2^2=1\)，\(\chi_1\chi_2=-\chi_2\chi_1\)。\(\mathfrak{so}(2)\) 是一维 Abel 代数，生成元 \(\mathsf A=\begin{pmatrix}0&1\\-1&0\end{pmatrix}\)，\(\mathcal Q(\mathsf A)=-\chi_1\chi_2\)。\(\ee^{\theta\mathcal Q(\mathsf A)}\chi_1\ee^{-\theta\mathcal Q(\mathsf A)}=\cos\theta\,\chi_1+\sin\theta\,\chi_2\)，即 \(\mathrm{SO}(2)\) 旋转。Fock 空间二维（真空和占据），\(\ee^{\theta\mathcal Q}\) 的本征值为 \(\ee^{\pm i\theta/2}\)，体现旋量表示的双覆盖性 \(\mathrm{Spin}(2)\cong\mathrm U(1)\to\mathrm{SO}(2)\)。

\emph{推广：玻色子的对称类似。}对满足 CCR \([b_i,b_j^\dagger]=\delta_{ij}\) 的玻色子，定义 Hermitian Quadratic \(\mathcal Q_{\rm B}=\sum A_{ij}b_i^\dagger b_j\)，则 \([\mathcal Q_{\rm B},b]=\mathsf A b\) 给出 \(\mathfrak{gl}(N)\) 而非 \(\mathfrak{so}(2N)\) 的作用，对应辛群 \(\mathrm{Sp}(2N)\) 而非旋量群。玻色子 Gaussian 的迹由 \(\det(1-\mathsf T)^{-1}\) 给出（分母幂），费米子由 \(\det(1+\mathsf T)\) 给出（分子幂），二者统一于 superdeterminant（Berezinian）\(\operatorname{sdet}(1-\mathsf T)=\det(1-\mathsf T_{\rm B})/\det(1+\mathsf T_{\rm F})\)，这是超对称量子力学的代数基础。

\emph{物理意义。}Clifford commutator 与 Lie 代数映射的物理核心是"Majorana 二次型生成 \(\mathfrak{so}(2N)\) 旋量表示"：每个实反对称矩阵 \(\mathsf A\in\mathfrak{so}(2N)\) 对应一个 Majorana 二次型 \(\mathcal Q(\mathsf A)=\frac14\sum_{ij}A_{ij}\chi_i\chi_j\)，对易子 \([\mathcal Q(\mathsf A),\mathcal Q(\mathsf B)]=\mathcal Q([\mathsf A,\mathsf B])\) 给出精确的 Lie 代数同态。物理上，\(\mathcal Q(\mathsf A)\) 是 \(N\) 个 Majorana 费米子的自由 Hamiltonian，\(\mathsf A\) 是单粒子轨道的混合矩阵——\(\mathcal Q\) 把经典正交变换 \(\mathsf T=e^{\mathsf A}\)（轨道旋转）提升为 Fock 空间的酉演化 \(e^{\mathcal Q}\)（多体态演化）。旋量表示的双覆盖性 \(\mathrm{Spin}(2N)\to\mathrm{SO}(2N)\) 体现为 \(2\pi\) 旋转给出 \(-1\)（费米子半整数自旋）。这一结构在拓扑超导体中描述 BdG Hamiltonian 的分类（Altland--Zirnbauer 对称类 D），在量子计算中用于 Majorana 量子比特的编织操作（非 Abel 统计来自旋量表示的非交换性）。
\end{derivation}
...
空间 NS/R 边界条件确定以后，还需要在时间方向区分是否插入 fermion parity。于是有限环自然出现四个 fermionic traces；它们与曲面上的四个 spin structures 对应，但并不等同于 \(\mathrm H_1(T^2;\F_2)\) 的四个同调类。
...
高温闭链扇区属于 \(\mathrm H_1(T^2;\F_2)\)，而 \((s,t)\) 是 fermionic boundary twists。在选择参考 spin structure 后，后者形成以 \(\mathrm H^1(T^2;\F_2)\) 为平移群的 affine torsor；两套四重对象通过 character Fourier 变换联系，而不是逐项同一。
...
\begin{derivation}[Majorana 零模与拓扑简并]
临界点处费米子色散 \(\epsilon(k)\) 在 \(k=0\)（R 扇区）软化，产生 Majorana 零模。这一零模的拓扑保护是 Ising 任意子非 Abel 统计的代数起源。分四步建立。

\emph{第一步：零模的精确条件。}BdG 方程 \(\epsilon(k)=0\) 的解要求 \(\cosh(2K_h)\cosh(2K_v^*)=\sinh(2K_h)\sinh(2K_v^*)\)，即 \(\tanh(2K_h)\tanh(2K_v^*)=1\)。由对偶关系 \(\tanh K_v^*=\ee^{-2K_v}\)，各向同性时化为 \(\sinh(2K_c)=1\)，即\eqnrefs{eq:phys-sm-square-critical-k}。R 扇区 \(k=0\) 模式在临界点成为零模：Bogoliubov 变换无法对角化（混合角 \(\theta_0\to\pi/4\) 奇点），对应 BdG 矩阵的 Jordan 块。物理上，零模意味着 Hamiltonian 中不含该模式，故其占据数不改变能量——这是简并的代数根源。

\emph{第二步：零模的 Majorana 算符。}临界 R 扇区中，\(k=0\) 的 BdG 块退化为 \(\begin{pmatrix}0&0\\0&0\end{pmatrix}\)，对应 Majorana 算符 \(\Gamma_0=c_0+c_0^\dagger\) 满足 \(\Gamma_0^2=1\) 且与 Hamiltonian 对易 \([H,\Gamma_0]=0\)。这是 Majorana 零模：不改变能量但改变宇称（\(\{\Gamma_0,\mathcal P\}=0\)）。R 扇区因此分裂为 \(\Gamma_0=\pm1\) 两个简并态，对应 \(\mathcal Z_{\mathrm R,0}\) 与 \(\mathcal Z_{\mathrm R,1}\) 在临界点等值——这正是\eqnrefs{eq:phys-sm-kaufman-sector-projection} 中 \(\mathcal Z_{\mathrm R,0}-\mathcal Z_{\mathrm R,1}\to0\) 的物理来源。零模的 Majorana 性质（\(\Gamma_0^\dagger=\Gamma_0\)，\(\Gamma_0^2=1\)）区别于普通复费米子零模（需要两个 Majorana 组成一个 Dirac 费米子），是 Ising 拓扑序的特征。

\emph{第三步：拓扑保护与 Kitaev 链对应。}Majorana 零模的拓扑保护体现在：对临界点附近的微扰 \(\delta K\)，零模获得质量 \(m\propto\delta K\)，但只要 \(|m|\) 小于谱隙（热力学极限下任意小），零模仍近似简并。这映射到 Kitaev 一维拓扑超导链：该链在 \(|m|<2|t|\)（拓扑相）中两端各有一个 Majorana 零模，在 \(|m|>2|t|\)（平庸相）中无零模。Ising 临界点对应拓扑相变边界 \(|m|=2|t|\)，零模从有到无。环面上的两个 Majorana 零模（对应两个同调方向）产生四重简并（\(2^{2g}=4\)，\(g=1\)），与\eqnrefs{eq:phys-sm-torus-h1} 的 \(2^{2g}\) 扇区一致。拓扑保护的本质是：零模由系统的全局拓扑（同调类）决定，不能被局部微扰消除——这与整数量子霍尔效应的边缘态拓扑保护同构。

\emph{第四步：Ising 任意子的熔接规则。}Majorana 零模的代数 \(\Gamma_i^2=1\)，\(\{\Gamma_i,\Gamma_j\}=2\delta_{ij}\) 定义 Ising 任意子理论。三个任意子类型：\(\mathbf 1\)（真空）、\(\sigma\)（Majorana 零模）、\(\psi\)（费米子）。熔接规则
\begin{equation*}
  \sigma\times\sigma=\mathbf 1+\psi,\quad\sigma\times\psi=\sigma,\quad\psi\times\psi=\mathbf 1.
\end{equation*}
\(\sigma\times\sigma=\mathbf 1+\psi\) 正是 R 扇区零模的二重简并：两个 \(\sigma\) 任意子熔接给出真空或费米子，对应 \(\Gamma_0=\pm1\)。编织两个 \(\sigma\) 任意子给出 \(\ee^{\pm\ii\pi/8}\) 相位（非 Abel 统计），其矩阵表示为 \(\frac{1}{\sqrt2}\begin{pmatrix}1&\ee^{-\ii\pi/4}\\\ee^{\ii\pi/4}&-1\end{pmatrix}\)，正是 Ising CFT 中 \(\sigma\) primary 的 conformal block 单值性。这把 Ising 模型的精确可解性、拓扑序和共形场论统一在 Majorana 零模的代数中。非 Abel 统计意味着编织操作不交换（矩阵乘法不可换），这是拓扑量子计算的基础——Ising 任意子虽不足以实现通用量子计算（需要额外的 \(\pi/8\) 门），但已能实现 Clifford 群操作。

\emph{第五步：实验实现。}Majorana 零模在拓扑超导纳米线中实验实现：InSb/InAs 纳瑟夫森结阵列在磁场下进入拓扑相，两端出现零偏导电导峰（Majorana 零模的签名）。Ising 任意子的编织操作可通过交换纳米线段实现，其非 Abel 统计使拓扑量子计算免受局部退相干——这是 Majorana 方案的根本优势。但实验验证仍面临困难：零模与非拓扑低能态的混合、有限长度效应、无序导致的零模劈裂等，使 Majorana 零模的明确实验鉴定至今仍是凝聚态物理的前沿问题。

\emph{零模的量子纠错编码。}Majorana 零模的简并子空间天然定义拓扑量子纠错码：环面 \(g=1\) 上 \(2^2=4\) 维简并空间编码 1 个逻辑量子比特（\(4=2^2\)，物理 2 比特，逻辑 1 比特，1 比特纠错）。亏格 \(g\) 曲面编码 \(g\) 个逻辑量子比特（\(2^{2g}\) 维物理空间），码距 \(d=g\)（需 \(g\) 个局域操作才能翻转逻辑态）。这与表面码（surface code）同构——Ising TQFT 的拓扑序就是 \(\mathbb Z_2\) 表面码的物理实现，是容错量子计算最有前景的方案之一。

\emph{拓扑不变量与分类。}Majorana 零模的拓扑不变量是 \(\mathbb Z_2\) 指标（Kitaev 链的 winding number 模 2）：\(\nu=\mathrm{sgn}[\epsilon(0)]\cdot\mathrm{sgn}[\epsilon(\pi)]\in\{+1,-1\}\)，\(\nu=-1\) 为拓扑相（有零模），\(\nu=+1\) 为平庸相（无零模）。这一指标在能隙不闭合的形变下不变，是拓扑相分类的 1D 示例。推广到 2D（Ising 任意子）给出 chiral central charge \(c=1/2\)（mod 8）作为拓扑不变量，与 modular T 矩阵的迹 \(\ee^{2\pi\ii c/24}\) 相关。

\emph{有限尺寸零模劈裂。}有限长度 \(L\) 的 Kitaev 链中，两端 Majorana 零模有指数小的耦合 \(\delta E\sim\ee^{-L/\xi}\)，使简并劈裂。这一劈裂是拓扑保护的定量度量：\(\delta E\) 随 \(L\) 指数衰减，衰减率 \(1/\xi\) 由体谱隙决定。在 Ising 模型中，有限环上 Majorana 零模的劈裂对应关联长度的倒数 \(\xi^{-1}\)，临界点 \(\xi\to\infty\) 时劈裂消失——这是有限尺寸标度 \(E\sim1/N\) 的微观起源。

\emph{编织矩阵的精确形式。}Ising 任意子的编织表示由 \(\sigma\) 任意子的 conformal block 给出。两个 \(\sigma\) 任意子的编织矩阵 \(B=\frac{1}{\sqrt2}\begin{pmatrix}1&\ee^{-\ii\pi/4}\\\ee^{\ii\pi/4}&-1\end{pmatrix}\) 满足 Yang--Baxter 方程 \(B_1B_2B_1=B_2B_1B_2\)（三任意子编织的一致性）。\(B\) 的本征值 \(\ee^{\pm\ii\pi/8}\) 给出拓扑相位，是 Ising 任意子统计角的精确值。编织群的表示（Burau 表示在 \(t=\ee^{\ii\pi/4}\) 的特化）是拓扑量子计算的门集——\(B\) 生成 Clifford 群，但不含 \(\pi/8\) 相位门，因此 Ising 任意子不通用（需补充非拓扑门）。

\emph{与其他任意子理论的比较。}Ising 任意子是 2D 拓扑序的最简单非 Abel 例子。其他常见任意子理论：(1) Fibonacci 任意子（\(\tau\times\tau=1+\tau\)），通用（可实现任意量子门），但实验实现更困难；(2) Abelian 任意子（如 \(\mathbb Z_n\) 玻色液体），编织只给相位（Abel 统计），不足以拓扑量子计算；(3) SU(2)\(_k\) Chern--Simons 任意子（\(k=2\) 即 Ising，\(k=3\) 即 Fibonacci）。Ising 任意子在可实验性和计算通用性之间取平衡：拓扑超导纳米线已接近实现，但需额外资源达到通用。

\emph{拓扑相变与临界点。}Ising 拓扑相变在 Kitaev 链中由质量项 \(m\) 驱动：\(m<0\) 拓扑相（有零模），\(m>0\) 平庸相（无零模），\(m=0\) 临界点（零模变为体零模，谱隙闭合）。这一相变属于 1D Ising 普适类（\(c=1/2\)），与 2D 经典 Ising 的热相变对偶——Kitaev 链的量子相变等价于 2D Ising 的行-行转移矩阵。拓扑不变量 \(\nu\) 在相变处不定义（能隙闭合），但可通过临界指数（\(\nu=1\)，\(\beta=1/8\)）描述相变的标度行为。

\emph{实验进展与挑战。}Majorana 零模的实验候选系统包括：(1) 超导-半导体纳米线（InSb/Al，InAs/Al），需满足拓扑条件 \(V_Z>\sqrt{\mu^2+\Delta^2}\)（Zeeman 能超过化学势和配对能）；(2) 拓扑绝缘体-超导异质结（FeTe\(_{0.55}\)Se\(_{0.45}\)），本征拓扑超导；(3) 分数量子霍尔边缘（\(\nu=5/2\)），Ising 任意子作为 \(\nu=5/2\) Laughlin 态的准粒子。2012--2024 年的实验进展包括零偏导峰观测、库仑阻塞谱测量等，但零模与非拓扑态的混合、无序效应、有限长度效应使"确凿"鉴定仍待实现——需要编织实验（非 Abel 统计的直接观测）作为最终判据。拓扑超导的判据可总结为
\begin{equation*}
\mathcal N_{\text{Maj}}=\dim\ker H_{\text{BdG}}\ (\text{mod}\ 2),\qquad \mathcal N_{\text{Maj}}>0\iff \text{拓扑非平凡}.
\end{equation*}
...
\emph{[有限尺寸自由能与 Ising 共形场论]}
临界点处有限尺寸自由能的精确系数由离散 Fourier 和的 Euler--Maclaurin 渐近给出，结果与 \(c=1/2\) Ising CFT 一致。分三步计算。

\emph{第一步：自由能的离散和表示。}由\eqnrefs{eq:phys-sm-fermion-dispersion}，各向同性临界 \(K=K_c\) 时 \(\epsilon(k)=2|k|\)（小 \(k\) 线性色散，cusp）。有限环 \(N\) 上每格点自由能
\begin{equation*}
...
\emph{第二步：Euler--Maclaurin 渐近。}对光滑函数 \(g(q)\)，\(\frac{1}{N}\sum_{m}g\!\left(\frac{2\pi m}{N}\right)=\int_0^{2\pi}\frac{g(q)\dd q}{2\pi}+\frac{g'(0)-g'(2\pi)}{2N}+\cdots\)。但 \(\log|q|\) 在 \(q=0\) 有 cusp，\(g'(0^+)-g'(0^-)=2/N\)，给出额外 \(O(N^{-1})\) 项。精确计算（Cardy 1984）给出 NS/R 扇区自由能差
\begin{equation*}
...
\emph{第三步：Ising CFT 的 primary fields。}\(c=1/2\) 极小模型 CFT 有三个 primary fields：
\begin{equation*}
...
\emph{第四步：有限尺寸谱与 operator content。}CFT 预言有限尺寸能谱 \(E_n-E_0=\frac{2\pi v}{N}(h_n+\bar h_n-1/12)\)，其中 \(v\) 是临界速度。Ising CFT 的三个 primary 给出三个最低能级：\(\mathbf 1\)（真空，\(E=E_0\)）、\(\sigma\)（自旋，\(E-E_0=\frac{\pi v}{8N}\)）、\(\psi\)（能量，\(E-E_0=\frac{\pi v}{N}\)）。这些能级在 transfer matrix 谱中精确可辨，对应 NS/R 扇区的不同 sector，是 CFT-operator correspondence 的格点验证。

\emph{modular 不变性与字符。}Ising CFT 的三个字符 \(\chi_0(q)=\frac12\left(\sqrt{\theta_3/\eta}+\sqrt{\theta_4/\eta}\right)\)、\(\chi_{1/2}(q)=\frac12\left(\sqrt{\theta_3/\eta}-\sqrt{\theta_4/\eta}\right)\)、\(\chi_{1/16}(q)=\sqrt{\theta_2/\eta}/\sqrt2\) 在 modular S 变换 \(q\to\tilde q=\ee^{-4\pi^2/\log q}\) 下混合为 \(S=\frac12\begin{pmatrix}1&1&\sqrt2\\1&1&-\sqrt2\\\sqrt2&-\sqrt2&0\end{pmatrix}\)。物理配分函数 \(Z=\chi_0\bar\chi_0+\chi_{1/2}\bar\chi_{1/2}+\chi_{1/16}\bar\chi_{1/16}\) 的 modular 不变性等价于 NS/R 扇区投影\eqnrefs{eq:phys-sm-kaufman-sector-projection}，是格点 Ising 与 CFT 等价的最终验证。

\emph{格点--CFT 对应的严格性。}从格点 Ising 到 \(c=1/2\) CFT 的对应可在两个严格意义上验证：(1) 有限尺寸自由能系数 \(-\pi c/(48N^2)\) 精确匹配（Cardy 1984）；(2) transfer matrix 谱的能级间距 \(\frac{2\pi v}{N}(h+\bar h-1/12)\) 精确匹配（Belavin--Polyakov--Zamolodchikov 1984）。两者都依赖 Ising 的自由费米子结构——Bogoliubov 谱给出精确能级，Euler--Maclaurin 给出精确自由能修正。对一般格点模型（如 3D Ising），无此精确对应，只有数值和重整化群证据，凸显了 2D Ising 精确可解性在格点--CFT 对应中的独特地位。
...
\emph{第一步：Pfaffian 的定义。}对 \(2n\times2n\) 反对称矩阵 \(A\)（\(A^T=-A\)），Pfaffian 定义为
\begin{equation*}
...
\emph{第二步：Ising 配分函数的图展开。}方格图 \(G=(V,E)\) 上 Ising 配分函数
\begin{equation*}
...
\emph{[离散 holomorphicity 与 s-holomorphic 观察]} Chelkak--Smirnov 引入的 s-holomorphic（spin holomorphic）观察是临界 Ising 严格收敛到 CFT 的关键工具。分四步概述。

\emph{第一步：离散复结构。}方格图 \(G\) 的对偶图 \(G^*\) 和 medial graph \(G_M\)（旋转 \(45°\)）共同定义离散复结构。每条边 \(e\in E(G)\) 对应对偶边 \(e^*\in E(G^*)\)，medial 边 \(e_M\) 连接 \(e\) 和 \(e^*\) 的中点。离散导数在 medial graph 上定义：函数 \(F:V(G_M)\to\mathbb C\) 的离散导数 \(\partial_M F\) 用相邻 medial 顶点的差商给出。

\emph{第二步：s-holomorphic 条件。}Ising 观察值 \(F(e_M)\)（定义在 medial 边上）满足 s-holomorphic 条件：对每个 medial 顶点 \(z\)，
\begin{equation*}
...
\emph{第三步：临界 s-holomorphic 与连续极限。}临界 Ising（\(\sinh(2K_c)=1\)）的 s-holomorphic 函数在网格细化 \(a\to0\) 时收敛到全纯函数：\(\operatorname{proj}_\ell F\to\operatorname{Re}(f)\) 或 \(\operatorname{Im}(f)\)（视 \(\ell\) 方向），因此 \(F\to f\) 全纯。这一收敛是指数快的（远离临界点衰减），给出临界关联函数的严格连续极限。特别地，自旋关联 \(\mathbb E[\sigma_{z_1}\cdots\sigma_{z_n}]\) 的离散 holomorphicity 保证其收敛到 CFT 关联函数 \(\langle\sigma(z_1)\cdots\sigma(z_n)\rangle_{\mathrm{CFT}}\)。

\emph{第四步：Smirnov 的收敛定理。}Smirnov (2010) 证明：临界方格 Ising 的自旋关联在网格细化下收敛到 \(c=1/2\) Ising CFT 的相应关联函数，收敛是局部一致的。这给出了从格点模型到 CFT 的严格收敛证明（而非仅形式匹配），是 2D Ising 精确可解性最深刻的严格结果。s-holomorphicity 是这一证明的核心工具：它把离散 Ising 观察量嵌入到离散复分析框架中，使连续极限成为全纯函数的收敛问题。这一方法已推广到其他临界格点模型（如 FK-Ising、loop O(1)），是现代统计力学严格收敛理论的基石。
\end{derivation}
