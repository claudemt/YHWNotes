\chapter{Onsager 自由能与临界谱}
\label{chap:phys-sm-onsager-exact-solution}

有限方格上的转移算符已经被 Jordan--Wigner 与 Clifford 线性化拆成独立的 \(\{q,-q\}\) 动量块，但这还不是 Onsager 自由能。每个块给出一条快度 \(\gamma(q)\)，NS/R 动量网格和物理奇偶投影再把这些快度组合成四个有限乘积。只有保留这套有限尺寸账目以后取热力学极限，才不会在临界点附近丢失扇区信息。临界线表现为转移谱的质量闭合；自旋关联长度还受到观测算符选择定则约束，不能简单等同于最小单粒子谱隙。
...
\end{equation}
表示。一个完整对称转移 step 与 \(\mathsf R_v\mathsf R_h(q)\) 相似，因此具有相同本征值。令
\[
...
\begin{proof}[解]
由\eqnrefs{eq:phys-sm-finite-spectrum-structure}，固定扇区 \(s\) 中单步转移的多体本征值为
\begin{equation*}
 \Lambda_s(\{n_q\})
 =\mathcal C_N\exp\!\left[\frac12\sum_{q\in s}(1-2n_q)\gamma(q)\right],
 \qquad n_q\in\{0,1\},
\end{equation*}
其中 \(n_q=0\) 对应 Bogoliubov 真空，\(n_q=1\) 对应单模激发。纵向 \(M\) 步等价于对每个准粒子模作 \(M\) 次乘幂，故对单模 \(q\)，
\begin{equation*}
 \Lambda_q(n)^M
...
对非自反块 \(\{q,-q\}\)，正则变换混合的是 \(q\) 与 \(-q\) 两个模，真空占据改变为偶数，故不翻转 \(p_s\)；只有自反模能够翻转。自反模 \(q=\pi\) 的未配对质量符号为 \(K_v^*+K_h>0\)，不穿零；\(q=0\) 的符号为 \(K_v^*-K_h\)，恰在临界线 \(K_v^*=K_h\) 穿零。因此铁磁耦合下
\begin{equation*}
 p_{\rm NS}=+1,
 \qquad
 p_{\rm R}=\operatorname{sgn}(K_v^*-K_h),
...
\end{equation}
这里 \(\Delta_{\rm tr}\) 是 dimensionless 初等转移质量。闭隙条件 \(K_v^*=K_h\) 与
\begin{equation}
...
\end{equation}
各向异性只通过临界转移 velocity \(v_c\) 改变度量尺度；无量纲系数仍与周期 Ising 临界理论的中心荷 \(c=1/2\) 一致。这里并未把共形场论作为推导输入：系数直接来自有限格点快度的尖点与离散 Fourier 取样。
