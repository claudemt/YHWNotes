...
转移 分解 \eqnrefs{eq:phys-sm-transfer-pauli-factorization} 已把二维方格 Ising 的 转移 算符 写成局域 Pauli 指数的乘积。要把指数维谱问题降成线性代数，必须同时处理两个结构：局域二次费米子化与有限环的全局奇偶约束。前者给出 Clifford 共轭作用，后者决定 NS/R 动量网格和环面四个 费米子 自旋结构。有限尺寸分析中二者不能交换顺序。

\section{轴旋转与 Jordan--Wigner 变换}
\label{sec:phys-sm-jw-majorana}

Ising 模型的 转移 因子 中，垂直传递因子 $\mathsf V_v$ 含 $\sigma^z$（在位），水平因子 $\mathsf V_h$ 含 $\sigma^z\sigma^z$（邻接）。Jordan--Wigner 变换要求 在位 项对角、邻接项在 $\tau^x$ 方向——因此需先做一个 $\pi/2$ 轴旋转，把 $\sigma^z\sigma^z$ 耦合转到 $\tau^x\tau^x$ 方向。定义旋转后的 Pauli 算符
\begin{equation}
 \tau_j^z=X_j,
 \qquad
 \tau_j^x=Z_j,
...
\end{equation}
由不同站点 Pauli 算符对易和同站点反对易关系，得到 正则反交换关系
\begin{equation}
 \{c_i,c_j\}=0,
 \qquad
...
\end{equation}
因此两个 转移 因子 的指数生成元都是费米子二次型。

\begin{derivation}[Jordan--Wigner 字符串与 正则反交换关系]
若 \(i<j\)，则
\[
 \mathscr S_j
 =\mathscr S_i\tau_i^z
...
\[
 ...
 =\frac{1-\tau_j^z}{2},
\]
故\eqnrefs{eq:phys-sm-tauz-number} 成立，并且 \(\{c_j,c_j^\dagger\}=1\)。这里 \(\{A,B\}=AB+BA\) 是反对易子，正则反交换关系 即 正则 anti-commutation 关系
\begin{equation*}
 \{c_i,c_j^\dagger\}=\delta_{ij},
 \qquad
...
\]
即得\eqnrefs{eq:phys-sm-neighbor-jw}。此式把邻接自旋耦合写成费米子双线性，是后续把 转移矩阵 表为 费米子 Gaussian 的关键。
...
对满足 正则反交换关系 的费米子，Gaussian 态下的 \(2n\) 点函数由 Pfaffian 给出：
\begin{equation*}
...
Jordan--Wigner 变换把自旋耦合写成费米子双线性，但要对 转移矩阵 做谱分析，还需知道二次型指数的共轭作用如何作用于费米子空间。引入 Majorana 算符后，这一问题化为 $2N$ 维实正交线性代数——Clifford 代数的核心结构。
...
\end{equation}
因此指数维 Fock-空间 算符的共轭作用，等价于 \(2N\) 维 Majorana 空间中的复正交线性变换。

\begin{derivation}[Clifford 对易子 与 Lie 代数映射]
...
对满足 CCR \([b_i,b_j^\dagger]=\delta_{ij}\) 的玻色子，定义 厄米 Quadratic \(\mathcal Q_{\rm B}=\sum A_{ij}b_i^\dagger b_j\)，则 \([\mathcal Q_{\rm B},b]=\mathsf A b\) 给出 \(\mathfrak{gl}(N)\) 而非 \(\mathfrak{so}(2N)\) 的作用，对应辛群 \(\mathrm{Sp}(2N)\) 而非旋量群。玻色子 Gaussian 的迹由 \(\det(1-\mathsf T)^{-1}\) 给出（分母幂），费米子由 \(\det(1+\mathsf T)\) 给出（分子幂），二者统一于 superdeterminant（Berezinian）\(\operatorname{sdet}(1-\mathsf T)=\det(1-\mathsf T_{\rm B})/\det(1+\mathsf T_{\rm F})\)，这是超对称量子力学的代数基础。
\end{derivation}
...
\[
 ...
 =-2c_N^\dagger,
\]
其中用了 \((c_N^\dagger)^2=0\)（正则反交换关系 的直接推论）。因此
\[
 \tau_N^z(c_N+c_N^\dagger)
...
边界条件 \(c_{N+1}=-\mathcal P c_1\) 表明费米子在绕环一周后获得 \(\mathcal P\) 决定的相位：偶奇偶扇区给反周期（NS），奇奇偶扇区给周期（R）。这正是 Euclidean 路径积分中 自旋结构 的离散对应：环面上四个 自旋结构 由 \((\alpha_x,\alpha_y)\in\{0,\tfrac12\}^2\) 标记，而 Ising 模型的四个 迹 扇区 \((s,t)\) 通过此边界条件自然实现。负号 \(-\mathcal P\) 中的整体 \(-1\) 来自 Jordan--Wigner 字符串的非局域性，是 Kauffman 圆环模型的拓扑特征，也是 Majorana 零模在拓扑相变中符号翻转的代数根源。

在亏格 \(g\) 曲面上，自旋结构 形成以 \(H^1(\Sigma_g;\mathbb Z_2)\cong(\mathbb Z_2)^{2g}\) 为平移群的 仿射 扭子空间，共有 \(2^{2g}\) 个。Ising 模型对应 \(g=1\) 的四个扇区；高亏格情形需用 Arf 不变量区分 偶/奇 自旋结构，这是 Kitaev toric code 与 Ising 任意子分类的共同代数基础。
\end{derivation}
...
空间 NS/R 边界条件确定以后，还需要在时间方向区分是否插入 费米子 宇称。于是有限环自然出现四个 费米子 迹；它们与曲面上的四个 自旋结构 对应，但并不等同于 \(\mathrm H_1(T^2;\mathbb F_2)\) 的四个同调类。

空间边界只有 NS/R 两种；环面还需要区分 转移 方向的费米子周期性。算符语言中，第二个二分由迹中是否插入 \(\mathcal P\) 实现。定义
\begin{equation}
 \mathcal Z_{s,t}
 =\Tr_{s}\!\left(
...
\end{equation}
这里 \(\mathsf T_s\) 是在相应空间费米子边界条件下得到的二次 转移 算符。若用 \(\alpha_x,\alpha_y\in\{0,\tfrac12\}\) 标记 Euclidean 费米子 在两个周期方向的 twist，约定 \(0\) 为周期 R、\(\tfrac12\) 为反周期 NS，则普通 迹 在 转移 方向给反周期条件，而插入 \(\mathcal P\) 后给周期条件。因此四个算符 迹 与 费米子 自旋结构 的字典为
\[
 \begin{array}{c|c}
 ...
 \end{array}
\]
其中时间方向的反周期性来自 费米子 迹 的标准 相干-态 sewing，而不是额外施加的 Ising 边界条件。物理自旋 Hilbert 空间要求 NS 只保留偶奇偶、R 只保留奇奇偶，因此
\begin{equation}
 \boxed{
 Z_{\rm Ising}
...
\end{equation}
这个投影式是有限尺寸严格恒等式；它比直接背诵某个 Pfaffian 的“\(+,+,+,-\)”整体符号更稳健，因为后者还依赖 Kasteleyn/Pfaffian 取向和 自旋-结构 标签 约定。

高温闭链扇区属于 \(\mathrm H_1(T^2;\mathbb F_2)\)，而 \((s,t)\) 是 费米子 边界 twists。在选择参考 自旋结构 后，后者形成以 \(\mathrm H^1(T^2;\mathbb F_2)\) 为平移群的 仿射 扭子空间；两套四重对象通过 特征标 Fourier 变换联系，而不是逐项同一。
...
有限尺寸谱的正确顺序因此是：先选 NS/R 网格；再对所有非自反 \(\{q,-q\}\) 块对角化；随后把自反模单独纳入；最后使用\eqnrefs{eq:phys-sm-kaufman-sector-projection} 做奇偶投影。只有在这些步骤之后才能取 \(N\to\infty\)。远离临界点时色散解析，NS/R 离散和的差异指数小；临界点出现 \(|q|\) 尖点 后，\(\log\Lambda\) 的扇区差变成 \(O(N^{-1})\)，对应每格点自由能的 \(O(N^{-2})\) 修正。临界离散和的显式 Fourier 渐近将进一步给出这个系数，而不只停留在量级判断。

\begin{derivation}[临界自反模与宇称投影]
临界点处 R 扇区的 \(k=0\) 自反模软化。这里讨论的是周期链的体临界模；它不同于有能隙拓扑相开放边界上的局域 Majorana 端模。

零模条件是 \(\epsilon(0)=0\)，并在各向同性情形化为 \(\sinh(2K_c)=1\)，即\eqnrefs{eq:phys-sm-square-critical-k}。此时 厄米 BdG 块仍可对角化，不会产生 Jordan 块；特殊之处只在于本征值为零，通常的“正能支”约定不再唯一。有限尺寸物理还必须连同全局费米子宇称投影处理。

把零能复费米子写成两个 厄米 分量时，每个分量都与临界二次 Hamiltonian 对易并翻转费米子宇称。因此未投影 Fock 空间出现成对态；物理转移矩阵只保留满足扇区宇称条件的组合。\(\mathcal Z_{\mathrm R,0}-\mathcal Z_{\mathrm R,1}\) 在临界点消失，正是零能占据与空置贡献相消的结果，而不是额外的环面拓扑简并。

离开临界点后该体模获得质量 \(m\propto K-K_c\)，并不继续保持零能。Jordan--Wigner 后的量子 Ising 链可与 Kitaev 链比较，但开放链端模、周期链体临界模与环面 自旋-结构 扇区是三个不同概念，不能用 \(2^{2g}\) 简并把它们等同。

\end{derivation}

临界有限尺寸自由能的次领项可用 Ising 共形场论解释。
临界点处有限尺寸自由能的精确系数由离散 Fourier 和的 Euler--Maclaurin 渐近给出，结果与 \(c=1/2\) Ising 共形场论 一致。分三步计算。

由\eqnrefs{eq:phys-sm-fermion-dispersion}，各向同性临界 \(K=K_c\) 时 \(\epsilon(k)=2|k|\)（小 \(k\) 线性色散，尖点）。有限环 \(N\) 上每格点自由能
\begin{equation*}
  -\beta f_N=\log 2+\frac{1}{2N}\sum_{q\in s}\log(2\sinh\frac{\epsilon(q)}{2})+\text{const},
\end{equation*}
其中 \(s\) 是 NS（\(q=(m+1/2)\cdot2\pi/N\)）或 R（\(q=m\cdot2\pi/N\)）网格。主导差异来自 \(q\to0\) 的 尖点 贡献。

对光滑函数 \(g(q)\)，\(\frac{1}{N}\sum_{m}g\!\left(\frac{2\pi m}{N}\right)=\int_0^{2\pi}\frac{g(q)\dd q}{2\pi}+\frac{g'(0)-g'(2\pi)}{2N}+\cdots\)。但 \(\log|q|\) 在 \(q=0\) 有 尖点，\(g'(0^+)-g'(0^-)=2/N\)，给出额外 \(O(N^{-1})\) 项。精确计算（Cardy 1984）给出 NS/R 扇区自由能差
\begin{equation*}
...
\(c=1/2\) 极小模型 共形场论 有三个 主 场：
\begin{equation*}
  \mathbf 1:\;(h,\bar h)=(0,0),\quad\sigma:\;(h,\bar h)=\left(\frac{1}{16},\frac{1}{16}\right),\quad\psi:\;(h,\bar h)=\left(\frac12,\frac12\right).
\end{equation*}
自旋关联 \(\langle\sigma\sigma\rangle\sim r^{-2h_\sigma}=r^{-1/8}\)（因 \(2h_\sigma=1/8\)），与\eqnrefs{eq:phys-sm-onsager-magnetization} 的 \(\beta=1/8\) 一致。能量关联 \(\langle\psi\psi\rangle\sim r^{-2h_\psi}=r^{-2}\)，对应能量-能量关联的 \(\eta_\epsilon=1\)。有限尺寸自由能\eqnrefs{eq:phys-sm-ising-cft-central-charge} 中 \(c=1/2\) 可从三个 主 的 Virasoro 代数表示验证：\(\sigma\) 的模不变性（模 S 矩阵）给出字符 \(\chi_0+\chi_{1/2}\)（NS 扇区）和 \(\chi_{1/16}\)（R 扇区），其组合在 模 变换下的不变性唯一确定 \(c=1/2\)。这把 Onsager 精确解的代数结构提升为共形不变性——临界 Ising 模型 \emph{就是} \(c=1/2\) Ising 共形场论 的格点正则化。

共形场论 预言有限尺寸能谱 \(E_n-E_0=\frac{2\pi v}{N}(h_n+\bar h_n-1/12)\)，其中 \(v\) 是临界速度。Ising 共形场论 的三个 主 给出三个最低能级：\(\mathbf 1\)（真空，\(E=E_0\)）、\(\sigma\)（自旋，\(E-E_0=\frac{\pi v}{8N}\)）、\(\psi\)（能量，\(E-E_0=\frac{\pi v}{N}\)）。这些能级在 转移矩阵 谱中精确可辨，对应 NS/R 扇区的不同 扇区，是 共形场论-算符 correspondence 的格点验证。
...
\]
保持不变。这与 NS/R 扇区投影\eqnrefs{eq:phys-sm-kaufman-sector-projection} 相容，是格点 Ising 与 (c=1/2) 共形场论 对应的谱学检验。

从格点 Ising 到 \(c=1/2\) 共形场论 的对应可在两个严格意义上验证：(1) 有限尺寸自由能系数 \(-\pi c/(48N^2)\) 精确匹配（Cardy 1984）；(2) 转移矩阵 谱的能级间距 \(\frac{2\pi v}{N}(h+\bar h-1/12)\) 精确匹配（Belavin--Polyakov--Zamolodchikov 1984）。两者都依赖 Ising 的自由费米子结构——Bogoliubov 谱给出精确能级，Euler--Maclaurin 给出精确自由能修正。对一般格点模型（如 3D Ising），无此精确对应，只有数值和重整化群证据，凸显了 2D Ising 精确可解性在格点--共形场论 对应中的独特地位。
...
\end{equation*}
其中求和遍历所有偶子图（每个顶点度数为偶）。方格图上偶子图等价于闭环集合——每个顶点度数为 0 或 2，因此 \(E'\) 分解为不相交的闭合多边形。这是 high-温度 展开的核心：配分函数的非平凡部分是所有闭环的带权求和。
...
与 转移矩阵 的等价。Kac--Ward 行列\eqnrefs{eq:phys-sm-kac-ward-pfaffian} 与 转移矩阵 对角化给出同一配分函数，但途径不同：转移矩阵 用 Jordan--Wigner → Bogoliubov → 谱乘积，Kac--Ward 用图展开 → Pfaffian → 行列式。两者的等价可在有限尺寸验证：Kac--Ward 行列式的本征值恰是 Bogoliubov 色散 \(\epsilon(q)\) 的乘积。这一等价性是 2D Ising 精确可解性的代数根源——Clifford 代数（转移矩阵）与 Pfaffian 代数（Kac--Ward）是同一数学结构的两种表现。

Kac--Ward 方法在 3D 失效：3D 方格的偶子图是闭合曲面而非路径，Pfaffian 的完全匹配（一维对象）无法表示二维曲面。3D Ising 的配分函数需要更高阶的代数结构（如 Grassmann 积分或 四元数 Pfaffian），目前无精确解。这一障碍是拓扑的：2D 偶子图由 \(\mathbb Z_2\) 同调分类（一阶上同调），3D 偶子图由 \(\mathbb Z_2\) 二阶上同调分类，后者无法用 Pfaffian（一阶对象）表示。这是 3D Ising 精确可解性困难的代数拓扑起源。

临界格点观察量的 s 全纯性 给出了通向连续共形场论的严格桥梁。

离散复结构。方格图 \(G\) 的对偶图 \(G^*\) 和 中间 图 \(G_M\)（旋转 \(45^\circ\)）共同定义离散复结构。每条边 \(e\in E(G)\) 对应对偶边 \(e^*\in E(G^*)\)，中间 边 \(e_M\) 连接 \(e\) 和 \(e^*\) 的中点。离散导数在 中间 图 上定义：函数 \(F:V(G_M)\to\mathbb C\) 的离散导数 \(\partial_M F\) 用相邻 中间 顶点的差商给出。

s 全纯 条件。Ising 观察值 \(F(e_M)\)（定义在 中间 边上）满足 s 全纯 条件：对每个 中间 顶点 \(z\)，
\begin{equation*}
  \sum_{e_M\ni z}\operatorname{proj}_{\ell(e_M)}F(e_M)=0,
\end{equation*}
其中 \(\ell(e_M)\) 是与边 \(e_M\) 关联的直线（方向由 Ising 耦合的 Kac--Ward 相位确定），\(\operatorname{proj}_\ell\) 是到 \(\ell\) 的正交投影。s 全纯 弱于全纯：它要求投影（而非函数本身）满足离散 Cauchy--Riemann 方程。

临界 s 全纯 与连续极限。临界 Ising（\(\sinh(2K_c)=1\)）的 s 全纯 函数在网格细化 \(a\to0\) 时收敛到全纯函数：\(\operatorname{proj}_\ell F\to\operatorname{Re}(f)\) 或 \(\operatorname{Im}(f)\)（视 \(\ell\) 方向），因此 \(F\to f\) 全纯。这一收敛是指数快的（远离临界点衰减），给出临界关联函数的严格连续极限。特别地，自旋关联 \(\mathbb E[\sigma_{z_1}\cdots\sigma_{z_n}]\) 的离散 holomorphicity 保证其收敛到 共形场论 关联函数 \(\langle\sigma(z_1)\cdots\sigma(z_n)\rangle_{\mathrm{CFT}}\)。

Smirnov 的收敛定理。Smirnov (2010) 证明：临界方格 Ising 的自旋关联在网格细化下收敛到 \(c=1/2\) Ising 共形场论 的相应关联函数，收敛是局部一致的。这给出了从格点模型到 共形场论 的严格收敛证明（而非仅形式匹配），是 2D Ising 精确可解性最深刻的严格结果。s 全纯性 是这一证明的核心工具：它把离散 Ising 观察量嵌入到离散复分析框架中，使连续极限成为全纯函数的收敛问题。这一方法已推广到其他临界格点模型（如 FK-Ising、回路 O(1)），是现代统计力学严格收敛理论的基石。
\end{derivation}
