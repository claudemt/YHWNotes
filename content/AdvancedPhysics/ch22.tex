...
电子 Hamiltonian \(\hat H_e(\mathbf R)\) 只作用于电子坐标，满足 \(\hat H_e|\phi_n\rangle=E_n(\mathbf R)|\phi_n\rangle\)。因此
\begin{equation*}
...
\end{itemize}
电子部分给出 \(E_m(\mathbf R)\chi_m\)，合并即得\eqnrefs{eq:born-huang-coupled}。绝热近似忽略 \(m\neq n\) 耦合，在大质量极限 \(M_A\to\infty\) 下成立；两绝热面简并时 \(\mathbf d_{mn}\) 发散，绝热近似失效（conical intersection）。
...
derivative coupling 的变换律。电子基变换 \(|\phi_n'\rangle=\sum_m|\phi_m\rangle U_{mn}(\mathbf R)\) 给出
\[
...
接下来考察 Born--Oppenheimer 近似的控制参数。
...
\end{equation*}
模 \(|\mathbf d_{mn}^{(A)}|\) 不变（相位因子模为 1）。因此 \(\Phi_n=\sum_A\frac{\hbar^2}{2M_A}\sum_{m\neq n}|\mathbf d_{mn}^{(A)}|^2\) 是规范标量——不依赖电子态的相位约定。\(\Phi_n\) 只含非对角项，故为规范标量。\(\Phi_n\ge0\) 意味着绝热近似使核有效势能升高；conical intersection 附近 \(\mathbf d_{mn}\) 发散，\(\Phi_n\) 随之发散。
