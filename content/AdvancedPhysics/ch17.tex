...
\begin{derivation}[FW 偶--奇 消元到相对阶 \(1/c^2\)]
第一步取生成元
\[
...
\]
中，$\ii[S_1,\beta mc^2]$ 恰好消去最低阶 奇 块。重复消元并保留目标阶，偶 部分为
\begin{equation}
H_{\rm FW}
=\beta mc^2+\mathcal E
...
\end{equation}
这就是 Land\'e interval 规则。它要求 LS 耦合 是良好近似；若自旋轨道与电子间静电分裂同阶，必须在固定 $J$ 子空间内整体对角化，即 intermediate 耦合。
...
两个不共线 Lorentz 推进 的乘积并不是纯 推进，而会附带空间旋转。对速度 $\mathbf v(t)$ 的粒子，相邻瞬时共动系之间的 Thomas--Wigner 角速度为
\begin{equation}
\boldsymbol\Omega_T
...
\end{equation}
电子在中心电场中的瞬时静止系会看到运动诱导磁场；若只计算该磁场引起的 Larmor 进动，会得到两倍于\eqnrefs{eq:pauli-from-dirac}的自旋轨道系数。共动坐标系自身还按\eqnrefs{eq:thomas-nonrel}旋转，恰好把多出的因子减半。因此 Thomas 因子是 推进 非交换性的经典几何表现，与 Dirac/FW 的量子结果一致。
...
\end{equation}
从而位置包含频率约 $2mc^2/\hbar$ 的正负能量干涉项，即 颤动。FW 表象把正负能量子空间在给定阶数上解耦后，这一快速干涉不再作为低能正能量波包的独立运动项。Pauli 理论因此应理解为 Dirac 谱子空间的受控块对角化，而不是“删去两个分量”。
...
自由 Dirac 电子的位置算符含快速振荡项\eqnrefs{eq:alpha-zitter}，频率 $\omega_Z=2mc^2/\hbar\approx1.55\times10^{21}\,\mathrm{rad/s}$，振幅 $\lambda_C=\hbar/(mc)\approx3.86\times10^{-13}\,\mathrm{m}$（电子 Compton 波长）。这一「颤动运动」来自正负能态的干涉，在 FW 表象中被精确消去——低能电子不表现 颤动。但在石墨烯（Dirac 锥有效理论）中，$\omega_Z$ 被压低到可实验观测的尺度（$\sim10^{13}\,\mathrm{rad/s}$），已在输运实验中间接探测。
...
FW 变换已在前框给出至 \(O(c^{-2})\) 的显式结果。其中自旋-轨道项系数 \(\frac{1}{2m^2c^2}\) 自动含 Thomas 半因子：直接投影正能态只给出 \(\frac{1}{m^2c^2}\)，而 FW 变换正确处理了正负能态耦合到二阶。
\end{derivation}
