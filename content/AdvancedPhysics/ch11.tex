...
对三种边界分别建立 coercivity（能量范数控制 Sobolev 范数），再用 Rellich 紧嵌入得到紧 resolvent，最后由谱定理给出离散谱。

椭圆上下界。式~\eqnrefs{eq:phys-cont-fourth-ellipticity} 给出 Hessian 层面的夹逼
\begin{equation*}
 c_0\|\nabla^2u\|_{L^2}^2
 \le a_{\mathsf C}[u,u]
 \le c_1\|\nabla^2u\|_{L^2}^2.
...
固支与简支的 coercivity。在 \(H_0^2(\mathcal D)\)（固支）与 \(H^2(\mathcal D)\cap H_0^1(\mathcal D)\)（简支）上，二阶 Poincar\'e 不等式
\begin{equation*}
...
紧 resolvent 与谱定理。三种情形的 form norm
\begin{equation*}
