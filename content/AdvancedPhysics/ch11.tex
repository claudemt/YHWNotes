...
先讨论椭圆上下界。式~\eqnrefs{eq:phys-cont-fourth-ellipticity} 给出 Hessian 层面的夹逼
\begin{equation*}
 c_0\|\nabla^2u\|_{L^2}^2
 \le a_{\mathsf C}[u,u]
 \le c_1\|\nabla^2u\|_{L^2}^2.
...
随后讨论固支与简支的 coercivity。在 \(H_0^2(\mathcal D)\)（固支）与 \(H^2(\mathcal D)\cap H_0^1(\mathcal D)\)（简支）上，二阶 Poincar\'e 不等式
\begin{equation*}
...
接着讨论紧 resolvent 与谱定理。三种情形的 form norm
\begin{equation*}
...
先讨论刚度谱 Weyl 计数。对 \(p_4=|\boldsymbol\xi|^4\)，条件 \(p_4\le\lambda\) 即 \(|\boldsymbol\xi|\le\lambda^{1/4}\)，是半径 \(\lambda^{1/4}\) 的二维圆盘，面积为
\begin{equation*}
...
随后讨论质量加权算子的主符号。广义本征问题 \(\mathsf L_X\phi_n=\omega_n^2\mu_A\phi_n\) 通过酉变换 \(\psi_n=\mu_A^{1/2}\phi_n\) 化为普通本征问题 \(\widetilde{\mathsf L}_X\psi_n=\omega_n^2\psi_n\)，其中 \(\widetilde{\mathsf L}_X=\mu_A^{-1/2}\mathsf L_X\mu_A^{-1/2}\)。乘法算子 \(\mu_A^{-1/2}\) 是零阶 PsDO，在最高阶只把主符号左右各乘一次，因此
\begin{equation*}
...
再讨论物理频率计数。阈值 \(\omega_n\le\varpi\) 等价于 \(p_{\widetilde{\mathsf L}}\le\varpi^2\)，即
\begin{equation*}
 p_4(\vb x,\boldsymbol\xi)
