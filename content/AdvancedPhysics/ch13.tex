...
\]
对时间作 Fourier 变换，并用
\(\mathcal Q^{\rm E}_{ij}=3I_{ij}-\delta_{ij}I_{kk}\) 与
\(\vb\Pi_\perp\cdot\vb n=0\)，便有
\[
 \vb\Pi_\perp\cdot\boldsymbol{\mathcal J}
 =-\ii\omega\vb\Pi_\perp\cdot\widetilde{\vb p}
 +\ii k\,\vb n\times\widetilde{\vb m}
...
\]
于是电偶极和磁偶极的横向角因子均为 \(8\pi/3\)。对称无迹四极张量则有
\[
 \int\dd\Omega\,
 |\vb\Pi_\perp\cdot(\vb{\mathcal Q}^{\rm E}\cdot\vb n)|^2
 =\frac{4\pi}{5}\mathcal Q^{\rm E}_{ij}
 (\mathcal Q^{\rm E}_{ij})^*.
\]
把这些结果代入精确母式，并使用
...
\[
 ...
 (\dot a^\mu u^\nu-\dot a^\nu u^\mu),
\]
即\eqnrefs{eq:phys-rr-regular-field-worldline}。有限项的系数也可在瞬时静止系
\(u^\mu=(c,\vb0)\)、\(a^\mu=0\) 上由
\(\tfrac12(\vb E_{\rm ret}-\vb E_{\rm adv})
 =q\vb j/(6\pi\varepsilon_0c^3)\) 校准；协变性随后唯一恢复上式。
\end{derivation}
...
\[
 ...
 =\frac{q^2}{8\pi\varepsilon_0c^2\ell}u^\mu .
\]
有限的局域束缚动量只能与 \(a^\mu\) 成正比；由正则场自力与世界管守恒匹配，
其系数为
\[
 P_{\rm bound}^\mu(\ell)
 =\frac{q^2}{8\pi\varepsilon_0c^2\ell}u^\mu
...
\begin{equation}
 ...
 \qquad \epsilon_{\rm grad}=\frac{\tau_{\rm rr}}T .
