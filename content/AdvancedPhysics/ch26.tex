...
Schur 引理与矩阵元正交。设有限群 \(G\) 有不可约表示 \(D^{(\alpha)}:G\to\mathrm{GL}(V_\alpha)\)，\(\dim V_\alpha=d_\alpha\)。Schur 引理：若线性算符 \(T:V_\alpha\to V_\beta\) 满足 \(T\,D^{(\alpha)}(g)=D^{(\beta)}(g)\,T\) 对所有 \(g\in G\)，则 \(T=0\)（\(\alpha\neq\beta\)）或 \(T=\lambda\,\mathbb I\)（\(\alpha=\beta\)）。首先构造 intertwining 算符
\begin{equation*}
...
对\eqnrefs{eq:matrix-element-orthogonality} 取迹（令 \(i=j\)、\(k=l\) 并求和）：
\begin{equation}
...
设 \(G\) 作用于有限集 \(X\)。不动点集 \(\mathrm{Fix}(g)=\{x\in X:gx=x\}\)。置换表示 \(\chi_{\rm perm}(g)=|\mathrm{Fix}(g)|\)。不等价轨道数等于置换表示中平凡表示 \(\mathbf 1\) 的重数，由\eqnrefs{eq:multiplicity-formula}（\(\alpha=\mathbf 1\)，\(\chi^{(\mathbf 1)}\equiv1\)）：
\begin{equation}
N_{\rm orbit}=\frac{1}{|G|}\sum_{g\in G}|\mathrm{Fix}(g)|.
\label{eq:burnside-orbit-count}
\end{equation}
即\eqnrefs{eq:burnside-orbit-count}。应用于 \(NH_3\)（\(C_{3v}\)，\(|G|=6\)）：\(\chi_V(e)=12\)、\(\chi_V(C_3)=0\)、\(\chi_V(\sigma_v)=2\)，得 \(n_{A_1}=3\)、\(n_{A_2}=1\)、\(n_E=4\)；减去平移与转动后 \(V_{\rm vib}=2A_1\oplus 2E\)，即 4 个振动模式。

\end{derivation}

把 \(3N\) 个核位移投影到不可约表示后，可直接判定正常模与光谱活性。

振动表示的构造。\(N\) 原子分子有 \(3N\) 个核位移坐标，总表示 \(\Gamma_{\rm tot}=\Gamma_{\rm trans}+\Gamma_{\rm rot}+\Gamma_{\rm vib}\)。平移表示 \(\Gamma_{\rm trans}\) 是位置向量 \((x,y,z)\) 在分子对称群 \(G\) 下的表示，转动表示 \(\Gamma_{\rm rot}\) 是轴向量 \((R_x,R_y,R_z)\) 的表示。振动表示 \(\Gamma_{\rm vib}=\Gamma_{\rm tot}-\Gamma_{\rm trans}-\Gamma_{\rm rot}\) 由减法给出。

Wilson--Decius 振动向量。对每个原子 \(i\) 和方向 \(\alpha\)，定义 Wilson--Decius 向量 \(\boldsymbol e_{i\alpha}\)（原子 \(i\) 沿 \(\alpha\) 方向单位位移，其余原子不动）。\(3N\) 个 \(\boldsymbol e_{i\alpha}\) 张成位移空间，群 \(G\) 的作用通过置换原子和旋转方向给出 \(\Gamma_{\rm tot}\) 的矩阵表示。特征标 \(\chi_{\rm tot}(R)=N_R(1+2\cos\theta_R)\)（\(N_R\) 是操作 \(R\) 下不动的原子数，\(\theta_R\) 是旋转角）——这是 Wilson--Decius 公式，把 \(\Gamma_{\rm tot}\) 的特征标从几何操作直接计算。

正规坐标的对称性。正规坐标 \(Q_k\) 是振动表示 \(\Gamma_{\rm vib}\) 的不可约基：\(R Q_k^{(\mu)}=\sum_{k'}D^{(\mu)}_{k'k}(R)Q_{k'}^{(\mu)}\)。同一不可约表示 \(\mu\) 的模式 \(Q_{k_1}^{(\mu)},Q_{k_2}^{(\mu)}\) 可通过对称性允许的势能项耦合，不同不可约表示的模式不耦合（对称性禁戒）。因此振动 Hamiltonian 在正规坐标下分块对角化，每块对应一个不可约表示——这是群论约化的物理效率。
