...
\end{equation*}
其中 \(\alpha_0=eE_0/(m_e\omega^2)\) 为 quiver 振幅。

quiver 动能为 \(\frac{1}{2}m_e\dot{\mathbf x}^2=\frac{1}{2}m_e\alpha_0^2\omega^2\cos^2\omega t\)。周期平均利用 \(\langle\cos^2\omega t\rangle=\frac{1}{2}\)：
\begin{equation*}
...
=U_p.
\end{equation*}
即 ponderomotive 能~\eqnrefs{eq:ponderomotive-energy-restated}。

电离后连续态总能量为 \(\varepsilon_p=p^2/(2m_e)+U_p\)——quiver 动能 \(U_p\) 作为有效势能出现在连续态中。能量守恒要求 \(n\hbar\omega=I_p+p^2/(2m_e)+U_p\)（吸收 \(n\) 个光子，克服电离能 \(I_p\)，剩余为光电子动能加 quiver 能）。因此光电子动能
\begin{equation*}
...
He 在 1600\,nm、\(2\times10^{14}\,\mathrm{W/cm^2}\) 的截止能量 \(\hbar\omega_{\rm cutoff}\approx176\,\mathrm{eV}\)，对应波长 \(\lambda\approx1240/176\approx7.0\,\mathrm{nm}\)，已接近水窗（280--540\,eV，即 2.3--4.4\,nm）的长波端。使用更短波长（如 2000\,nm 中红外）可进一步推高 \(U_p\propto\lambda^2\)，使截止进入水窗区域——这是中红外 HHG 用于生物成像的物理基础。
\end{exbox}

\begin{derivation}[Lewenstein HHG 模型与量子轨道理论]
谐波辐射由电子复合偶极矩 \(\vb d(t)=\langle\psi(t)|\vb r|\psi(t)\rangle\) 的 Fourier 分量 \(\vb d(\omega)=\int\vb d(t)\ee^{\ii\omega t}\dd t\) 决定。SFA 下 \(\psi(t)=\psi_0(t)+\psi_{\rm Volkov}(t)\)，复合只来自 \(\psi_{\rm Volkov}\to\psi_0\) 的跃迁。Lewenstein 公式
\begin{equation*}
...
\end{equation*}
其中 \(S=\int_{t'}^{t''}[\frac{(\vb p-\vb A)^2}{2}+I_p]\dd\tau-\omega t''\) 为半经典作用量。
...
\end{align*}
电离条件要求 \(t'\) 为复数（\(\mathrm{Im}\,t'>0\)），对应隧穿；\(t''\) 为实数。

每个鞍点 \((t'_k,t''_k,\vb p_k)\) 对应一条量子轨道：\(t'_k\) 复时间隧穿、\(t''_k\) 实时间回碰。谐波谱
\begin{equation*}
  \vb d(\omega)\approx\sum_k C_k\exp[-\ii S_k(\omega)],
\end{equation*}
短波长极限下最短轨道主导，极值给出 \(3.17U_p\) 截止。

多轨道相位差 \(\Delta S=S_k-S_{k'}\) 产生干涉，可由双色场等脉冲形状控制。SFA 忽略的母态耗尽、Coulomb 传播与宏观相位匹配不改鞍点结构，但定量预测强度时须另行加入。

外场连续态基底由 Volkov 解给出：平面波场中非相对论极限下
\begin{equation}
...
\end{equation}
SFA 即将波函数展开为束缚态加 Volkov 连续态。Coulomb 对连续态的修正在低能区显著，主要影响谱形而不改截止能 \(3.17U_p\)。
\end{derivation}
