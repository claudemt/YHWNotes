...
\eqnrefs{eq:minimal-coupling}是后续所有规范选择的共同起点。对电子 \(Q=Q_e=-e\)、质量为 \(m_e\)。只有在这一一般形式建立以后，我们才进入强场常用的偶极近似 \(k a\ll1\)，把原子尺度内的矢势写成 \(\mathbf A(\mathbf r,t)\simeq\mathbf A(t)\)。在 速度规范 且取 \(\Phi=0\) 时，电子 Hamiltonian 为
...
对电子 \(Q=-e\)，利用 \(\mathbf E(t)=-\partial_t\mathbf A(t)\)，得到 长度规范
...
精确理论中，由规范变换联系的 Hamiltonian 彼此幺正等价。有限基组截断或不一致的 SFA 分割可能破坏这种等价性；因此，近似计算中的 规范 dependence 应被视为近似误差的诊断，而不是新的可观测量。
\end{note}
...
\end{equation}
其中 $A-\partial\chi$ 使用四维记号。任何有限基或 SFA 截断若不同时变换态、算符与边界相位，就可能人为产生 规范 dependence。
...
\end{equation*}
其中 \(\alpha_0=eE_0/(m_e\omega^2)\) 为 抖动 振幅。

抖动 动能为 \(\frac{1}{2}m_e\dot{\mathbf x}^2=\frac{1}{2}m_e\alpha_0^2\omega^2\cos^2\omega t\)。周期平均利用 \(\langle\cos^2\omega t\rangle=\frac{1}{2}\)：
\begin{equation*}
...
=U_p.
\end{equation*}
即 有质动力 能\eqnrefs{eq:ponderomotive-energy-restated}。

电离后连续态总能量为 \(\varepsilon_p=p^2/(2m_e)+U_p\)——抖动 动能 \(U_p\) 作为有效势能出现在连续态中。能量守恒要求 \(n\hbar\omega=I_p+p^2/(2m_e)+U_p\)（吸收 \(n\) 个光子，克服电离能 \(I_p\)，剩余为光电子动能加 抖动 能）。因此光电子动能
\begin{equation*}
...
SFA 没有一个在所有强场参数区间都普适的小参数。它通常在电子离核后 抖动 动能远大于 Coulomb 势修正、且目标可观测量不强烈依赖低能长程散射时最可靠。若需要系统恢复 Coulomb 效应，可以进一步使用 Coulomb--Volkov、eikonal-Volkov 或 Coulomb-corrected SFA 等方法。
...
不同复鞍点之间的相位差会产生 ATI 峰内结构、半周期干涉以及所谓 intracycle/intercycle interference。所谓“量子 轨道”并不是电子具有一条可直接观测的经典路径，而是每一个复鞍点及其对应的半经典作用量对积分作出的驻相贡献。
...
鞍点的虚部控制指数抑制，但不能在没有额外测量定义时直接等同于唯一的可观测“隧穿时间”。不同 操作性 定义 可能对应 Wigner time、Larmor time、attoclock offset 等不同量。SFA 的复时间首先是解析延拓后的驻相坐标。
\end{note}
...
因此 \(\gamma_K\) 同时编码束缚能、抖动 动能和驱动时间尺度。提高场强 \(E_0\) 会减小 \(\gamma_K\)，提高角频率 \(\omega\) 会增大 \(\gamma_K\)，不同原子的 \(I_p\) 也直接改变动力学区间。
...
则 \eqnrefs{eq:floquet-eigenproblem} 变成“物理 Hilbert 空间 \(\otimes\) Fourier 格点”的无限维定态本征问题。这才是 穿衣 态 与 光子 sideband 的一般数学结构。
...
Keldysh 参数 \(\gamma_K=\sqrt{I_p/(2U_p)}\) 区分多光子（\(\gamma_K\gg1\)）与隧穿（\(\gamma_K\ll1\)）区域，其中 \(I_p\) 是电离势，\(U_p=e^2E_0^2/(4m\omega^2)\) 是 有质动力 能。
