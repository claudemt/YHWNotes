...
\begin{derivation}[三维薄板能量到 Kirchhoff--Love 约束]
令面内尺度为 \(L\)、厚度为 \(h\)，并取
\[
...
于是
\[
 \varepsilon_{\alpha\beta}=O(hw_\star/L^2),\qquad
 \varepsilon_{\alpha z}
 =\frac{w_\star}{2L}
 (\widehat\partial_\alpha\widehat w+\partial_\zeta\widehat u_\alpha).
\]
以弯曲能 \(E_\star h^3w_\star^2/L^2\) 归一化时，剪切能前带
\(\epsilon_h^{-2}=(L/h)^2\)。有限能量首先要求主阶剪切相消。三维面内平衡
\[
 \partial_zT_{\alpha z}
 =-\partial_\beta T_{\alpha\beta}
 +O(\epsilon_h^2)T_{\alpha\beta}/L
\]
再结合上下表面 \(T_{\alpha z}=0\)，给出
\(T_{\alpha z}=O(\epsilon_h)T_{\alpha\beta}\) 与
\(T_{zz}=O(\epsilon_h^2)T_{\alpha\beta}\)。厚度反射对称材料中，
\[
 \varepsilon_{\alpha z}
 =O(\epsilon_h)\varepsilon_{\alpha\beta}^{(b)},\qquad
 u_\alpha=u_\alpha^{(0)}-z\partial_\alpha w
 +O(\epsilon_h^3w_\star),
\]
这就是\eqnrefs{eq:phys-cont-kl-kinematics}的首阶运动学。

法向应变不能同时强行置零；首阶条件是平面应力
\[
 0=T_{zz}=C_{zz\alpha\beta}e_{\alpha\beta}
...
\[
 ...
 -\frac{C_{\alpha\beta zz}C_{zz\gamma\delta}}{C_{zzzz}},
\]
即\eqnrefs{eq:phys-cont-plane-stress-schur}。最后将
...
普适性说明。 此证明只用到了：(a) 算子的自伴性与正定性，(b) 本征函数在节域上满足 Dirichlet 边界，(c) 极小极大原理。这三条对任意维数、任意有界区域、任意自伴椭圆算子（包括 Laplace、双调和、甚至 Schr\"odinger 算子 \(-\nabla_t^2+V\)）都成立。因此 Courant 定理是谱几何中最普适的节线约束之一。

紧致性。 对紧流形（如球面 \(\mathbb S^2\)）上的 Laplace--Beltrami 算子，同一证明给出 \(\mu(\Psi_j)\le j\)，这就是球谐函数节线理论的出发点。对薄壳（曲面上的双调和算子），只要壳的曲率有界且算子仍自伴正定，Courant 界同样有效。
\end{derivation}
...
随后讨论证明骨架。设 \(\mu_j\) 是 \(\phi_j\) 的节域数。每个节域 \(\Omega_k\) 上 \(\phi_j\) 是第一本征函数（Dirichlet，因 \(\phi_j|_{\partial\Omega_k}=0\)），故 \(\lambda_j\ge\lambda_1(\Omega_k)\)。由 Faber--Krahn 不等式（\(\lambda_1(\Omega)\ge\pi j_0^2/|\Omega|\)），\(\lambda_j\ge\pi j_0^2/|\Omega_k|\)，即 \(|\Omega_k|\ge\pi j_0^2/\lambda_j\)。节域互斥：\(\sum_k|\Omega_k|\le|\mathcal D|\)，故
\begin{equation*}
...
再讨论双调和算子的推广。对双调和算子 \(\Delta^2\)（板振动），本征值 \(\lambda_j=\omega_j^2\)，Weyl 定律 \(j\sim\frac{|\mathcal D|}{2\pi}\sqrt{\lambda_j}\)。Faber--Krahn 型不等式对 \(\Delta^2\) 给出 \(\lambda_1(\Omega)\ge C/|\Omega|^2\)（Talenti 型），代入得
\begin{equation*}
