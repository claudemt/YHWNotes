...
\begin{derivation}[径向测度的酉消去与有效逆平方项]
 令 \(q=(d-1)/2\)，作代换 \(R_l=r^{-q}u_l\) 消去径向 Laplacian 的一阶导数项，使方程化为标准 Sturm--Liouville 形式，再定出逆平方系数与原点端点分类。
 ...
 \begin{equation*}
   ...
   u_l^{(2)}(r)\sim r^{-\nu_l+1/2},
 \end{equation*}
 当 \(\nu_l\ge1\) 时 \(u_l^{(2)}\) 不平方可积，原点为 limit-point；当 \(0\le\nu_l<1\) 时两支皆平方可积，原点为 limit-circle。于是得到\eqnrefs{eq:phys-hdqm-radial-u}。同时
 \begin{equation*}
 ...
 常用维数中 \(d=3\) 给出 \(u_l=rR_l\)、\(\nu_l=l+1/2\) 与离心项 \(l(l+1)/r^2\)；\(d=2\) 给出 \(\nu_l=l\)，其 \(s\) 波恰处系数 \(-1/4\) 的临界逆平方势。三维与二维的 \(s\) 波均属 limit-circle，通常的正则性条件只是相应自伴实现中的一个选择。
 \end{derivation}
...
\]
该算子对称但未必自伴。由 von Neumann 延拓理论，其自伴扩张的存在性与个数由 deficiency indices
\begin{equation*}
...
\end{equation*}
故二维 deficiency 解亦平方可积。一维去掉一点把直线分成两个半轴，每段端点独立，deficiency indices 为 $(1,1)$。

综上，$d=1,2,3$ 时 $n_+=n_-=1$，去点 Laplacian 有单参数族自伴扩张，可经原点边界条件引入一个点相互作用参数；$d\ge4$ 时奇异 deficiency 解不再平方可积，$n_\pm=0$，算子本质自伴，单点不能产生这种零程相互作用。临界维数 $d=4$ 处 $\psi\sim r^{-2}$ 的范数积分 $\int r^{-1}\dd r$ 在对数尺度发散，恰处边界。一维通常的 $\delta(x)$ 势只是这一自伴连接条件的特例。
\end{derivation}
