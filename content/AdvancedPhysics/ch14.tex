...
例如，三维 ($d=3$) 有 $g_l=2l+1$（$m=-l,\ldots,l$），即标准球谐函数 $Y_l^m$；二维 ($d=2$) 有 $g_0=1$、$g_l=2$（$l\ge1$，对应 $\cos l\theta$ 和 $\sin l\theta$），即 Fourier 模；四维 ($d=4$) 有 $g_l=(l+1)^2$。
\end{derivation}
...
\begin{derivation}[径向测度的酉消去与有效逆平方项]
 令 \(q=(d-1)/2\)，通过 \(R_l=r^{-q}u_l\) 消去径向 Laplacian 的一阶导数项，使方程化为标准 Sturm--Liouville 形式，再分析逆平方项系数与原点自伴性。
 ...
 在常用维数中，\(d=3\) 给出 \(u_l=rR_l\)、\(\nu_l=l+1/2\) 与离心项 \(l(l+1)/r^2\)；\(d=2\) 则有 \(\nu_l=l\)，其中 \(s\) 波处在系数 \(-1/4\) 的临界逆平方势。统一写法还直接给出端点分类：\(\nu_l\ge1\) 时原点为 limit-point 型，而 \(0\le\nu_l<1\) 时为 limit-circle 型，需由原物理问题选定边界条件。特别地，三维与二维的 \(s\) 波都属于后一类；通常的正则性条件只是相应自伴实现中的一个选择。
 \end{derivation}
...
先对非整数阶 Bessel 函数做 Frobenius 展开，提取首项幂次，得到奇维情形的解析有效程展开；再对整数阶做极限 \(\nu\to n\)，分离出运动学对数，证明减除后剩余部分解析。
...
把上述展开以及\eqnrefs{eq:phys-hdqm-logderivative-threshold}代入\eqnrefs{eq:phys-hdqm-phase-log-derivative}，分子从 \(z^\nu\) 起始，分母从 \(z^{-\nu}\) 起始，因此
\begin{equation*}
  k^{2\nu_l}\cot\delta_l(k)
  =c_0+c_2k^2+c_4k^4+\cdots .
\end{equation*}
左端在阈值附近按 \(k^2\) 解析；常数项编码散射长度，下一项编码有效程，其系数由内部对数导数在 \(k=0\) 处的展开决定。
...
二维 \(s\) 波对应 \(n=0\)，对数直接出现在最低阶；重新吸收常数后便得到\eqnrefs{eq:phys-hdqm-ere-2d}。因此三维低能散射由 \(k\cot\delta_0=-1/a_0+r_0k^2/2+\cdots\) 控制，而二维散射长度进入 \(\cot\delta_0^{(2)}\) 的对数。更一般地，偶维的整数阶 Bessel 第二解带来运动学 \(\ln k\) 项，奇维的半整数阶展开则不含这一项。
\end{derivation}
...
先讨论超位力算符。对径向 Hamiltonian \(H=-\frac{\hbar^2}{2m}\frac{\dd^2}{\dd r^2}+V_{\rm eff}(r)\)（\(V_{\rm eff}=V(r)+\frac{\hbar^2\ell(\ell+1)}{2mr^2}\)），定义超位力算符 \(G=r^s\frac{\dd}{\dd r}+\frac{s}{2}r^{s-1}\)（\(s\) 为非负整数）。对束缚态 \(|n\rangle\)，超位力关系来自
\begin{equation*}
  0=\langle n|[G,H]|n\rangle.
\end{equation*}

随后讨论对易子的显式计算。由 \([r^s\frac{\dd}{\dd r},H]\) 的逐项计算（利用 \([\frac{\dd}{\dd r},V(r)]=V'(r)\) 和 \([\frac{\dd}{\dd r},\frac{\dd^2}{\dd r^2}]=0\)），
\begin{equation*}
...
再讨论Kramers--Pasternack 递推。对幂律势 \(V(r)=\lambda r^q\)，\eqnrefs{eq:phys-hdqm-hypervirial-recurrence} 化为
\begin{equation*}
...
接着讨论Coulomb 势的精确矩阵元。对 \(V=-\frac{e^2}{r}\)（\(q=-1\)，\(\lambda=-e^2\)），递推
\begin{equation*}
...
超位力递推可以绕开对中间态的显式求和，直接生成所需径向矩阵元；在 Stark、Zeeman 等微扰计算中，这通常比逐项展开更有效。


束缚态 Coulomb 问题还具有由推广 Runge--Lenz 向量生成的 \(SO(d+1)\) 隐对称性。

先讨论\(d\) 维 Coulomb Hamiltonian。\(d\) 维空间中 Coulomb Hamiltonian \(H=-\frac{\hbar^2}{2m}\nabla_d^2-\frac{e^2}{r}\)，球坐标分离变量给出径向方程含有效势 \(V_{\rm eff}(r)=-\frac{e^2}{r}+\frac{\hbar^2\ell(\ell+d-2)}{2mr^2}\)（\(\ell=0,1,2,\ldots\) 是 \(d\) 维角动量量子数）。能级 \(E_n=-\frac{me^4}{2\hbar^2(n+\frac{d-3}{2})^2}\)（\(n=\ell+1,\ell+2,\ldots\)），简并度 \(\binom{n+d-2}{d-1}-\binom{n+d-4}{d-1}\)。

随后讨论推广的 Runge--Lenz 向量。定义 \(d\) 维 Runge--Lenz 向量
\begin{equation*}
...
再讨论能级简并的群论解释。\(SO(d+1)\) 的不可约表示标记最高权 \((n+\frac{d-3}{2},0,\ldots,0)\)，维数即简并度。\(SO(d)\)（角动量）是 \(SO(d+1)\) 的子群，分支规则给出各 \(\ell\) 的贡献。\(d=3\) 时 \(SO(4)\cong SU(2)\times SU(2)\)，简并度 \(n^2\) 来自 \((n/2,n/2)\) 表示——这是经典 Fock 1935 年结果的推广。\(d=2\) 时 \(SO(3)\) 给出简并度 \(2n-1\)（含 \(\pm m\) 简并）。

接着讨论Bertrand 定理与唯一性。Bertrand 定理：唯一使所有束缚轨道闭合的中心势是 Coulomb（\(V\propto1/r\)）和谐振子（\(V\propto r^2\)）。Coulomb 的闭合性由 Runge--Lenz 向量保证（\(A\) 守恒 → 轨道是固定 \(A\) 方向的椭圆），谐振子的闭合性由 \(SU(d)\) 对称性保证。高维推广中，只有这两种势的隐藏对称性足以产生额外简并——所有其他中心势的简并度只有 \(SO(d)\) 的最小简并。这把 Coulomb 和谐振子的精确可解性提升为动力学定理：闭合轨道 \(\leftrightarrow\) 隐藏对称性 \(\leftrightarrow\) 精确可解，是经典—量子对应的深刻体现。
\end{derivation}
