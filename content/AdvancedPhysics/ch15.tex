\chapter{自场、ALD 方程与 Landau--Lifshitz 动力学}
...
本章现在形成一条单向依赖关系。首先，Maxwell 方程和 \(\partial_\mu J^\mu=0\) 给出任意守恒源的辐射壳振幅与一般频谱；世界线表示、多极展开和软极限都只是该精确频谱的后续特化。其次，当源的运动不再预先给定时，总能动量守恒与推迟 Green 函数定义平滑源的因果自作用。点粒子极限要求 Dirac 推迟/超前分解和世界管质量重整化，由此得到有限 ALD 自力；Schott 动量和 Liénard 辐射则是同一四动量守恒的近场与远场两部分。最后，在 \(\epsilon_{\rm LL}\ll1\) 时，ALD 的无失控分支才可约化为 Landau--Lifshitz 方程。

因此，任何具体辐射反作用模型都应按以下次序判断适用性：先确定电流是否可视为给定源；若不可，判断是否需要有限尺寸记忆；只有在点粒子近似成立后才使用 ALD；只有在辐射反作用时间相对于外部时间尺度足够小时才进一步使用 Landau--Lifshitz。这个层级把频谱、守恒、Green 函数和有效动力学放在同一经典场论框架中。下一部分转入量子理论时，Green/谱方法仍被保留，但动力学对象改为具有自伴定义域的哈密顿量；随后开放系统再把封闭量子演化推广为约化动力学。
