\chapter{Fano--Feshbach 共振与 Landau--Zener 跃迁}
\label{ch:resonance-lz}

离散态若与连续谱耦合，原来的能级会获得位移和有限寿命；两个随时间靠近的能级若彼此耦合，系统又可能在避免交叉处改变绝热分支。前者是定态散射问题，后者是含时演化问题，却共享同一种消元思想：把 Hilbert 空间分成保留子空间 \(P\) 与其余子空间 \(Q\)，解出 \(Q\) 分量，再把它反馈到 \(P\) 中。定态反馈形成能量依赖的自能，含时反馈形成局部二能级耦合。Fano--Feshbach 与 Landau--Zener 模型正是这两种情形最清楚的可解代表。
...
更一般地，对复谱参数 $z$，完整预解式 $G(z)=(z-H)^{-1}$ 的 $P$ 块满足精确 Schur-补恒等式
\begin{equation}
PG(z)P
...
\]
虚部产生宽度 \(\Gamma=-2\operatorname{Im}\langle\psi_P|H_{PQ}\delta(E-H_{QQ})H_{QP}|\psi_P\rangle\)，即 Fermi 黄金法则中的衰变率。有效哈密顿量的非厄米部分正由这一步产生，把实能级移动到复平面 \(E\to E-\ii\Gamma/2\)，给出指数衰变 \(e^{-\Gamma t/2}\)。
\end{proof}
...
就是能量依赖自能。它的实部移动共振位置，虚部产生有限寿命。只要 \(Q\) 空间包含连续谱，\(H_{\rm eff}^{(+)}\) 一般就是非厄米的；这不是基本哈密顿量失去 Hermiticity，而是出射边界条件和连续谱消元后的有效描述。
...
仅知道复极点还不能解释不对称 Fano 线形，因为不对称来自“背景连续路径”和“经离散态再回到连续谱的路径”相干叠加。设固定总能量为 \(E\) 的精确 eigenstate 展开为
...
（若 \(V_E\) 为复数，相位可吸收到连续态约定中）。定义穿衣 discrete 态
...
\begin{proof}[解]
设外部跃迁算符为 \(T\)，初态为 \(|i\rangle\)。由 \eqnrefs{eq:fano-normalized-state}，散射态 $|\Psi_E\rangle$ 已分解为连续背景 $|E\rangle$ 和穿衣离散态 $|\Phi_E\rangle$ 两条路径，跃迁振幅为
\[
\mathcal M(E)
...
在单通道、时间反演对称且无吸收的常见约定中，可以通过相位选择把 $q$ 取为实数，得到\eqnrefs{eq:fano-profile}。若存在不可消去的多通道相位、吸收或时间反演破缺，$q$ 一般可以是复数，此时线型应保留 $|q+\epsilon|^2/(1+\epsilon^2)$，不能强行用实 $q$ 描述所有不对称性。
...
如果不仅关心单次跃迁概率，还关心多次交叉的 Stückelberg interference，就必须保留振幅相位。单次 Landau--Zener 散射矩阵除了 \(P_{\rm LZ}\) 外还包含 Stokes 相
...
\eqnrefs{eq:landau-zener} 要求：非绝热能差在相关时间窗内可线性化、耦合 \(V\) 近似常数、演化从远离交叉的 \(t\to-\infty\) 延伸到 \(t\to+\infty\)，并且系统在这段时间内可视为封闭二能级系统。有限脉冲、非线性 sweep、多个近邻能级或显著耗散都会改变连接问题，应回到一般含时传播子或多能级模型。
\end{note}
...
\(^6\)Li 在 834\,G 的宽共振（\(\Delta B=300\,\mathrm{G}\)）使 \(a(B)\) 可在 \(-\infty\) 到 \(+\infty\) 间连续调节。在 \(B>B_0\)（\(a>0\)）侧实现分子 BEC（紧束缚 Feshbach 分子），在 \(B<B_0\)（\(a<0\)）侧实现 BCS 超流。共振点 \(B=B_0\) 处 \(a\to\pm\infty\)，系统进入幺正极限：散射截面达到量子极限 \(\sigma=4\pi/k^2\)，所有长度尺度被散射长度抹平，方程只依赖密度与温度之比。MIT (2006) 在此区域观测到超流涡旋，确认了 BEC-BCS 渡越。
\end{exbox}
...
Fano 模型。哈密顿量 \(H=H_0+V\)，\(H_0|\phi\rangle=E_0|\phi\rangle\)（离散态），\(H_0|E\rangle=E|E\rangle\)（连续态），\(V\) 耦合离散与连续：\(V_\phi(E)=\langle\phi|V|E\rangle\)。Fano 假设 \(V_\phi(E)\) 在共振附近缓变（常数近似），自能 \(\Sigma(E)=\Delta(E)-i\Gamma(E)/2\)，\(\Gamma=2\pi|V_\phi(E_0)|^2\)（衰变率），\(\Delta=\mathcal P\int\frac{|V_\phi(E')|^2}{E_0-E'}\dd E'\)（能移）。
...
Feshbach 共振与磁场调控。在冷原子物理中，Feshbach 共振使散射长度随磁场变化为
\[
  a(B)=a_{\rm bg}\left(1-\frac{\Delta B}{B-B_0}\right),
\]
其中 \(a_{\rm bg}\) 是背景散射长度，\(B_0\) 是共振磁场，\(\Delta B\) 是共振宽度。开通道中的碰撞态与闭通道中的分子束缚态通过超精细相互作用混合；磁场改变 Zeeman 能移，使闭通道能级穿过开通道阈值。在 \(B=B_0\) 处，\(a\to\infty\)，散射达到幺正极限；随着磁场扫过 \(B_0\)，分子束缚态越过阈值而成为虚态。Feshbach 共振因此给了实验者一只连续调节相互作用强度的“旋钮”。
