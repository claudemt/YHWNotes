...
将运动分解为快振荡与慢漂移，对快运动求精确解后周期平均，残余项给出慢运动有效势。以一维场 $E(x,t)=E(X)\cos\omega t$ 为例，令 $x=X+\xi$（$X$ 慢、$\xi$ 快），控制参数 $\alpha_0/L\ll1$ 保证场在 quiver 幅值内可线性化。
...
\end{equation*}
即 quiver 幅值 $\alpha_0=eE(X)/(m_e\omega^2)$，quiver 速度幅值 $v_q=c\,a_0$；$a_0\ll1$ 保证非相对论近似自洽。
...
\]
两项分别为 quiver 与 drift 位移，抵消方能回原点。无量纲回碰动能为
\[
...
对不回碰而直接离开的电子，canonical momentum 守恒给出 $p_f=-eA(t_0)$（脉冲消失后 canonical = mechanical），而 $|A|\le E_0/\omega$，故
\[
