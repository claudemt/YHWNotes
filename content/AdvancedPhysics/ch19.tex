\chapter{从弱耦合到 GKSL 主方程}
\label{chap:born-markov-gksl}

精确约化态一般依赖整个历史，因此还不能用一个局域微分方程推进。只有把环境相关时间、系统驰豫时间和 Bohr 频率间隔逐一比较，Born、马尔可夫与久期近似才有明确含义。

\section{先用一个量子比特看懂主方程}

\chapref{chap:phys-oqs-open-quantum-systems}已经说明：系统和环境整体可以保持纯态，系统的约化态却会因纠缠而失去相干。本章要进一步回答一个计算问题：什么条件下，这种约化演化能够由只依赖当前时刻的微分方程描述？在进入投影算符和环境谱以前，先把最简单的二能级例子完整算一遍。
...
\end{equation}
正半定性\eqnrefs{eq:phys-oqs-gamma-psd} 在这里承担了关键作用：它保证所有 $\gamma_q(\omega)$ 非负，因此耗散部分可以写成一组真实非负速率的 Lindblad 通道。反对易项使 $\Tr\rho$ 的一阶变化精确抵消回收项，而非负速率又使生成元满足 GKSL 定理的完全正性条件。后文\eqnrefs{eq:phys-oqs-infinitesimal-completeness} 会进一步把这个结论写成显式的无穷小 Kraus 完备关系；因此“环境谱矩阵半正定 $\Rightarrow$ 生成元完全正”不是额外假设，而是从平稳环境相关函数到完全正迹保持半群的闭合链条。
