...
记平衡态为 \(\rho_\beta=Z^{-1}\ee^{-\beta H}\)。对任意满足
...
\]
这条等式比单独的极小性更强：偏离 Gibbs 态的程度直接等于自由能超额，而不需要把某个理想气体实例拼接进证明。
...
Kubo 累积量 \(\kappa_n\) 与普通矩 \(\mu_n=\langle A^n\rangle\) 通过 cumulant--moment 关系联系：
\begin{equation*}
 \kappa_1=\mu_1,\quad
...
涨落-耗散定理 (FDT) 把响应函数（耗散）与关联函数（涨落）联系起来。从 Kubo--Mori 内积出发，利用 KMS 条件和谱分解导出 FDT。

先讨论谱表示。对任意算符 \(A\)，定义谱函数（动态结构因子）
\begin{equation*}
...
在经典极限 \(\hbar\to0\) 中，\(\coth(\beta\hbar\omega/2)\to 2/(\beta\hbar\omega)\)，FDT 退化为
\begin{equation*}
 S_{AA}^{\rm cl}(\omega)=\frac{2}{\beta\omega}\chi_{AA}''(\omega),
\end{equation*}
即 Einstein 关系 \(D=\mu\kB T\)（扩散系数 = 迁移率 × 温度）的频域版本。

再取零频极限 \(\omega\to0\)，此时 \(\chi''(\omega)/\omega\to\chi'(0)\)（静态磁化率），FDT 给出
\begin{equation*}
...
先讨论\(\log\Xi_\Lambda\) 的解析对数支。取定参考点 \(z_0\in U\) 及实数 \(\theta_0\) 使 \(\log\Xi_\Lambda(z_0)=\log|\Xi_\Lambda(z_0)|+\ii\theta_0\)。对任意 \(z\in U\)，沿 \(U\) 中从 \(z_0\) 到 \(z\) 的路径 \(\gamma\) 定义
\[
 \log\Xi_\Lambda(z)
 :=\log\Xi_\Lambda(z_0)
...
随后讨论\(p_\Lambda\) 全纯。由\eqnrefs{eq:phys-sm-pressure-finite-general}，
\[
 p_\Lambda(z)=\frac{1}{\beta\mathcal V_\Lambda}\log\Xi_\Lambda(z),
\]
...
再讨论一致极限的全纯性。取任意紧集 \(\mathcal K\Subset U\)。假设 \(p_\Lambda\to p\) 在 \(\mathcal K\) 上一致。由 Weierstrass 定理，全纯函数列在紧集上的一致极限仍全纯：对 \(\mathcal K\) 内任意三角形 \(\triangle\subset\mathcal K\)，由一致收敛可交换极限与积分，
\[
...
上述全纯性结论 presuppose 极限 \(p=\lim p_\Lambda\) 存在。对稳定、短程、平移不变体系，存在性由次可加性给出：取立方盒 \(\Lambda_L=[-L,L]^d\)，则
\begin{equation*}
...
对稳定、短程、平移不变体系，体自由能 \(f_\Lambda=F_\Lambda/\mathcal V_\Lambda\) 的热力学极限存在性由次可加性给出。以下给出完整证明，包括边界项控制和边界条件无关性。

先讨论稳定性与能量分解。设相互作用势 \(\Phi\) 满足稳定性条件
\begin{equation*}
...
随后讨论次可加性。取 \(L=L_1+L_2\)，将 \(\Lambda_L\) 分成 \(\Lambda_{L_1}\)（左半）和 \(\Lambda_{L_2}\)（右半）加边界带。由稳定性和短程条件：
\begin{equation*}
...
再讨论极限存在。由\eqnrefs{eq:phys-sm-subadditivity-free-energy} 取 \(L_1=L_2=L/2\)（等分），得 \(f_L\ge f_{L/2}\)，即 \(f_L\) 沿 \(L\to2L\) 单调不增。由稳定性 \(f_L\ge-B\)（有下界），单调有下界序列收敛。对一般 \(L\to\infty\)，取 \(L_n=2^n L_0\)，\(f_{L_n}\) 收敛到 \(f^*=\inf_L f_L\)。对任意 \(L\)，取 \(n\) 使 \(2^n L_0\le L<2^{n+1}L_0\)，由次可加性 \(f_L\ge f_{2^n L_0}\ge f^*\)，且 \(f_L\le f^*+O(2^{-n})\)，故 \(f_L\to f^*\)。

接着讨论边界条件无关性。不同边界条件（周期、自由、固定、Dirichlet）只改变边界带 \(H_{\partial\Lambda_L}\) 中的相互作用项，其贡献 \(\le B''|\partial\Lambda_L|\)。归一化后边界差异 \(\sim|\partial\Lambda_L|/\mathcal V_L\to0\)（van Hove），故体自由能极限与边界条件无关。这一无关性是热力学量的"广延性"的数学表述：只有体量（正比于 \(\mathcal V_\Lambda\)）在极限中保留，面量（正比于 \(|\partial\Lambda|\)）和更低阶量消失。

进一步讨论一般区域。上述证明对立方盒给出。对满足 van Hove 条件 \(|\partial\Lambda|/\mathcal V_\Lambda\to0\) 的一般区域，用内接和外接立方盒夹逼：取最大内接盒 \(\Lambda^{\rm in}\) 和最小外接盒 \(\Lambda^{\rm out}\)，由稳定性 \(F_{\Lambda^{\rm in}}\le F_\Lambda\le F_{\Lambda^{\rm out}}+B'|\partial\Lambda|\)，van Hove 条件保证两者归一化后趋于同一极限。
\end{derivation}
...
先讨论Griffiths 第一不等式。对铁磁 Ising Hamiltonian \(H=-\sum_{A}J_A\prod_{x\in A}\sigma_x\)（\(J_A\ge0\)），任意多项式 \(P(\sigma)=\prod_{x\in B}\sigma_x\)（\(B\subset V\)）的期望
\begin{equation*}
...
接着讨论monotonicity 与相变存在性。Griffiths 不等式给出四条物理推论：
\begin{itemize}
