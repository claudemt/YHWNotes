...
\]
再除以 \(N\) 即得\eqnrefs{eq:phys-sm-strip-free-energy}。主本征值的严格性来自 \(\mathsf T_N\) 的全部元素为正（Perron--Frobenius），而铁磁 Ising 的转移矩阵恰满足此条件。
\end{proof}

例：取 $N=1$（单列即单自旋），$\mathsf T_1$ 退化为 $2\times2$ 矩阵 $\mathsf T_1=\begin{pmatrix}\ee^{\beta J}&\ee^{-\beta J}\\ \ee^{-\beta J}&\ee^{\beta J}\end{pmatrix}$，本征值 $\Lambda_0=2\cosh(\beta J)$、$\Lambda_1=2\sinh(\beta J)$，谱隙 $\Delta_1=\log\coth(\beta J)$；配分函数 $Z_{1,M}=\Lambda_0^M+\Lambda_1^M$，自由能 $-\beta f_1=\log(2\cosh\beta J)+o(1)$ 与 Onsager 解的高温极限一致，纵向关联长度 $\xi_\parallel=1/\log\coth(\beta J)$ 即一维 Ising 的精确值。Perron--Frobenius 定理可推广至紧正算子（Krein--Rutman 定理）：若 $\mathscr T$ 是 Banach 格上的紧正算子且谱半径 $r(\mathscr T)>0$，则 $r(\mathscr T)$ 是具有正特征向量的单本征值——这把有限维转移矩阵的主本征值论证直接延拓到 $N\to\infty$ 柱面极限，是 Ruelle 证明 $\log\Lambda_0(N)$ 凸性从而存在热力学极限的关键工具；次主本征值在临界点处与主本征值等模，对应关联长度发散与相变。

\section{谱表示与依赖观测量的相关长度}
...
\]
对一行局域观测量 \(O\)，定义对角算符 \(\widehat O\)，并设两观测点纵向间隔 \(r\) 行。由转移算子的物理意义，未归一化两点函数为
\[
...
\[
 ...
 \end{aligned}
\]
由 \(|0\rangle\) 是 Perron--Frobenius 主本征矢，\(\langle0|\widehat O|0\rangle=\langle O\rangle\) 正是观测量 \(O\) 的热力学平均，因此 \(a=0\) 项为 \(\langle O\rangle^2\)。减去它便得连通关联
\[
 \langle O_0O_r\rangle_c
...
\[
 ...
 \log\frac{\Lambda_0}{|\Lambda_a|},
\]
即\eqnrefs{eq:phys-sm-observable-correlation-length}。若 \(O\) 与所有非主谱态正交，则连通关联恒为零，相关长度为无穷；选择定则因此不是技术细节，而是观测量与谱的物理耦合条件。
\end{proof}
...
\begin{proof}[解]
Jordan--Wigner 变换把 \(N\) 个 Pauli 自旋映射为 \(N\) 个费米子模式，使转移矩阵从 \(2^N\) 维一般矩阵降为 \(N\) 维自由费米子问题。先验证反交换关系，再处理周期边界留下的宇称符号。
...
Pauli 二次型的费米子化。翻转算符 \(X_j=S_j\gamma_j\)（因 \(S_j^2=\mathbb I\)），但弦因子 \(S_j\) 使 \(\sum_j X_j\) 不是简单费米子二次型。关键技巧：做自旋轴旋转 \(X\to Y, Y\to-X\)（等价于 \(\gamma_j\to\tilde\gamma_j, \tilde\gamma_j\to-\gamma_j\)），使转移矩阵中两因子的弦因子结构互补。旋转后
\begin{equation*}
...
\end{equation}
即 Onsager 色散关系。转移矩阵本征值为各模式能量的乘积，自由能由 \(\frac{1}{N}\sum_k\epsilon(k)\to\int\frac{\dd k}{2\pi}\epsilon(k)\) 给出。临界条件 \(\epsilon(0)=0\)（R 扇区 \(k=0\) 模式软化）恢复\eqnrefs{eq:phys-sm-square-critical-k}。

Jordan--Wigner 弦 \(S_j=\prod_{k<j}Z_k\) 是一维有序化：它利用链的线性序把局域 Pauli 映射为局域费米子。二维中弦因子变为面填充算符，一般无法消除——这是二维 Ising 精确可解但三维不可解的根本原因。环面边界处弦因子绕回，产生 \(R_N\) 宇称分支，对应\eqnrefs{eq:phys-sm-torus-h1} 的两个同调生成元。这一联系把 Jordan--Wigner 边界项与曲面同调统一：弦绕行数 = 同调绕行数 = 自旋结构选择。
\end{proof}

上述二次费米子形式可用 Bogoliubov 变换直接求出谱隙。
自由费米子二次型由 Bogoliubov 正则变换对角化。对每个动量块求一个 \(2\times2\) 本征值问题，便得到精确色散和谱隙。

一般自由费米子哈密顿量 \(H=\sum_{jk}(A_{jk}c_j^\dagger c_k+\frac12 B_{jk}c_j^\dagger c_k^\dagger+\text{h.c.})\)，\(A\) Hermite，\(B\) 反对称。Bogoliubov 变换 \(d_k=\sum_j(u_{kj}c_j+v_{kj}c_j^\dagger)\) 要求 \(d_k\) 满费米子关系且 \(H=\sum_k\epsilon_k d_k^\dagger d_k+\text{const}\)。代入得 Bogoliubov--de Gennes (BdG) 方程
\begin{equation*}
...
\end{equation*}
本征值 \(\epsilon(k)=\pm\sqrt{(K_h\cos k+K_v^*)^2+(K_h\sin k)^2}=\pm\sqrt{K_h^2+K_v^{*2}+2K_hK_v^*\cos k}\)。这与\eqnrefs{eq:phys-sm-fermion-dispersion} 在参数化 \(\cosh\epsilon=\cosh(2K_h)\cosh(2K_v^*)-\sinh(2K_h)\sinh(2K_v^*)\cos k\) 下一致（差一个整体重标，来自转移矩阵与哈密顿量的指数关系）。
...
有限环 \(N\) 上 NS 扇区最低动量 \(k_{\min}=\pi/N\)，有限尺寸谱隙 \(\Delta_N\approx\Delta+\frac{\pi^2}{2N^2\epsilon(0)}\)（小 \(k\) 展开）。临界点 \(\Delta=0\) 时 \(\Delta_N\sim\pi/N\)，对应中心荷 \(c=1/2\) 的 Ising 共形场论：有限尺寸自由能修正 \(f_N=f_\infty-\frac{\pi c}{6N^2}\)，\(c=1/2\) 由三个主场 \((1,\sigma,\epsilon)\) 的共形权 \((0,1/16,1/2)\) 确定。这把转移矩阵谱与共形场论直接联系起来。
...
Suzuki--Trotter 分解把 \(d\) 维经典统计模型的转移矩阵写成 \((d-1)\) 维量子模型的虚时演化。所需计算只是把不对易的两部分交替切成短时间片，再控制时间片宽度趋零时的误差。
...
2D 经典 Ising 的转移矩阵 \(\mathsf T_N=\mathsf V_h^{1/2}\mathsf V_v\mathsf V_h^{1/2}\)（\eqnrefs{eq:phys-sm-transfer-pauli-factorization}）。连续极限 \(M\to\infty\)（纵向层数），\(\mathsf T_N^M=\ee^{-M H_{\rm Q}}\)，量子哈密顿量
\begin{equation*}
...
\begin{itemize}
 ...
 \item[(iii)] 蒙特卡洛：Suzuki--Trotter 分解把量子统计力学化为经典配分函数（世界线表示），使量子蒙特卡洛成为可能——连续极限误差 \(O(\Delta\tau^2)\) 由分解阶数控制。
 \item[(iv)] AdS/共形场论对应：量子--经典对应的推广，\(d\) 维量子场论映射到 \((d+1)\) 维经典引力，Ising/共形场论对应是其离散版本。
\end{itemize}
