...
令 \(A=\lambda a^\dagger\)，\(B=-\lambda a\)，\([A,B]=\lambda^2\) 为 c-number，BCH 公式 \(\ee^{A+B}=\ee^A\ee^B\ee^{-[A,B]/2}\) 给出
\begin{equation*}
...
\end{equation*}
即 \(a(D|0\rangle)=\lambda(D|0\rangle)\)。位移算符 \(D(\lambda_{\rm FC})\) 把基态振动真空平移到激发态振动真空，平移量 \(\lambda_{\rm FC}=\Delta Q/(2Q_{\rm zpf})\) 是两个势阱平衡位置之差以零点宽度为单位。FC 因子由 Huang--Rhys 因子 \(S_{\rm HR}=\lambda_{\rm FC}^2\) 决定。关键代数性质为
\begin{equation*}
...
接下来考察 Huang--Rhys 因子与 vibronic 进展。

单模位移谐振子的 Huang--Rhys 因子 \(S_{\rm HR}=(\Delta Q)^2/(2\ell_v^2)\) 决定 vibronic 谱线强度分布：\(I_n=e^{-S_{\rm HR}}S_{\rm HR}^{\,n}/n!\)（Poisson 分布）。\(S_{\rm HR}\) 的物理意义是基态到末态振动波包中心的位移（以零点宽度为单位）。
...
设初态 \(g\) 有 \(N\) 个 normal mode \(\{Q_i\}\)，频率 \(\{\omega_i\}\)；末态 \(e\) 有 \(N\) 个 mode \(\{Q'_j\}\)，频率 \(\{\omega'_j\}\)。Duschinsky 变换
\begin{equation}
