...
\textbf{物理意义。}Maxwell 算子的自伴性保证电磁场可按正交模式展开，每个模式独立量子化为独立谐振子。这是腔量子电动力学的数学基础：腔中电磁场 $\vb E(\vb r,t)=\sum_\lambda \vb f_\lambda(\vb r)\,a_\lambda(t)+\text{h.c.}$，不同模式互不耦合，各自有频率 $\omega_\lambda$。实例：矩形腔 $\vb f_{lmn}$ 由波矢 $\vb k=(l\pi/a,m\pi/b,n\pi/c)$ 标记，频率 $\omega=c|\vb k|$，正交性确保不同模式光子独立——这正是激光多模理论的出发点。
\end{derivation}
...
\textbf{物理意义。}场能分解为独立谐振子的关键不是"类比"，而是 Maxwell 模的正交性使交叉项严格为零。这意味着不同模式的光子不耦合——激光多模理论、腔 QED 中多模 Fock 态、以及光子统计中各模独立的 Poisson/Bose--Einstein 分布，都根植于此正交分解。
\end{derivation}
...
\emph{第一步：矩阵元。}由 $\hat{\mathbf E}^{(-)}=-\ii\sum_{\mathbf k'\lambda'}\sqrt{\hbar\omega_{k'}/(2\varepsilon_0 V)}\,\mathbf e_{\mathbf k'\lambda'}^*\hat a_{\mathbf k'\lambda'}^\dagger\ee^{-\ii(\mathbf k'\cdot\mathbf r-\omega_{k'}t)}$，矩阵元为
\begin{equation*}
M_{\mathbf k\lambda}
...
\emph{第二步：模平方。}取 $|M_{\mathbf k\lambda}|^2$，相位因子 $\ee^{-\ii\mathbf k\cdot\mathbf r_0}$ 的模为 1，因此
\begin{equation*}
|M_{\mathbf k\lambda}|^2
...
\emph{第三步：连续极限与偏振求和。}把 $\sum_{\mathbf k}\to V/(2\pi)^3\int\dd^3 k$ 代入 Fermi 黄金法则，$V$ 消去：
\begin{equation*}
...
\emph{第四步：角积分。}取 $\mathbf d$ 沿 $z$ 轴，则 $|\mathbf d\cdot\hat{\mathbf k}|^2=|\mathbf d|^2\cos^2\theta$，
\begin{equation*}
...
\emph{第五步：径向积分。}由 $k=\omega/c$ 得 $k^2\dd k=\omega^2\dd\omega/c^3$，delta 函数 $\delta(\omega-\omega_0)$ 将积分锁定在 $\omega_0$：
\begin{equation}
...
\textbf{物理意义。}自发辐射率 $A_{eg}=\omega_0^3|\mathbf d_{ge}|^2/(3\pi\varepsilon_0\hbar c^3)$ 完全由原子参数（偶极矩阵元 $\mathbf d_{ge}$、跃迁频率 $\omega_0$）和真空电磁场参数（$\varepsilon_0,c$）决定，不涉及任何经典外场。$\omega_0^3$ 依赖使高频跃迁的辐射率急剧增大——Lyman-$\alpha$（121.6\,nm）的 $A\approx6.27\times10^8\,\mathrm{s^{-1}}$ 远大于钠 D 线（589\,nm）的 $A\approx6.1\times10^7\,\mathrm{s^{-1}}$。将此结果与 Planck 热分布结合，可恢复 Einstein $A$-$B$ 关系 $A/B=8\pi h\nu^3/c^3$，说明 $A$ 系数的量子推导与热力学一致性。
\end{derivation}

这说明自发辐射不是经典外场触发的跃迁，而是原子偶极与真空电磁场连续模耦合的结果。\(\omega_0^3\) 来自单光子场振幅中的 \(\sqrt\omega\) 与三维 photon density of states 中的 \(\omega^2\) 共同作用。把 \eqnrefs{eq:spontaneous-emission-rate} 与 Planck 谱结合，即可恢复 Einstein \(A\)-\(B\) 关系。
...
接下来考察Green dyadic、局域态密度与一般环境。

在非均匀无耗散环境中，逐个求模并非总是最方便。定义 outgoing electromagnetic Green dyadic
\begin{equation}
\left[\nabla\times\nabla\times
...
\begin{derivation}[从模式谱表示到 Green dyadic 辐射率]
对\eqnrefs{eq:maxwell-normal-mode-eigenproblem}的正交本征模 $\{\mathbf f_\lambda\}$，满足 $\int\dd^3r\,\epsilon_r(\mathbf r)\,\mathbf f_\lambda\cdot\mathbf f_{\lambda'}^*=\delta_{\lambda\lambda'}$，Green dyadic 的谱表示为
\begin{equation*}
\mathbf G(\mathbf r,\mathbf r';\omega)
= c^2\sum_\lambda
...
= \frac{2\omega_0^2}{\hbar\epsilon_0 c^2}\,
 ...
 \mathbf d_{ge},
\end{equation*}
即\eqnrefs{eq:spontaneous-green-dyadic}。Green dyadic 是"模式密度乘局域场强"的连续谱压缩表示。

\textbf{物理意义。}Green 并矢 $\vb G(\vb r_0,\vb r_0;\omega_0)$ 把环境对原子辐射的影响编码为单一张量：其虚部给出局域态密度 (LDOS)，直接决定自发辐射率 $\Gamma\propto\vb d^*\cdot\operatorname{Im}\vb G\cdot\vb d$。在自由空间 $\operatorname{Im}\vb G=\omega^2/(6\pi c^3)\vb I$，还原 $\Gamma=\omega_0^3|\vb d|^2/(3\pi\varepsilon_0\hbar c^3)$。在光子带隙材料内 $\operatorname{Im}\vb G\to0$，自发辐射被完全抑制。在微腔内 $\operatorname{Im}\vb G$ 在共振频率发散，自发辐射被增强（Purcell 效应 $F_P\propto Q/V$）。
\end{derivation}

\section{Fock、相干态与一般线性驱动}
接下来考察Fock 态、相干态与光子统计。
...
之所以采用 normal ordering，正是因为理想吸收型探测器先由 \(E^{(+)}\) 湮灭一个光量子，再由共轭振幅组成概率。
...
\begin{derivation}[相干态的 Poisson 统计与 quadrature 涨落]
\emph{第一步：递推关系。}由 $a|\alpha\rangle=\alpha|\alpha\rangle$ 和 $|\alpha\rangle=\sum_n c_n|n\rangle$，左乘 $\langle n|$：
\begin{equation*}
...
\emph{第二步：归一化。}由 $\langle\alpha|\alpha\rangle=1$：
\begin{equation*}
...
\emph{第三步：Poisson 分布。}光子数分布 $P_n=|\langle n|\alpha\rangle|^2=\ee^{-|\alpha|^2}|\alpha|^{2n}/n!$，这正是 Poisson 分布 with 参数 $\lambda=|\alpha|^2$。平均光子数
\begin{equation*}
...
\emph{第四步：quadrature 统计。}由 $a|\alpha\rangle=\alpha|\alpha\rangle$ 和 $Q=(a+a^\dagger)/\sqrt{2}$，
\begin{equation*}
...
\textbf{物理意义。}相干态的 Poisson 统计意味着光子数涨落 $\Delta N=\sqrt{\langle N\rangle}$（shot noise），这是激光远高于阈值时的典型行为。quadrature 方差 $(\Delta Q)^2=(\Delta P)^2=1/2$ 等于真空涨落——相干态在相空间中只是把真空波包平移到 $(\sqrt2\,\operatorname{Re}\alpha,\sqrt2\,\operatorname{Im}\alpha)$，不改变噪声分布。这与压缩态形成对比：压缩态在一个 quadrature 方向低于真空涨落，代价是另一个方向增大。相干态的统计性质可总结为
\begin{equation*}
P(n)=\ee^{-|\alpha|^2}\frac{|\alpha|^{2n}}{n!}\ (\text{Poisson}),\qquad \Delta Q\cdot\Delta P=\frac12\ (\text{Heisenberg 下界}).
\end{equation*}
...
是纯虚 c-number，因此所有三重及更高嵌套交换子严格为零。第二阶 Magnus 项只是整体相位：
...
这里的 geometric-like phase 来自不同时刻位移在相空间中的非交换性；虽然它不影响单个相干态的光子数，但在多路径干涉中可以产生可观测相位。由真空出发立刻得到
...
\begin{derivation}[Bogoliubov 约束与 quadrature 压缩]
\emph{思路：}分三步推导：(1) 从含参量下转换的 Heisenberg 方程看出解是 \((a,a^\dagger)\) 的线性变换（Bogoliubov 形式），(2) 由正则对易关系保持要求导出 \(|u|^2-|v|^2=1\) 约束，(3) 计算 quadrature 方差展示压缩效应。

\emph{第一步：Heisenberg 方程的线性性。}含参量下转换的 Hamiltonian \(\hat H=\hbar\omega a^\dagger a+\frac{\hbar}{2}(\kappa a^{\dagger2}+\kappa^*a^2)+\cdots\) 中，\(a^{\dagger2}\) 项成对产生光子。Heisenberg 方程
\begin{equation*}
\frac{\dd a}{\dd t}=\frac{1}{\ii\hbar}[a,\hat H]
=-\ii\omega a-\ii\kappa^* a^\dagger+\cdots
\end{equation*}
对 \((a,a^\dagger)^T\) 是线性方程组（省略号代表线性驱动项，只产生位移不改变 Bogoliubov 结构）。线性 ODE 的解是线性变换：\(a(t)=u(t)a(0)+v(t)a^\dagger(0)+\text{c-number}\)。系数 \(u(t)\) 和 \(v(t)\) 由运动方程和初始条件 \(u(0)=1,v(0)=0\) 确定。

\emph{第二步：正则约束。}量子力学要求对易关系在时间演化下保持：由 \([a(0),a^\dagger(0)]=1\) 要求 \([a(t),a^\dagger(t)]=1\)。计算：
\begin{equation*}
...
|u|^2-|v|^2=1.
\end{equation*}
这是 \(\mathrm{SU}(1,1)\) 群的约束——双曲旋转保持 Minkowski 度规 \(\text{diag}(+1,-1)\)，而非 \(\mathrm{SU}(2)\) 的幺正约束 \(|u|^2+|v|^2=1\)。区别源于 \(a\) 和 \(a^\dagger\) 不对易，使交叉项产生负号。

\emph{第三步：quadrature 方差。}在压缩相位 \(\phi=0\) 时，Bogoliubov 变换为 \(S^\dagger a S=a\cosh r-a^\dagger\sinh r\)（由\eqnrefs{eq:squeezing-bogoliubov-transform}，\(u=\cosh r\)，\(v=-\sinh r\)，满足 \(|u|^2-|v|^2=\cosh^2 r-\sinh^2 r=1\)）。由 \(Q=(a+a^\dagger)/\sqrt{2}\)：
\begin{equation*}
S^\dagger Q S=\frac{S^\dagger a S+S^\dagger a^\dagger S}{\sqrt2}
=\frac{(a\cosh r-a^\dagger\sinh r)+(a^\dagger\cosh r-a\sinh r)}{\sqrt2}
...
\end{equation*}
因为真空 \(\langle 0|Q^2|0\rangle=1/2\)。同理 \((\Delta P)^2=\frac{1}{2}e^{2r}\)。不确定乘积 \(\Delta Q\cdot\Delta P=\frac{1}{2}e^{-r}\cdot\frac{1}{2}e^{r}\cdot 2=\frac{1}{2}\) 保持 Heisenberg 下界——压缩态是最小不确定态，但噪声分布非对称：\(Q\) 方向压缩 \(e^{-r}\)，\(P\) 方向放大 \(e^r\)。

\emph{第四步：物理意义。}Bogoliubov 约束 \(|u|^2-|v|^2=1\) 保证压缩变换不违反正则对易关系——这不是任意变换，而是 \(\mathrm{SU}(1,1)\) 双曲旋转。\(Q\) 方向压缩、\(P\) 方向放大的物理机制是参量下转换中 \(a^{\dagger2}\) 项成对产生光子，使光子数只含偶数项，在 \(Q\) 方向建立相干叠加导致振幅压缩。实例：LIGO 注入 6\,dB 压缩真空（\(r\approx0.69\)），\(Q\) 方向噪声降低 \(e^{-r}\approx0.50\) 倍，用于降低高频散粒噪声。
...
\end{equation}
这一步同时证明了 squeezing 是保持 canonical commutator 的双曲旋转，而不是对 \(a\) 的普通酉相位旋转。

\textbf{物理意义。}Bogoliubov 变换 $a\to a\cosh r-e^{i\phi}a^\dagger\sinh r$ 把湮灭算符混合产生算符，是量子光学中从真空产生压缩态的机制。双曲旋转保持 $[a,a^\dagger]=1$（$\cosh^2 r-\sinh^2 r=1$），但把真空涨落从 $x$ 方向压缩 $e^{-r}$、$p$ 方向放大 $e^r$——测不准原理保持。实例：参量下转换 $\chi^{(2)}$ 非线性晶体在简并配置下直接实现 $S(\zeta)$，产生压缩光用于引力波探测（LIGO 注入压缩真空使量子噪声低于标准量子极限）。
\end{derivation}

对 squeezed vacuum \(|0;\zeta\rangle=S(\zeta)|0\rangle\)，由\eqnrefs{eq:squeezing-bogoliubov-transform}可知平均光子数
...
所以 squeezing 并不是“把真空噪声椭圆压扁”的纯几何图像；从原 Fock 基看，它等价于成对地产生偶数个量子。二次 Hamiltonian 中的 \(a^{\dagger2}\) 和 \(a^2\) 正是这种 pair creation/annihilation 结构。

压缩态的生成机制不止参量下转换一种。谐振子频率的突然变化也会产生压缩——这是一个纯量子效应，经典力学中不存在类比。

接下来考察频率突变作为 squeezing 的具体例子。
...
``突然近似''的控制条件已经由 \(\tau_q\omega_{0,1}\ll1\) 明确给出；相反，若频率变化极慢，绝热定理使系统跟随瞬时本征态而不会产生同样的 squeezing 激发。
...
压缩真空 & \(3+\frac{1}{\langle N\rangle}\) & \(2\langle N\rangle+1\) & OPO 低于阈值 \\
\hline
\end{tabular}
\end{center}

\textbf{推导要点。}对 Fock 态 \(|n\rangle\)，\(\langle N(N-1)\rangle=n(n-1)\)，故 \(g^{(2)}(0)=1-1/n\)；单光子 \(n=1\) 给出 \(g^{(2)}(0)=0\)，这是单光子源的判据。对相干态，Poisson 分布使 \(\langle N(N-1)\rangle=\langle N\rangle^2\)，故 \(g^{(2)}(0)=1\)。对热光场，Bose--Einstein 分布 \(P_n=\bar n^n/(1+\bar n)^{n+1}\) 给出 \(\langle N(N-1)\rangle=2\bar n^2\)，故 \(g^{(2)}(0)=2\)（光子 bunching）。对压缩真空，只含偶数光子使 \(\langle N(N-1)\rangle=3\sinh^2 r\cosh^2 r\)，故 \(g^{(2)}(0)=3+\coth^2 r>3\)（强 bunching）。
...
相干态的实验产生依赖激光器工作于远高于阈值区间。压缩态则需非线性光学过程，主要方案是参量下转换（SPDC），探测则依赖 balanced homodyne 技术。

\textbf{参量下转换 (SPDC)。}二阶非线性晶体（\(\chi^{(2)}\)）在简并参量下转换配置下实现压缩算符 \(S(\zeta)\)。用 Nd:YAG 泵浦光（532\,nm）注入 PPKTP 晶体，信号场频率为 1064\,nm。OPO 低于阈值时输出压缩真空，压缩参量
\[
...
\textbf{Balanced homodyne 探测。}压缩态的噪声压缩不在光子数分布中直接可见（压缩真空的光子数只含偶数项），而需通过 balanced homodyne 探测测量 quadrature \(X_\theta=(ae^{-i\theta}+a^\dagger e^{i\theta})/\sqrt{2}\) 的方差。本地振荡光 (LO) 与信号在 50/50 分束器上混合，两路光电流差
\[
\hat i_-\propto|\alpha_{\rm LO}|\,\hat X_\theta
\]
直接给出 quadrature 的概率分布。对压缩真空 \(S(\zeta)|0\rangle\)（\(\zeta=r\)，\(\phi=0\)），\(X_0\) 的方差为 \(\frac12 e^{-2r}\)，\(X_{\pi/2}\) 的方差为 \(\frac12 e^{2r}\)。

\begin{exbox}[LIGO 的压缩光注入]
Advanced LIGO 在 2013 年注入 6\,dB 压缩真空，将量子噪声降低约 2 倍。信号频带 10--100\,Hz 中，辐射压力噪声（低频）和散粒噪声（高频）的交叉频率 \(f_c\approx100\,\mathrm{Hz}\)。注入压缩后，高频散粒噪声降低 \(\sim e^{-r}\approx0.50\)，低频辐射压力噪声相应增加 \(e^r\approx2.0\)——但通过 filter cavity 将压缩角旋转至频带依赖，可同时降低两个频带（频率依赖压缩, FDC），2020 年 O3 观测运行中实现了 \(\sim3\) 倍的灵敏度提升。
\end{exbox}

Green 并矢\eqnrefs{eq:spontaneous-green-dyadic}不仅适用于自由空间，在腔中其虚部在共振频率处急剧增强，给出 Purcell 效应——自发辐射率被腔共振放大。

接下来考察Purcell 效应与腔量子电动力学。
...
\]
对 \(r>0\) 严格小于真空极限 1，验证 EPR 纠缠。2000 年 Bowen 实验用 OPO 产生 \(r\approx0.6\)（3\,dB）双模压缩，测得 Reid 乘积 \(0.64<1\)，首次实验验证连续变量 EPR 纠缠。
\end{exbox}

\begin{derivation}[单模压缩态 Wigner 函数的显式推导与边缘分布，即\eqnrefs{eq:squeezed-wigner-function}]
\emph{策略路线图。}首先定义 Wigner 函数的位移算符表示并写出真空态 Wigner 函数；其次利用压缩算符的 Bogoliubov 变换性质化简压缩态的特征函数；再次通过 Fourier 逆变换得到压缩态 Wigner 函数的封闭表达式；然后计算两个正交分量的边缘分布；最后验证不确定性乘积饱和 Heisenberg 下界。

\emph{第一步：Wigner 函数与特征函数。}单模 Wigner 函数定义为
\begin{equation}
...
\emph{第二步：压缩态特征函数。}其次考虑单模压缩态 \(\rho_{\rm sq}=S(\zeta)|0\rangle\langle0|S^\dagger(\zeta)\)，\(\zeta=r\ee^{\ii\phi}\)。特征函数
\begin{equation*}
...
\end{equation}
此式由 \(S^\dagger a S=a\cosh r+a^\dagger\ee^{\ii\phi}\sinh r\) 直接代入 \(D(\xi)=\exp(\xi a^\dagger-\xi^*a)\) 并用 BCH 公式得到。故
\begin{equation*}
...
\emph{第三步：Wigner 函数封闭表达式。}再次取 \(\phi=0\)（沿 \(x_1\) 压缩）简化计算。此时 \(\xi=\xi_1+\ii\xi_2\)，
\begin{equation*}
...
\end{equation}
即\eqnrefs{eq:squeezed-wigner-function}。相空间中 Wigner 函数是椭圆 Gaussian：沿 \(x_1\) 方向半轴 \(\ee^r\)（展宽），沿 \(x_2\) 方向半轴 \(\ee^{-r}\)（压缩），面积保持 \(\pi\ee^r\cdot\pi\ee^{-r}=\pi^2\) 与真空态相同——相空间面积不变是 Liouville 定理的量子体现。

\emph{第四步：边缘分布。}然后计算正交分量 \(X_i=x_i\) 的边缘分布
\begin{equation*}
P(x_1)=\int_{-\infty}^{+\infty}\!\dd x_2\;W_{\rm sq}(\alpha)
=\frac{1}{\sqrt\pi}\,\ee^{-x_1^2\ee^{-2r}},
\end{equation*}
\begin{equation*}
P(x_2)=\int_{-\infty}^{+\infty}\!\dd x_1\;W_{\rm sq}(\alpha)
=\frac{1}{\sqrt\pi}\,\ee^{-x_2^2\ee^{2r}}.
\end{equation*}
方差
\begin{equation}
...
\end{equation}
即\eqnrefs{eq:squeezed-quadrature-variance}。\(r>0\) 时 \(X_1\) 压缩、\(X_2\) 反压缩。

\emph{第五步：不确定性原理饱和。}最后计算乘积
\begin{equation*}
\Delta X_1\,\Delta X_2
=\frac{\ee^{-r}}{\sqrt2}\cdot\frac{\ee^r}{\sqrt2}
=\frac12,
\end{equation*}
恰为 Heisenberg 下界 \(\Delta X_1\,\Delta X_2\ge1/2\) 的等号——压缩态是最小不确定态。真空态 \(r=0\) 给出 \(\Delta X_1=\Delta X_2=1/\sqrt2\)（对称噪声），压缩态 \(r>0\) 将噪声从 \(X_1\) 转移到 \(X_2\) 而不违反不确定性原理。在引力波探测器 LIGO 中，\(r\approx 0.6\)（3\,dB）的压缩真空注入使信号频带噪声降低约 40\%，这是 2015 年首次探测到引力波的关键技术之一。


\emph{[压缩态的不可测性与 Holevo 界的量子信息应用]} 压缩态的量子信息性质——超越标准量子极限的测量精度和 Holevo 界的信道容量——是量子信息论在连续变量系统中的核心应用。分四步建立。

\emph{第一步：标准量子极限与压缩超越。}相干态 \(|\alpha\rangle\) 的正交分量 \(\Delta X=\Delta P=1/\sqrt{2}\)（最小不确定乘积 \(\Delta X\Delta P=1/2\)），给出标准量子极限（SQL）。压缩态 \(S(r)|\alpha\rangle\)（\(S(r)=\exp[\frac{r}{2}(a^2-a^{\dagger2})]\)）满足 \(\Delta X=\ee^{-r}/\sqrt{2}\)，\(\Delta P=\ee^r/\sqrt{2}\)——一个分量压缩，另一个反压缩，乘积仍 \(1/2\)。压缩 \(r\) 使 \(\Delta X\) 低于 SQL，在引力波探测（LIGO 注入压缩光提升灵敏度）和精密测量中直接应用。

\emph{第二步：Holevo 界与高斯信道容量。}连续变量量子信道的经典信息容量由 Holevo 界给出：\(C\le\chi=S(\rho_{\rm out})-\int p(x)S(\rho_x)\dd x\)（\(S\) von Neumann 熵）。对高斯信道（保高斯态的 CP 映射），Holevo 界可精确计算：\(C=g(\nu_1)+g(\nu_2)-g(\nu_3)-g(\nu_4)\)（\(g(x)=(x+1)\log(x+1)-x\log x\)，\(\{\nu_i\}\) 是信道参数的辛本征值）。压缩态优化 Holevo 界——在能量约束下，压缩态（而非相干态）达到高斯信道容量，是连续变量量子通信的最优编码。

\emph{第三步：纠缠辅助容量与超加性。}纠缠辅助容量 \(C_E\)（发送方与接收方共享纠缠时的容量）可超过未辅助容量 \(C\)（超加性 \(C_E\ge C\)）。对高斯信道，\(C_E=g(N_S)-g(N_N)\)（\(N_S\) 信号光子数，\(N_N\) 噪声光子数），而 \(C\) 需要优化输入态。对纯损耗信道，\(C=C_E=\log(1+\eta N_S/(1-\eta))\)（\(\eta\) 透射率），纠缠不增加容量；对噪声信道，\(C_E>C\)，纠缠提供真实增益——这是量子通信中纠缠资源价值的定量体现。

\emph{第四步：多模压缩与连续变量纠缠。}双模压缩态 \(S_2(r)|0,0\rangle=\frac{1}{\cosh r}\sum_n(\tanh r)^n|n,n\rangle\) 是连续变量纠缠的典型态。其 Schmidt 分解 \(\sum_n\lambda_n|n\rangle_A|n\rangle_B\)（\(\lambda_n=(\tanh r)^n/\cosh r\)）给出纠缠熵 \(E=-\sum|\lambda_n|^2\log|\lambda_n|^2=g(\sinh^2 r)\)。双模压缩态在连续变量量子密钥分发（CV-QKD）中用作纠缠源，安全性的 Shor--Preskill 证明在连续变量中通过 Holevo 界给出窃听者信息上界 \(I_{\rm Eve}\le\chi\)——压缩态参数 \(r\) 直接决定密钥率 \(K=\beta I_{AB}-\chi\)（\(\beta\) 重复率，\(I_{AB}\) Alice--Bob 互信息），是连续变量量子通信的物理基础。
\end{derivation}
