...
先讨论矩阵元。由 $\hat{\mathbf E}^{(-)}=-\ii\sum_{\mathbf k'\lambda'}\sqrt{\hbar\omega_{k'}/(2\varepsilon_0 V)}\,\mathbf e_{\mathbf k'\lambda'}^*\hat a_{\mathbf k'\lambda'}^\dagger\ee^{-\ii(\mathbf k'\cdot\mathbf r-\omega_{k'}t)}$，矩阵元为
\begin{equation*}
M_{\mathbf k\lambda}
...
随后讨论模平方。取 $|M_{\mathbf k\lambda}|^2$，相位因子 $\ee^{-\ii\mathbf k\cdot\mathbf r_0}$ 的模为 1，因此
\begin{equation*}
|M_{\mathbf k\lambda}|^2
...
再讨论连续极限与偏振求和。把 $\sum_{\mathbf k}\to V/(2\pi)^3\int\dd^3 k$ 代入 Fermi 黄金法则，$V$ 消去：
\begin{equation*}
...
接着讨论角积分。取 $\mathbf d$ 沿 $z$ 轴，则 $|\mathbf d\cdot\hat{\mathbf k}|^2=|\mathbf d|^2\cos^2\theta$，
\begin{equation*}
...
进一步讨论径向积分。由 $k=\omega/c$ 得 $k^2\dd k=\omega^2\dd\omega/c^3$，delta 函数 $\delta(\omega-\omega_0)$ 将积分锁定在 $\omega_0$：
\begin{equation}
...
先讨论递推关系。由 $a|\alpha\rangle=\alpha|\alpha\rangle$ 和 $|\alpha\rangle=\sum_n c_n|n\rangle$，左乘 $\langle n|$：
\begin{equation*}
...
随后讨论归一化。由 $\langle\alpha|\alpha\rangle=1$：
\begin{equation*}
...
接着讨论quadrature 统计。由 $a|\alpha\rangle=\alpha|\alpha\rangle$ 和 $Q=(a+a^\dagger)/\sqrt{2}$，
\begin{equation*}
...
分三步推导：(1) 从含参量下转换的 Heisenberg 方程看出解是 \((a,a^\dagger)\) 的线性变换（Bogoliubov 形式），(2) 由正则对易关系保持要求导出 \(|u|^2-|v|^2=1\) 约束，(3) 计算 quadrature 方差展示压缩效应。

先讨论Heisenberg 方程的线性性。含参量下转换的 Hamiltonian \(\hat H=\hbar\omega a^\dagger a+\frac{\hbar}{2}(\kappa a^{\dagger2}+\kappa^*a^2)+\cdots\) 中，\(a^{\dagger2}\) 项成对产生光子。Heisenberg 方程
\begin{equation*}
...
随后讨论正则约束。量子力学要求对易关系在时间演化下保持：由 \([a(0),a^\dagger(0)]=1\) 要求 \([a(t),a^\dagger(t)]=1\)。计算：
\begin{equation*}
...
再讨论quadrature 方差。在压缩相位 \(\phi=0\) 时，Bogoliubov 变换为 \(S^\dagger a S=a\cosh r-a^\dagger\sinh r\)（由\eqnrefs{eq:squeezing-bogoliubov-transform}，\(u=\cosh r\)，\(v=-\sinh r\)，满足 \(|u|^2-|v|^2=\cosh^2 r-\sinh^2 r=1\)）。由 \(Q=(a+a^\dagger)/\sqrt{2}\)：
\begin{align*}
S^\dagger Q S
...
接着讨论物理意义。Bogoliubov 约束 \(|u|^2-|v|^2=1\) 保证压缩变换不违反正则对易关系——这不是任意变换，而是 \(\mathrm{SU}(1,1)\) 双曲旋转。\(Q\) 方向压缩、\(P\) 方向放大的物理机制是参量下转换中 \(a^{\dagger2}\) 项成对产生光子，使光子数只含偶数项，在 \(Q\) 方向建立相干叠加导致振幅压缩。实例：LIGO 注入 6\,dB 压缩真空（\(r\approx0.69\)），\(Q\) 方向噪声降低 \(e^{-r}\approx0.50\) 倍，用于降低高频散粒噪声。
...
参量下转换 (SPDC)。 二阶非线性晶体（\(\chi^{(2)}\)）在简并参量下转换配置下实现压缩算符 \(S(\zeta)\)。用 Nd:YAG 泵浦光（532\,nm）注入 PPKTP 晶体，信号场频率为 1064\,nm。OPO 低于阈值时输出压缩真空，压缩参量
\[
...
Balanced homodyne 探测。 压缩态的噪声压缩不在光子数分布中直接可见（压缩真空的光子数只含偶数项），而需通过 balanced homodyne 探测测量 quadrature \(X_\theta=(ae^{-i\theta}+a^\dagger e^{i\theta})/\sqrt{2}\) 的方差。本地振荡光 (LO) 与信号在 50/50 分束器上混合，两路光电流差
\[
...
先讨论Wigner 函数与特征函数。单模 Wigner 函数定义为
\begin{equation}
...
随后讨论压缩态特征函数。其次考虑单模压缩态 \(\rho_{\rm sq}=S(\zeta)|0\rangle\langle0|S^\dagger(\zeta)\)，\(\zeta=r\ee^{\ii\phi}\)。特征函数
\begin{equation*}
...
\begin{derivation}[压缩态与连续变量信息量]
压缩态的量子信息性质可由正交分量方差和 Gaussian 熵继续分析。

先讨论标准量子极限与压缩超越。相干态 \(|\alpha\rangle\) 的正交分量 \(\Delta X=\Delta P=1/\sqrt{2}\)（最小不确定乘积 \(\Delta X\Delta P=1/2\)），给出标准量子极限（SQL）。压缩态 \(S(r)|\alpha\rangle\)（\(S(r)=\exp[\frac{r}{2}(a^2-a^{\dagger2})]\)）满足 \(\Delta X=\ee^{-r}/\sqrt{2}\)，\(\Delta P=\ee^r/\sqrt{2}\)——一个分量压缩，另一个反压缩，乘积仍 \(1/2\)。压缩 \(r\) 使 \(\Delta X\) 低于 SQL，在引力波探测（LIGO 注入压缩光提升灵敏度）和精密测量中直接应用。

随后讨论Holevo 界与高斯信道容量。连续变量量子信道的经典信息容量由 Holevo 界给出：\(C\le\chi=S(\rho_{\rm out})-\int p(x)S(\rho_x)\dd x\)（\(S\) von Neumann 熵）。对高斯信道（保高斯态的 CP 映射），Holevo 界可精确计算：\(C=g(\nu_1)+g(\nu_2)-g(\nu_3)-g(\nu_4)\)（\(g(x)=(x+1)\log(x+1)-x\log x\)，\(\{\nu_i\}\) 是信道参数的辛本征值）。压缩态优化 Holevo 界——在能量约束下，压缩态（而非相干态）达到高斯信道容量，是连续变量量子通信的最优编码。

再讨论纠缠辅助容量与超加性。纠缠辅助容量 \(C_E\)（发送方与接收方共享纠缠时的容量）可超过未辅助容量 \(C\)（超加性 \(C_E\ge C\)）。对高斯信道，\(C_E=g(N_S)-g(N_N)\)（\(N_S\) 信号光子数，\(N_N\) 噪声光子数），而 \(C\) 需要优化输入态。对纯损耗信道，\(C=C_E=\log(1+\eta N_S/(1-\eta))\)（\(\eta\) 透射率），纠缠不增加容量；对噪声信道，\(C_E>C\)，纠缠提供真实增益——这是量子通信中纠缠资源价值的定量体现。

接着讨论多模压缩与连续变量纠缠。双模压缩态 \(S_2(r)|0,0\rangle=\frac{1}{\cosh r}\sum_n(\tanh r)^n|n,n\rangle\) 是连续变量纠缠的典型态。其 Schmidt 分解 \(\sum_n\lambda_n|n\rangle_A|n\rangle_B\)（\(\lambda_n=(\tanh r)^n/\cosh r\)）给出纠缠熵 \(E=-\sum|\lambda_n|^2\log|\lambda_n|^2=g(\sinh^2 r)\)。双模压缩态在连续变量量子密钥分发（CV-QKD）中用作纠缠源，安全性的 Shor--Preskill 证明在连续变量中通过 Holevo 界给出窃听者信息上界 \(I_{\rm Eve}\le\chi\)——压缩态参数 \(r\) 直接决定密钥率 \(K=\beta I_{AB}-\chi\)（\(\beta\) 重复率，\(I_{AB}\) Alice--Bob 互信息），是连续变量量子通信的物理基础。
\end{derivation}
