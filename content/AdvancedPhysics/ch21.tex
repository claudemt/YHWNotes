...
以最简单的 \(J_g=1/2\to J_e=3/2\) 模型为例。对基态 \(m_g=+1/2\)，\(\sigma^+\) 跃迁到 \(m_e=+3/2\) 的 CG 强度为 1，而 \(\sigma^-\) 到 \(m_e=-1/2\) 的强度为 \(1/3\)；对 \(m_g=-1/2\) 两者交换。因此在远失谐、低饱和条件下，若局域圆偏振分量强度为 \(I_\pm(z)\)，两条基态 light 位移 具有结构
...
原子沿势坡上爬，动能转化为势能；在接近势顶时被 光学 pumping 到另一个更低的子态，再次开始爬坡。重复这一过程就像希腊神话中的 Sisyphus 不断推石上山，因此称 Sisyphus cooling。
...
这既可从广义相对论理解，也可借助等效原理在加速电梯中用 Doppler 位移 得到。
...
因此各分量满足 Laplace 方程。静态自由空间磁场不能产生稳定的 \(|B|\) 局域极大值，因此 high-场-seeking 态 不能通过普通静态磁场极大值稳定俘获。

这个结论可以直接从 \(|\mathbf B|\) 的 拉普拉斯算子 算出来。记 \(B=|\mathbf B|\)，则
