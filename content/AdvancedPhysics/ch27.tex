...
对质量 $m$、电荷 $Q$ 的相对论性点粒子，在给定外电磁场 $(\Phi,\mathbf A)$ 和额外标量势 $V(\mathbf r)$ 中，正则哈密顿量为
\begin{equation}
...
上述经典简单人链条需要同时满足若干独立条件：$a_0\ll1$ 保证非相对论，$k\ell\ll1$ 保证偶极近似，连续态传播中激光力相对 Coulomb 力占主导，且脉冲包络在单个回碰周期内变化不剧烈。母离子势会导致 Coulomb focusing、轨道相位修正和低能结构；有限横向隧穿动量会降低实际回碰概率；当 $a_0$ 接近一时，磁漂移与相对论质量修正都不可忽略。
...
\end{equation}
比较束缚态隧穿时间尺度与外场周期。$\gamma_K\gg1$ 更接近多光子区，$\gamma_K\ll1$ 更接近准静态隧穿区；它并不单独保证简单人轨道的全部近似成立。下一章将从规范协变的量子传播出发，把本章轨道提升为 Volkov/SFA 的复时间量子轨道，并系统说明这些经典截止为什么会在鞍点极限中重新出现。
