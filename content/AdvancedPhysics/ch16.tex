...
约化矩阵元 $\langle j'||T^{(k)}||j\rangle$ 只需由一个方便的非零矩阵元反解，其余角动量依赖均由 3-j 符号给出。例如 秩-$2$ 电四极算符满足三角条件 $|j-2|\le j'\le j+2$，并在特定磁量子数组合下受到 3-j 符号的附加奇偶约束。同一分离也适用于超精细结构：磁偶极与电四极常数包含动力学信息，全部 $F,m_F$ 依赖由角动量耦合系数承担。
\end{derivation}
