...
把张量算符包装成 intertwiner。固定初态不可约表示 $\mathcal H_j$ 与末态 $\mathcal H_{j'}$。把 rank-$k$ 张量算符 $T_q^{(k)}$ 视为线性映射
\begin{equation*}
...
$\Lambda$ 为 Hermitian（约束与泛函皆实），故可用酉矩阵 $U$ 对角化 $\Lambda U=U\,\text{diag}(\varepsilon_i)$。令新轨道 $\widetilde\varphi_i=\sum_j\varphi_j U_{ji}$；酉变换不改变 Slater determinant（$\det[\widetilde\varphi]=\det[\varphi]\det U$，而 $|\det U|=1$），故总波函数不变。在新基下右端化为对角形式，得到 canonical Hartree--Fock 方程
\begin{equation}
\boxed{
