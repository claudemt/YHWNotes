...
\textbf{物理意义。}色散关系 $\gamma(q)$ 给出 Ising 自旋关联长度 $\xi=1/\gamma(0)$。临界点 $K_v^*=K_h$ 处 $\gamma(0)=0$，关联长度发散——这是相变的标志。各向同性临界点 $K_c=\frac12\ln(1+\sqrt2)$ 代入给出 Onsager 结果 $\xi\sim|T-T_c|^{-1}$，即 $\nu=1$ 临界指数。
\end{derivation}
...
\textbf{物理意义。}真空宇称 $p_{\rm R}=\operatorname{sgn}(K_v^*-K_h)$ 标识自发磁化方向：$K_v^*>K_h$ 时竖向耦合主导，磁化沿竖向；反之沿横向。各向同性点 $K_v^*=K_h$ 处宇称翻转，对应自发对称破缺的相变点。这一结论在有限格上精确成立，说明自发对称破缺不需要热力学极限即可在 Bogoliubov 真空层面识别。
\end{derivation}
...
\emph{思路：}自由能的非解析部分只来自低 $q$ 模式的积分。在临界点附近，色散关系 $\omega(q)\approx\sqrt{\Delta_K^2+v_c^2q^2}$ 产生 $\sqrt{\Delta_K^2+v_c^2q^2}-v_cq$ 的被积函数，其对 $\Delta_K^2$ 的导数含 $\operatorname{arsinh}$，积分后产生 $\Delta_K^2\log|\Delta_K|$ 型奇性。

\emph{第一步：局部模型。}减去临界点的光滑背景，自由能非解析部分只来自小 $q$ 区域：
\begin{equation*}
 \mathcal J(\Delta_K)
 =\int_0^{q_0}
...
\emph{第四步：温度依赖。}由 $K_h=J_h/(\kB T)$、$K_v=J_v/(\kB T)$，对偶关系 $\sinh(2K_v)\sinh(2K_v^*)=1$ 微分给出
\begin{equation*}
 \frac{\dd K_v^*}{\dd K_v}
 =-\frac{1}{\sinh(2K_v)}.
\end{equation*}
...
\emph{结论。}自由能奇异部是 $(T-T_c)^2\log|T-T_c|$，二阶温度导数表现为 $\log|T-T_c|$ 型对数发散——这正是二维 Ising 模型的著名对数比热奇性。

\textbf{物理意义。}对数比热 ($\alpha=0$) 介于不连续（$\alpha<0$）和幂律发散（$\alpha>0$）之间，是二维 Ising 模型的标志性特征。三维 Ising 模型有 $\alpha\approx0.11$（幂律发散），一维无相变。Onsager 1944 年首次解析得到这一结果，是统计力学精确解的里程碑。
\end{derivation}
...
\textbf{物理意义。}$N^{-2}$ 有限尺寸修正是共形场论 (CFT) 的普适预言：$f_\infty-\frac{\pi c v}{6N^2}$ 中 $c=1/2$（Ising CFT 的中心荷）、$v=2$（临界传播速度）。这一修正不依赖微观细节，只由普适类决定——任何与 Ising 同普适类的模型（如三态 Potts 在不同 CFT）都有类似结构但不同 $c$。数值上，$N=100$ 的条带比 $N\to\infty$ 的自由能高出 $\pi/(12\times100^2)\approx2.6\times10^{-5}$，可用于从有限尺寸数据提取中心荷。
\end{derivation}

transfer/Clifford 谱、有限 Fock traces、热力学自由能和临界 finite-size sum 均由前述有限尺寸算符直接推出。自旋二点函数本身不是二次费米子局域算符；把它化为 Toeplitz/Pfaffian 仍需独立的相关函数定理。结合这一输入与 strong Szegő theorem 后，Yang 自发磁化可由剩余 Fourier 求和得到。
...
\emph{第一步：Kaufman 旋转算符表示。}Kaufman 把 Ising 自旋算符表示为 Clifford 代数中的投影算符。对一行内 \(N\) 个自旋，定义旋转算符
\begin{equation*}
...
\emph{第二步：Clifford 表示与 Toeplitz 结构。}Jordan--Wigner 变换后 \(\boldsymbol\sigma_N=\ii^{N/2}\prod_{j=1}^N(c_j+c_j^\dagger)\)（弦算符乘积）。在自由费米子 Fock 空间中，这一乘积的期望值化为 \(N\times N\) 行列式，其元素是费米子 Green 函数
\begin{equation*}
...
\emph{第三步：strong Szegő 定理。}对充分光滑且零环绕数（winding number \(=0\)）的符号 \(\Phi\)，strong Szegő 定理给出
\begin{equation}
...
\emph{第四步：Yang 磁化公式的推导。}自发磁化 \(m=\lim_{N\to\infty}\langle\sigma_{0,0}\sigma_{M,N}\rangle^{1/(MN)}\)（取适当极限）。由\eqnrefs{eq:phys-sm-szego-strong}，体积项给出 \(\log m=\log G_0\)，常数项给出有限尺寸修正。对 Ising 符号 \(\Phi(\theta)=[1-k^2\sin^2(\theta/2)]^{1/2}\)（\(k=\sinh^{-2}(2K)\)），Fourier 系数
\begin{equation*}
...
\emph{Fisher--Hartwig 奇点与临界行为。}临界点 \(K=K_c\)（\(k=1\)）时 \(\Phi(\theta)\) 在 \(\theta=0\) 有 cusp 奇点，strong Szegő 定理不适用。Fisher--Hartwig 把符号分解为 \(\Phi=\Phi_B\prod_j t_{\alpha_j,\beta_j}\)，其中 \(\Phi_B\) 满足 Szegő 条件，\(t_{\alpha,\beta}\) 是奇点因子。Ising 临界对应单个 \((\alpha,\beta)=(0,1/2)\) 奇点，行列式渐近
\begin{equation*}
  D_N\sim E\,N^{\alpha^2-\beta^2}\,\exp\!\left[N\log G_0+\sum n(\log\Phi_B)_n(\log\Phi_B)_{-n}\right],
\end{equation*}
