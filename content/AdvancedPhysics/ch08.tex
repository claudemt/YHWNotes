...
Jordan--Wigner 与 Clifford 线性化已经把 转移 算符 化成独立动量块，同时保留了 NS/R 网格和物理奇偶投影。从这些有限尺寸对象出发，先得到单粒子 快度 \(\gamma(q)\)，再写出四个 费米子 迹 的有限乘积，最后取热力学极限得到 Onsager 自由能。临界线由单粒子 转移质量 闭合确定；具体自旋相关长度还需结合观测算符的 形式-因子 选择定则，不能与单粒子谱隙直接等同。

\section{动量块的 Clifford 线性变换}
\label{sec:phys-sm-onsager-solution}

在固定 NS 或 R 空间扇区内，Fourier 变换把平移不变二次型分成 \(\{q,-q\}\) 块。选取一个 Nambu/Majorana 基后，垂直和水平 转移 因子 的共轭作用分别由 行列式-one 矩阵
\begin{equation}
 \mathsf R_v=
 \begin{pmatrix}
 \cosh 2K_v^*&\sinh 2K_v^*\\
...
\end{equation}
表示。一个完整对称 转移 step 与 \(\mathsf R_v\mathsf R_h(q)\) 相似，因此具有相同本征值。令
\[
...
\]
在固定空间扇区 \(s\in\{\mathrm{NS},\mathrm R\}\) 中，二次 转移 算符 可作 Bogoliubov 正规形，使多体本征值写成
\begin{equation}
 \Lambda_s(\{n_q\})
 =\mathcal C_N
 \exp\!\left[
...
\end{equation}
这里对自反模也把 \(n_q\) 视为独立单模占据；非自反 \(q,-q\) 的 正则变换 则成对组织。令 \(p_s=\pm1\) 表示 Bogoliubov 真空相对于原 Jordan--Wigner 奇偶算符的奇偶，即
\[
 \mathcal P
 =p_s(-1)^{\sum_q n_q}.
\]
非自反的 \(\{q,-q\}\) Bogoliubov 变换只成对改变原费米子占据，因此不会改变真空奇偶。自反模中，\(q=\pi\) 的未配对质量具有符号 \(K_v^*+K_h>0\)，而 R 扇区的 \(q=0\) 未配对质量具有符号 \(K_v^*-K_h\)。于是对铁磁耦合 \(K_h,K_v>0\) 且离开临界线，当前 Jordan--Wigner 约定 给出
\begin{equation}
 p_{\rm NS}=+1,
 \qquad
...
\end{equation}
在临界线 \(K_v^*=K_h\) 上，R 扇区的 \(q=0\) 快度 为零，此时含奇偶插入的 R 迹 本身为零，所以 \(p_{\rm R}\) 在该点无需单独指定。

对纵向周期数 \(M\)，四个 迹 由独立 准粒子 模逐项相乘：
\begin{align}
 \mathcal Z_{s,0}
 &=\mathcal C_N^M
 \prod_{q\in s}
...
\end{equation}
得到。若存在零 快度，自带 \(\sinh(M\gamma/2)=0\) 会自动使相应奇偶插入 迹 消失；这正是不能把特殊模藏在“所有 \(q\) 成对”近似中的原因。

\begin{derivation}[为什么奇偶插入把 \(\cosh\) 变成 \(\sinh\)]
由\eqnrefs{eq:phys-sm-finite-spectrum-structure}，固定扇区 \(s\) 中单步 转移 的多体本征值为
\begin{equation*}
 \Lambda_s(\{n_q\})
 =\mathcal C_N\exp\!\left[\frac12\sum_{q\in s}(1-2n_q)\gamma(q)\right],
 \qquad n_q\in\{0,1\},
\end{equation*}
其中 \(n_q=0\) 对应 Bogoliubov 真空，\(n_q=1\) 对应单模激发。纵向 \(M\) 步等价于对每个 准粒子 模作 \(M\) 次乘幂，故对单模 \(q\)，
\begin{equation*}
 \Lambda_q(n)^M
...
对非自反块 \(\{q,-q\}\)，正则变换 混合的是 \(q\) 与 \(-q\) 两个模，真空占据改变为偶数，故不翻转 \(p_s\)；只有自反模能够翻转。自反模 \(q=\pi\) 的未配对质量符号为 \(K_v^*+K_h>0\)，不穿零；\(q=0\) 的符号为 \(K_v^*-K_h\)，恰在临界线 \(K_v^*=K_h\) 穿零。因此铁磁耦合下
\begin{equation*}
 p_{\rm NS}=+1,
 \qquad
 p_{\rm R}=\operatorname{sgn}(K_v^*-K_h),
...
对长柱面自由能只需各扇区主导本征值的体积阶。奇偶投影最多改变有限个 准粒子 的占据，所以
\begin{equation}
 \log\Lambda_{\max}(N)
...
\end{equation}
这里 \(\Delta_{\rm tr}\) 是 dimensionless 初等 转移质量。闭隙条件 \(K_v^*=K_h\) 与
\begin{equation}
 \boxed{
...
自由能奇性由最软 转移 模 控制，但具体观测量的相关长度还取决于它能耦合到哪些激发。因而在进入有限尺寸临界修正前，需要把单粒子 转移质量 与自旋相关长度严格区分。

由 转移 谱得到的 \(\Delta_{\rm tr}=\gamma(0)\) 首先只是最低 准粒子 mass。具体观测量的相关长度还要使用\eqnrefs{eq:phys-sm-observable-correlation-length} 的 选择定则。二维 Ising 的精确 形式-因子 理论 给出额外输入：无序相中的 自旋 场 可耦合到一粒子通道，而有序纯相的 连通 自旋 关联 首个非真空贡献是两粒子通道。因此在接近临界点时
\begin{equation}
...
\end{equation}
这里第二式指有序纯相的 连通关联。非零 形状因子 本身是独立的 Ising 关联-理论 输入；仅凭自由费米单粒子谱不能证明其系数非零。两侧相关长度振幅可以不同，而 \(\Delta_{\rm tr}=|a_T|\,|T-T_c|+O((T-T_c)^2)\) 仍给出共同临界指数 \(\nu=1\)。

\section{临界 NS/R 和的有限尺寸修正}

有限扇区在临界点不会只留下“\(O(1/N)\)”的模糊量级；系数可以直接从各向异性色散的 尖点 算出。在临界线 \(K_v^*=K_h\) 上，令
\[
 v_c:=\sinh(2K_{h,c})>0.
\]
...
\end{equation}
各向异性只通过临界 转移 velocity \(v_c\) 改变度量尺度；无量纲系数仍与周期 Ising 临界理论的 中心 荷 \(c=1/2\) 一致。这里并未把共形场论作为推导输入：系数直接来自有限格点 快度 的 尖点 与离散 Fourier 取样。

\begin{derivation}[各向异性临界 尖点 到 NS/R 的 \(1/N\) 系数]
...
传递矩阵的 Clifford 谱、有限 Fock 空间中的迹、热力学自由能和临界有限尺寸求和均由前述有限尺寸算符直接推出。自旋二点函数本身不是二次费米子局域算符；把它化为 Toeplitz 行列式或 Pfaffian 仍需独立的相关函数定理。结合这一输入与 强 Szegő 定理后，Yang 自发磁化可由剩余 Fourier 求和得到。
...
\end{equation*}
使 \(\sigma_j^z\mathbf W=\mathbf W\tau_j^x\)（把 \(Z\) 旋转到 \(X\) 方向）。两点函数化为 转移矩阵 的矩阵元
\begin{equation*}
...
\end{equation*}
求和 \(\sum_{n=1}^\infty n(\log\Phi)_n(\log\Phi)_{-n}=\frac{1}{4}\log(1-k^2)\)，给出\eqnrefs{eq:phys-sm-onsager-magnetization}。\(T>T_c\) 时 \(k>1\)，符号 \(\Phi\) 有零点（环绕数 \(\neq0\)），强 Szegő 定理失效，需用 广义 Szegő 定理（Fisher--Hartwig），给出磁化为零。

临界点 \(K=K_c\)（\(k=1\)）时 \(\Phi(\theta)\) 在 \(\theta=0\) 有 尖点 奇点，强 Szegő 定理不适用。Fisher--Hartwig 把符号分解为 \(\Phi=\Phi_B\prod_j t_{\alpha_j,\beta_j}\)，其中 \(\Phi_B\) 满足 Szegő 条件，\(t_{\alpha,\beta}\) 是奇点因子。Ising 临界对应单个 \((\alpha,\beta)=(0,1/2)\) 奇点，行列式渐近
\begin{align*}
  D_N
  ...
  \right],
\end{align*}
幂律 \(N^{-1/4}\) 给出 \(\eta=1/4\)。这把 Szegő/Fisher--Hartwig 渐近与 共形场论 标度维度 \(h_\sigma=1/16\)（因 \(2h_\sigma=1/8\)，\(\eta=2h_\sigma=1/4\)）精确对应。
\end{derivation}
