...
随后讨论交错性（特征 \(2\) 下的对称性）。在 \(\mathbb F_2\) 上，\(\omega(\alpha,\beta)=|\alpha\cap\beta|\pmod2=|\beta\cap\alpha|\pmod2=\omega(\beta,\alpha)\)，故 \(\omega\) 对称。但曲面的相交形式在整数系数下是交错的：\(I_\mathbb Z(\alpha,\beta)=-I_\mathbb Z(\beta,\alpha)\)。模二化后交错性变为对称性，\(\omega(\alpha,\alpha)=0\)（自交数为偶：闭链可微扰使自交成对出现）。因此 \(\omega\) 是 \(\mathbb F_2\)-辛形式：对称且自交为零。
