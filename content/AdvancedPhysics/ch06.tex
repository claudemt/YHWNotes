...
Perron--Frobenius 定理可推广至紧正算子（Krein--Rutman 定理）：若 \(\mathscr T\) 是 Banach 格上的紧正算子且谱半径 \(r(\mathscr T)>0\)，则 \(r(\mathscr T)\) 是具有正特征向量的单本征值。这把有限维 转移矩阵 的主本征值论证直接延拓到 \(N\to\infty\) 的柱面极限，是 Ruelle 证明 \(\log\Lambda_0(N)\) 凸性从而存在 热力学极限 的关键工具。次主本征值在临界点处与主本征值等模，对应关联长度发散与相变。
\end{derivation}
