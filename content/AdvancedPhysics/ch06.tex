...
图展开把二维 Ising 的配分函数组织成边集之和；要得到完整自由能和有限尺寸谱，更有效的做法是把一个方向的统计求和改写为线性算子的幂。先建立 转移 算符 的一般有限条带结构：精确矩阵元、Perron--Frobenius 主本征值、谱表示的相关函数以及 Pauli 算符分解。随后再引入费米子化，从而把“转移算子存在”与“方格 Ising 恰好能被 Clifford 代数线性化”分开。
...
\end{equation}
这里的 右矢 只是经典行配置的线性编码，不是额外的量子时间演化 Hilbert 空间。

横向取周期边界 \(\sigma_{N+1}=\sigma_1\)。把一行内部的水平 Boltzmann 权重平均分到相邻两次纵向传播，定义对称 转移 算符 \(\mathsf T_N\) 的矩阵元
\begin{equation}
...
\]
再除以 \(N\) 即得\eqnrefs{eq:phys-sm-strip-free-energy}。主本征值的严格性来自 \(\mathsf T_N\) 的全部元素为正（Perron--Frobenius），而铁磁 Ising 的 转移矩阵 恰满足此条件。
\end{derivation}

例：取 $N=1$（单列即单自旋），$\mathsf T_1$ 退化为 $2\times2$ 矩阵 $\mathsf T_1=\begin{pmatrix}\ee^{\beta J}&\ee^{-\beta J}\\ \ee^{-\beta J}&\ee^{\beta J}\end{pmatrix}$，本征值 $\Lambda_0=2\cosh(\beta J)$、$\Lambda_1=2\sinh(\beta J)$，谱隙 $\Delta_1=\log\coth(\beta J)$；配分函数 $Z_{1,M}=\Lambda_0^M+\Lambda_1^M$，自由能 $-\beta f_1=\log(2\cosh\beta J)+o(1)$ 与 Onsager 解的高温极限一致，纵向关联长度 $\xi_\parallel=1/\log\coth(\beta J)$ 即一维 Ising 的精确值。Perron--Frobenius 定理可推广至紧正算子（Krein--Rutman 定理）：若 $\mathscr T$ 是 Banach 格上的紧正算子且谱半径 $r(\mathscr T)>0$，则 $r(\mathscr T)$ 是具有正特征向量的单本征值——这把有限维 转移矩阵 的主本征值论证直接延拓到 $N\to\infty$ 柱面极限，是 Ruelle 证明 $\log\Lambda_0(N)$ 凸性从而存在 热力学极限 的关键工具；次主本征值在临界点处与主本征值等模，对应关联长度发散与相变。

\section{谱表示与 observable-dependent 相关长度}

转移谱 不只决定自由能，也决定沿纵向的相关衰减。令 \(\widehat O\) 是由一行局域观测量定义的对角算符，取 \(M\to\infty\) 后，间隔 \(r\) 行的 连通关联 可写成
\begin{equation}
...
\end{equation}
这条 选择定则 在后面区分单费米 转移质量 与具体自旋相关长度时至关重要。

\begin{derivation}[转移 谱到 连通关联]
...
\]
对一行局域观测量 \(O\)，定义对角算符 \(\widehat O\)，并设两观测点纵向间隔 \(r\) 行。由 转移 算子的物理意义，未归一化两点函数为
\[
...
\[
 ...
 \end{aligned}
\]
由 \(|0\rangle\) 是 Perron--Frobenius 主本征矢，\(\langle0|\widehat O|0\rangle=\langle O\rangle\) 正是观测量 \(O\) 的热力学平均，因此 \(a=0\) 项为 \(\langle O\rangle^2\)。减去它便得 连通关联
\[
 \langle O_0O_r\rangle_c
...
\[
 ...
 \log\frac{\Lambda_0}{|\Lambda_a|},
\]
即\eqnrefs{eq:phys-sm-observable-correlation-length}。若 \(O\) 与所有非主谱态正交，则 连通关联 恒为零，相关长度为无穷；选择定则 因此不是技术细节，而是观测量与谱的物理耦合条件。
\end{derivation}
...
Jordan--Winger 变换把 \(N\) 个 Pauli 自旋映射为 \(N\) 个费米子模式，使 转移矩阵 从 \(2^N\) 维一般矩阵降为 \(N\) 维自由费米子问题。分四步建立。
...
Pauli 二次型的费米子化。翻转算符 \(X_j=S_j\gamma_j\)（因 \(S_j^2=\mathbb I\)），但弦因子 \(S_j\) 使 \(\sum_j X_j\) 不是简单费米子二次型。关键技巧：做自旋轴旋转 \(X\to Y, Y\to-X\)（等价于 \(\gamma_j\to\tilde\gamma_j, \tilde\gamma_j\to-\gamma_j\)），使 转移矩阵 中两因子的弦因子结构互补。旋转后
\begin{equation*}
...
\end{equation}
即 Onsager 色散关系。转移矩阵 本征值为各模式能量的乘积，自由能由 \(\frac{1}{N}\sum_k\epsilon(k)\to\int\frac{\dd k}{2\pi}\epsilon(k)\) 给出。临界条件 \(\epsilon(0)=0\)（R 扇区 \(k=0\) 模式软化）恢复\eqnrefs{eq:phys-sm-square-critical-k}。

Jordan--Wigner 弦 \(S_j=\prod_{k<j}Z_k\) 是一维有序化：它利用链的线性序把局域 Pauli 映射为局域费米子。二维中弦因子变为面填充算符，一般无法消除——这是二维 Ising 精确可解但三维不可解的根本原因。环面边界处弦因子绕回，产生 \(R_N\) 宇称分支，对应\eqnrefs{eq:phys-sm-torus-h1} 的两个同调生成元。这一联系把 Jordan--Wigner 边界项与曲面同调统一：弦绕行数 = 同调绕行数 = 自旋结构 选择。
\end{derivation}
...
对 Ising 转移矩阵，Fourier 后 \(A_k=K_h\cos k+K_v^*\)，\(B_k=K_h\sin k\)（各向异性推广直截了当）。BdG 矩阵
\begin{equation*}
  \begin{pmatrix}K_h\cos k+K_v^*&K_h\sin k\\-K_h\sin k&-(K_h\cos k+K_v^*)\end{pmatrix},
\end{equation*}
本征值 \(\epsilon(k)=\pm\sqrt{(K_h\cos k+K_v^*)^2+(K_h\sin k)^2}=\pm\sqrt{K_h^2+K_v^{*2}+2K_hK_v^*\cos k}\)。这与\eqnrefs{eq:phys-sm-fermion-dispersion} 在参数化 \(\cosh\epsilon=\cosh(2K_h)\cosh(2K_v^*)-\sinh(2K_h)\sinh(2K_v^*)\cos k\) 下一致（差一个整体重标，来自 转移矩阵 与 Hamiltonian 的指数关系）。
...
有限环 \(N\) 上 NS 扇区最低动量 \(k_{\min}=\pi/N\)，有限尺寸谱隙 \(\Delta_N\approx\Delta+\frac{\pi^2}{2N^2\epsilon(0)}\)（小 \(k\) 展开）。临界点 \(\Delta=0\) 时 \(\Delta_N\sim\pi/N\)，对应 中心 荷 \(c=1/2\) 的 Ising 共形场论：有限尺寸自由能修正 \(f_N=f_\infty-\frac{\pi c}{6N^2}\)，\(c=1/2\) 由三个 主 场 \((1,\sigma,\epsilon)\) 的 共形 权 \((0,1/16,1/2)\) 确定。这把 转移矩阵 谱与共形场论直接联系起来。
...
Suzuki--Trotter 分解把 \(d\) 维经典统计模型的 转移矩阵 映射为 \((d-1)\) 维量子模型的虚时演化，建立量子--经典对应。分四步建立。

对算符 \(A,B\)，\(\ee^{A+B}=\lim_{n\to\infty}(\ee^{A/n}\ee^{B/n})^n\)。误差估计：\(\ee^{A/n}\ee^{B/n}=\ee^{(A+B)/n+[A,B]/(2n^2)+O(n^{-3})}\)，故 \(n\) 步分解误差 \(O(1/n)\)。对 Ising 转移矩阵 \(\mathsf T=\mathsf V_h\mathsf V_v\)，Trotter 分解把非对易乘积化为交替指数的乘积，每步可显式计算。

2D 经典 Ising 的 转移矩阵 \(\mathsf T_N=\mathsf V_h^{1/2}\mathsf V_v\mathsf V_h^{1/2}\)（\eqnrefs{eq:phys-sm-transfer-pauli-factorization}）。连续极限 \(M\to\infty\)（纵向层数），\(\mathsf T_N^M=\ee^{-M H_{\rm Q}}\)，量子 Hamiltonian
\begin{equation*}
...
\begin{itemize}
 ...
 \item[(iii)] 蒙特卡洛：Suzuki--Trotter 分解把量子统计力学化为经典配分函数（世界线表示），使量子 蒙特卡洛 成为可能——连续极限误差 \(O(\Delta\tau^2)\) 由分解阶数控制。
 \item[(iv)] AdS/共形场论 对应：量子--经典对应的推广，\(d\) 维量子场论映射到 \((d+1)\) 维经典引力，Ising/共形场论 对应是其离散版本。
\end{itemize}
