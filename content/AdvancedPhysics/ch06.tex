\chapter{Jordan--Wigner、Clifford 线性化与有限扇区}
...
Ising 模型的转移因子中，垂直传递因子 $\mathsf V_v$ 含 $\sigma^z$（在位），水平因子 $\mathsf V_h$ 含 $\sigma^z\sigma^z$（邻接）。Jordan--Wigner 变换要求在位项对角、邻接项在 $\tau^x$ 方向——因此需先做一个 $\pi/2$ 轴旋转，把 $\sigma^z\sigma^z$ 耦合转到 $\tau^x\tau^x$ 方向。定义旋转后的 Pauli 算符
\begin{equation}
 \tau_j^z=X_j,
 \qquad
 \tau_j^x=Z_j,
...
对满足正则反交换关系的费米子，高斯态下的 \(2n\) 点函数由 Pfaffian 给出：
\begin{equation*}
...
Jordan--Wigner 变换把自旋耦合写成费米子双线性，但要对转移矩阵做谱分析，还需知道二次型指数的共轭作用如何作用于费米子空间。引入 Majorana 算符后，这一问题化为 $2N$ 维实正交线性代数——Clifford 代数的核心结构。
...
对满足 CCR \([b_i,b_j^\dagger]=\delta_{ij}\) 的玻色子，定义厄米 Quadratic \(\mathcal Q_{\rm B}=\sum A_{ij}b_i^\dagger b_j\)，则 \([\mathcal Q_{\rm B},b]=\mathsf A b\) 给出 \(\mathfrak{gl}(N)\) 而非 \(\mathfrak{so}(2N)\) 的作用，对应辛群 \(\mathrm{Sp}(2N)\) 而非旋量群。玻色子高斯的迹由 \(\det(1-\mathsf T)^{-1}\) 给出（分母幂），费米子由 \(\det(1+\mathsf T)\) 给出（分子幂），二者统一于 superdeterminant（Berezinian）\(\operatorname{sdet}(1-\mathsf T)=\det(1-\mathsf T_{\rm B})/\det(1+\mathsf T_{\rm F})\)，这是超对称量子力学的代数基础。
\end{proof}
...
\[
 ...
 =-2c_N^\dagger,
\]
其中用了 \((c_N^\dagger)^2=0\)（正则反交换关系的直接推论）。因此
\[
 \tau_N^z(c_N+c_N^\dagger)
...
边界条件 \(c_{N+1}=-\mathcal P c_1\) 表明费米子在绕环一周后获得 \(\mathcal P\) 决定的相位：偶奇偶扇区给反周期（NS），奇奇偶扇区给周期（R）。这正是 Euclidean 路径积分中自旋结构的离散对应：环面上四个自旋结构由 \((\alpha_x,\alpha_y)\in\{0,\tfrac12\}^2\) 标记，而 Ising 模型的四个迹扇区 \((s,t)\) 通过此边界条件自然实现。负号 \(-\mathcal P\) 中的整体 \(-1\) 来自 Jordan--Wigner 字符串的非局域性，是 Kauffman 圆环模型的拓扑特征，也是 Majorana 零模在拓扑相变中符号翻转的代数根源。

在亏格 \(g\) 曲面上，自旋结构形成以 \(H^1(\Sigma_g;\mathbb Z_2)\cong(\mathbb Z_2)^{2g}\) 为平移群的仿射扭子空间，共有 \(2^{2g}\) 个。Ising 模型对应 \(g=1\) 的四个扇区；高亏格情形需用 Arf 不变量区分偶/奇自旋结构，这是 Kitaev toric code 与 Ising 任意子分类的共同代数基础。
\end{proof}
...
零模条件是 \(\epsilon(0)=0\)，并在各向同性情形化为 \(\sinh(2K_c)=1\)，即\eqnrefs{eq:phys-sm-square-critical-k}。此时厄米 BdG 块仍可对角化，不会产生 Jordan 块；特殊之处只在于本征值为零，通常的“正能支”约定不再唯一。有限尺寸物理还必须连同全局费米子宇称投影处理。

把零能复费米子写成两个厄米分量时，每个分量都与临界二次哈密顿量对易并翻转费米子宇称。因此未投影 Fock 空间出现成对态；物理转移矩阵只保留满足扇区宇称条件的组合。\(\mathcal Z_{\mathrm R,0}-\mathcal Z_{\mathrm R,1}\) 在临界点消失，正是零能占据与空置贡献相消的结果，而不是额外的环面拓扑简并。

离开临界点后该体模获得质量 \(m\propto K-K_c\)，并不继续保持零能。Jordan--Wigner 后的量子 Ising 链可与 Kitaev 链比较，但开放链端模、周期链体临界模与环面自旋-结构扇区是三个不同概念，不能用 \(2^{2g}\) 简并把它们等同。

\end{proof}

临界有限尺寸自由能的次领项可用 Ising 共形场论解释。
临界点处有限尺寸自由能的精确系数来自离散 Fourier 和的 Euler--Maclaurin 渐近。把光滑部分与动量零点附近的尖点分开求和，便得到与 \(c=1/2\) Ising 共形场论一致的系数。
...
\end{equation*}
其中 \(s\) 是 NS（\(q=(m+1/2)\cdot2\pi/N\)）或 R（\(q=m\cdot2\pi/N\)）网格。主导差异来自 \(q\to0\) 的尖点贡献。

对光滑函数 \(g(q)\)，\(\frac{1}{N}\sum_{m}g\!\left(\frac{2\pi m}{N}\right)=\int_0^{2\pi}\frac{g(q)\dd q}{2\pi}+\frac{g'(0)-g'(2\pi)}{2N}+\cdots\)。但 \(\log|q|\) 在 \(q=0\) 有尖点，\(g'(0^+)-g'(0^-)=2/N\)，给出额外 \(O(N^{-1})\) 项。精确计算（Cardy 1984）给出 NS/R 扇区自由能差
\begin{equation*}
...
\end{equation*}
自旋关联 \(\langle\sigma\sigma\rangle\sim r^{-2h_\sigma}=r^{-1/8}\)（因 \(2h_\sigma=1/8\)），与\eqnrefs{eq:phys-sm-onsager-magnetization} 的 \(\beta=1/8\) 一致。能量关联 \(\langle\psi\psi\rangle\sim r^{-2h_\psi}=r^{-2}\)，对应能量-能量关联的 \(\eta_\epsilon=1\)。有限尺寸自由能\eqnrefs{eq:phys-sm-ising-cft-central-charge} 中 \(c=1/2\) 可从三个主的 Virasoro 代数表示验证：\(\sigma\) 的模不变性（模 S 矩阵）给出字符 \(\chi_0+\chi_{1/2}\)（NS 扇区）和 \(\chi_{1/16}\)（R 扇区），其组合在模变换下的不变性唯一确定 \(c=1/2\)。这把 Onsager 精确解的代数结构提升为共形不变性——临界 Ising 模型 \emph{就是} \(c=1/2\) Ising 共形场论的格点正则化。

共形场论预言有限尺寸能谱 \(E_n-E_0=\frac{2\pi v}{N}(h_n+\bar h_n-1/12)\)，其中 \(v\) 是临界速度。Ising 共形场论的三个主给出三个最低能级：\(\mathbf 1\)（真空，\(E=E_0\)）、\(\sigma\)（自旋，\(E-E_0=\frac{\pi v}{8N}\)）、\(\psi\)（能量，\(E-E_0=\frac{\pi v}{N}\)）。这些能级在转移矩阵谱中精确可辨，对应 NS/R 扇区的不同扇区，是共形场论-算符 correspondence 的格点验证。
...
\]
保持不变。这与 NS/R 扇区投影\eqnrefs{eq:phys-sm-kaufman-sector-projection} 相容，是格点 Ising 与 (c=1/2) 共形场论对应的谱学检验。

从格点 Ising 到 \(c=1/2\) 共形场论的对应可在两个严格意义上验证：(1) 有限尺寸自由能系数 \(-\pi c/(48N^2)\) 精确匹配（Cardy 1984）；(2) 转移矩阵谱的能级间距 \(\frac{2\pi v}{N}(h+\bar h-1/12)\) 精确匹配（Belavin--Polyakov--Zamolodchikov 1984）。两者都依赖 Ising 的自由费米子结构——Bogoliubov 谱给出精确能级，Euler--Maclaurin 给出精确自由能修正。对一般格点模型（如 3D Ising），无此精确对应，只有数值和重整化群证据，凸显了 2D Ising 精确可解性在格点--共形场论对应中的独特地位。
...
与转移矩阵的等价。Kac--Ward 行列\eqnrefs{eq:phys-sm-kac-ward-pfaffian} 与转移矩阵对角化给出同一配分函数，但途径不同：转移矩阵用 Jordan--Wigner → Bogoliubov → 谱乘积，Kac--Ward 用图展开 → Pfaffian → 行列式。两者的等价可在有限尺寸验证：Kac--Ward 行列式的本征值恰是 Bogoliubov 色散 \(\epsilon(q)\) 的乘积。这一等价性是 2D Ising 精确可解性的代数根源——Clifford 代数（转移矩阵）与 Pfaffian 代数（Kac--Ward）是同一数学结构的两种表现。

Kac--Ward 方法在 3D 失效：3D 方格的偶子图是闭合曲面而非路径，Pfaffian 的完全匹配（一维对象）无法表示二维曲面。3D Ising 的配分函数需要更高阶的代数结构（如 Grassmann 积分或四元数 Pfaffian），目前无精确解。这一障碍是拓扑的：2D 偶子图由 \(\mathbb Z_2\) 同调分类（一阶上同调），3D 偶子图由 \(\mathbb Z_2\) 二阶上同调分类，后者无法用 Pfaffian（一阶对象）表示。这是 3D Ising 精确可解性困难的代数拓扑起源。

临界格点观察量的 s 全纯性给出了通向连续共形场论的严格桥梁。

离散复结构。方格图 \(G\) 的对偶图 \(G^*\) 和中间图 \(G_M\)（旋转 \(45^\circ\)）共同定义离散复结构。每条边 \(e\in E(G)\) 对应对偶边 \(e^*\in E(G^*)\)，中间边 \(e_M\) 连接 \(e\) 和 \(e^*\) 的中点。离散导数在中间图上定义：函数 \(F:V(G_M)\to\mathbb C\) 的离散导数 \(\partial_M F\) 用相邻中间顶点的差商给出。

s 全纯条件。Ising 观察值 \(F(e_M)\)（定义在中间边上）满足 s 全纯条件：对每个中间顶点 \(z\)，
\begin{equation*}
  \sum_{e_M\ni z}\operatorname{proj}_{\ell(e_M)}F(e_M)=0,
\end{equation*}
其中 \(\ell(e_M)\) 是与边 \(e_M\) 关联的直线（方向由 Ising 耦合的 Kac--Ward 相位确定），\(\operatorname{proj}_\ell\) 是到 \(\ell\) 的正交投影。s 全纯弱于全纯：它要求投影（而非函数本身）满足离散 Cauchy--Riemann 方程。

临界 s 全纯与连续极限。临界 Ising（\(\sinh(2K_c)=1\)）的 s 全纯函数在网格细化 \(a\to0\) 时收敛到全纯函数：\(\operatorname{proj}_\ell F\to\operatorname{Re}(f)\) 或 \(\operatorname{Im}(f)\)（视 \(\ell\) 方向），因此 \(F\to f\) 全纯。这一收敛是指数快的（远离临界点衰减），给出临界关联函数的严格连续极限。特别地，自旋关联 \(\mathbb E[\sigma_{z_1}\cdots\sigma_{z_n}]\) 的离散全纯性保证其收敛到共形场论关联函数 \(\langle\sigma(z_1)\cdots\sigma(z_n)\rangle_{\mathrm{CFT}}\)。

Smirnov 的收敛定理。Smirnov (2010) 证明：临界方格 Ising 的自旋关联在网格细化下收敛到 \(c=1/2\) Ising 共形场论的相应关联函数，收敛是局部一致的。这给出了从格点模型到共形场论的严格收敛证明（而非仅形式匹配），是 2D Ising 精确可解性最深刻的严格结果。s 全纯性是这一证明的核心工具：它把离散 Ising 观察量嵌入到离散复分析框架中，使连续极限成为全纯函数的收敛问题。这一方法已推广到其他临界格点模型（如 FK-Ising、回路 O(1)），是现代统计力学严格收敛理论的基石。
\end{proof}
