...
先讨论Asano contraction 的代数表述。设 \(P(z_1,z_2)\) 是双变量多项式，在 \(\mathbb D^2=\{|z_1|<1,|z_2|<1\}\) 内无零点（即 \(P\) 在双单位圆盘上稳定）。定义 contract
\begin{equation*}
 (CP)(z)=P(z,z).
\end{equation*}
...
随后讨论Ruelle 推广到多变量。Ruelle 把 Asano contraction 推广到多变量：设 \(P(z_1,\ldots,z_n)\) 在 \(\mathbb D^n\) 内无零点。对任意 \(i\ne j\)，contract \(z_i,z_j\to z\)（即令 \(z_i=z_j=z\)）后，\(P(z,\ldots,z,\ldots,z,\ldots)\) 在 \(\mathbb D\) 内无零点。证明对 \(z_i\) 固定其余变量，应用第一步的双变量结果。

再讨论从单边到一般图。对有限铁磁图 \(G=(\mathsf V,\mathsf E)\)，配分函数 \(\Phi_G(\{z_x\})\) 可逐边构造：
\begin{itemize}
...
Asano--Ruelle 收缩可以保留多变量零点禁区，某些连续单点测度也具有 Lee--Yang 性质；但这些推广都要求额外的稳定性假设，不能只从单变量圆定理的形式推出。反铁磁或受挫耦合通常破坏圆形零点约束。因此后文只在已经验证铁磁性与单点测度条件的模型中调用圆定理；一维链的解析性则使用本章已经给出的转移矩阵谱隙，不再重复一套彼此矛盾的本征值计算。
