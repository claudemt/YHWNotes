\chapter{Lee--Yang 零点与相变}
\label{chap:phys-sm-partition-function-zeros-phase-transitions}

有限体积的配分函数是解析函数；热力学极限若要产生相变，它的复零点必须不断逼近物理参数轴。铁磁 Ising 模型的特殊之处在于，这些零点的几何位置能够被精确限制。多变量 Lee--Yang 禁区对任意有限图成立，均匀外场时零点进一步落到单位圆上；零点在极限中的密度直接控制磁化的不连续性。这里不需要二维方格的可解性。
...
把局部结论推广到任意有限图需要 Asano--Ruelle 收缩引理。该引理是独立的多变量稳定多项式定理：它说明在逐边合并变量的特定收缩操作下，上述单位圆盘零点禁区保持不变。简单代入 \(z_i=z_j\) 与收缩并不等价；从单边稳定性推广到一般图所需的保持定理正是 Asano--Ruelle 引理。
...
\eqnrefs{eq:phys-sm-two-spin-stability} 可由二自旋多项式直接计算；从单边稳定性到任意图的逐边收缩则依赖 Asano--Ruelle 引理。两者的证明层级必须分开。
\end{note}

\begin{proof}[解]
Asano--Ruelle 收缩引理把单边稳定性\eqnrefs{eq:phys-sm-two-spin-stability} 逐边推广到任意有限图。核心操作是"收缩"：把双变量多项式中 \(z_1,z_2\) 同时替换为 \(z\)，在特定条件下保持零点禁区。

Asano 收缩的代数表述。设 \(P(z_1,z_2)\) 是双变量多项式，在 \(\mathbb D^2=\{|z_1|<1,|z_2|<1\}\) 内无零点（即 \(P\) 在双单位圆盘上稳定）。定义收缩
\begin{equation*}
 (CP)(z)=P(z,z).
\end{equation*}
...
对每个 \(z_1\in\mathbb D\)，定义 Rouché 变换
\begin{equation*}
...
\begin{itemize}
 ...
 \item 合并：添加边 \(\{x,y\}\) 后，新多项式是旧多项式与 \(P_{K_{xy}}\) 的乘积。乘积在 \(\mathbb D^n\) 内无零点当且仅当每个因子无零点。但合并后 \(z_x,z_y\) 成为同一多项式的变量——需用 Asano--Ruelle 收缩保证合并后仍稳定。
\end{itemize}
...
圆定理依赖铁磁耦合和 Ising 型二值自旋，不是任意统计模型的普遍零点定理；零点积聚导致非解析性的逻辑则更一般。Lee--Yang 理论负责外场方向的解析结构；零场二维 Ising 的自由能则要用另一套方法求解：先把配分函数改写成偶子图，再利用平面对偶 Duality、转移矩阵与费米子化。两种分析最终在 Yang 自发磁化处相接，因为 Yang 自发磁化既决定长距离关联，也通过\eqnrefs{eq:phys-sm-mag-jump} 决定 \(z=1\) 处的零点密度。
