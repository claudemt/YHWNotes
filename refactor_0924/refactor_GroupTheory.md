...
下面最初的 22 章拆分方案适合列任务，不适合直接成为讲义目录。正式成书把源码单元合并为 11 个读者可见章节：群与子群、群的结构、线性表示、特征标与张量积、点群与空间群、量子力学中的对称性、旋转群与旋量、角动量、置换群与全同粒子、Lie 群根系与 \(SU(3)\)、Lorentz--Poincaré 与规范对称。源码仍保留 `ch01`--`ch22`，文件边界不再等同章边界。

写作单位应是连续论证，而不是“定义框—推导框—例题框”的卡片序列。定义和例子保留编号与交叉引用，但使用无边框正文；长推导写成普通证明或连续计算。章首从上一问题尚未解决之处进入新对象，章末停在一个具体结论或下一问题上，不添加“本章主线”“闭环”“路线图”式总结。标题使用数学对象或物理主题的名称，问句只留在真正需要提出问题的正文中。
