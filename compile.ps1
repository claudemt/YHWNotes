...
$selChapters = @{}; $selSections = @{}
while ($true) {
    $selection = Read-Host "`nContent IDs (e.g. c1,c2,12; Enter=all)"
    if ([string]::IsNullOrWhiteSpace($selection)) { foreach ($ch in $chapters) { $selChapters[[string]$ch.No] = $true }; break }
...
$tempTex = Join-Path $ROOT_DIR "__compile_temp.tex"
[IO.File]::WriteAllLines($tempTex, $mainLines, [Text.UTF8Encoding]::new($false))
...
$tempPdf = Join-Path $buildDir "__compile_temp.pdf"
if (Test-Path $tempPdf) {
    ...
    Move-Item $tempPdf $outPath -Force
    Get-ChildItem "$buildDir\__compile_temp.*" -ErrorAction SilentlyContinue | Remove-Item -Force
    Remove-Item "$buildDir\*.aux","$buildDir\*.log","$buildDir\*.out","$buildDir\*.toc" -Force -ErrorAction SilentlyContinue
    Start-Process $outPath
} else {
    Write-Host "Failed" -ForegroundColor Red
    Get-Content (Join-Path $buildDir "__compile_temp.log") -ErrorAction SilentlyContinue | Select-String "^!" | Select-Object -First 10
    Write-Host "Check build\__compile_temp.log for details." -ForegroundColor Yellow
    exit 1
}
